import{t as e}from"./AppToast-BfaQtGFE.js";import{n as t,t as n}from"./AppModal-C8uxOJPY.js";var r=null,i=null;function a(a={}){let{fecha:o=``,claseId:s=``,clases:c=[],alumnos:l=[],maestroId:u=null,onSave:d=null}=a;r=d,i=u,n.open({title:`Nueva Clase Emergente`,size:`lg`,saveText:`Crear Clase`,cancelText:`Cancelar`,body:`
    ...
    `,onShow:e=>{let n=e.querySelector(`#modal-es_co-docencia`),r=e.querySelector(`#codocencia-fields`),i=e.querySelector(`#modal-alumnos-lista`),a=e.querySelector(`#modal-alumnos-buscar`),o=e.querySelector(`#modal-seleccionar-todos`),s=e.querySelector(`#modal-alumnos-resumen`);n?.addEventListener(`change`,e=>{r.style.display=e.target.checked?`block`:`none`});let c=Array.from(new Map((l||[]).filter(Boolean).map(e=>[e.id,e])).values()).sort((e,t)=>(e.nombre_completo||``).localeCompare(t.nombre_completo||``)),u=new Set,d=()=>{let e=u.size;s.textContent=e>0?`${e} alumno(s) seleccionado(s).`:`Selecciona al menos un alumno para crear el registro.`},f=()=>{let e=(a?.value||``).trim().toLowerCase();return c.filter(t=>!e||[t.nombre_completo||``,t.instrumento_principal||``,...t.clase_nombres||[]].join(` `).toLowerCase().includes(e))},p=()=>{let e=f(),t=e.length>0&&e.every(e=>u.has(e.id));o.checked=t,o.indeterminate=!t&&e.some(e=>u.has(e.id))},m=()=>{let n=f();i.innerHTML=n.length?n.map(e=>`
            <label class="pm-emergente-student">
              <input type="checkbox" class="modal-alumno-check" value="${e.id}" ${u.has(e.id)?`checked`:``}>
              <span>${t(e.nombre_completo||`Alumno sin nombre`)}</span>
              <span class="pm-emergente-student-tags">
                ${e.instrumento_principal?`<span class="pm-emergente-tag pm-emergente-tag-instrument">${t(e.instrumento_principal)}</span>`:``}
                ${(e.clase_nombres||[]).map(e=>`<span class="pm-emergente-tag">${t(e)}</span>`).join(``)}
              </span>
            </label>
          `).join(``):`<p class="pm-emergente-hint" style="margin:0;">No hay alumnos que coincidan con los filtros.</p>`,i.querySelectorAll(`.modal-alumno-check`).forEach(t=>{t.addEventListener(`change`,()=>{t.checked?u.add(t.value):u.delete(t.value),e.dataset.selectedAlumnoIds=Array.from(u).join(`,`),p(),d()})}),p(),d()};o?.addEventListener(`change`,()=>{let t=f();o.checked?t.forEach(e=>u.add(e.id)):t.forEach(e=>u.delete(e.id)),e.dataset.selectedAlumnoIds=Array.from(u).join(`,`),m()}),a?.addEventListener(`input`,m),m()},onSave:async t=>{let n=(t.dataset.selectedAlumnoIds||``).split(`,`).map(e=>e.trim()).filter(Boolean),a={fecha:t.querySelector(`#modal-fecha`).value,actividad:t.querySelector(`#modal-clase_id`).value,clase_id:null,hora_inicio:t.querySelector(`#modal-hora_inicio`).value,hora_fin:t.querySelector(`#modal-hora_fin`).value,tema_principal:t.querySelector(`#modal-tema`).value.trim(),contenido:t.querySelector(`#modal-contenido`).value.trim(),motivo:t.querySelector(`#modal-motivo`).value.trim(),es_codocencia:t.querySelector(`#modal-es_co-docencia`).checked,maestro_auxiliar_id:t.querySelector(`#modal-maestro_auxiliar_id`)?.value||null,estado:`pendiente`,maestro_id:i,asistencia:n.map(e=>({alumno_id:e,estado:null}))};return!a.fecha||!a.actividad||!a.hora_inicio||!a.hora_fin||!a.tema_principal||!a.motivo?(e.error(`Todos los campos obligatorios deben completarse`),!1):n.length===0?(e.error(`Selecciona al menos un alumno participante`),!1):a.hora_inicio>=a.hora_fin?(e.error(`La hora de inicio debe ser menor que la hora de fin`),!1):(r&&await r(a),!0)}})}export{a as t};