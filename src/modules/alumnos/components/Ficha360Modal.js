...
// Datos arquetípicos con granularidad histórica profunda para los 4 pilares
const FIXTURES_360 = {
  'sofia': {
    id: 'alu-sofia-demo',
...
export const Ficha360Modal = {
  activeTab: 'asistencia',

  abrir(alumnoOClave = 'sofia', defaultTab = 'asistencia') {
    let key = 'sofia'
    let data = FIXTURES_360.sofia
    this.activeTab = defaultTab

    if (typeof alumnoOClave === 'string' && FIXTURES_360[alumnoOClave]) {
      key = alumnoOClave
      data = FIXTURES_360[alumnoOClave]
    } else if (alumnoOClave && typeof alumnoOClave === 'object') {
      const nombre = (alumnoOClave.nombre_completo || alumnoOClave.nombre || '').toLowerCase()
      if (nombre.includes('mateo') || nombre.includes('morales') || nombre.includes('cello') || nombre.includes('chelo')) {
        key = 'mateo'
        data = FIXTURES_360.mateo
      } else {
        key = 'sofia'
        data = FIXTURES_360.sofia
      }
    }

    this.renderModal(key, data)
  },

  renderModal(activeKey, data) {
    const esSofia = activeKey === 'sofia'
    const waUrl = whatsappLink(data.representante_tlf, `Hola ${data.representante_nombre}, le contactamos desde El Sistema Punta Cana sobre ${data.nombre_completo}.`)

    const headerHTML = `
      <div class="d-flex align-items-center justify-content-between w-100 pe-3">
        <div class="d-flex align-items-center gap-2">
          <i class="bi bi-stars text-warning fs-5"></i>
          <span class="fw-bold text-white">Ficha 360° del Alumno</span>
        </div>
        <div class="btn-group btn-group-sm" role="group">
          <button type="button" class="btn ${esSofia ? 'btn-light active fw-bold text-primary' : 'btn-outline-light'}" id="modal-btn-sofia" style="font-size:0.75rem;">
            ⭐ Sofía (Violín)
          </button>
          <button type="button" class="btn ${!esSofia ? 'btn-warning active fw-bold text-dark' : 'btn-outline-light'}" id="modal-btn-mateo" style="font-size:0.75rem;">
            ⚠️ Mateo (Cello)
          </button>
        </div>
      </div>
    ...
    const bodyHTML = `
      <div class="p-1">
        ...
        <div class="p-3 mb-2 rounded-3 text-white shadow-sm" style="background: linear-gradient(135deg, #1e1b4b 0%, #312e81 100%); border: 1px solid rgba(99, 102, 241, 0.35);">
          <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div class="d-flex align-items-center gap-3">
              <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 48px; height: 48px; font-size: 1.15rem; background: ${esSofia ? '#4f46e5' : '#d97706'}; color: white;">
                ${esSofia ? 'SR' : 'MM'}
              </div>
              <div>
                <div class="d-flex align-items-center gap-2">
                ...
                <div class="text-light opacity-75 small font-monospace" style="font-size:0.75rem;">
            ...
            <div class="d-flex align-items-center gap-2">
              <button class="btn btn-sm btn-light text-primary fw-bold shadow-sm d-inline-flex align-items-center gap-1.5 px-3 py-1.5 rounded-pill" id="btn-generar-ia" style="font-size:0.78rem;">
                <i class="bi bi-robot text-primary"></i>
                <span id="btn-ia-text">Analizar con IA</span>
              </button>
              <button class="btn btn-sm btn-warning text-dark fw-bold shadow-sm d-inline-flex align-items-center gap-1.5 px-3 py-1.5 rounded-pill" id="btn-escalar-hermes" style="font-size:0.78rem;" title="Crear ticket o protocolo interdepartamental en Hermes">
        ...
        <!-- Panel de Análisis con IA (Colapsable) -->
        <div id="panel-resena-ia" class="p-3 mb-2 rounded-3 border d-none animate-in fade-in" style="background: var(--soi-bg-subtle, var(--bs-tertiary-bg, #f8fafc)); border-color: rgba(99, 102, 241, 0.3) !important;">
          <div class="d-flex align-items-center justify-content-between mb-1.5">
            ...
            <span class="badge" id="badge-riesgo-ia" style="font-size:0.7rem;"></span>
          </div>
          <p class="small mb-2 text-body" id="texto-diagnostico-ia" style="font-size:0.82rem; line-height:1.45;"></p>
          <div class="p-2 rounded-2 bg-body border border-secondary-subtle small">
            ...
            <div class="text-body-secondary" id="texto-recomendacion-ia" style="font-size:0.78rem;"></div>
          </div>
        </div>

        <!-- Interactive Bento Grid 4 Pilares (Clickable Cards with Active State) -->
        <div class="row g-2 mb-2">
          
          <!-- 1. Asistencia -->
          <div class="col-sm-6 col-lg-3">
            <div class="card h-100 border rounded-3 bg-body border-secondary-subtle shadow-2xs cursor-pointer pillar-card ${this.activeTab === 'asistencia' ? 'ring-2 ring-primary border-primary' : ''}" id="card-pilar-asistencia" style="padding: 0.80rem; cursor: pointer; transition: all 0.2s;" title="Clic para ver histórico de asistencias">
          ...
          <div class="col-sm-6 col-lg-3">
            <div class="card h-100 border rounded-3 bg-body border-secondary-subtle shadow-2xs cursor-pointer pillar-card ${this.activeTab === 'academico' ? 'ring-2 ring-primary border-primary' : ''}" id="card-pilar-academico" style="padding: 0.80rem; cursor: pointer; transition: all 0.2s;" title="Clic para ver contenidos vistos en clase">
              <div class="d-flex justify-content-between align-items-center mb-1">
          ...
          <!-- 3. Finanzas -->
          <div class="col-sm-6 col-lg-3">
            <div class="card h-100 border rounded-3 bg-body border-secondary-subtle shadow-2xs cursor-pointer pillar-card ${this.activeTab === 'finanzas' ? 'ring-2 ring-primary border-primary' : ''}" id="card-pilar-finanzas" style="padding: 0.80rem; cursor: pointer; transition: all 0.2s;" title="Clic para ver pagos y compras en tiendita">
              <div class="d-flex justify-content-between align-items-center mb-1">
          ...
          <!-- 4. Luthería -->
          <div class="col-sm-6 col-lg-3">
            <div class="card h-100 border rounded-3 bg-body border-secondary-subtle shadow-2xs cursor-pointer pillar-card ${this.activeTab === 'lutheria' ? 'ring-2 ring-primary border-primary' : ''}" id="card-pilar-lutheria" style="padding: 0.80rem; cursor: pointer; transition: all 0.2s;" title="Clic para ver hoja de vida y reparaciones del instrumento">
              <div class="d-flex justify-content-between align-items-center mb-1">
        ...
        <!-- Contenedor del Drilldown Profundo (Detalle Dinámico según Tarjeta Seleccionada) -->
        <div id="contenedor-drilldown" class="p-3 mb-2 rounded-3 border bg-body border-secondary-subtle shadow-xs">
          ${this.generarHTMLDrilldown(this.activeTab, data)}
        </div>

        <!-- Compact Contact & Legal Bar -->
    ...
    setTimeout(() => {
      document.getElementById('modal-btn-sofia')?.addEventListener('click', () => {
        AppModal.close()
        setTimeout(() => this.abrir('sofia', this.activeTab), 180)
      })
      document.getElementById('modal-btn-mateo')?.addEventListener('click', () => {
        AppModal.close()
        setTimeout(() => this.abrir('mateo', this.activeTab), 180)
      })

      // Card Clicks to switch Drilldown
      const setPilar = (tab) => {
        this.activeTab = tab
        const cont = document.getElementById('contenedor-drilldown')
        if (cont) {
          cont.innerHTML = this.generarHTMLDrilldown(tab, data)
        }
        document.querySelectorAll('.pillar-card').forEach(c => {
          c.classList.remove('border-primary')
          c.style.border = '1px solid var(--bs-border-color)'
        })
        const activeCard = document.getElementById(`card-pilar-${tab}`)
        if (activeCard) {
      ...
      document.getElementById('card-pilar-asistencia')?.addEventListener('click', () => setPilar('asistencia'))
      document.getElementById('card-pilar-academico')?.addEventListener('click', () => setPilar('academico'))
      document.getElementById('card-pilar-finanzas')?.addEventListener('click', () => setPilar('finanzas'))
      document.getElementById('card-pilar-lutheria')?.addEventListener('click', () => setPilar('lutheria'))

      // AI Button Handler
      const btnIa = document.getElementById('btn-generar-ia')
      const panelIa = document.getElementById('panel-resena-ia')
      const btnIaText = document.getElementById('btn-ia-text')
      const diagIa = document.getElementById('texto-diagnostico-ia')
      const recIa = document.getElementById('texto-recomendacion-ia')
      const badgeRiesgo = document.getElementById('badge-riesgo-ia')
      ...
      // Hermes Escalation Handler
      const btnEscalar = document.getElementById('btn-escalar-hermes')
      btnEscalar?.addEventListener('click', async () => {
        ...
        try {
          ...
          console.warn('[Ficha360] Error al escalar tarea:', err)
