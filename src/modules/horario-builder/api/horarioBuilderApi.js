...
const mockClases = [
  ...
  {
    id: 'c-003',
    nombre: 'Piano Inicial A',
    instrumento: 'Piano',
    maestro_principal_id: 'm-002',
    capacidad_maxima: 12,
    total_alumnos: 10,
    horarios: [],
  },
  {
    id: 'c-004',
    nombre: 'Teoría y Solfeo I',
    instrumento: 'Solfeo',
    maestro_principal_id: 'm-006',
    capacidad_maxima: 25,
    total_alumnos: 18,
    horarios: [],
  },
  ...
  {
    id: 'c-006',
    nombre: 'Guitarra Clásica I',
    instrumento: 'Guitarra',
    maestro_principal_id: 'm-005',
    capacidad_maxima: 15,
    total_alumnos: 11,
    horarios: [],
  },
  {
    id: 'c-007',
    nombre: 'Cello y Cámara',
    instrumento: 'Cello',
    maestro_principal_id: 'm-004',
    capacidad_maxima: 8,
    total_alumnos: 3,
    horarios: [],
  },
...
async function getClasesReal() {
  ...
  const { data: horarios } = await supabase.from('clase_horarios').select('*')

  const { data: inscripciones } = await supabase.from('alumnos_clases').select('clase_id')
...
export async function applyScheduleRun(runId, assignments) {
  ...
  try {
    ...
    const { error: insertErr } = await supabase.from('clase_horarios').insert(newHorarios)
