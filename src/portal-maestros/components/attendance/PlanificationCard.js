...
import { abrirMapaDeRutas } from '../teacherRouteMapPanel.js'
...
export function createPlanificationCard(container, opts) {
  ...
  // Antes, sin guía ACM, esta card caía a un mini-CRUD propio sobre la tabla
  // `planificaciones` (objetivosEstructurados) — un cuarto sistema de registro
  // de contenido, aislado de maestro_routes. Auditoría: 0 de 3 filas reales en
  // esa tabla tenían objetivos estructurados; nadie lo estaba usando. Se
  // reemplaza por un CTA directo al Mapa de Rutas real (maestro_routes), el
  // mismo sistema que ya usan "Hoy", Asistencia (header) y la vista "Plan".
  function renderNoRouteMap() {
    if (!treeContainer) return

    treeContainer.innerHTML = `
      <div class="pm-plan-sin-guia-chip" style="margin-top:0; margin-bottom:0.75rem;">
        <i class="bi bi-info-circle me-1"></i>Esta clase no tiene guía ACM asignada
      </div>
      <div class="text-center p-4 text-muted bg-body-tertiary rounded-4 border border-secondary-subtle mb-3 w-100" style="font-size:0.85rem;">
        <i class="bi bi-signpost-2-fill d-block mb-2 fs-3 text-secondary"></i>
        <span>Usa tu propio Mapa de Rutas (Unidad → Objetivo → Indicador) para planificar y calificar esta clase.</span>
        <button type="button" class="btn btn-sm btn-primary mt-3" id="pm-btn-abrir-mapa-rutas" style="font-weight:700;">
          <i class="bi bi-signpost-2-fill me-1"></i> Abrir Mapa de Rutas
        </button>
      </div>
    `

    const btnAbrir = treeContainer.querySelector('#pm-btn-abrir-mapa-rutas')
    if (btnAbrir) {
