...
`;async function Au(e,t=`maestro`){if(!e)return;let n=t===`admin`;try{let r=ku;if(n?r+=`
      ...
      <div class="planificacion-filter-toolbar mb-4">
        ...
        <div class="premium-select-container">
          ...
          <select class="form-select premium-filter-select" id="select-estado">
            ...
            ${Ue.getEstados().map(e=>`<option value="${e.value}">${e.label}</option>`).join(``)}
          </select>
        </div>
      </div>
  ...
  `}function Ku(e){if(!e||e.length===0)return``;let t=U.viewMode===`admin`;return e.map(e=>{let n=Ue.getEstadoConfig(e.estado),r=e.estado===`revisado`?`border-accent-success`:e.estado===`ejecutado`?`border-accent-warning`:`border-accent-secondary`;return`
