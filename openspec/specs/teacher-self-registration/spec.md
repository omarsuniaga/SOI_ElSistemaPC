# Teacher Self-Registration — Especificación

## Propósito

Habilitar el auto-registro de maestros en el Portal Maestros con un flujo de aprobación administrativa, reparando la cadena de autenticación `auth.users → profiles → maestros` que presentaba inconsistencias de seguridad y naming. La fuente única de verdad para el estado de activación es `profiles.estado`.

### Problemas resueltos

1. No existía trigger que creara `maestros.user_id` al registrarse un teacher
2. Políticas RLS referenciaban tablas inexistentes (`students`, `progresos_academicos`, `observaciones`)
3. `is_admin()` buscaba columna `role` en vez de `rol`
4. RLS de `class_sessions` comparaba `maestro_id` contra `auth.uid()` en vez de subquery contra `maestros`
5. No existía vista de registro en Portal Maestros
...
#### A3. Helper function `public.user_profile()` para RLS
...
| `openspec/changes/archive/2026-05-17-auth-rls-rearchitecture/` | Cambio SDD que implementó esta especificación |
