    ...
    `}_buildEntry(e){let t=l[e.tipo]||l.logro,n=new Date(e.created_at).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`numeric`});return`
      <div class="pe-entry" data-testid="pe-entry" data-id="${e.id}">
        ...
        <div class="pe-entry__body">
          ...
          <p class="pe-entry__titulo" data-testid="pe-entry-titulo">${r(e.titulo)}</p>
          ${e.descripcion?`<p class="pe-entry__desc">${r(e.descripcion)}</p>`:``}
...
`).trim():``}var y={LOGRADO:{label:`Logrado`,color:`#198754`,bg:`#19875418`,icon:`✅`},EN_PROGRESO:{label:`En progreso`,color:`#0d6efd`,bg:`#0d6efd18`,icon:`📈`},INICIADO:{label:`Iniciado`,color:`#6c757d`,bg:`#6c757d18`,icon:`🔰`},DIFICULTAD:{label:`Dificultad`,color:`#dc3545`,bg:`#dc354518`,icon:`⚠️`}};async function b(t,i,a=0){let o=t.querySelector(`#pm-alumno-progresos-root`);if(!o)return;let s=a===0;s&&(o.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`);try{let{data:n,error:c}=await e.from(`progresos`).select(`id, contenido_dsl, estado_cualitativo, calificacion, observaciones, fecha_evaluacion, clase_id, indicadores`).eq(`alumno_id`,i).not(`contenido_dsl`,`is`,null).neq(`contenido_dsl`,``).order(`fecha_evaluacion`,{ascending:!1}).range(a,a+19);if(c)throw c;if(!n||n.length===0){s&&(o.innerHTML=`<p style="font-size:0.82rem;color:var(--pm-text-muted);text-align:center;padding:1rem 0;">Sin registros de progreso generados por IA aún.</p>`);return}let l=[...new Set(n.map(e=>e.clase_id).filter(Boolean))],{data:u}=l.length?await e.from(`clases`).select(`id, nombre`).in(`id`,l):{data:[]},d=new Map((u||[]).map(e=>[e.id,e.nombre])),f=new Map;for(let e of n){let t=e.contenido_dsl;f.has(t)||f.set(t,{contenido:t,entries:[]}),f.get(t).entries.push(e)}let p=Array.from(f.values()).map(({contenido:e,entries:t})=>{let n=t[0],i=y[n.estado_cualitativo]??y.EN_PROGRESO,a=new Date(n.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`2-digit`});return`
        <details class="pm-prog-card">
          <summary class="pm-prog-card__summary">
            <span class="pm-prog-card__icon" style="color:${i.color}">${i.icon}</span>
            <div class="pm-prog-card__info">
              <span class="pm-prog-card__name">${r(e)}</span>
              <span class="pm-prog-card__meta">${t.length} registro${t.length===1?``:`s`} · último: ${a}</span>
            </div>
            <span class="pm-prog-card__badge" style="color:${i.color};background:${i.bg}">${i.label}</span>
            <i class="bi bi-chevron-down pm-prog-card__chevron"></i>
          </summary>
          <div class="pm-prog-card__timeline">
            ${t.map(e=>{let t=y[e.estado_cualitativo]??y.EN_PROGRESO,n=new Date(e.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`numeric`}),i=d.get(e.clase_id)||`Clase`,a=e.indicadores?.tarea,o=e.calificacion==null?null:`${e.calificacion}/5`;return`
                <div class="pm-prog-entry">
                  ...
                  <div class="pm-prog-entry__body">
                    <div class="pm-prog-entry__header">
                      ...
                      <span class="pm-prog-entry__clase">${r(i)}</span>
                    ...
                    ${e.observaciones?`<p class="pm-prog-entry__obs">${r(e.observaciones)}</p>`:``}
                    ${a?`<p class="pm-prog-entry__tarea">📋 ${r(a)}</p>`:``}
                  </div>
                </div>
              `}).join(``)}
          </div>
        ...
        <div class="pm-zen-hero" style="--accent-gradient: ${G}">
          ...
          <div class="pm-zen-hero__content">
            ...
            <div class="pm-zen-info">
              <h1 class="pm-zen-name">${r(t.nombre_completo)}</h1>
              <p class="pm-zen-instrument">${r(t.instrumento_principal||`Estudiante`)}</p>
        ...
        <div class="pm-zen-body">
          ...
          <div class="pm-zen-section">
            ...
            <div class="pm-zen-clases-grid">
              ${l.map(e=>{let t=z.get(e);if(!t)return``;let n=I[e]||{P:0,A:0,J:0,T:0,total:0},i=n.total>0?Math.round(n.P/n.total*100):null,a=m?.filter(t=>t.covered_by_clase_id===e&&t.nota!=null&&t.nota!==0)||[],o=a.length>0?Math.round(a.reduce((e,t)=>e+t.nota,0)/a.length*10)/10:null;return`
                  <div class="pm-zen-clase-card pm-glass">
                    <div class="pm-zen-clase-header">
                      <strong>${r(t.nombre)}</strong>
                      ${t.nivel?`<span class="pm-zen-clase-nivel">Nivel ${t.nivel}</span>`:``}
                    </div>
                    <div class="d-flex justify-content-between align-items-center mb-2">
                      <p class="pm-zen-clase-inst" style="margin:0;">${r(t.instrumento||``)}</p>
                      ${o===null?`
                    ...
                    <div class="pm-zen-clase-stats">
                      <div class="pm-zen-clase-stat" style="flex:1.2;">
                        <div style="display:flex;align-items:baseline;gap:3px;">
                          <span class="pm-zen-stat-value" style="color:${i===null?`var(--pm-text-muted)`:i>=75?`var(--pm-success)`:i>=50?`var(--pm-warning)`:`var(--pm-danger)`};">${n.P}</span>
                          <span style="font-size:0.7rem;color:var(--pm-text-muted);">/ ${n.total||`—`}</span>
                        </div>
                        <span class="pm-zen-stat-label">${i===null?`Sin datos`:i+`%`}</span>
                      </div>
              ...
              ${k.slice(0,8).map(e=>{let t=new Date(e.created_at),n=e.nota>=4?`var(--pm-success)`:e.nota>=2?`var(--pm-warning)`:`var(--pm-danger)`;return`
                  <div class="pm-zen-eval-item">
                    ...
                    <div class="pm-zen-eval-content">
                      ...
                      ${e.tarea?`<p class="pm-zen-eval-tarea">${r(e.tarea)}</p>`:``}
                      ${e.observations?`<p class="pm-zen-eval-obs">${r(e.observations.substring(0,80))}${e.observations.length>80?`...`:``}</p>`:``}
                    </div>
                  </div>
                `}).join(``)}
            </div>
              ...
              ${V.map(e=>{let t=e.nota>=4?`var(--pm-success)`:e.nota>=2?`var(--pm-warning)`:`var(--pm-danger)`;return`
                  <div class="pm-zen-desenv-item">
                    ...
                    <div class="pm-zen-desenv-content">
                      ...
                      <p class="pm-zen-desenv-obs">${r(e.observations)}</p>
                      ${e.tarea?`<p class="pm-zen-desenv-tarea">📋 ${r(e.tarea)}</p>`:``}
                    </div>
                  </div>
                `}).join(``)}
            </div>
          ...
          <details class="pm-zen-section pm-zen-accordion" ${p.length>0?`open`:``}>
            ...
            <div class="pm-zen-asistencia-timeline" style="margin-top:0.75rem;">
              ${p.length===0?`<p class="pm-zen-empty">Sin registros de asistencia</p>`:p.slice(0,30).map(e=>{let t={P:`Presente`,A:`Ausente`,J:`Justificado`,T:`Tardanza`},n={P:`var(--pm-success)`,A:`var(--pm-danger)`,J:`var(--pm-warning)`,T:`#FF9500`},i=z.get(e.clase_id),a=e.estado===`J`?S.get(e.sesion_id):null;return`
                      <div class="pm-zen-asistencia-item">
                        ...
                        <div class="pm-zen-asistencia-content">
                          ...
                          <span class="pm-zen-asistencia-clase">${r(i?.nombre||`Clase`)}</span>
                          ${a?`
                            <div class="pm-zen-justif-box">
                              ...
                              <p class="pm-zen-justif-motivo">${r(a.motivo)}</p>
                          ...
                          ${e.observaciones?`<span class="pm-zen-asistencia-obs">${r(e.observaciones)}</span>`:``}
                        </div>
                      </div>
                    `}).join(``)}
            </div>
          ...
          <div class="pm-zen-section">
            ...
            <div class="pm-zen-ausencias">
              ${y.map(e=>{let t=new Date(e.fecha_inicio).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`}),n=e.fecha_fin?new Date(e.fecha_fin).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`}):t,i={pendiente:`var(--pm-warning)`,aprobada:`var(--pm-success)`,rechazada:`var(--pm-danger)`};return`
                  <div class="pm-zen-ausencia-item">
                    <div class="pm-zen-ausencia-icon" style="background:${i[e.estado]||`var(--pm-border)`}20">
                      <i class="bi bi-calendar-x" style="color:${i[e.estado]||`var(--pm-text-muted)`}"></i>
                    </div>
                    <div class="pm-zen-ausencia-content">
                      <div class="pm-zen-ausencia-header">
                        <strong>${t===n?t:`${t} - ${n}`}</strong>
                        <span class="pm-zen-ausencia-estado" style="color:${i[e.estado]||`var(--pm-text-muted)`}">${e.estado||`pendiente`}</span>
                      </div>
                      ${e.motivo?`<p class="pm-zen-ausencia-motivo">${r(e.motivo.substring(0,60))}${e.motivo.length>60?`...`:``}</p>`:``}
                    </div>
                  </div>
                `}).join(``)}
            </div>
            ...
            <div class="pm-zen-details-grid">
              <div class="pm-zen-detail">
                ...
                <div style="flex:1;min-width:0;">
                  ...
                  <strong>${i(t.tlf_alumno||t.representante_tlf)||`—`}</strong>
                </div>
              ...
              <div class="pm-zen-detail">
                ...
                <div>
                  ...
                  <strong>${r(t.representante_nombre)}</strong>
                </div>
              </div>`:``}
              ...
              <div class="pm-zen-detail">
                ...
                <div>
                  ...
                  <strong>${r(t.correo_representante)}</strong>
                </div>
              </div>`:``}
              ${t.direccion?`
              <div class="pm-zen-detail">
    ...
    `,n.querySelector(`#pm-alumno-back`).onclick=()=>window.history.back();let K=f(t.tlf_alumno||t.representante_tlf);if(K){let e=t.nombre_completo||`el alumno`,i=t.representante_nombre||e,a={alumno:e,contacto:i},s=document.createElement(`div`);s.id=`pm-wa-modal`,s.innerHTML=`
        ...
        <div class="pm-wa-dialog">
          ...
          <div id="pm-wa-view-send">
            <div class="pm-wa-header">
              ...
              <span>Mensaje para <strong>${r(i)}</strong></span>
