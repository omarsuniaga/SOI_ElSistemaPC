...
describe('repertorioView', () => {
  ...
  it('enters explicit multiselect on long press, selects the first measure and exposes the action sheet', async () => {
    const adapter = makeAdapter()
    const container = document.createElement('main')
    await renderRepertoireView(container, { adapter })
    container.querySelector('.repertoire-card[data-montaje-id="montaje-1"]').click()
    const measure = container.querySelector('.repertoire-measure')
    measure.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true }))
    await new Promise((resolve) => setTimeout(resolve, 600))
    expect(container.querySelector('.repertoire-measure').getAttribute('aria-selected')).toBe('true')
    expect(container.querySelector('#repertoire-measure-actions').classList.contains('is-open')).toBe(true)
    expect(container.querySelector('#repertoire-measure-actions').textContent).toContain('1 compases seleccionados')
    expect(adapter.updateMeasureState).not.toHaveBeenCalled()
  })

  it('supports explicit selection, non-contiguous toggle, clear and fila scope labeling', async () => {
    const container = document.createElement('main')
    await renderRepertoireView(container, { adapter: makeAdapter() })
    container.querySelector('.repertoire-card[data-montaje-id="montaje-1"]').click()
    container.querySelector('.repertoire-multi').click()
    container.querySelectorAll('.repertoire-measure')[0].click()
    container.querySelectorAll('.repertoire-measure')[2].click()
    expect(container.querySelectorAll('.repertoire-measure[aria-selected="true"]')).toHaveLength(2)
    expect(container.querySelector('.repertoire-action-sheet-scope').textContent).toContain('Fila')
    container.querySelector('#repertoire-measure-actions [data-action="close"]').click()
    expect(container.querySelectorAll('.repertoire-measure[aria-selected="true"]')).toHaveLength(0)
    expect(container.querySelector('#repertoire-measure-actions')).toBeNull()
  })

  it('clears selection when changing student scope and fails closed for unsupported student bulk writes', async () => {
    const adapter = makeAdapter()
    const container = document.createElement('main')
    await renderRepertoireView(container, { adapter })
    container.querySelector('.repertoire-card[data-montaje-id="montaje-1"]').click()
    ...
    container.querySelector('.repertoire-multi').click()
    container.querySelector('.repertoire-measure').click()
    container.querySelector('.repertoire-sheet-preparation[data-state="DOMINADO"]').click()
    await Promise.resolve()
    expect(container.querySelector('.repertoire-sync').textContent).toContain('Actualización de backend requerida')
    expect(adapter.updateStudentState).not.toHaveBeenCalled()
  })
