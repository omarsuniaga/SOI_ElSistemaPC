...
export async function createAiService() {
  ...
  async function _requestWithFallback(messages, temp = 0.7, cacheKey = null) {
    const { canMakeRequest, withRateLimit, getCachedResponse } =
      await import('./groqRateLimiter.js')
    ...
    throw (
      lastError || new Error('No hay proveedores de IA configurados o las API keys son inválidas.')
    )
  }
  ...
  return {
    ...
    async chat(message, context = {}) {
      ...
      if (
        cleanMessage.includes('hola') ||
        cleanMessage.includes('buenos') ||
        cleanMessage.includes('buenas')
      ) {
        intent = 'saludo'
        ...
        cleanMessage.includes('asistencia') ||
        cleanMessage.includes('falt') ||
        cleanMessage.includes('presente')
      ) {
        intent = 'asistencia'
      }
...
${
  opts.sesiones
    ?.slice(historyLimit)
    .map((s) => `- Fecha: ${s.fecha}, Calif: ${s.calif || 'N/A'}, Estado: ${s.estado}`)
    .join('\n') || 'Sin sesiones'
}
