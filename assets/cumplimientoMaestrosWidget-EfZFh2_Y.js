...
import{n as e}from"./rolldown-runtime-tcWNtVWY.js";import{i as t}from"./supabase-DJmkTfk1.js";import{t as n}from"./preload-helper-CsoeaaUJ.js";async function r(){try{let{data:e,error:n}=await t.from(`maestro_desempeño`).select(`
    ...
    `}attachEventListeners(){let e=document.getElementById(`filterCategoria`),t=document.getElementById(`filterDiasAtraso`),r=document.getElementById(`btnRefresh`);e?.addEventListener(`change`,e=>{this.applyFilter({categoria:e.target.value||null})}),t?.addEventListener(`change`,e=>{if(!e.target.value)this.applyFilter({diasAtrasoMin:0,diasAtrasoMax:999});else{let t=e.target.value.split(`-`);this.applyFilter({diasAtrasoMin:t[0]?parseInt(t[0]):0,diasAtrasoMax:t[1]?parseInt(t[1]):999})}}),r?.addEventListener(`click`,()=>{this.init()}),document.getElementById(`btnGotoNotificaciones`)?.addEventListener(`click`,()=>{n(async()=>{let{router:e}=await import(`./router-CcRIuSbB.js`).then(e=>e.n);return{router:e}},__vite__mapDeps([0,1,2,3])).then(({router:e})=>{e.navigate(`admin-notificaciones`)})}),this.container.querySelectorAll(`.btn-contactar`).forEach(e=>{e.addEventListener(`click`,e=>{let t=e.target.closest(`.btn-contactar`).dataset.maestroId;this.onContactarMaestro(t)})}),this.container.querySelectorAll(`.btn-detalle`).forEach(e=>{e.addEventListener(`click`,e=>{let t=e.target.closest(`.btn-detalle`).dataset.maestroId;this.onDetalleMaestro(t)})})}onContactarMaestro(e){let t=this.maestros.find(t=>t.maestro_id===e);if(!t)return;let n=t.maestros?.email;n?window.location.href=`mailto:${n}?subject=Seguimiento%20Registros%20Asistencias`:alert(`No hay email disponible para este maestro`)}onDetalleMaestro(e){let t=this.maestros.find(t=>t.maestro_id===e);t&&(console.log(`View detail for maestro:`,e,t),window.location.href=`/admin/maestros/${e}/detail`)}};export{a as n,o as t};