-- ============================================================================
-- vw_seguimiento_ausentes v5: agrega maestro_tlf (para el mensaje de retencion
-- al maestro) y retencion_id (para levantar la retencion desde el modal).
-- DROP + CREATE (cambia posicion de columnas). Aplicado a zmhmdvmyeyswunurcyow.
-- ============================================================================
DROP VIEW IF EXISTS public.vw_seguimiento_ausentes;
CREATE VIEW public.vw_seguimiento_ausentes
WITH (security_invoker = false) AS
WITH periodo_activo AS (
  SELECT id, nombre, fecha_inicio, fecha_fin FROM public.periodos WHERE activo = true ORDER BY fecha_inicio DESC LIMIT 1
),
regla AS (
  SELECT COALESCE((config->>'nivel1')::int, 3) AS n1, COALESCE((config->>'nivel2')::int, 6) AS n2, COALESCE((config->>'nivel3')::int, 10) AS n3
...
dias AS (
  SELECT ast.alumno_id,
    COUNT(DISTINCT ast.fecha) FILTER (WHERE ast.estado = 'ausente'
      AND NOT EXISTS (SELECT 1 FROM public.asistencias p WHERE p.alumno_id = ast.alumno_id AND p.fecha = ast.fecha AND p.estado = 'presente')) AS dias_ausente,
...
SELECT
  ...
  cls.clase_nombres, cls.maestro_id, cls.maestro_nombre, cls.maestro_tlf,
  d.dias_ausente, d.dias_clase, d.ultima_ausencia_fecha,
  CASE WHEN d.dias_ausente >= (SELECT n3 FROM regla) THEN 3 WHEN d.dias_ausente >= (SELECT n2 FROM regla) THEN 2 ELSE 1 END AS nivel,
  ...
  ret.retencion_id, (ret.retencion_id IS NOT NULL) AS retencion_activa,
  (SELECT id FROM periodo_activo) AS periodo_id, (SELECT nombre FROM periodo_activo) AS periodo_nombre
...
LEFT JOIN LATERAL (
  ...
  ) s WHERE public.normalizar_tel_rd(s.contacto_telefono) IS NOT NULL ORDER BY s.ord LIMIT 1
) ctc ON true
LEFT JOIN LATERAL (
  ...
  FROM public.comunicaciones_seguimiento cs WHERE cs.alumno_id = a.id AND cs.origen = 'ausentismo' ORDER BY cs.fecha DESC LIMIT 1
) seg ON true
LEFT JOIN LATERAL (
  SELECT ri.id AS retencion_id FROM public.retenciones_instrumento ri WHERE ri.alumno_id = a.id AND ri.estado = 'retenido' ORDER BY ri.retenido_en DESC LIMIT 1
) ret ON true
WHERE a.activo = true
  AND NOT EXISTS (SELECT 1 FROM public.alumno_suspensiones sp WHERE sp.alumno_id = a.id AND sp.estado = 'activa' AND CURRENT_DATE >= sp.desde AND (sp.hasta IS NULL OR CURRENT_DATE <= sp.hasta));

GRANT SELECT ON public.vw_seguimiento_ausentes TO authenticated;
COMMENT ON VIEW public.vw_seguimiento_ausentes IS 'v5: agrega maestro_tlf y retencion_id. Contador acumulado del periodo, umbrales 3/6/10. security_invoker=false.';
