-- ============================================================================
-- MIGRATION: 20260827000001_harden_portal_access_system.sql
-- Hardening y seguridad en funciones de acceso a portales:
-- 1. search_path = public, pg_temp en todas las funciones SECURITY DEFINER
-- 2. REVOKE de PUBLIC y GRANT explícito a authenticated
-- 3. Blindaje contra escalamiento de privilegios (solo superadmin puede asignar SUPERADMIN)
-- 4. Protección IDOR en consultas de otros usuarios
-- ============================================================================

-- 1. has_portal_access (Hardened)
...
BEGIN
  ...
  WITH accessible AS (
    ...
    WHERE a.user_id = v_target_user_id AND c.is_active = true
...
-- 3. set_user_portales (Hardened con control de escalamiento de privilegios)
...
BEGIN
  ...
  IF p_portal_ids IS NOT NULL AND array_length(p_portal_ids, 1) > 0 THEN
    FOREACH v_clean_id IN ARRAY p_portal_ids LOOP
      ...
      IF NOT EXISTS (SELECT 1 FROM public.portal_catalog WHERE portal_id = v_clean_id AND is_active = true) THEN
...
-- 4. Mínimo privilegio de ejecución: Revocar de PUBLIC y conceder solo a authenticated
