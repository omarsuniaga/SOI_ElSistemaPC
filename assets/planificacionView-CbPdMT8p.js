import{i as e,r as t}from"./maestroDataService-9gztUGVK.js";import{t as n}from"./vendor-BUMNmNsE.js";import{t as r}from"./AppToast-3qbHkRVc.js";import{i,l as a,m as o,o as s,r as c}from"./weeklyPlanAdapter-BulGBGlw.js";import{t as l}from"./a11yUtils-Bm1YcVfS.js";var u={achieved:{label:`Dominado`,icon:`🟢`,cardClass:`estado-completado`},exceeded:{label:`Sobresaliente`,icon:`🔵`,cardClass:`estado-completado`},in_process:{label:`En proceso`,icon:`🟡`,cardClass:`estado-parcial`},needs_reinforcement:{label:`Requiere refuerzo`,icon:`🟠`,cardClass:`estado-parcial`},failed:{label:`No logrado`,icon:`🔴`,cardClass:`estado-no_iniciado`},not_started:{label:`Sin iniciar`,icon:`⚪`,cardClass:`estado-no_iniciado`}},d={violin:`🎻`,viola:`🎻`,cello:`🎻`,contrabajo:`🎻`,chelo:`🎻`,piano:`🎹`,teclado:`🎹`,guitarra:`🎸`,bajo:`🎸`,ukulele:`🎸`,flauta:`🪈`,clarinete:`🎵`,oboe:`🎵`,fagot:`🎵`,saxofon:`🎵`,trompeta:`🎺`,trombon:`🎺`,tuba:`🎺`,corno:`🎺`,corneta:`🎺`,percusion:`🥁`,bateria:`🥁`,marimba:`🥁`,xilofono:`🥁`,timbal:`🥁`,canto:`🎤`,voz:`🎤`,vocal:`🎤`,arpa:`🪗`,acordeon:`🪗`,teoria:`📖`,solfeo:`📖`,armonia:`📖`,historia:`📖`};function f(e){return u[e]||u.not_started}function p(e){return String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function m(e){if(!e)return`🎼`;let t=e.toLowerCase().normalize(`NFD`).replace(/[\u0300-\u036f]/g,``);return Object.entries(d).find(([e])=>t.includes(e))?.[1]||`🎼`}function h(e){return e>=70?`#10b981`:e>=30?`#f59e0b`:`#ef4444`}function g(e){let t=new Set;return(e?.items||[]).filter(e=>{let n=e.indicator_id||`${e.node_id}:${e.topic}`;return t.has(n)?!1:(t.add(n),!0)})}function _(e,t,n){let r=n.length;return g(e).map(e=>{let i=n.map(n=>t[`${n}_${e.indicator_id}`]?.status||`not_started`),a=i.filter(e=>[`achieved`,`exceeded`].includes(e)).length,o=i.filter(e=>[`in_process`,`needs_reinforcement`].includes(e)).length,s=`not_started`;a>0&&a===r&&r>0?s=`achieved`:(a>0||o>0)&&(s=`in_process`);let c=f(s);return{id:e.indicator_id||e.node_id||e.id,weekNumber:e.week_number,topic:e.topic,objective:e.objective,evidence:e.evidence,assessmentMethod:e.assessment_method,teacherStrategy:e.teacher_strategy,progressPercentage:r>0?Math.round(a/r*100):0,achievedCount:a,totalStudents:r,overallStatus:s,meta:c}})}function v(e,t,n){let r=n.map(e=>e.alumno_id).filter(Boolean),i=g(e?.plan),a=i.length,o=0;if(a>0&&r.length>0){let e=0;i.forEach(n=>{r.forEach(r=>{let i=t[`${r}_${n.indicator_id}`]?.status||`not_started`;[`achieved`,`exceeded`].includes(i)&&e++})}),o=Math.round(e/(a*r.length)*100)}return{progressPercentage:o,totalStudents:r.length}}function y(e=3){return`
  ...
  `;let T=u.querySelector(`#pm-planning-content`),E=null;(function(){let e=u.querySelector(`#pm-step-slide`),t=u.querySelectorAll(`#pm-step-dots .pm-stepper-dot`),n=u.querySelector(`#pm-step-prev`),r=u.querySelector(`#pm-step-next`);if(!e)return;let i=t.length,a=0;function o(o){a=Math.max(0,Math.min(i-1,o)),e.style.transform=`translateX(-${a*100}%)`,t.forEach((e,t)=>{e.classList.toggle(`active`,t===a),e.setAttribute(`aria-selected`,String(t===a))}),n&&(n.disabled=a===0),r&&(r.disabled=a===i-1)}n?.addEventListener(`click`,()=>o(a-1)),r?.addEventListener(`click`,()=>o(a+1)),t.forEach(e=>e.addEventListener(`click`,()=>o(Number(e.dataset.step))));let s=setInterval(()=>{a<i-1?o(a+1):clearInterval(s)},5e3);u.querySelector(`#pm-guide-stepper`)?.addEventListener(`pointerdown`,()=>clearInterval(s)),o(0)})();async function D(e){let n=T.querySelector(`[data-clase-id="${e}"]`);if(n)try{let[r,i,o]=await Promise.all([s(e,d).catch(()=>null),a(e).catch(()=>({})),t([e]).catch(()=>[])]),{progressPercentage:c}=v(r,i,o),l=h(c),u=n.querySelector(`.pm-card-progress-bar`),f=n.querySelector(`.pm-progress-pct`);u&&(u.style.width=`${c}%`,u.style.backgroundColor=l),f&&(f.textContent=`${c}%`)}catch(e){console.warn(`[planning] No se pudo actualizar la tarjeta:`,e)}}async function O(){T.innerHTML=y(3);try{let n=await e();if(n.length===0){T.innerHTML=`
          ...
          </div>`,l(`No tienes clases asignadas actualmente.`);return}let r=(await Promise.allSettled(n.map(async e=>{let[n,r,i]=await Promise.all([s(e.id,d).catch(()=>null),a(e.id).catch(()=>({})),t([e.id]).catch(()=>[])]),{progressPercentage:o,totalStudents:c}=v(n,r,i);return{...e,currentWeek:n?.route?.current_week||1,hasGuide:!!n,progressPercentage:o,totalStudents:c}}))).map((e,t)=>e.status===`fulfilled`?e.value:{...n[t],currentWeek:1,hasGuide:!1,progressPercentage:0,totalStudents:0});T.innerHTML=`
      ...
      `,l(`${r.length} clases cargadas.`),T.querySelectorAll(`.pm-class-card-interactive`).forEach(e=>{let t=async()=>{let t=e.dataset.claseId,n=r.find(e=>String(e.id)===String(t));n&&(f=n.id,await k(),M(n))};e.addEventListener(`click`,t),e.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),t())})})}catch(e){console.error(`[planning] Error renderizando clases:`,e),r.error(`No se pudieron cargar las clases asignadas.`),b(T,`No se pudieron cargar tus clases.`)}}async function k(){if(f)try{let[e,n,r]=await Promise.all([s(f,d).catch(()=>null),t([f]).catch(()=>[]),a(f).catch(()=>({}))]);g=e,w=n,C=r,S=await A(e?.route?.weekly_plan_id);let i=n.map(e=>e.alumno_id).filter(Boolean);x=_(e?.plan,r,i)}catch(e){console.error(`[planning] Error al refrescar datos:`,e)}}async function A(e){return!f||!d||!e?{}:(await i(f,d,e).catch(()=>[])).reduce((e,t)=>(e[String(t.week_number)]=t,e),{})}function j(e){let t=S[String(e.week_number)]||null;return{...e,teacher_strategy:t?.teacher_strategy??e.teacher_strategy,student_activity:t?.student_activity??e.student_activity,homework:t?.homework??e.homework,evidence:t?.evidence??e.evidence,teacher_notes:t?.teacher_notes??``,hasTeacherAdjustment:!!t}}function M(e,t=`general`){E&&=(n.getInstance(E)?.dispose(),E.remove(),null);let i=g?.route,a=g?.plan?.items||[],s=i?.current_week||1,u={general:``,temas:a.length>0?a.length:``,indicadores:x.length>0?x.length:``};function _(e,n,r){let i=u[e];return`
        <button class="pm-tab-btn ${t===e?`active`:``}" data-tab="${e}" type="button">
          ${n} ${r}
          ${i?`<span class="pm-tab-count">${i}</span>`:``}
        </button>
      `}E=document.createElement(`div`),E.className=`modal fade`,E.setAttribute(`tabindex`,`-1`),E.setAttribute(`aria-hidden`,`true`),E.setAttribute(`aria-label`,`Detalle de clase: ${e.nombre}`),E.innerHTML=`
      <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content" style="background:var(--pm-surface); color:var(--pm-text); border:1px solid var(--pm-border); border-radius:20px; box-shadow:0 24px 64px rgba(0,0,0,0.15);">
          ...
          <div class="modal-body" style="padding:1.5rem; min-height:360px;">
            ...
            <div class="pm-tab-pane ${t===`general`?``:`d-none`}" data-pane="general">
              <div class="row g-3">
                <div class="col-12 col-md-6">
                  <div style="padding:1.1rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.015)); height:100%;">
                    ...
                    <div style="font-weight:700; font-size:1.05rem; margin-bottom:0.5rem; color:var(--pm-text);">${p(e.plan_estudio||`Sin plan asignado`)}</div>
                    <p style="font-size:0.82rem; color:var(--pm-text-muted); margin:0; line-height:1.5;">${p(e.descripcion||`Clase de formación activa dentro del plan institucional.`)}</p>
                  </div>
                </div>
                <div class="col-12 col-md-6">
                  <div style="padding:1.1rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.015)); height:100%;">
                    ...
                    ${[[`Instrumento`,p(e.instrumento||`General`)],[`Alumnos inscritos`,`${e.totalStudents||0}`],[`Semanas en plan`,`${a.length}`],[`Semana activa`,`${s}`]].map(([e,t])=>`
                ...
                <div class="col-12">
                  <div style="padding:1.25rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.01));">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem;">
                      ...
                      <span style="font-size:1.6rem; font-weight:800; color:${h(e.progressPercentage)}; min-width:56px; text-align:right;">${e.progressPercentage}%</span>
                    </div>
                    <div style="height:12px; border-radius:999px; background:var(--pm-border); overflow:hidden;">
                      <div style="height:100%; width:${e.progressPercentage}%; background:${h(e.progressPercentage)}; border-radius:999px; transition:width 0.6s ease;"></div>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:0.72rem; color:var(--pm-text-muted); margin-top:0.5rem;">
                      <span>0%</span>
                      <span style="color:${h(e.progressPercentage)}; font-weight:700;">
                        ${e.progressPercentage<30?`Inicial`:e.progressPercentage<70?`En progreso`:`Avanzado`}
                      </span>
                      <span>100%</span>
                    </div>
                  </div>
            ...
            <div class="pm-tab-pane ${t===`temas`?``:`d-none`}" data-pane="temas">
              ${a.length===0?`
                <div class="pm-planning-empty">
            ...
            <!-- ── Pestaña: Indicadores ── -->
            <div class="pm-tab-pane ${t===`indicadores`?``:`d-none`}" data-pane="indicadores">
              ${x.length===0?`
                <div class="pm-planning-empty">
                  <div style="font-size:2.5rem; margin-bottom:0.75rem;">🎯</div>
    ...
    `,document.body.appendChild(E);let v=new n(E),y=E.querySelectorAll(`.pm-tab-btn`),b=E.querySelectorAll(`.pm-tab-pane`);y.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.tab;y.forEach(e=>e.classList.toggle(`active`,e.dataset.tab===t)),b.forEach(e=>e.classList.toggle(`d-none`,e.dataset.pane!==t))})}),E.querySelectorAll(`.pm-week-header`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.week,n=E.querySelector(`#pm-week-body-${t}`),r=e.querySelector(`.pm-week-chevron`),i=n?.classList.contains(`open`);n?.classList.toggle(`open`,!i),r?.classList.toggle(`open`,!i),e.setAttribute(`aria-expanded`,String(!i))})}),E.querySelectorAll(`.pm-week-adjustment-form`).forEach(t=>{t.addEventListener(`submit`,async n=>{n.preventDefault();let a=Number(t.dataset.week),o=new FormData(t),s=t.querySelector(`button[type="submit"]`),u=s?.querySelector(`.btn-text`),f=s?.querySelector(`.spinner-border`);s&&(s.disabled=!0),f?.classList.remove(`d-none`),u&&(u.textContent=`Guardando...`);try{await c({group_id:e.id,teacher_id:d,weekly_plan_id:i?.weekly_plan_id,week_number:a,teacher_strategy:String(o.get(`teacher_strategy`)||``).trim(),student_activity:String(o.get(`student_activity`)||``).trim(),homework:String(o.get(`homework`)||``).trim(),evidence:String(o.get(`evidence`)||``).trim(),teacher_notes:String(o.get(`teacher_notes`)||``).trim()}),r.success(`Ajustes guardados — Semana ${a}.`),l(`Ajuste de la semana ${a} guardado correctamente.`),await k(),M(e,`temas`)}catch(e){console.error(`[planning] Error guardando ajuste:`,e),r.error(e.message||`No se pudieron guardar los ajustes.`)}finally{s?.isConnected&&(s.disabled=!1,f?.classList.add(`d-none`),u&&(u.textContent=`Guardar ajuste`))}})}),E.querySelectorAll(`.btn-mark-seen-group`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t.dataset.indicatorId,i=t.querySelector(`.btn-text`),a=t.querySelector(`.spinner-border`);t.disabled=!0,a?.classList.remove(`d-none`),i&&(i.textContent=`Procesando...`);try{let t=w.filter(t=>String(t.clase_id)===String(e.id)).map(e=>e.alumno_id).filter(Boolean);if(t.length===0){r.warning(`Esta clase no tiene alumnos inscritos para calificar.`);return}await Promise.all(t.map(e=>o(e,n,`achieved`,`Aprobado masivamente`,``,null))),r.success(`Indicador marcado como Dominado para todo el grupo.`),l(`Indicador marcado como dominado para todos los alumnos.`),await k(),M(e,`indicadores`)}catch(e){console.error(`[planning] Error al calificar indicador grupal:`,e),r.error(`Error al actualizar el progreso del indicador.`)}finally{t?.isConnected&&(t.disabled=!1,a?.classList.add(`d-none`),i&&(i.textContent=`🟢 Marcar Grupo`))}})}),E.querySelectorAll(`.btn-toggle-individual`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.indicatorId,i=E.querySelector(`#individual-eval-${n}`),a=E.querySelector(`#alumnos-list-ind-${n}`);if(!i)return;if(!i.classList.contains(`d-none`)){i.classList.add(`d-none`),t.classList.replace(`btn-secondary`,`btn-outline-secondary`);return}if(i.classList.remove(`d-none`),t.classList.replace(`btn-outline-secondary`,`btn-secondary`),a.children.length>0)return;let s=w.filter(t=>String(t.clase_id)===String(e.id)).map(e=>e.alumnos).filter(Boolean);if(s.length===0){a.innerHTML=`<div class="col-12 text-muted" style="font-size:0.82rem;">Sin alumnos inscritos en esta clase.</div>`;return}a.innerHTML=s.map(e=>{let t=(C||{})[`${e.id}_${n}`]?.status||`not_started`;return`
            <div class="col-12 col-sm-6" style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem 0.65rem; border-radius:10px; border:1px dashed var(--pm-border); gap:0.5rem;">
              ...
              <select class="form-select form-select-sm select-student-indicator"
                data-student-id="${e.id}" data-indicator-id="${n}"
          ...
          `}).join(``),a.querySelectorAll(`.select-student-indicator`).forEach(i=>{i.addEventListener(`change`,async()=>{let a=i.dataset.studentId,s=i.value;i.disabled=!0;try{await o(a,n,s,`Calificación individual`,``,null),r.success(`Calificación guardada.`),l(`Calificación del alumno guardada.`),await k();let i=C||{},c=w.filter(t=>String(t.clase_id)===String(e.id)).map(e=>e.alumno_id).filter(Boolean),u=c.filter(e=>[`achieved`,`exceeded`].includes(i[`${e}_${n}`]?.status||`not_started`)).length,d=c.length,f=d>0?Math.round(u/d*100):0,p=h(f),m=t.closest(`.pm-indicator-card`),g=m?.querySelector(`.ind-progress-bar`),_=m?.querySelector(`.ind-progress-bar`)?.parentElement?.previousElementSibling?.querySelector(`span:last-child`);g&&(g.style.width=`${f}%`,g.style.background=p),_&&(_.textContent=`${f}%`),t.textContent=`👥 ${u}/${d}`}catch(e){console.error(`[planning] Error actualizando indicador:`,e),r.error(`No se pudo guardar la calificación.`)}finally{i.disabled=!1}})})})}),E.addEventListener(`hidden.bs.modal`,()=>{n.getInstance(E)?.dispose(),E.remove(),E=null,f&&D(f)},{once:!0}),v.show(),l(`Panel de clase ${e.nombre} abierto.`)}await O()}export{x as renderPlanificacionView};