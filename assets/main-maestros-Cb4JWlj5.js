...
`).trim():``}var _o={LOGRADO:{label:`Logrado`,color:`#198754`,bg:`#19875418`,icon:`✅`},EN_PROGRESO:{label:`En progreso`,color:`#0d6efd`,bg:`#0d6efd18`,icon:`📈`},INICIADO:{label:`Iniciado`,color:`#6c757d`,bg:`#6c757d18`,icon:`🔰`},DIFICULTAD:{label:`Dificultad`,color:`#dc3545`,bg:`#dc354518`,icon:`⚠️`}};async function vo(e,t,n=0){let r=e.querySelector(`#pm-alumno-progresos-root`);if(!r)return;let i=n===0;i&&(r.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`);try{let{data:a,error:o}=await j.from(`progresos`).select(`id, contenido_dsl, estado_cualitativo, calificacion, observaciones, fecha_evaluacion, clase_id, indicadores`).eq(`alumno_id`,t).not(`contenido_dsl`,`is`,null).neq(`contenido_dsl`,``).order(`fecha_evaluacion`,{ascending:!1}).range(n,n+19);if(o)throw o;if(!a||a.length===0){i&&(r.innerHTML=`<p style="font-size:0.82rem;color:var(--pm-text-muted);text-align:center;padding:1rem 0;">Sin registros de progreso generados por IA aún.</p>`);return}let s=[...new Set(a.map(e=>e.clase_id).filter(Boolean))],{data:c}=s.length?await j.from(`clases`).select(`id, nombre`).in(`id`,s):{data:[]},l=new Map((c||[]).map(e=>[e.id,e.nombre])),u=new Map;for(let e of a){let t=e.contenido_dsl;u.has(t)||u.set(t,{contenido:t,entries:[]}),u.get(t).entries.push(e)}let d=Array.from(u.values()).map(({contenido:e,entries:t})=>{let n=t[0],r=_o[n.estado_cualitativo]??_o.EN_PROGRESO,i=new Date(n.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`2-digit`});return`
        <details class="pm-prog-card">
          ...
          <div class="pm-prog-card__timeline">
            ${t.map(e=>{let t=_o[e.estado_cualitativo]??_o.EN_PROGRESO,n=new Date(e.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`numeric`}),r=l.get(e.clase_id)||`Clase`,i=e.indicadores?.tarea,a=e.calificacion==null?null:`${e.calificacion}/5`;return`
              ...
              <div class="pm-eval-indicador ${g(e.latest.nota)}" data-ind-id="${e.id}">
                <div class="pm-eval-indicador-header">
                  ...
                  <div class="pm-eval-info">
                    <span class="pm-eval-nombre">${I(e.nombre||`Indicador`)}</span>
                    ${e.nodeName?`<span class="pm-eval-node">${I(e.nodeName)}</span>`:``}
                  </div>
                ...
                <div class="pm-eval-timeline" style="display:none;">
                  ${e.history.map(e=>`
                    <div class="pm-eval-entry">
                      ...
                      ${e.observations?`<p class="pm-eval-entry-obs">${I(e.observations)}</p>`:``}
                      ${e.tarea?`<p class="pm-eval-entry-tarea">📋 ${I(e.tarea)}</p>`:``}
                    </div>
                  `).join(``)}
                </div>
              </div>
    ...
    `,n.querySelectorAll(`.pm-eval-indicador`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.querySelector(`.pm-eval-timeline`),n=e.querySelector(`.pm-eval-toggle`),r=t.style.display!==`none`;t.style.display=r?`none`:`block`,n.classList.toggle(`bi-chevron-down`,r),n.classList.toggle(`bi-chevron-up`,!r)})})}catch(t){let n=e.querySelector(`#pm-alumno-progreso-root`);n&&(n.innerHTML=`<p class="pm-empty" style="color:var(--pm-danger);">Error al cargar evaluaciones: ${I(t.message)}</p>`)}}async function bo(e,{alumnoId:t}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let n=M();if(!n){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}if(!t){e.innerHTML=`<p class="pm-empty">No se especificó el alumno.</p>`;return}try{let{data:r,error:i}=await j.from(`alumnos`).select(`id, nombre_completo, instrumento_principal, tlf_alumno, fecha_nacimiento, created_at, nivel_actual, representante_nombre, representante_tlf, correo_representante, direccion`).eq(`id`,t).single();if(i||!r){console.error(`[AlumnoPerfil] Error al obtener alumno:`,i),e.innerHTML=`
        ...
        <div class="pm-zen-body">
          ...
          <div class="pm-zen-section">
            ...
            <div class="pm-zen-bitacora">
              ${k.map(e=>{let t=O.get(e.clase_id),n=go(e.contenido_dsl);return`
    ...
    `,e.querySelector(`#pm-alumno-back`).onclick=()=>window.history.back();let ie=lo(r.tlf_alumno||r.representante_tlf);if(ie){let t=r.nombre_completo||`el alumno`,i=r.representante_nombre||t,a={alumno:t,contacto:i},o=document.createElement(`div`);o.id=`pm-wa-modal`,o.innerHTML=`
    ...
    `}}var xo={scales:`🎼`,arpeggios:`🎹`,left_hand:`✋`,bow:`🎻`,sound:`🔊`,intonation:`🎵`,studies:`⚙️`,repertoire:`📖`},So={approved:`#34C759`,in_process:`#007AFF`,pending:`#ccc`,failed:`#FF3B30`},Co={approved:`Aprobado`,in_process:`En proceso`,pending:`Pendiente`,failed:`Fallido`};function wo(e){let t=(e||``).toLowerCase();for(let[e,n]of Object.entries(xo))if(t.includes(e))return n;return t.includes(`escala`)?`🎼`:t.includes(`arpegio`)?`🎹`:t.includes(`mano`)||t.includes(`izquierda`)?`✋`:t.includes(`arco`)?`🎻`:t.includes(`sonido`)?`🔊`:t.includes(`afinación`)||t.includes(`entonación`)?`🎵`:t.includes(`estudio`)?`⚙️`:t.includes(`repertorio`)||t.includes(`obra`)?`📖`:`📋`}async function To(e){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let t=M();if(!t){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}try{let{data:n}=await j.from(`clases`).select(`id, nombre`).eq(`maestro_id`,t.id).order(`nombre`);if(!n||n.length===0){e.innerHTML=`<p class="pm-empty">No tienes clases asignadas.</p>`;return}let{data:r}=await j.from(`inscripciones`).select(`alumno_id, alumnos(id, nombre, apellido)`).in(`clase_id`,n.map(e=>e.id));e.innerHTML=`
                  ...
                  ${n.map(e=>{let t=s?.find(t=>t.node_id===e.id),n=t?.status||`pending`,r=e.indicators||[],i=t?.indicator_attempts||[];return`
                      <div class="pm-node-card" onclick="this.classList.toggle('expanded')">
                        ...
                        <div class="pm-node-info">
                          ...
                          <span class="pm-node-status">${Co[n]}</span>
                        </div>
                        ...
                        <div class="pm-indicators">
                          ${r.length===0?`<p style="font-size:0.7rem;color:var(--pm-text-muted);">Sin indicadores</p>`:r.map(e=>{let t=i.find(t=>t.indicator_id===e.id);return`
                                <div class="pm-indicator">
                                  <span class="pm-indicator-dot" style="background:${So[t?.status||`pending`]};"></span>
  ...
  `,e.querySelector(`.block-section-header`)?.addEventListener(`click`,()=>{o(n)})}var Mo=()=>typeof global<`u`&&global.getMaestroLocal?global.getMaestroLocal():M(),No=(...e)=>typeof global<`u`&&global.getMisClases?global.getMisClases(...e):O(...e),Po=(...e)=>typeof global<`u`&&global.loadRouteTree?global.loadRouteTree(...e):Oo(...e),Y={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:null},Fo=!1;async function Io(e,{onTopicSelected:t}={}){if(Y={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:t},e.innerHTML=`<div class="pm-ruta-gamificada"><div class="pm-loading"><div class="pm-spinner"></div></div></div>`,!Mo()){e.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No hay sesión activa.</p></div>`;return}try{if(T(),Y.clases=await No(!0),!Y.clases?.length){e.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No tenés clases asignadas.</p></div>`;return}Y.activeClaseId=Y.clases[0].id,await Lo(),Ro(e),Ho(e),Uo(e)}catch(t){console.error(`[rutaGameificadaView]`,t),e.innerHTML=`<div style="color:red;padding:20px;"><i class="bi bi-exclamation"></i> Error: ${t.message}</div>`}}async function Lo(){Y.loading=!0,Y.rutaId=await Do(Y.activeClaseId),Y.rutaId?Y.blocks=await Po(Y.rutaId,Y.activeClaseId):Y.blocks=[],Y.loading=!1}function Ro(e){e.innerHTML=`
  ...
  `;let r=Y.expandedLevels.has(e.id);return n.innerHTML=`
    <div class="level-header" style="padding: 12px 16px; display: flex; align-items: center; gap: 12px; cursor: ${e.locked?`not-allowed`:`pointer`};">
      <div class="level-icon" style="width: 32px; height: 32px; border-radius: 8px; background: ${Vo(e.semaphore)}; display: flex; align-items: center; justify-content: center; color: white;">
      ...
      <div style="flex: 1;">
        ...
        <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
          <div style="flex: 1; height: 6px; background: #e2e8f0; border-radius: 3px; overflow: hidden;">
            <div style="width: ${e.percentage}%; height: 100%; background: ${Vo(e.semaphore)}; transition: width 0.3s ease;"></div>
          </div>
      ...
      ${(e.nodes||[]).map(t=>`
        <div class="node-item" style="margin-bottom: 12px;">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
            <span style="width: 8px; height: 8px; border-radius: 50%; background: ${Vo(t.semaphore)};"></span>
  ...
  `,e.locked||n.querySelector(`.level-header`).addEventListener(`click`,()=>{Y.expandedLevels.has(e.id)?Y.expandedLevels.delete(e.id):Y.expandedLevels.add(e.id),Ro(t)}),n.querySelectorAll(`.indicator-row`).forEach(e=>{e.addEventListener(`click`,n=>{n.stopPropagation(),Y.selectedIndicator={id:e.dataset.id,nombre:e.dataset.nombre,nodeNombre:e.dataset.node,levelNombre:e.dataset.level,blockNombre:e.dataset.block},Ro(t)})}),n}function Bo(e){let t=e.querySelector(`#ruta-action-panel`);if(!Y.selectedIndicator){t.innerHTML=``;return}t.innerHTML=`
  ...
  `,t.querySelector(`#btn-cancel-select`).addEventListener(`click`,()=>{Y.selectedIndicator=null,Ro(e)}),t.querySelector(`#btn-use-topic`).addEventListener(`click`,()=>{Si({...Y.selectedIndicator,indicatorId:Y.selectedIndicator.id,claseId:Y.activeClaseId}),Y.onTopicSelected&&Y.onTopicSelected(Y.activeClaseId)})}function Vo(e){switch(e){case`green`:return`#22c55e`;case`yellow`:return`#eab308`;case`gray`:return`#94a3b8`;default:return`#94a3b8`}}function Ho(e){e.querySelector(`#ruta-clase-select`)?.addEventListener(`change`,async t=>{Y.activeClaseId=t.target.value,e.innerHTML=`<div class="pm-ruta-gamificada"><div class="pm-loading"><div class="pm-spinner"></div></div></div>`,await Lo(),Ro(e),Ho(e)})}function Uo(e){Fo||(Fo=!0,Ao.on(`node-covered`,()=>{Lo().then(()=>{Ro(e),Ho(e)})}))}var Wo=[`lunes`,`martes`,`miércoles`,`jueves`,`viernes`,`sábado`,`domingo`];async function Go(e){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let t=M();if(!t){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}try{let{data:n}=await j.from(`system_config`).select(`value`).eq(`key`,`maestros_pueden_crear_clases`).maybeSingle();if(n?.value!==`true`){e.innerHTML=`
          ...
          <div id="nueva-clase-horarios">
            <div class="pm-horario-row" data-index="0">
              <select class="pm-input pm-horario-dia">
                ${Wo.map(e=>`<option value="${e}">${e.charAt(0).toUpperCase()+e.slice(1)}</option>`).join(``)}
              </select>
    ...
    `}}async function Ko(e,{alumnoId:t}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;try{let n=await F.getStudent(t),r=await F.fetchRoutes();e.innerHTML=`
      <div class="pm-asist-header">
    ...
    `,this.loadRoutes(e),e},async loadRoutes(e){let t=e.querySelector(`#routes-grid`);try{let n=await F.fetchRoutes();if(!n||n.length===0){t.innerHTML=`
      ...
      `}}},Yo={async render(e){let t=e?.id,n=new URLSearchParams(window.location.hash.split(`?`)[1]).get(`studentId`),r=document.createElement(`div`);return r.className=`pm-view pm-animate-fade-in`,r.innerHTML=`
    ...
    `,r.querySelector(`#back-to-library`).onclick=()=>{window.location.hash=`#/ruta-libreria`},t?this.loadRouteDetail(r,t,n):r.innerHTML=`<div class="pm-placeholder"><i class="bi bi-x-circle"></i><p>ID de ruta no proporcionado.</p></div>`,r},async loadRouteDetail(e,t,n){let r=e.querySelector(`#route-header-content`),i=e.querySelector(`#route-hierarchy`);try{let[e,a]=await Promise.all([F.getRouteDetail(t),n?F.getStudentProgress(n,t):Promise.resolve([])]),o=new Map(a.map(e=>[e.node_id,e]));if(r.innerHTML=`
      ...
      `).join(``)}catch(e){console.error(`Error al cargar detalle de ruta:`,e),i.innerHTML=`<p class="pm-error-msg">Error al cargar la estructura académica.</p>`}}},X={maestro:null,permisos:null,clases:[],submitting:!1,checked:!1};async function Xo(e){let t=M();if(!t){e.innerHTML=`
      <div class="pm-settings-empty">
        <i class="bi bi-shield-lock"></i>
        <p>No hay sesión activa. Inicia sesión para continuar.</p>
      </div>`;return}X.maestro=t,X.submitting=!1;let n=await Ke(t.id);if(X.permisos=n,X.checked=!0,!n.puede_registrar_alumnos){Zo(e,n,t.id);return}try{let e=await O();X.clases=Array.isArray(e)?e:[]}catch(e){console.warn(`[RegistroAlumno] Error cargando clases:`,e.message),X.clases=[]}Qo(e),ts(),rs()}function Zo(e,t,n){let r=t?.solicitudes||[],i=t?.solicitud_actual;e.innerHTML=`
        ...
        <div id="pm-register-action-container">
          ...
          `}
            ...
            <div style="background: rgba(234, 179, 8, 0.1); border: 1px solid rgba(234, 179, 8, 0.3); padding: 0.85rem; border-radius: 10px; display: inline-flex; align-items: center; gap: 8px; color: #eab308; font-weight: 600; font-size: 0.85rem; animation: pm-error-fade-in 0.3s ease;">
              <i class="bi bi-clock-history"></i> Solicitud Pendiente de Aprobación
            </div>`)}catch(t){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Error al solicitar: `+t.message,type:`danger`}})),a.disabled=!1,a.innerHTML=e}})}function Qo(e){e.innerHTML=`
    <div class="pm-settings pm-fade-in registro-alumno-view" role="main" aria-label="Registro de alumnos">
      <header class="pm-settings-header">
  ...
  `}function hs(e){let t=is(e.nombre||`Clase sin nombre`),n=os(e.horarios||[]),r=is(e.nivel||``),i=e.capacidad_maxima??e.max_alumnos??`–`;return`
  ...
  `}async function gs(e,t){ss=e,document.querySelectorAll(`.gcv-clase-card`).forEach(e=>e.classList.remove(`active`)),document.getElementById(`gcv-card-${e}`)?.classList.add(`active`);let n=document.getElementById(`gcv-panel`);if(!n)return;let r=t.find(t=>t.id===e);if(r){n.innerHTML=`<div class="gcv-loading"><div class="gcv-spinner"></div></div>`;try{let i=await He(e),a=i.map(e=>e.alumno).filter(Boolean);ls=new Set(i.map(e=>e.alumno_id)),n.innerHTML=_s(r,a,cs.filter(e=>!ls.has(e.id))),xs(e,t)}catch(e){n.innerHTML=ws(`bi-exclamation-circle`,`Error al cargar alumnos`,is(e.message))}}}function _s(e,t,n){return`
    <div class="gcv-panel-inner">
      <div class="gcv-panel-header">
        <h3 class="gcv-panel-title"><i class="bi bi-people-fill"></i> ${is(e.nombre||`Clase`)}</h3>
      ...
      <div class="gcv-section">
        ...
        <div id="gcv-lista-inscritos" class="gcv-student-list">
          ${t.length===0?`<p class="gcv-empty-list">Sin alumnos inscritos aún.</p>`:t.map(e=>vs(e)).join(``)}
        </div>
      </div>
      ...
      <div class="gcv-section">
        ...
        <div id="gcv-lista-disponibles" class="gcv-student-list gcv-available-list">
          ${n.length===0?`<p class="gcv-empty-list">Todos los alumnos activos ya están inscritos.</p>`:n.map(e=>ys(e)).join(``)}
        </div>
        ${n.length>0?`
  ...
  `}function vs(e){let t=is(e.nombre_completo||e.nombre||`Alumno`),n=is(e.instrumento_principal||e.instrumento||``);return`
    <div class="gcv-student-row inscrito-item"
         data-alumno-id="${e.id}"
         data-name="${t.toLowerCase()}"
         data-instrumento="${n.toLowerCase()}">
      <div class="gcv-student-avatar gcv-avatar-success">${as(t)}</div>
  ...
  `}var Ts=null,Es=!1,Ds={enfermedad:`Médica`,personal:`Personal`,capacitacion:`Capacitación`,vacaciones:`Vacaciones`,otro:`Otro`};function Os(e){if(!e)return``;let[t,n,r]=e.split(`-`);return`${r}/${n}/${t}`}function ks(e){return e===`alta`?`<i class="bi bi-exclamation-circle-fill" style="color:#ef4444"></i>`:e===`media`?`<i class="bi bi-exclamation-circle-fill" style="color:#f59e0b"></i>`:`<i class="bi bi-info-circle-fill" style="color:#22c55e"></i>`}function As(){if(document.getElementById(`admin-ausencias-insights-styles`))return;let e=document.createElement(`style`);e.id=`admin-ausencias-insights-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}function js(e){document.querySelectorAll(`[data-route="admin-ausencias"],[data-route="admin-notificaciones"]`).forEach(t=>{let n=t.querySelector(`.aai-nav-badge`);e>0?(n||(n=document.createElement(`span`),n.className=`aai-nav-badge`,t.appendChild(n)),n.textContent=e>99?`99+`:String(e),n.style.display=`flex`):n&&(n.style.display=`none`)})}function Ms(){return Ts||(Ts=document.createElement(`div`),Ts.id=`admin-ausencias-insights`,Ts.setAttribute(`role`,`status`),Ts.setAttribute(`aria-live`,`polite`),document.body.appendChild(Ts),Ts)}function Ns(e){let t=Ms(),n=e.length,r=e[0],i=n===1&&r?`
    <div class="aai-preview">
      <div class="aai-preview-row">
        ${ks(r.urgencia)}
        <span>${r.maestros?.nombre_completo??`Maestro`}</span>
        <span class="aai-chip">${Ds[r.tipo_ausencia]??r.tipo_ausencia??`—`}</span>
        <span style="opacity:.7">${Os(r.fecha_inicio)}${r.fecha_fin&&r.fecha_fin!==r.fecha_inicio?` → `+Os(r.fecha_fin):``}</span>
      </div>
    </div>
  `:``;t.innerHTML=`
    <div class="aai-row">
  ...
  `,requestAnimationFrame(()=>{t.classList.add(`visible`)}),t.querySelector(`#aai-btn-revisar`)?.addEventListener(`click`,()=>{Ps(),window.router&&window.router.navigate(`admin-ausencias`)}),t.querySelector(`#aai-btn-dismiss`)?.addEventListener(`click`,()=>{Fs()})}function Ps(){Ts&&Ts.classList.remove(`visible`)}function Fs(){Ps();let e=Date.now()+1800*1e3;localStorage.setItem(`admin-ausencias-insights-dismiss`,String(e))}function Is(){let e=localStorage.getItem(`admin-ausencias-insights-dismiss`);return e?Date.now()<Number(e):!1}var Ls={init(){window.adminAusenciasInsights=this,As(),Ms(),this.evaluate()},async evaluate(){if(!Es){Es=!0;try{if((window.router?.currentRoute?.()??``).split(`?`)[0]===`admin-ausencias`){Ps(),js((await D())?.length??0);return}if(Is())return;let e=await D();if(js(e?.length??0),!e||e.length===0){Ps();return}Ns(e)}catch(e){console.warn(`[adminAusenciasInsights] Error evaluando:`,e),Ps()}finally{Es=!1}}}},Rs=new class{constructor(){this.storageKey=`portal-maestros-theme`,this.init()}init(){let e=localStorage.getItem(this.storageKey),t=window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`;this.currentTheme=e||t,this.applyTheme(this.currentTheme),window.matchMedia(`(prefers-color-scheme: dark)`).addEventListener(`change`,e=>{localStorage.getItem(this.storageKey)||(this.currentTheme=e.matches?`dark`:`light`,this.applyTheme(this.currentTheme))})}applyTheme(e){document.documentElement.setAttribute(`data-bs-theme`,e),document.documentElement.setAttribute(`data-portal-theme`,e),this.updateCustomProperties(e)}updateCustomProperties(e){let t=document.documentElement;e===`dark`?(t.style.setProperty(`--pm-glass-bg`,`rgba(30, 41, 59, 0.8)`),t.style.setProperty(`--pm-glass-border`,`rgba(255, 255, 255, 0.1)`),t.style.setProperty(`--pm-header-glass`,`rgba(15, 23, 42, 0.95)`)):(t.style.setProperty(`--pm-glass-bg`,`rgba(255, 255, 255, 0.8)`),t.style.setProperty(`--pm-glass-border`,`rgba(0, 0, 0, 0.1)`),t.style.setProperty(`--pm-header-glass`,`rgba(242, 242, 247, 0.95)`))}toggle(){this.currentTheme=this.currentTheme===`dark`?`light`:`dark`,this.applyTheme(this.currentTheme),localStorage.setItem(this.storageKey,this.currentTheme),window.dispatchEvent(new CustomEvent(`themeChanged`,{detail:{theme:this.currentTheme}}))}getCurrentTheme(){return this.currentTheme}createToggleButton(){let e=document.createElement(`button`);return e.className=`pm-theme-toggle`,e.setAttribute(`aria-label`,`Cambiar tema`),e.innerHTML=`
    ...
    `,e.addEventListener(`click`,()=>{this.toggle(),this.updateButtonIcon(e)}),window.addEventListener(`themeChanged`,()=>{this.updateButtonIcon(e)}),e}updateButtonIcon(e){let t=e.querySelector(`.pm-theme-icon`);t&&(t.className=`bi ${this.currentTheme===`dark`?`bi-moon-fill`:`bi-sun-fill`} pm-theme-icon`)}};`serviceWorker`in navigator&&navigator.serviceWorker.addEventListener(`message`,e=>{if(e.data?.type===`NAVIGATE_TO`){let t=e.data.url||e.data.hash;t&&(window.location.hash=t.startsWith(`#`)?t.slice(1):t)}});var zs=null;function Bs(e){let t=new Date(e),n=new Date-t,r=Math.floor(n/1e3),i=Math.floor(r/60),a=Math.floor(i/60),o=Math.floor(a/24),s=new Intl.RelativeTimeFormat(`es`,{numeric:`auto`});return o>0?s.format(-o,`day`):a>0?s.format(-a,`hour`):i>0?s.format(-i,`minute`):`hace un momento`}var Vs={init(){document.getElementById(`pm-notificaciones-drawer-overlay`)||(zs=document.createElement(`div`),zs.innerHTML=`
    ...
    `,document.body.appendChild(zs),document.getElementById(`pm-notificaciones-close`).addEventListener(`click`,this.close),document.getElementById(`pm-notificaciones-drawer-overlay`).addEventListener(`click`,e=>{e.target.id===`pm-notificaciones-drawer-overlay`&&this.close()}),document.getElementById(`pm-notif-mark-all`).addEventListener(`click`,()=>{_a()}),da(e=>{this.renderList(e)}),pa())},_updateDedupBadge(){let e=document.getElementById(`pm-notif-dedup-badge`);if(!e)return;let t=oa();t>0?(e.textContent=`🔄 ${t} dedup`,e.style.display=`inline-flex`):e.style.display=`none`},renderList(e){let t=document.getElementById(`pm-notificaciones-list`);if(t){if(this._updateDedupBadge(),e.length===0){t.innerHTML=`
      ...
      `;return}t.innerHTML=Ws(e).map(e=>{let t=e.count>1,n=e.items.some(e=>e.estado!==`leida`),r=Gs(e.tipo,e.items[0]),i=e.tipo===`sesion_sin_registrar`;return`
        <div
        ...
        >
          ...
          <div class="pm-notif-content">
            ...
            <div class="pm-notif-footer-row">
              <span class="pm-notif-time">${Bs(e.items[0].created_at)}</span>
    ...
    <div class="pm-main-area">
      ...
      <header class="pm-header" id="pm-header">
        <div class="pm-header-left" id="pm-header-left">
          <span class="pm-header-greeting">${Ks?`Panel Admin`:`Portal Maestros`}</span>
          <span class="pm-header-title" style="font-size:clamp(1rem,3.5vw,1.5rem);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:52vw;">
            ${Ks?t?.nombre_completo??`Administrador`:`Prof. `+(t?.nombre_completo??``)}
  ...
  `,tc();let i=document.getElementById(`pm-theme-toggle-container`);i&&i.appendChild(Rs.createToggleButton());let a=document.getElementById(`pm-footer-nav`);a&&a.querySelectorAll(`.pm-nav-tab`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),Z.navigate(e.dataset.route)})});let o=document.getElementById(`pm-sidebar`);o&&o.querySelectorAll(`.pm-sidebar-link`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),Z.navigate(e.dataset.route)})}),document.getElementById(`pm-btn-perfil`).addEventListener(`click`,e=>{e.preventDefault(),Z.navigate(`perfil`)});let s=document.getElementById(`pm-header`),c=document.getElementById(`pm-header-search-input`),l=document.getElementById(`pm-search-toggle-btn`),u=document.getElementById(`pm-search-back-btn`),d=()=>{s?.classList.add(`search-active`),setTimeout(()=>{c?.focus()},50)},f=()=>{s?.classList.remove(`search-active`),c&&(c.value=``),document.getElementById(`pm-header-search-dropdown`)?.remove()};l?.addEventListener(`click`,e=>{e.stopPropagation(),d()}),u?.addEventListener(`click`,e=>{e.stopPropagation(),f()});let p=null,m=null,h=()=>{p?.remove(),p=null},g=e=>{if(h(),!e.length)return;let t=document.createElement(`div`);t.id=`pm-header-search-dropdown`,t.setAttribute(`role`,`listbox`),t.innerHTML=e.map(e=>`
      ...
      </div>`).join(``),document.body.appendChild(t);let n=c.getBoundingClientRect();t.style.cssText=`position:fixed;top:${n.bottom+4}px;left:${Math.max(8,n.left)}px;width:${Math.min(320,window.innerWidth-16)}px;z-index:9999;background:var(--pm-surface);border:1px solid var(--pm-border);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.18);overflow:hidden;`,p=t,t.querySelectorAll(`.pm-hsd-item`).forEach(e=>{let t=()=>{f(),h(),Z.navigate(`alumno?id=${e.dataset.id}`)};e.addEventListener(`click`,t),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&t()})})};if(c?.addEventListener(`input`,()=>{let e=c.value.trim();if(clearTimeout(m),e.length<1){h();return}let t=qt();if(t){let n=e.toLowerCase();g(t.filter(e=>e.nombre_completo?.toLowerCase().includes(n)).slice(0,8).map(e=>({...e,instrumento_principal:e.clases?.join(`, `)||null})));return}m=setTimeout(async()=>{try{let{data:t}=await j.from(`alumnos`).select(`id, nombre_completo, instrumento_principal`).ilike(`nombre_completo`,`%${e}%`).limit(8);g(t||[])}catch{h()}},200)}),c?.addEventListener(`keydown`,e=>{e.key===`Escape`&&(f(),h())}),!document.getElementById(`pm-hsd-styles`)){let e=document.createElement(`style`);e.id=`pm-hsd-styles`,e.textContent=`.pm-hsd-item{display:flex;align-items:center;gap:0.625rem;padding:0.75rem 1rem;cursor:pointer;border-bottom:1px solid var(--pm-border);transition:background 0.1s}.pm-hsd-item:last-child{border-bottom:none}.pm-hsd-item:hover,.pm-hsd-item:focus{background:var(--pm-surface-2);outline:none}.pm-hsd-icon{font-size:1rem;color:var(--pm-primary);flex-shrink:0}.pm-hsd-info{flex:1;min-width:0}.pm-hsd-name{display:block;font-size:0.875rem;font-weight:500;color:var(--pm-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pm-hsd-meta{font-size:0.7rem;color:var(--pm-text-muted)}.pm-hsd-arrow{color:var(--pm-text-muted);font-size:0.75rem}`,document.head.appendChild(e)}document.addEventListener(`click`,e=>{!c?.contains(e.target)&&!p?.contains(e.target)&&h()}),document.getElementById(`pm-sync-indicator`).addEventListener(`click`,async e=>{e.target.classList.contains(`error`)&&await nc()}),document.getElementById(`pm-bell-btn`)?.addEventListener(`click`,()=>{Vs.open()});let _=(Z.currentRoute?.()||`hoy`).split(`?`)[0];fc(_)}var uc=!1;function dc(){if(uc)return;if(uc=!0,da(()=>{let e=document.getElementById(`pm-notif-badge`);if(!e)return;let t=va();t>0?(e.textContent=t>9?`9+`:t,e.style.display=`flex`):e.style.display=`none`}),pa(),ba(),!Ks){qs!==void 0&&qs&&(j.removeChannel(qs),qs=null);let e=Zs;e?.id&&(qs=j.channel(`permisos-maestro:${e.id}`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`permisos_maestros`,filter:`maestro_id=eq.${e.id}`},async t=>{console.log(`[Realtime] Permisos actualizados:`,t.new);try{let t=await Ke(e.id),n=document.getElementById(`portal-app`);if(!n)return;let r=[];t.puede_registrar_alumnos&&!Qs?.puede_registrar_alumnos&&r.push(`Registrar Alumnos`),t.puede_inscribir_clases&&!Qs?.puede_inscribir_clases&&r.push(`Gestionar e Inscribir Clases`);let i=[];Qs?.puede_registrar_alumnos&&!t.puede_registrar_alumnos&&i.push(`Registrar Alumnos`),Qs?.puede_inscribir_clases&&!t.puede_inscribir_clases&&i.push(`Gestionar e Inscribir Clases`);let a=(Z.currentRoute?.()||`perfil`).split(`?`)[0],o=a===`registrar-alumno`&&!t.puede_registrar_alumnos||a===`gestionar-clases`&&!t.puede_inscribir_clases?`hoy`:a;a===`pending-approval`&&r.length>0&&(o=`hoy`),lc(n,e,t),hc(),oc(),Z.setAuthGuard(()=>yt.isAuthenticated(),[`login`,`register`,`pending-approval`]),mc.clear(),await $(o),Z.navigate(o),r.length>0?P.success(`¡Nuevos permisos activados: ${r.join(`, `)}! Ahora podés acceder desde el Perfil o la barra de navegación.`):i.length>0?P.show(`El administrador removió tu acceso a: ${i.join(`, `)}.`,`warning`):P.show(`Tus permisos fueron actualizados por el administrador.`,`info`)}catch(e){console.warn(`[Realtime] Error actualizando permisos:`,e.message)}}).subscribe(e=>{console.log(`[Realtime] Canal permisos_maestros:`,e)}),window.addEventListener(`beforeunload`,()=>{j.removeChannel(permisosChannel)},{once:!0}))}document.addEventListener(`keydown`,e=>{if(sc()!==`desktop`||e.target.tagName===`INPUT`||e.target.tagName===`TEXTAREA`)return;window._globalAppKeys||(window._globalAppKeys=[]);let t=window._globalAppKeys;if(t.push(e.key.toLowerCase()),t[t.length-2]===`g`)switch(e.key.toLowerCase()){case`h`:Z.navigate(`hoy`),t.length=0;break;case`c`:Z.navigate(`calendario`),t.length=0;break;case`r`:Z.navigate(`ruta`),t.length=0;break;case`m`:Z.navigate(`metricas`),t.length=0;break;case`p`:Z.navigate(`perfil`),t.length=0;break;default:break}t.length>3&&t.splice(0,t.length-2)});let e=null;window.addEventListener(`resize`,()=>{clearTimeout(e),e=setTimeout(()=>{let e=sc();if(e!==cc){cc=e,document.body.dataset.pmLayout=e,lc(document.getElementById(`portal-app`),Zs),hc();let t=(Z.currentRoute?.()||`hoy`).split(`?`)[0];fc(t)}},250)},{passive:!0})}function fc(e){document.querySelectorAll(`.pm-nav-tab`).forEach(t=>{t.classList.toggle(`active`,t.dataset.route===e)}),document.querySelectorAll(`.pm-sidebar-link`).forEach(t=>{t.classList.toggle(`active`,t.dataset.route===e)})}var pc={},Q=null,mc=new Set;function hc(){let e=document.getElementById(`pm-view-container`);e&&(e.innerHTML=``,[`login`,`logout`,`register`,`pending-approval`,`calendario`,`clases`,`hoy`,`asistencia`,`metricas`,`perfil`,`clase-emergente`,`planificacion`,`alumno`,`gamificacion`,`ruta`,`crear-clase`,`ruta-plan-builder`,`ruta-semanal`,`ruta-libreria`,`ruta-detalle`,`registrar-alumno`,`gestionar-clases`].forEach(t=>{let n=document.createElement(`div`);n.id=`pm-view-${t}`,n.className=`pm-view-content`,n.style.display=`none`,e.appendChild(n),pc[t]=n}),Ks&&[`admin-alumnos`,`admin-programas`,`admin-maestros`,`admin-metricas`,`admin-config`,`admin-clases`,`admin-sesiones`,`admin-aprobacion`,`admin-ausencias`,`admin-notificaciones`].forEach(t=>{let n=document.createElement(`div`);n.id=`pm-view-${t}`,n.className=`pm-view-content`,n.style.display=`none`,e.appendChild(n),pc[t]=n}))}async function $(e,t={},{silent:n=!1}={}){let r=window.location.hash.includes(`?`)?window.location.hash.split(`?`)[1]:``,i=new URLSearchParams(r),a=e.split(`?`)[0];if(!n){let e=document.getElementById(`pm-header`);if(e&&e.classList.contains(`search-active`)){e.classList.remove(`search-active`);let t=document.getElementById(`pm-header-search-input`);t&&(t.value=``)}fc(a),window.pwaInstaller&&window.pwaInstaller.evaluateInsights(),window.adminAusenciasInsights&&window.adminAusenciasInsights.evaluate()}let o=pc[a];if(!o){console.warn(`[Router] Contenedor no encontrado: ${a}`);return}if(n||(typeof Q==`function`&&(console.log(`[Router] Ejecutando cleanup de vista anterior...`),Q(),Q=null),Object.values(pc).forEach(e=>{e.style.display=`none`,e.classList.remove(`active`)}),o.style.display=`block`,o.offsetHeight,o.classList.add(`active`)),mc.has(a))return;let s=setTimeout(()=>{o.querySelectorAll(`.pm-loading-overlay`).forEach(e=>e.remove());let e=document.createElement(`div`);e.className=`pm-loading pm-loading-overlay`,e.innerHTML=`<div class="pm-spinner"></div>`,o.prepend(e)},300);try{switch(a){case`login`:Ot(o,{onSuccess:()=>yc()});break;case`register`:kt(o,{onSuccess:()=>Z.navigate(`pending-approval`)});break;case`pending-approval`:At(o,{onBackToLogin:()=>Z.navigate(`login`)});break;case`logout`:ac(),xa(),bt().then(()=>window.location.reload());break;case`calendario`:case`clases`:Q=await Rt(o);break;case`hoy`:Q=await jt(o,{onClaseClick:e=>Z.navigate(`asistencia?clase=${e}`)});break;case`asistencia`:Q=await ja(o,{claseId:i.get(`clase`),fecha:i.get(`fecha`)});break;case`metricas`:Q=Jt(o);break;case`perfil`:Q=Va(o);break;case`clase-emergente`:Q=Fa(o,{maestroId:Zs?.id});break;case`planificacion`:Q=await so(o);break;case`alumno`:Q=bo(o,{alumnoId:i.get(`id`)});break;case`gamificacion`:await To(o);break;case`ruta`:await Io(o,{onTopicSelected:e=>Z.navigate(`asistencia?clase=${e}`)});break;case`crear-clase`:Go(o);break;case`ruta-plan-builder`:Ko(o,{alumnoId:i.get(`id`)});break;case`ruta-semanal`:qo(o,{alumnoId:i.get(`id`)});break;case`ruta-libreria`:Jo.render().then(e=>{o.innerHTML=``,o.appendChild(e)});break;case`ruta-detalle`:Yo.render(t).then(e=>{o.innerHTML=``,o.appendChild(e)});break;case`admin-alumnos`:v(o);break;case`admin-programas`:x(o);break;case`admin-maestros`:g(o);break;case`admin-metricas`:k(o);break;case`admin-config`:w(o);break;case`admin-clases`:f?.(o);break;case`admin-sesiones`:break;case`admin-aprobacion`:await y(o),Q=null;break;case`admin-ausencias`:await S(o),Q=null;break;case`admin-notificaciones`:await ee(o),Q=null;break;case`registrar-alumno`:if(!Qs?.puede_registrar_alumnos){Z.navigate(`hoy`);return}Xo(o);break;case`gestionar-clases`:if(!Qs?.puede_inscribir_clases){Z.navigate(`hoy`);return}Q=await us(o);break;default:}clearTimeout(s),o.querySelector(`.pm-loading-overlay`)?.remove(),new Set([`hoy`,`calendario`,`metricas`,`perfil`,`ruta`,`gamificacion`,`crear-clase`,`planificacion`,`ruta-libreria`]).has(a)&&mc.add(a)}catch(e){clearTimeout(s),o.innerHTML=`<p class="pm-error">Error cargando vista: ${e.message}</p>`}}async function gc(){if(Zs)try{let e=new Date,t=e.toLocaleDateString(`es-ES`,{weekday:`long`}).toLowerCase(),n=e.toISOString().split(`T`)[0],[r,i,a]=await Promise.all([O(),O().then(e=>te(e.map(e=>e.id))),O().then(()=>p(Zs.id,n,n))]),o=Object.fromEntries(r.map(e=>[e.id,e]));await ge(i.filter(e=>e.dia?.toLowerCase()===t).map(e=>({...e,clase_nombre:o[e.clase_id]?.nombre||`Clase`})),a.filter(e=>e.borrador===!1||e.estado===`registrada`).map(e=>e.clase_id))}catch(e){console.warn(`[Alerts] Error programando alertas:`,e.message)}}function _c(){mc.clear()}function vc(e){mc.delete(e)}async function yc(){let e=document.getElementById(`portal-app`);if(!e)return;console.log(`[Init] Iniciando Portal...`),console.log(`[Init] Llamando usePortalAuth.init()...`);let t=await yt.init();console.log(`[Init] Auth completado:`,t?`con maestro`:`sin maestro`),Ks=t?.es_admin===!0,console.log(`[Init] IS_ADMIN:`,Ks);let n=window.router||St(),r=[`login`,`register`,`pending-approval`],i=n.currentRoute().split(`?`)[0],a=r.includes(i);if(!t&&!a){console.log(`[Init] No maestro y ruta privada, mostrando login screen`),ac();return}if(!t&&a){console.log(`[Init] No maestro pero ruta pública detectada:`,i),document.getElementById(`pm-view-container`)||(e.innerHTML=`<main class="pm-view" id="pm-view-container"></main>`),hc(),oc(),Z.setAuthGuard(()=>yt.isAuthenticated(),r),Z.start();return}let o=null;if(!Ks)try{o=await Ke(t.id)}catch(e){console.warn(`[Init] Error fetching permissions:`,e.message)}console.log(`[Init] Renderizando shell...`),lc(e,t,o),console.log(`[Init] Shell renderizado`),Ks&&Ls.init(),hc(),dc(),Br(vc,_c),oc(),Z.setAuthGuard(()=>yt.isAuthenticated(),[`login`,`register`,`pending-approval`]),Z.start(),A().then(async()=>{let e=[`hoy`,`calendario`,`metricas`],t=(Z.currentRoute?.()||`hoy`).split(`?`)[0];await e.filter(e=>e!==t&&!mc.has(e)).reduce((e,t)=>e.then(()=>{if(pc[t])return $(t,{},{silent:!0})}),Promise.resolve()),gc(),window.pwaInstaller&&window.pwaInstaller.evaluateInsights(),window.adminAusenciasInsights&&window.adminAusenciasInsights.evaluate()}).catch(e=>console.warn(`[Prefetch] Error:`,e.message)),nc()}window.addEventListener(`error`,e=>{let t=[`useCache`,`WebSocket`,`content.js`],n=e.message||``;if(t.some(e=>n.includes(e))){console.warn(`[Ignored Error]`,n);return}et(Error(e.message),{context:`window.error`,filename:e.filename,lineno:e.lineno});let r=document.getElementById(`portal-app`);r&&(r.innerHTML=`
    ...
    </div>`)}),yc().catch(e=>{let t=document.getElementById(`portal-app`);t&&(t.innerHTML=`<div style="padding:20px;color:red;font-family:monospace;background:#fff;z-index:9999;position:fixed;top:0;left:0;right:0;bottom:0;overflow:auto;"><h2>❌ initPortal() falló</h2><pre>${e?.message||e}\n${e?.stack||``}</pre></div>`)});