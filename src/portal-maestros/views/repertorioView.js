...
function mapMarkup(montaje, selected, selectionMode, pickerMeasure, canEditApplicability, syncMessage, activeStudentId, passages = [], groups = [], action = null, linkedScope = null, measuresPerRow = DEFAULT_MEASURES_PER_ROW, historyEvents = [], trajectoryVisible = false, trajectoryTargets = [], canEditTargets = false, priorityVisible = false, priorityCandidates = [], mode = 'demo') {
  ...
  return `<section class="repertoire-map" aria-label="Mapa de preparación de ${montaje.obra.titulo}">
    <div class="repertoire-map__header"><div><button class="btn btn-link repertoire-back">← Repertorio</button><h1>${montaje.obra.titulo}</h1><p>${activeFila.nombre || 'Fila pendiente'} · ${montaje.evento?.nombre || 'Preparación pedagógica'} · ${montaje.evento?.fecha || ''}</p></div><span class="repertoire-mode-badge">${mode === 'real' ? 'REAL · Supabase' : 'DEMO · persistencia local'}</span></div>
    <nav class="repertoire-fila-tabs" aria-label="Filas de la obra">${(montaje.filas || []).map((fila) => `<button type="button" class="btn btn-sm ${fila.id === activeFilaId ? 'btn-primary' : 'btn-outline-secondary'} repertoire-fila-tab" data-fila-id="${fila.id}">${fila.nombre}</button>`).join('')}</nav>
    <nav class="repertoire-detail-tabs" aria-label="Detalle de fila"><button type="button" class="btn btn-sm btn-primary">Resumen</button><button type="button" class="btn btn-sm btn-outline-secondary">Preparación</button><button type="button" class="btn btn-sm btn-outline-secondary">Alumnos</button><button type="button" class="btn btn-sm btn-outline-secondary">Pasajes</button><button type="button" class="btn btn-sm btn-outline-secondary">Sesiones</button><button type="button" class="btn btn-sm btn-outline-secondary">Historial</button></nav>
    ...
    <section class="repertoire-fila-summary" aria-label="Resumen de fila"><h2>Fila: ${activeFila.nombre || 'Pendiente'}</h2><div class="repertoire-fila-summary__columns"><div><strong>EVALUACIÓN COLECTIVA</strong><span>Estado del montaje: ${STATE_LABELS[montaje.compases?.[0]?.estado_preparacion] || 'Sin evaluar'}</span></div><div><strong>EVIDENCIA DERIVADA DE ALUMNOS</strong><span>${derivedStates.length ? Object.entries(derivedSummary).map(([state, count]) => `${count} ${STATE_LABELS[state]}`).join(' · ') : 'Sin evidencia registrada'}</span><small>${derivedStates.length ? `Derivado de ${derivedStates.length} alumnos` : 'La evidencia individual permanece independiente.'}</small></div></div></section>
    <section class="repertoire-students" aria-label="Preparación individual"><h2>Alumnos de ${activeFila.nombre || 'la fila'}</h2><p>El estado colectivo de la fila no reemplaza estos estados individuales.</p><div class="repertoire-student-list"><button type="button" class="btn btn-sm ${activeStudentId === null ? 'btn-primary' : 'btn-outline-secondary'} repertoire-student" data-student-id="">Fila colectiva</button>${filaStudents.map((student) => `<button type="button" class="btn btn-sm ${student.id === activeStudentId ? 'btn-primary' : 'btn-outline-secondary'} repertoire-student" data-student-id="${student.id}">${student.nombre}: ${STATE_LABELS[student.estado_preparacion]}</button>`).join('')}</div></section>
