import{i as e,r as t}from"./AppModal-B8f8dDnR.js";import{a as n,n as r,t as i}from"./MapaContenidoSVG-C-qzEw6i.js";import{c as a}from"./asistenciasApi-Ck2FRxpB.js";import{t as o}from"./router-DPk1oCHJ.js";import{c as s,d as c,f as l,i as u,l as d,n as f,o as p,p as m,r as h,s as g,t as _,u as v}from"./mapaClaseService-UtEI55gM.js";function y({claseId:e,niveles:t=[],objetivo:n=null,maestroId:r=null,onSaved:i=null,onClosed:a=null}){document.querySelectorAll(`.objetivo-editor-modal-overlay`).forEach(e=>e.remove());let o={objetivo:n,indicadores:[],archivarPendienteId:null,archivarObjetivoPendiente:!1},s=document.createElement(`div`);if(s.className=`objetivo-editor-modal-overlay`,document.body.appendChild(s),!document.getElementById(`objetivo-editor-modal-styles`)){let e=document.createElement(`style`);e.id=`objetivo-editor-modal-styles`,e.textContent=E(),document.head.appendChild(e)}let c=()=>{s.remove(),a?.()},l=()=>{s.innerHTML=S({claseId:e,niveles:t,state:o}),x({overlay:s,claseId:e,niveles:t,maestroId:r,state:o,close:c,render:l,onSaved:i})};l(),o.objetivo?.id&&b(o,l)}async function b(e,t){try{e.indicadores=await c(e.objetivo.id)}catch(t){console.error(`[objetivoEditorModal] Error cargando indicadores:`,t),e.indicadores=[]}t()}function x({overlay:e,claseId:t,niveles:n,maestroId:r,state:i,close:a,render:o,onSaved:c}){e.querySelector(`.objetivo-editor-modal-close-x`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-modal-backdrop`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-cancelar-btn`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-guardar-btn`)?.addEventListener(`click`,async()=>{let n=e.querySelector(`#objetivo-editor-nombre`)?.value?.trim()||``,a=e.querySelector(`#objetivo-editor-descripcion`)?.value?.trim()||``,s=e.querySelector(`#objetivo-editor-nivel`)?.value||null;if(!n){w(e,`El nombre del objetivo es requerido`);return}try{if(i.objetivo?.id){let e=await f(i.objetivo.id,{nombre:n,descripcion:a});i.objetivo={...i.objetivo,...e},c?.(i.objetivo)}else{if(!s){w(e,`Seleccioná un nivel`);return}let o=await g({clase_id:t,level_id:s,nombre:n,descripcion:a,created_by:r});i.objetivo=o,c?.(o)}o(),i.objetivo?.id&&b(i,o)}catch(t){console.error(`[objetivoEditorModal] Error guardando objetivo:`,t),w(e,`No se pudo guardar el objetivo`)}}),e.querySelector(`.objetivo-editor-borrar-objetivo-btn`)?.addEventListener(`click`,async()=>{try{await d(i.objetivo.id),a()}catch(t){if(t instanceof _){i.archivarObjetivoPendiente=!0,o();return}console.error(`[objetivoEditorModal] Error borrando objetivo:`,t),w(e,`No se pudo borrar el objetivo`)}}),e.querySelector(`.objetivo-editor-archivar-objetivo-btn`)?.addEventListener(`click`,async()=>{try{await u(i.objetivo.id),a()}catch(t){console.error(`[objetivoEditorModal] Error archivando objetivo:`,t),w(e,`No se pudo archivar el objetivo`)}}),e.querySelector(`.objetivo-editor-agregar-indicador-btn`)?.addEventListener(`click`,async()=>{let n=e.querySelector(`#objetivo-editor-nuevo-indicador`)?.value?.trim()||``;if(n)try{await p({objetivo_id:i.objetivo.id,clase_id:t,descripcion:n}),await b(i,o)}catch(t){console.error(`[objetivoEditorModal] Error agregando indicador:`,t),w(e,`No se pudo agregar el indicador`)}}),e.querySelectorAll(`.objetivo-editor-indicador-row`).forEach(t=>{let n=t.dataset.indicadorId;t.querySelector(`.btn-borrar-indicador`)?.addEventListener(`click`,async()=>{try{await s(n),await b(i,o)}catch(t){if(t instanceof _){i.archivarPendienteId=n,o();return}console.error(`[objetivoEditorModal] Error borrando indicador:`,t),w(e,`No se pudo borrar el indicador`)}}),t.querySelector(`.btn-archivar-indicador`)?.addEventListener(`click`,async()=>{try{await h(n),i.archivarPendienteId=null,await b(i,o)}catch(t){console.error(`[objetivoEditorModal] Error archivando indicador:`,t),w(e,`No se pudo archivar el indicador`)}})})}function S({claseId:e,niveles:t,state:n}){let{objetivo:r}=n,i=!!r?.id,a=!i&&t.length===0,o=t.map(e=>`<option value="${e.id}">${T(e.nombre)}</option>`).join(``);return`
    ...
    <div class="objetivo-editor-modal-dialog">
      ...
      <div class="objetivo-editor-modal-body">
        ...
        <input type="text" id="objetivo-editor-nombre" class="form-control" value="${T(r?.nombre||``)}" placeholder="Ej: La 3ra posición" />

        <label class="objetivo-editor-label" for="objetivo-editor-descripcion">Descripción</label>
        <textarea id="objetivo-editor-descripcion" class="form-control" rows="2">${T(r?.descripcion||``)}</textarea>
  ...
  `}function C(e){let t=e.indicadores.map(t=>`
      <div class="objetivo-editor-indicador-row" data-indicador-id="${t.id}">
        <span class="objetivo-editor-indicador-desc">${T(t.descripcion)}</span>
  ...
  `}function w(e,t){let n=e.querySelector(`.objetivo-editor-error-msg`);n&&(n.textContent=t,n.classList.remove(`d-none`),setTimeout(()=>n.classList.add(`d-none`),3e3))}function T(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}function E(){return`
  ...
  `}var D=[1,2,3,4,5];function O({claseId:e,claseIndicadorId:t,indicadorDescripcion:i=``,presentes:a=[],fecha:o=null,evaluadoPor:s=null,onGuardado:c=null,onClosed:l=null}){document.querySelectorAll(`.calificacion-indicador-overlay`).forEach(e=>e.remove());let u=new Map,d=document.createElement(`div`);if(d.className=`calificacion-indicador-overlay`,d.innerHTML=k({indicadorDescripcion:i,presentes:a,fecha:o}),document.body.appendChild(d),!document.getElementById(`calificacion-indicador-styles`)){let e=document.createElement(`style`);e.id=`calificacion-indicador-styles`,e.textContent=j(),document.head.appendChild(e)}let f=()=>{d.remove(),l?.()};d.querySelector(`.calificacion-panel-close-x`)?.addEventListener(`click`,f),d.querySelector(`.calificacion-panel-backdrop`)?.addEventListener(`click`,f),d.querySelector(`.calificacion-panel-cancelar-btn`)?.addEventListener(`click`,f),d.querySelectorAll(`.calificacion-alumno-row`).forEach(e=>{let t=e.dataset.alumnoId;e.querySelectorAll(`.btn-nota`).forEach(n=>{n.addEventListener(`click`,()=>{let i=Number(n.dataset.nota);u.set(t,i),e.querySelectorAll(`.btn-nota`).forEach(e=>e.classList.toggle(`selected`,e===n));let a=e.querySelector(`.calificacion-badge-slot`);a&&(a.innerHTML=r(i)?`<span class="calificacion-superado-badge">Superado</span>`:``)})})}),d.querySelector(`.calificacion-panel-guardar-btn`)?.addEventListener(`click`,async()=>{let r=d.querySelector(`.calificacion-panel-guardar-btn`);r.disabled=!0,r.textContent=`Guardando...`;let i=0;for(let[r,a]of u.entries())try{await n({alumno_id:r,clase_indicador_id:t,clase_id:e,nota:a,evaluado_por:s}),i++}catch(e){console.error(`[calificacionIndicadorPanel] Error guardando evaluación:`,r,e)}c?.({guardados:i,total:u.size}),f()})}function k({indicadorDescripcion:e,presentes:t,fecha:n}){let r=t.map(e=>`
      <div class="calificacion-alumno-row" data-alumno-id="${e.id}">
        ...
        <div class="calificacion-notas-btns">
          ${D.map(e=>`<button type="button" class="btn btn-sm btn-outline-primary btn-nota" data-nota="${e}">${e}</button>`).join(``)}
        </div>
      </div>
    `).join(``);return`
    ...
    <div class="calificacion-panel-dialog">
      <div class="calificacion-panel-header">
        <div>
          ...
          <p class="calificacion-panel-subtitle">${A(e)}${n?` — ${A(n)}`:``}</p>
        </div>
  ...
  `}function A(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}function j(){return`
  ...
  `}function M(){return new Date().toISOString().slice(0,10)}async function N(e,{claseId:t,maestroId:n=null}={}){if(!e||!t)return;e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5" style="min-height: 300px;">
      <div class="spinner-border text-primary" role="status"></div>
    </div>
  `;let r={claseId:t,maestroId:n,niveles:[],objetivos:[],estrellasMap:new Map,modo:`diseno`,gateBloqueado:!1,presentes:[],selectedObjetivoId:null,indicadoresSeleccionados:[]};await P(r),await F(r),L(e,r)}async function P(e){try{e.niveles=await l(e.claseId)}catch(t){console.error(`[MapaClaseView] Error cargando niveles asignados:`,t),e.niveles=[]}}async function F(e){try{let[t,n]=await Promise.all([m(e.claseId),v(e.claseId)]);e.objetivos=t||[],e.estrellasMap=new Map((n||[]).map(e=>[e.objetivoId,e]))}catch(t){console.error(`[MapaClaseView] Error cargando objetivos/estrellas:`,t),e.objetivos=[],e.estrellasMap=new Map}}function I(e){return e.objetivos.map(t=>{let n=e.estrellasMap.get(t.id);return{id:t.id,titulo:t.nombre,...n?{estrellas:n.estrellas,pctAvance:n.pctAvance,estadoVisual:n.estadoVisual}:{}}})}function L(e,t){let n=t.niveles.length===0;e.innerHTML=`
  ...
  `;let r=e.querySelector(`#mapa-clase-svg-canvas`);r&&i({container:r,nodos:I(t),modo:t.modo,onNodeClick:n=>B(e,t,n),onAddNodeClick:()=>z(e,t)}),t.modo===`sesion`&&t.selectedObjetivoId&&V(e,t),e.querySelector(`#btn-modo-diseno`)?.addEventListener(`click`,()=>{t.modo=`diseno`,t.gateBloqueado=!1,t.selectedObjetivoId=null,t.indicadoresSeleccionados=[],L(e,t)}),e.querySelector(`#btn-modo-sesion`)?.addEventListener(`click`,()=>R(e,t)),e.querySelector(`#btn-ir-asistencias`)?.addEventListener(`click`,()=>o.navigate(`asistencias`))}async function R(e,t){let n=M(),r={tomada:!1,presentes:[]};try{r=await a({claseId:t.claseId,fecha:n})}catch(e){console.error(`[MapaClaseView] Error verificando asistencia del día:`,e)}if(!r.tomada){t.gateBloqueado=!0,L(e,t);return}t.gateBloqueado=!1,t.presentes=r.presentes||[],t.modo=`sesion`,L(e,t)}function z(t,n){if(n.niveles.length===0){e.warning(`Esta clase no tiene niveles asignados. Asigná un nivel en la matriz ACM primero.`);return}y({claseId:n.claseId,niveles:n.niveles,objetivo:null,maestroId:n.maestroId,onSaved:()=>H(t,n)})}function B(e,t,n){if(t.modo===`diseno`){let r=t.objetivos.find(e=>String(e.id)===String(n.id))||null;y({claseId:t.claseId,niveles:t.niveles,objetivo:r,maestroId:t.maestroId,onSaved:()=>H(e,t)});return}t.selectedObjetivoId=n.id,c(n.id).then(r=>{t.selectedObjetivoId===n.id&&(t.indicadoresSeleccionados=r||[],L(e,t))}).catch(n=>{console.error(`[MapaClaseView] Error cargando indicadores del objetivo:`,n),t.indicadoresSeleccionados=[],L(e,t)})}function V(e,n){let r=e.querySelector(`#mapa-clase-panel-sesion`);if(!r)return;let i=n.objetivos.find(e=>String(e.id)===String(n.selectedObjetivoId)),a=n.indicadoresSeleccionados.map(e=>`
  ...
  `,r.querySelectorAll(`.btn-calificar-indicador`).forEach(t=>{t.addEventListener(`click`,()=>{let r=n.indicadoresSeleccionados.find(e=>String(e.id)===t.dataset.indicadorId);r&&O({claseId:n.claseId,claseIndicadorId:r.id,indicadorDescripcion:r.descripcion,presentes:n.presentes,fecha:M(),evaluadoPor:n.maestroId,onGuardado:()=>H(e,n)})})})}async function H(e,t){await F(t),L(e,t)}export{N as renderMapaClaseView};