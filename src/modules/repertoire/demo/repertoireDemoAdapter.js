...
export function createRepertoireDemoAdapter() {
  ...
  return {
    ...
    async updateLinkedGroupState(groupId, nextState) { const group = groups.find((item) => item.id === groupId); if (!group) throw new Error('Grupo no encontrado'); group.measureIds.forEach((id) => { const measure = state[0].compases.find((item) => item.id === id); if (measure) measure.estado_preparacion = nextState }); return { affected: group.measureIds.length } },
