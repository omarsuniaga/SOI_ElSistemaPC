  ...
  return `
    <form id="wiz-form-step3" novalidate>
      ${renderFormField({
        ...
        type: 'radio',
        value:
          draft.tiene_alergias === true ? 'true' : draft.tiene_alergias === false ? 'false' : '',
        options: [
          { value: 'true', label: 'Sí' },
      ...
      ${
        draft.tiene_alergias === true
          ? renderFormField({
              ...
              type: 'textarea',
              value: draft.alergias_descripcion ?? '',
            })
          : ''
      }
      ${renderFormField({
        ...
        value: tieneCondicion
          ...
          : draft.tiene_condicion_transmisible === false
            ? 'false'
            : '',
        error: errors.tiene_condicion_transmisible ?? '',
        options: [
          { value: 'true', label: 'Sí' },
          { value: 'false', label: 'No' },
        ],
      ...
      ${renderFormField({
        ...
        type: 'radio',
        value: tieneAlergiaMediacmento
          ? 'true'
          : draft.tiene_alergia_medicamento === false
            ? 'false'
            : '',
        options: [
          { value: 'true', label: 'Sí' },
      ...
      ${renderFormField({
        ...
        label: '¿Tiene impedimento social?',
        type: 'radio',
        value:
...
export function getState(container) {
  ...
  return {
    ...
    alergias_descripcion:
      form.querySelector('[name="alergias_descripcion"]')?.value?.trim() ?? null,
    tiene_condicion_transmisible: boolField('tiene_condicion_transmisible'),
    condicion_transmisible_descripcion:
      form.querySelector('[name="condicion_transmisible_descripcion"]')?.value?.trim() ?? null,
    tiene_alergia_medicamento: boolField('tiene_alergia_medicamento'),
    alergia_medicamento_descripcion:
      form.querySelector('[name="alergia_medicamento_descripcion"]')?.value?.trim() ?? null,
