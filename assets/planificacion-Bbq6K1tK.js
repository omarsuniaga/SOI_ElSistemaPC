...
[
  ...
  `;let r=``;e.querySelector(`[data-role="feedback-textarea"]`).addEventListener(`input`,e=>{r=e.target.value}),e.querySelector(`[data-action="publicar"]`).addEventListener(`click`,async()=>{try{await Re(t.id),n()}catch(e){console.error(`[acmPropuestasView] Error al publicar:`,e),window.alert(`Error al publicar: ${e.message}`)}}),e.querySelector(`[data-action="devolver"]`).addEventListener(`click`,async()=>{if(!r.trim()){window.alert(`Escriba un feedback antes de devolver la propuesta.`);return}try{await ze(t.id,r),n()}catch(e){console.error(`[acmPropuestasView] Error al devolver:`,e),window.alert(`Error al devolver: ${e.message}`)}})}function Ve(e){return e.length?e.slice().sort((e,t)=>(e.level_number||0)-(t.level_number||0)).map(e=>`
    ...
    `).join(``):`<p class="acm-placeholder">Esta propuesta no tiene niveles.</p>`}var He=[`borrador`,`activo`,`archivado`];async function Ue(e,t){if(!e||!t)throw Error(`claseId y routeVersionId son requeridos`);let{data:n}=await i.from(`class_curriculum_plan`).select(`id`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();n?.id&&await i.from(`class_curriculum_plan`).update({estado:`archivado`}).eq(`id`,n.id);let{data:r,error:a}=await i.from(`class_curriculum_plan`).insert({clase_id:e,route_version_id:t,estado:`activo`}).select().single();if(a)throw a;return r}async function We(e){let{data:t,error:n}=await i.from(`class_curriculum_plan`).select(`*`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();if(n)throw n;return t}async function Ge(e,t){if(!He.includes(t))throw Error(`Estado no válido`);let{data:n,error:r}=await i.from(`class_curriculum_plan`).update({estado:t}).eq(`clase_id`,e).select().single();if(r)throw r;return n}async function Ke(e){if(!Array.isArray(e)||e.length===0)throw Error(`Se requiere al menos un objetivo`);let t=e.map(e=>({planificacion_id:e.planificacion_id,class_curriculum_plan_id:e.class_curriculum_plan_id,node_id:e.node_id,indicator_id:e.indicator_id,semana:e.semana||null,estado:e.estado||`pendiente`})),{data:n,error:r}=await i.from(`clase_objetivos`).insert(t).select();if(r)throw r;return n}async function z(e){let{data:t,error:n}=await i.from(`clase_objetivos`).select(`*`).eq(`planificacion_id`,e).order(`created_at`);if(n)throw n;return t||[]}async function qe(e,t){let n=[`estado`,`semana`],r={};for(let e of n)t[e]!==void 0&&(r[e]=t[e]);let{data:a,error:o}=await i.from(`clase_objetivos`).update(r).eq(`id`,e).select().single();if(o)throw o;return a}async function Je(e){let{error:t}=await i.from(`clase_objetivos`).delete().eq(`planificacion_id`,e);if(t)throw t}var Ye=class{constructor(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._listeners=[]}subscribe(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}_notify(){this._listeners.forEach(e=>e(this))}async fetchRutaDeClase(e){this.cargando=!0,this.error=null,this._notify();try{return this.rutaAsignada=await We(e),this.cargando=!1,this._notify(),this.rutaAsignada}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async asignarRuta(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await Ue(e,t);return this.rutaAsignada=n,this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async cambiarEstado(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await Ge(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{return this.objetivos=await z(e),this.cargando=!1,this._notify(),this.objetivos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async agregarObjetivos(e,t,n){this.cargando=!0,this.error=null,this._notify();try{let r=await Ke(n.map(n=>({planificacion_id:e,class_curriculum_plan_id:t,node_id:n.node_id,indicator_id:n.indicator_id,semana:n.semana||null})));return this.objetivos=await z(e),this.cargando=!1,this._notify(),r}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async actualizarObjetivo(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await qe(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async eliminarObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{await Je(e),this.objetivos=[],this.cargando=!1,this._notify()}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async evaluarAlumno(e){this.cargando=!0,this.error=null,this._notify();try{let t=await m(e);return this.evaluaciones=await _(e.clase_id),this.cargando=!1,this._notify(),t}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchEvaluaciones(e){this.cargando=!0,this.error=null,this._notify();try{return this.evaluaciones=await _(e),this.cargando=!1,this._notify(),this.evaluaciones}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchProgresoAlumnos(e){this.cargando=!0,this.error=null,this._notify();try{return this.progresoAlumnos=await y(e),this.cargando=!1,this._notify(),this.progresoAlumnos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}reset(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._notify()}},B=null;function Xe(){return B||=new Ye,B}var V=Xe();function Ze(e){if(!e)return;let t=V.subscribe(()=>H(e));return H(e),V.fetchRutaDeClase(``).catch(()=>{}),()=>t()}function H(e){if(V.cargando&&!V.rutaAsignada&&V.objetivos.length===0){e.innerHTML=`
  ...
  `,tt(e)}function Qe(e){let t=e.estado===`activo`?`Activa`:e.estado===`borrador`?`Borrador`:`Archivada`,n=e.estado===`activo`?`success`:e.estado===`borrador`?`warning`:`secondary`;return`
    ...
    </div>`}function et(e){if(e.length===0)return`
      <div class="page-glass rounded p-3 mb-4">
    ...
    <div class="page-glass rounded p-3 mb-4">
      ...
      ${t}
    </div>`}function tt(e){e.querySelectorAll(`[data-action="evaluar"]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.indicatorId;document.dispatchEvent(new CustomEvent(`evaluacion:open`,{detail:{indicatorId:t}}))})})}function U(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}var nt=n({renderDisenadorCurricularView:()=>it}),rt=[{id:`nivel-0`,nombre:`Nivel 0: Iniciación / Descubrimiento`,color:`success`},{id:`nivel-1`,nombre:`Nivel 1: Básico / Formación Técnica`,color:`primary`},{id:`nivel-2`,nombre:`Nivel 2: Intermedio / Desarrollo Solista`,color:`warning`},{id:`nivel-3`,nombre:`Nivel 3: Avanzado / Maestría Institucional`,color:`danger`}];async function it(e){if(!e)return;e.innerHTML=`
  ...
  `;let t=[];try{t=await o()}catch(e){console.error(`[DisenadorCurricularView] Error:`,e)}let n={claseId:t[0]?.id||``,nivelId:`nivel-1`,frecuenciaSemanal:2,frecuenciaOrigen:`manual`,semanasTotales:24,objetivos:[{id:`obj-1`,titulo:`Dominio de Postura y Emisión Sonora Libre`,indicadores:[{id:`ind-1`,titulo:`Postura corporal equilibrada y relajada`,prerrequisitoId:null},{id:`ind-2`,titulo:`Distribución fluida del arco en cuerdas abiertas`,prerrequisitoId:`ind-1`}]},{id:`obj-2`,titulo:`Afinación y Articulación Digital`,indicadores:[{id:`ind-3`,titulo:`Colocación exacta de 1er y 2do dedo`,prerrequisitoId:`ind-2`},{id:`ind-4`,titulo:`Independencia digital a pulso 60 BPM`,prerrequisitoId:`ind-3`}]}]};at(e,t,n)}function at(e,n,r){let i={list:[],porNodo:new Map,currentNodoId:null,datosListos:!0},a=async()=>{i.porNodo.clear(),i.datosListos=!0,r.claseId&&(i.list=await g(r.claseId))},o=()=>{e.innerHTML=`
    ...
    `,ot(e,n,r,i,a),W(e,r,i)};a().then(()=>o())}function W(e,n,r){let i=e.querySelector(`#lista-objetivos-container`);if(!i)return;if(n.objetivos.length===0){i.innerHTML=`
  ...
  `).join(``),st(e,n,r),G(e,n,r)}function ot(t,n,r,i,a){t.querySelector(`#btn-volver-acm`)?.addEventListener(`click`,()=>{x.navigate(`planificacion-acm`)}),t.querySelector(`#select-clase-full`)?.addEventListener(`change`,e=>{r.claseId=e.target.value,a().then(()=>W(t,r,i))}),t.querySelector(`#input-frecuencia-full`)?.addEventListener(`input`,e=>{r.frecuenciaSemanal=parseFloat(e.target.value||`2`),ct(t,r)}),t.querySelector(`#btn-auto-frecuencia`)?.addEventListener(`click`,()=>{let i=n.find(e=>String(e.id)===String(r.claseId));if(!i){e.show(`Selecciona primero una clase para detectar su horario`,`warning`);return}let a=2;if(Array.isArray(i.diasSemana))a=i.diasSemana.length;else if(typeof i.horario==`string`){let e=i.horario.match(/lunes|martes|miércoles|jueves|viernes|sábado|domingo/gi);e&&(a=new Set(e.map(e=>e.toLowerCase())).size)}r.frecuenciaSemanal=a;let o=t.querySelector(`#input-frecuencia-full`);o&&(o.value=a),e.show(`Horario detectado: ${a} clases/semana`,`info`),ct(t,r)}),t.querySelector(`#btn-ia-generar-full`)?.addEventListener(`click`,async()=>{let n=t.querySelector(`#btn-ia-generar-full`);n&&(n.disabled=!0),e.show(`Generando Estructura Curricular con IA (GROQ)...`,`info`);let a=await me({instrumento:`Música e Instrumento`,nivelIndex:1});a&&a.length>0&&(r.objetivos=a.map((e,t)=>({id:`obj-full-ia-${t}`,titulo:e.titulo||`Unidad Didáctica ${t+1}`,indicadores:(e.indicadores||[`Evaluación de desempeño`]).map((e,n)=>({id:`ind-full-ia-${t}-${n}`,titulo:typeof e==`string`?e:e.titulo||`Contenido Evaluables`,prerrequisitoId:n>0?`ind-full-ia-${t}-${n-1}`:null}))})),W(t,r,i),e.show(`Estructura generada por IA cargada exitosamente`,`success`)),n&&(n.disabled=!1)}),t.querySelector(`#btn-guardar-plan-full`)?.addEventListener(`click`,async()=>{if(!r.claseId){e.show(`Debes seleccionar una Clase o Agrupación`,`warning`);return}let n=[];r.objetivos.forEach(e=>{n.push(`[OBJETIVO] ${e.titulo}`),e.indicadores.forEach(e=>{n.push(`  • [CLASE] ${e.titulo}`)})});let i={clase_id:r.claseId,semana:1,titulo:`Plan Didáctico Semestral - ${r.frecuenciaSemanal} clases/sem`,nivelId:t.querySelector(`#select-nivel-full`)?.value||`nivel-1`,frecuenciaSemanal:r.frecuenciaSemanal,semanasTotales:24,objetivosEstructurados:r.objetivos,contenidos:n,estado:`publicada`,esPlantillaOficial:!0,fecha:new Date().toISOString().slice(0,10)};try{await l(i),e.show(`Plan Institucional Oficial publicado con éxito ⭐`,`success`),x.navigate(`planificacion-acm`)}catch(t){e.show(`Error al publicar plan: ${t.message}`,`error`)}}),t.querySelector(`#btn-add-objetivo-full`)?.addEventListener(`click`,()=>{let e=r.objetivos.length+1;r.objetivos.push({id:`obj-${Date.now()}`,titulo:`Unidad Didáctica ${e}`,indicadores:[{id:`ind-${Date.now()}-1`,titulo:`Contenido Evaluables 1`,prerrequisitoId:null}]}),W(t,r,i)})}function st(e,t,n){e.querySelectorAll(`.input-obj-title`).forEach(r=>{r.addEventListener(`input`,r=>{let i=parseInt(r.target.dataset.objIdx,10);t.objetivos[i]&&(t.objetivos[i].titulo=r.target.value,G(e,t,n))})}),e.querySelectorAll(`.input-ind-title`).forEach(r=>{r.addEventListener(`input`,r=>{let i=parseInt(r.target.dataset.objIdx,10),a=parseInt(r.target.dataset.indIdx,10);t.objetivos[i]?.indicadores[a]&&(t.objetivos[i].indicadores[a].titulo=r.target.value,G(e,t,n))})}),e.querySelectorAll(`.select-prereq`).forEach(r=>{r.addEventListener(`change`,r=>{let i=parseInt(r.target.dataset.objIdx,10),a=parseInt(r.target.dataset.indIdx,10);t.objetivos[i]?.indicadores[a]&&(t.objetivos[i].indicadores[a].prerrequisitoId=r.target.value||null,G(e,t,n))})}),e.querySelectorAll(`.btn-del-obj`).forEach(r=>{r.addEventListener(`click`,()=>{let i=parseInt(r.dataset.objIdx,10);t.objetivos.splice(i,1),W(e,t,n)})}),e.querySelectorAll(`.btn-add-ind`).forEach(r=>{r.addEventListener(`click`,()=>{let i=parseInt(r.dataset.objIdx,10);if(t.objetivos[i]){let r=`ind-${Date.now()}`;t.objetivos[i].indicadores.push({id:r,titulo:`Objetivo de Clase ${t.objetivos[i].indicadores.length+1}`,prerrequisitoId:null}),W(e,t,n)}})}),e.querySelectorAll(`.btn-del-ind`).forEach(r=>{r.addEventListener(`click`,()=>{let i=parseInt(r.dataset.objIdx,10),a=parseInt(r.dataset.indIdx,10);t.objetivos[i]?.indicadores&&(t.objetivos[i].indicadores.splice(a,1),W(e,t,n))})})}function ct(e,t){let n=t.frecuenciaSemanal,r=Math.round(n*24),i=Math.max(r-4,15),a=e.querySelector(`#lbl-ritmo-calculado`);a&&(a.textContent=`${n} clase(s)/semana ➔ ~${r} clases en 6 meses (24 semanas). Meta real: ~${i} Indicadores.`)}function G(e,t,n){let r=e.querySelector(`#full-svg-canvas-container`);if(!r)return;let i=[];if(t.objetivos.forEach(e=>{e.indicadores.forEach(e=>{i.push({id:e.id,titulo:e.titulo,estado:e.prerrequisitoId?`en_proceso`:`logrado`})})}),i.length===0){r.innerHTML=`<div class="text-body-secondary small py-5 text-center">Agregá indicadores para visualizar el mapa SVG en tiempo real.</div>`;return}v({container:r,nodos:i,onNodeClick:r=>{lt(e,r,n,t)}})}function lt(n,r,i,a){let o=n.querySelector(`#nodo-alumnos-eval-panel`),s=n.querySelector(`#lbl-nodo-eval-titulo`),c=n.querySelector(`#tbody-alumnos-eval`);if(!o||!c)return;o.style.display=`block`,s&&(s.textContent=`Evaluación: ${r.titulo}`),i.currentNodoId=r.id;let l=n.querySelector(`#badge-nodo-eval-estado`),u=()=>{c.innerHTML=(i.list||[]).map(e=>`
    ...
    `).join(``),c.querySelectorAll(`.row-alumno-eval`).forEach(t=>{t.addEventListener(`click`,()=>{if(!i.datosListos)return;let n=t.dataset.id,o=i.list.find(e=>String(e.id)===String(n));o&&(o.estrellas=b.siguienteEstrella(o.estrellas),h.guardarLocal({alumnoId:o.id,claseId:a.claseId,nodoId:r.id,estrellas:o.estrellas}),u(),e.show(`${o.nombre}: ${o.estrellas}★ registrados`,`info`))})})},d=i.porNodo.get(r.id);if(d){i.list=d,i.datosListos=!0,u();return}i.datosListos=!1,u(),l&&(l.textContent=`Actualizando…`),g(a.claseId,r.id).then(e=>{i.currentNodoId===r.id&&(i.porNodo.set(r.id,e),i.list=e,i.datosListos=!0,u(),l&&(l.textContent=`En Tiempo Real`))})}function ut(e){let t=``;for(let n=1;n<=5;n++)n<=e?t+=`<i class="bi bi-star-fill text-warning"></i>`:t+=`<i class="bi bi-star text-secondary opacity-50"></i>`;return t}var dt=n({renderRutaPedagogicaView:()=>ft});async function ft(e){if(!e)return;e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5" style="min-height: 450px;">
      <div class="text-center">
        <div class="spinner-border text-primary mb-3" style="width: 3rem; height: 3rem;" role="status"></div>
  ...
  `;let t=[],n=[];try{let[e,r]=await Promise.all([o(),s()]);t=e||[],n=r||[]}catch(e){console.error(`[RutaPedagogicaView] Error:`,e)}pt(e,t,n)}function pt(n,r,i){let a=r[0]?.id||``,o=null,s=[],c=new Map,l=!0,u=async()=>{c.clear(),l=!0,s=await g(a),p()},d=()=>{let e=n.querySelector(`#panel-alumnos-evaluacion-nodo`),r=n.querySelector(`#lbl-nodo-seleccionado`);e&&(e.style.display=o?`block`:`none`),r&&(r.innerHTML=`<i class="bi bi-award-fill text-warning me-2"></i>${t(o?.titulo||`Selecciona un Nodo`)}`)},f=()=>{let e=n.querySelector(`#tbody-alumnos-ruta`);if(!e)return;e.innerHTML=s.map(e=>{let n=e.nombre.split(` `).slice(0,2).map(e=>e[0]).join(``).toUpperCase();return`
          <tr class="row-alumno-ruta${l?``:` opacity-50`}" data-id="${e.id}" style="cursor: pointer;">
            ...
            <td class="text-center">
              ...
              <small class="fw-bold text-body-secondary">${e.estrellas>0?`${e.estrellas} Estrellas (${gt(e.estrellas)})`:`Sin Registrar (0★)`}</small>
            </td>
