 ...
 * Client-side validation with DOMPurify sanitization
 */
...
export function validateObservation(data) {
  ...
  if (data.indicators && Array.isArray(data.indicators)) {
    if (data.indicators.length > 10) {
      errors.push('Maximum 10 indicators allowed')
    }
  }
 ...
 * @param {Object} data - Lesson plan data
 */
...
export function sanitizeInput(input, options = {}) {
  if (typeof window !== 'undefined' && window.DOMPurify) {
    return window.DOMPurify.sanitize(input, options)
  }
