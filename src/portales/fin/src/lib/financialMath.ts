...
/**
 * Retorna una etiqueta legible y humana para el perfil de pago (ISP)
 */
export function getISPEtiqueta(categoria?: string): {
  titulo: string;
  badgeClass: string;
  descripcion: string;
} {
  switch (categoria) {
    case 'A':
      return {
        ...
        descripcion: 'Pagos al día y alta puntualidad'
      };
    case 'B':
      return {
        titulo: 'Bueno',
        badgeClass: 'bg-sky-500/10 text-sky-400 border-sky-500/20',
        descripcion: 'Historial regular y solvente'
      };
    case 'C':
      return {
        titulo: 'Atraso Ocasional',
        badgeClass: 'bg-amber-500/10 text-amber-400 border-amber-500/20',
        descripcion: 'Atrasos leves, requiere seguimiento'
      };
    case 'D':
    case 'E':
      return {
        titulo: 'Mora Frecuente',
        badgeClass: 'bg-rose-500/10 text-rose-400 border-rose-500/20',
        descripcion: 'Atrasos recurrentes, prioritario para convenio'
      };
    default:
      return {
        titulo: 'Nuevo Ingreso',
        badgeClass: 'bg-zinc-800 text-zinc-400 border-zinc-700',
        descripcion: 'Sin historial previo de cuotas'
      };
  }
}
