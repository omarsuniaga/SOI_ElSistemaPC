 ...
 * Lista todos los proyectos con es_macro_evento = true y su semáforo de salud.
 */
...
export function renderProyectoManagerView(containerId) {
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
      <div>
        <h4 class="fw-bold mb-1">
          <i class="bi bi-kanban-fill text-primary me-2"></i>Project Manager Institucional
        </h4>
  ...
  async function cargarProyectos() {
    ...
    if (!eventos || eventos.length === 0) {
      document.getElementById('pm-lista').innerHTML =
        `<div class="col-12 text-center py-5 text-muted">
          <i class="bi bi-inbox fs-1 d-block mb-2"></i>No hay macro-eventos activos.
        </div>`
      return
    }
  ...
  function renderLista(proyectos) {
    ...
    lista.innerHTML = filtrados.map(({ evento, salud }) => {
      ...
      const fechaFmt = new Date(evento.fecha_inicio)
