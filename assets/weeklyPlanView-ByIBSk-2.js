import{i as e}from"./supabase-KnARm58N.js";import{i as t}from"./portalUtils-DbrsCFDo.js";import{t as n}from"./academicService-dKTfSUQ8.js";async function r(i,{alumnoId:a}){i.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;try{let{data:o,error:s}=await e.from(`academic_plans`).select(`*, route_versions(route_id, version_number, routes(name, instrument_id))`).eq(`student_id`,a).eq(`status`,`in_process`).maybeSingle();if(s)throw s;if(!o){i.innerHTML=`
      ...
      `;return}let c=await n.getRouteDetail(o.route_versions.route_id),{data:l,error:u}=await e.from(`weekly_plan_entries`).select(`*`).eq(`academic_plan_id`,o.id).order(`start_date`,{ascending:!1});if(u)throw u;i.innerHTML=`
      <div class="pm-asist-header">
        <h2 class="apple-display-md">Planificación Semanal</h2>
        <p class="apple-caption">${t(o.route_versions.routes.name)} - v${o.route_versions.version_number}</p>
      </div>
        ...
        `:l.map(e=>`
          <div class="card-apple mb-3 pm-animate-slide-up" style="padding: 1rem;">
            ...
            <div class="mt-2" style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
              ${(e.planned_nodes||[]).map(e=>`<span class="badge-apple" style="background: var(--pm-bg-alt); font-size: 0.7rem;">${t(e.title)}</span>`).join(``)}
            </div>
          </div>
        `).join(``)}
      </div>
        ...
        <div class="pm-modal-content">
          ...
          <div class="mb-3">
            ...
            <div id="nodes-checklist" style="max-height: 200px; overflow-y: auto; background: var(--pm-bg-alt); padding: 0.5rem; border-radius: 8px;">
              ${c.map(e=>`
                <div class="mb-2">
                  <small style="font-weight: 700; color: var(--pm-text-muted); text-transform: uppercase; font-size: 0.65rem;">${t(e.name)}</small>
                  ${e.levels.map(e=>`
                    ${e.nodes.map(e=>`
                      <div class="form-check" style="padding-left: 1.5rem; margin-top: 0.25rem;">
                        <input class="form-check-input node-checkbox" type="checkbox" value="${e.id}" data-title="${t(e.title)}" id="node-${e.id}">
