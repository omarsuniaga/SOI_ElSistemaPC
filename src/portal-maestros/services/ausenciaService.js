...
export function buildAbsencePayload({ maestro, formState, archivoUrl = null }) {
  const selectedClasses = (formState.clasesAfectadas || []).filter(
    (clase) => clase.selected !== false,
  )
  ...
  return {
    ...
    duracion_tipo:
      formState.duracionTipo ||
      (formState.fechaInicio === formState.fechaFin ? 'un_dia' : 'varios_dias'),
...
export async function createAbsenceRequest({
  ...
  const notification =
    notifyDirector && ausenciasApi.crearNotificacionAusencia
