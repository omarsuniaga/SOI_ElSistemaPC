    ...
    tbody.innerHTML = paginated
      .map((item) => {
        ...
        if (item.accion === 'APROBACION_FINAL')
          actionBadge = 'bg-success bg-opacity-15 text-success border border-success-subtle'
        if (item.accion === 'CREACION')
          actionBadge = 'bg-primary bg-opacity-15 text-primary border border-primary-subtle'
        if (item.accion === 'RECHAZO')
          actionBadge = 'bg-danger bg-opacity-15 text-danger border border-danger-subtle'
