import{i as e,r as t,s as n,t as r}from"./AppModal-B8f8dDnR.js";import{i}from"./supabase-Cgh_dhNB.js";import{t as a}from"./config-CNiOV0RX.js";import{a as o,c as s,i as c,n as l,o as u,r as d,t as f}from"./planificacionAdapter-Dyi8LwW_.js";import{a as p,i as m,r as h,t as ee}from"./MapaContenidoSVG-C-qzEw6i.js";import{i as g,n as _,r as v,t as te}from"./IndicadorLogro-CINYes1F.js";import{t as y}from"./router-DPk1oCHJ.js";import{n as b}from"./aiEvaluacionService-Cl48ShsO.js";import{a as ne,f as x,m as re,o as ie,s as ae}from"./mapaClaseService-UtEI55gM.js";var oe=n({actualizarPermiso:()=>ce,aprobarSolicitud:()=>fe,crearSolicitud:()=>le,obtenerPermisoPorMaestro:()=>C,obtenerPermisos:()=>se,obtenerSolicitudPorMaestro:()=>ue,obtenerSolicitudesPendientes:()=>de,rechazarSolicitud:()=>pe});function S(e){return e?{...e,id:e.id,maestro_id:e.maestro_id??``,maestro_nombre:e.maestros?.nombre_completo??``,maestro_email:e.maestros?.correo??``,puede_registrar_alumnos:e.puede_registrar_alumnos??!1,puede_inscribir_clases:e.puede_inscribir_clases??!1,permisos:Array.isArray(e.permisos)?e.permisos:[],solicitudes:Array.isArray(e.solicitudes)?e.solicitudes:[],concedido_por:e.concedido_por??null,concedido_por_nombre:null,creado_en:e.creado_en||null,actualizado_en:e.actualizado_en||null}:null}async function se(){let{data:e,error:t}=await i.from(`permisos_maestros`).select(`*, maestros!permisos_maestros_maestro_id_fkey(nombre_completo, correo)`).order(`creado_en`,{ascending:!1});if(t)throw console.error(`Error cargando permisos:`,t.message),Error(`No se pudieron cargar los permisos`);return e.map(S)}async function C(e){let{data:t,error:n}=await i.from(`permisos_maestros`).select(`*, maestros!permisos_maestros_maestro_id_fkey(nombre_completo, correo)`).eq(`maestro_id`,e).maybeSingle();if(n)throw console.error(`Error cargando permiso:`,n.message),Error(`No se pudo cargar el permiso`);return S(t)}async function ce(e,t){let{data:n}=await i.from(`permisos_maestros`).select(`puede_registrar_alumnos, puede_inscribir_clases, permisos, solicitudes`).eq(`maestro_id`,e).maybeSingle(),r={maestro_id:e,puede_registrar_alumnos:`puede_registrar_alumnos`in t?t.puede_registrar_alumnos??!1:n?.puede_registrar_alumnos??!1,puede_inscribir_clases:`puede_inscribir_clases`in t?t.puede_inscribir_clases??!1:n?.puede_inscribir_clases??!1,permisos:Array.isArray(t.permisos)?t.permisos:n?.permisos??[],solicitudes:Array.isArray(t.solicitudes)?t.solicitudes:n?.solicitudes??[],concedido_por:t.concedido_por||null},{data:a,error:o}=await i.from(`permisos_maestros`).upsert(r,{onConflict:`maestro_id`}).select().single();if(o)throw console.error(`Error actualizando permiso:`,o.message),Error(`No se pudo actualizar el permiso`);return S(a)}function w(e){return e?{id:e.id,maestro_id:e.maestro_id??``,maestro_nombre:e.maestros?.nombre_completo??``,maestro_email:e.maestros?.correo??``,solicita_alumnos:e.solicita_alumnos??!1,solicita_clases:e.solicita_clases??!1,estado:e.estado??`pendiente`,creado_en:e.creado_en||null,actualizado_en:e.actualizado_en||null,aprobado_en:e.aprobado_en||null,aprobado_por:e.aprobado_por||null,aprobado_por_nombre:e.maestros_aprobado?.nombre_completo??``,motivo_rechazo:e.motivo_rechazo||null}:null}async function le(e,t,n){if(!e)throw Error(`maestroId es requerido`);let{data:r}=await i.from(`solicitudes_permisos`).select(`id, estado`).eq(`maestro_id`,e).eq(`estado`,`pendiente`).maybeSingle();if(r)throw Error(`Ya existe una solicitud pendiente para este maestro`);let a=[];t&&a.push(`alumnos:create`),n&&a.push(`clases:enroll`);let{data:o,error:s}=await i.from(`solicitudes_permisos`).insert([{maestro_id:e,solicita_alumnos:t??!1,solicita_clases:n??!1,tipos:a,estado:`pendiente`}]).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(s)throw console.error(`Error creando solicitud:`,s.message),Error(`No se pudo crear la solicitud de permisos: ${s.message}`);return w(o)}async function ue(e){let{data:t,error:n}=await i.from(`solicitudes_permisos`).select(`*, maestros!maestro_id(nombre_completo, correo)`).eq(`maestro_id`,e).order(`creado_en`,{ascending:!1}).maybeSingle();if(n)throw console.error(`Error obteniendo solicitud:`,n.message),Error(`No se pudo obtener la solicitud`);return w(t)}async function de(){let{data:e,error:t}=await i.from(`solicitudes_permisos`).select(`*, maestros!maestro_id(nombre_completo, correo)`).eq(`estado`,`pendiente`).order(`creado_en`,{ascending:!0});if(t)throw console.error(`Error obteniendo solicitudes pendientes:`,t.message),Error(`No se pudieron cargar las solicitudes pendientes`);return(e||[]).map(w)}async function fe(e,t){if(!e||!t)throw Error(`solicitudId y adminId son requeridos`);let{data:n,error:r}=await i.from(`solicitudes_permisos`).update({estado:`aprobado`,aprobado_en:new Date().toISOString(),aprobado_por:t}).eq(`id`,e).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(r)throw console.error(`Error aprobando solicitud:`,r.message),Error(`No se pudo aprobar la solicitud`);if(n?.maestro_id){let e=w(n),r=[];e.solicita_alumnos&&r.push(`registrar_alumnos`,`alumnos:create`),e.solicita_clases&&r.push(`inscribir_clases`,`clases:enroll`,`clases:create`);let i=await C(n.maestro_id),a=Array.isArray(i?.permisos)?i.permisos:[];await ce(n.maestro_id,{puede_registrar_alumnos:e.solicita_alumnos||(i?.puede_registrar_alumnos??!1),puede_inscribir_clases:e.solicita_clases||(i?.puede_inscribir_clases??!1),permisos:[...new Set([...a,...r])],concedido_por:t})}return w(n)}async function pe(e,t,n){if(!e||!t)throw Error(`solicitudId y adminId son requeridos`);let{data:r,error:a}=await i.from(`solicitudes_permisos`).update({estado:`rechazado`,aprobado_en:new Date().toISOString(),aprobado_por:t,motivo_rechazo:n||``}).eq(`id`,e).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(a)throw console.error(`Error rechazando solicitud:`,a.message),Error(`No se pudo rechazar la solicitud`);return w(r)}var me=class{constructor(e=`default`){this.scope=e,this.subscriptions=[],this.intervals=[],this.listeners=[]}registerChannel(e){e&&!this.subscriptions.includes(e)&&this.subscriptions.push(e)}registerInterval(e){e!==null&&!this.intervals.includes(e)&&this.intervals.push(e)}registerListener(e,t,n){e&&t&&n&&this.listeners.push({el:e,event:t,fn:n})}destroy(){this.subscriptions.forEach(e=>{try{e&&i.removeChannel(e)}catch(e){console.warn(`[LifecycleManager] Error removing channel (${this.scope}):`,e)}}),this.intervals.forEach(e=>{try{clearInterval(e)}catch(e){console.warn(`[LifecycleManager] Error clearing interval (${this.scope}):`,e)}}),this.listeners.forEach(({el:e,event:t,fn:n})=>{try{e&&t&&n&&e.removeEventListener(t,n)}catch(e){console.warn(`[LifecycleManager] Error removing listener (${this.scope}):`,e)}}),this.subscriptions=[],this.intervals=[],this.listeners=[],console.log(`[LifecycleManager] Cleanup completed for scope: ${this.scope}`)}};async function he(e){let{error:t}=await i.from(`sesiones_clase`).delete().eq(`id`,e);if(t)throw console.error(`Error eliminando sesión:`,t.message),Error(`No se pudo eliminar la sesión`);return{success:!0}}var ge=[{id:`nivel-0`,nombre:`Nivel 0: Iniciación / Descubrimiento`,color:`success`},{id:`nivel-1`,nombre:`Nivel 1: Básico / Formación Técnica`,color:`primary`},{id:`nivel-2`,nombre:`Nivel 2: Intermedio / Desarrollo Solista`,color:`warning`},{id:`nivel-3`,nombre:`Nivel 3: Avanzado / Maestría Institucional`,color:`danger`}];async function T({plan:n=null,esACM:i=!1,onSaved:a=null}={}){let s=[];try{s=await o()}catch(e){console.error(`[EditorPlanificacionModal] Error cargando clases:`,e)}let c=!!(n&&n.id),u=c?`Editar Estructura Curricular`:i?`Diseñador Curricular Institucional (ACM)`:`Nueva Planificación Didáctica`,d={nivelId:n?.nivelId||`nivel-1`,frecuenciaSemanal:n?.frecuenciaSemanal||2,semanasTotales:n?.semanasTotales||24,objetivos:n?.objetivosEstructurados||[{id:`obj-1`,titulo:`Dominio de Postura y Emisión Sonora`,indicadores:[{id:`ind-1`,titulo:`Postura corporal equilibrada y relajada`,prerrequisitoId:null},{id:`ind-2`,titulo:`Distribución fluida del arco en cuerdas abiertas`,prerrequisitoId:`ind-1`}]},{id:`obj-2`,titulo:`Afinación y Articulación Digital`,indicadores:[{id:`ind-3`,titulo:`Colocación exacta de 1er y 2do dedo`,prerrequisitoId:`ind-2`},{id:`ind-4`,titulo:`Independencia digital a pulso 60 BPM`,prerrequisitoId:`ind-3`}]}]},p=n?.clase_id||n?.claseId||``,m=n?.titulo||`Plan Didáctico Semestral (6 Meses)`,h=n?.semana||1,g=`
  ...
  `;try{_e(e,await s())}catch(n){console.error(`[MaestroPlanificacionView] Error:`,n),e.innerHTML=`
    ...
    `}}}function _e(n,r){let i=`todos`,a=()=>{let a=r.filter(e=>i===`todos`?!0:i===`borrador`?e.estado===`borrador`:i===`revisada`?e.estado===`revisada`:i!==`publicada`||e.estado===`publicada`),o=n.querySelector(`#maestro-planes-list`);if(o){if(a.length===0){o.innerHTML=`
      ...
      `,o.querySelector(`#btn-crear-primer-plan`)?.addEventListener(`click`,()=>{T({onSaved:()=>E(n)})});return}o.innerHTML=a.map(e=>{let n=ve(e.estado),r=Array.isArray(e.contenidos)?e.contenidos:typeof e.contenidos==`string`?[e.contenidos]:[];return`
  ...
  `,n.querySelector(`#btn-crear-disenador-full`)?.addEventListener(`click`,()=>{y.navigate(`planificacion-disenador`)}),n.querySelector(`#btn-ver-ruta-full`)?.addEventListener(`click`,()=>{y.navigate(`planificacion-ruta`)}),n.querySelector(`#chk-select-all`)?.addEventListener(`change`,e=>{let t=e.target.checked;n.querySelectorAll(`.chk-plan`).forEach(e=>{e.checked=t,t?a.add(e.dataset.id):a.delete(e.dataset.id)}),s()}),n.querySelector(`#btn-aprobar-masivo`)?.addEventListener(`click`,async()=>{if(a.size!==0)try{await c(Array.from(a)),e.show(`${a.size} planificaciones aprobadas con éxito`,`success`),a.clear(),O(n)}catch(t){e.show(`Error en aprobación masiva: ${t.message}`,`error`)}}),n.querySelectorAll(`#acm-filter-tabs button`).forEach(e=>{e.addEventListener(`click`,()=>{n.querySelectorAll(`#acm-filter-tabs button`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),i=e.dataset.filter,a.clear(),s(),o()})}),o()}function xe(e){return e===`publicada`?`<span class="badge bg-success-subtle text-success border border-success-subtle"><i class="bi bi-check-circle me-1"></i>Aprobado</span>`:e===`revisada`?`<span class="badge bg-warning-subtle text-warning-emphasis border border-warning-subtle"><i class="bi bi-clock me-1"></i>Pendiente ACM</span>`:`<span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle"><i class="bi bi-pencil-square me-1"></i>Borrador</span>`}async function Se(e){if(e){e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5">
      <div class="spinner-border text-primary" role="status">
  ...
  `;let n=e.querySelector(`#acm-propuestas-list`),r=e.querySelector(`#acm-propuestas-detail`),i=[];try{i=await Le()}catch(e){console.error(`[acmPropuestasView] Error cargando propuestas:`,e),n.innerHTML=`<p class="acm-error">Error al cargar propuestas: ${t(e.message)}</p>`;return}if(!i.length){n.innerHTML=`<p class="acm-placeholder">No hay propuestas pendientes de revisión.</p>`;return}n.innerHTML=i.map(e=>`
    ...
    `).join(``),n.querySelectorAll(`[data-propuesta-id]`).forEach(t=>{t.addEventListener(`click`,()=>{let n=i.find(e=>e.id===t.dataset.propuestaId);n&&Ve(r,n,{onResolved:()=>Be(e)})})})}function Ve(e,t,{onResolved:n}){e.innerHTML=`
    <div class="acm-propuesta-detail-inner">
      <h3>Árbol de contenido propuesto</h3>
      <div class="acm-tree">${He(t.levels||[])}</div>
  ...
  `;let r=``;e.querySelector(`[data-role="feedback-textarea"]`).addEventListener(`input`,e=>{r=e.target.value}),e.querySelector(`[data-action="publicar"]`).addEventListener(`click`,async()=>{try{await Re(t.id),n()}catch(e){console.error(`[acmPropuestasView] Error al publicar:`,e),window.alert(`Error al publicar: ${e.message}`)}}),e.querySelector(`[data-action="devolver"]`).addEventListener(`click`,async()=>{if(!r.trim()){window.alert(`Escriba un feedback antes de devolver la propuesta.`);return}try{await ze(t.id,r),n()}catch(e){console.error(`[acmPropuestasView] Error al devolver:`,e),window.alert(`Error al devolver: ${e.message}`)}})}function He(e){return e.length?e.slice().sort((e,t)=>(e.level_number||0)-(t.level_number||0)).map(e=>`
    ...
    `).join(``):`<p class="acm-placeholder">Esta propuesta no tiene niveles.</p>`}var Ue=[`borrador`,`activo`,`archivado`];async function We(e,t){if(!e||!t)throw Error(`claseId y routeVersionId son requeridos`);let{data:n}=await i.from(`class_curriculum_plan`).select(`id`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();n?.id&&await i.from(`class_curriculum_plan`).update({estado:`archivado`}).eq(`id`,n.id);let{data:r,error:a}=await i.from(`class_curriculum_plan`).insert({clase_id:e,route_version_id:t,estado:`activo`}).select().single();if(a)throw a;return r}async function Ge(e){let{data:t,error:n}=await i.from(`class_curriculum_plan`).select(`*`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();if(n)throw n;return t}async function Ke(e,t){if(!Ue.includes(t))throw Error(`Estado no válido`);let{data:n,error:r}=await i.from(`class_curriculum_plan`).update({estado:t}).eq(`clase_id`,e).select().single();if(r)throw r;return n}var qe=[`nombre`,`descripcion`,`order_index`];async function Je(e){if(!Array.isArray(e)||e.length===0)throw Error(`Se requiere al menos un objetivo`);let t=e.map(e=>({clase_id:e.clase_id,level_id:e.level_id,nombre:e.nombre,descripcion:e.descripcion||null,orden_objetivo:e.orden_objetivo??null,origen_node_id:e.origen_node_id||null,origen_objetivo_id:e.origen_objetivo_id||null,created_by:e.created_by||null})),{data:n,error:r}=await i.from(`clase_mapa_objetivos`).insert(t).select();if(r)throw r;return n}async function L(e){let{data:t,error:n}=await i.from(`clase_mapa_objetivos`).select(`*`).eq(`clase_id`,e).is(`archived_at`,null).order(`order_index`);if(n)throw n;return t||[]}async function Ye(e,t){let n={};for(let e of qe)t[e]!==void 0&&(n[e]=t[e]);if(Object.keys(n).length===0)throw Error(`No hay campos válidos para actualizar (permitidos: nombre, descripcion, order_index)`);let{data:r,error:a}=await i.from(`clase_mapa_objetivos`).update(n).eq(`id`,e).select().single();if(a)throw a;return r}async function Xe(e){let{error:t}=await i.from(`clase_mapa_objetivos`).delete().eq(`clase_id`,e);if(t)throw t}var Ze=class{constructor(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._listeners=[]}subscribe(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}_notify(){this._listeners.forEach(e=>e(this))}async fetchRutaDeClase(e){this.cargando=!0,this.error=null,this._notify();try{return this.rutaAsignada=await Ge(e),this.cargando=!1,this._notify(),this.rutaAsignada}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async asignarRuta(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await We(e,t);return this.rutaAsignada=n,this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async cambiarEstado(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await Ke(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{return this.objetivos=await L(e),this.cargando=!1,this._notify(),this.objetivos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async agregarObjetivos(e,t,n){this.cargando=!0,this.error=null,this._notify();try{let r=await Je(n.map(n=>({planificacion_id:e,class_curriculum_plan_id:t,node_id:n.node_id,indicator_id:n.indicator_id,semana:n.semana||null})));return this.objetivos=await L(e),this.cargando=!1,this._notify(),r}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async actualizarObjetivo(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await Ye(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async eliminarObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{await Xe(e),this.objetivos=[],this.cargando=!1,this._notify()}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async evaluarAlumno(e){this.cargando=!0,this.error=null,this._notify();try{let t=await p(e);return this.evaluaciones=await h(e.clase_id),this.cargando=!1,this._notify(),t}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchEvaluaciones(e){this.cargando=!0,this.error=null,this._notify();try{return this.evaluaciones=await h(e),this.cargando=!1,this._notify(),this.evaluaciones}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchProgresoAlumnos(e){this.cargando=!0,this.error=null,this._notify();try{return this.progresoAlumnos=await m(e),this.cargando=!1,this._notify(),this.progresoAlumnos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}reset(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._notify()}},R=null;function Qe(){return R||=new Ze,R}var z=Qe();function $e(e){if(!e)return;let t=z.subscribe(()=>B(e));return B(e),z.fetchRutaDeClase(``).catch(()=>{}),()=>t()}function B(e){if(z.cargando&&!z.rutaAsignada&&z.objetivos.length===0){e.innerHTML=`
      ...
      </div>`;return}if(z.error&&!z.rutaAsignada){e.innerHTML=`
      <div class="clase-plan-view">
        <div class="alert alert-warning d-flex align-items-start gap-3 m-3" role="alert">
          ...
          <div>
            ...
            <p class="mb-0 small">${V(z.error)}</p>
          </div>
        </div>
      </div>`;return}let t=z.rutaAsignada,n=z.objetivos||[];e.innerHTML=`
    <div class="clase-plan-view">
  ...
  `,rt(e)}function et(e){let t=e.estado===`activo`?`Activa`:e.estado===`borrador`?`Borrador`:`Archivada`,n=e.estado===`activo`?`success`:e.estado===`borrador`?`warning`:`secondary`;return`
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            <small class="text-muted">ID: ${V(e.route_version_id||e.id)}</small>
          </div>
        </div>
    ...
    <div class="page-glass rounded p-4 mb-4 text-center">
    ...
    </div>`}function nt(e){if(e.length===0)return`
      <div class="page-glass rounded p-3 mb-4">
      ...
      <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
        <div class="d-flex align-items-center gap-2">
          ...
          <span class="small">${V(e.indicator_description||e.indicator_id)}</span>
        </div>
        <div class="d-flex align-items-center gap-2">
          ${t}
    ...
    <div class="page-glass rounded p-3 mb-4">
      ...
      ${t}
    </div>`}function rt(e){e.querySelectorAll(`[data-action="evaluar"]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.indicatorId;document.dispatchEvent(new CustomEvent(`evaluacion:open`,{detail:{indicatorId:t}}))})})}function V(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}var it=n({renderDisenadorCurricularView:()=>H});async function H(e){if(!e)return;e.innerHTML=`
  ...
  `;let t=[];try{t=await o()}catch(e){console.error(`[DisenadorCurricularView] Error cargando clases:`,e)}let n={claseId:t[0]?.id||``,niveles:[],plantillas:[],plantillaId:``,nivelIdIA:``};await U(n),W(e,t,n)}async function U(e){if(!e.claseId){e.niveles=[],e.plantillas=[],e.plantillaId=``,e.nivelIdIA=``;return}try{e.niveles=await x(e.claseId)}catch(t){console.error(`[DisenadorCurricularView] Error cargando niveles asignados:`,t),e.niveles=[]}try{let t=await re(),n=new Set(e.niveles.map(e=>e.id));e.plantillas=t.filter(e=>n.has(e.level_id))}catch(t){console.error(`[DisenadorCurricularView] Error cargando plantillas:`,t),e.plantillas=[]}e.plantillaId=e.plantillas[0]?.id||``,e.nivelIdIA=e.niveles[0]?.id||``}function W(e,n,r){let i=!!r.claseId&&r.niveles.length===0,a=!r.claseId||i;e.innerHTML=`
  ...
  `,at(e,n,r)}function at(t,n,r){t.querySelector(`#btn-volver-acm`)?.addEventListener(`click`,()=>{y.navigate(`planificacion-acm`)}),t.querySelector(`#select-clase-disenador`)?.addEventListener(`change`,async e=>{r.claseId=e.target.value,await U(r),W(t,n,r)}),t.querySelector(`#select-plantilla-disenador`)?.addEventListener(`change`,e=>{r.plantillaId=e.target.value}),t.querySelector(`#select-nivel-ia-disenador`)?.addEventListener(`change`,e=>{r.nivelIdIA=e.target.value}),t.querySelector(`#btn-clonar-plantilla`)?.addEventListener(`click`,async()=>{if(!r.claseId||!r.plantillaId)return;let n=t.querySelector(`#btn-clonar-plantilla`);n.disabled=!0;try{await ne(r.claseId,r.plantillaId),e.show(`Plantilla clonada a la clase con éxito ⭐`,`success`),y.navigate(`planificacion-acm`)}catch(t){e.show(`Error al clonar la plantilla: ${t.message}`,`error`),n.disabled=!1}}),t.querySelector(`#btn-generar-ia`)?.addEventListener(`click`,async()=>{if(!r.claseId||!r.nivelIdIA)return;let i=t.querySelector(`#btn-generar-ia`);i.disabled=!0,e.show(`Generando estructura curricular con IA (GROQ)...`,`info`);try{let t=n.find(e=>String(e.id)===String(r.claseId)),i=r.niveles.findIndex(e=>e.id===r.nivelIdIA),a=await b({instrumento:t?.nombre||t?.name||`Música`,nivelIndex:i>=0?i:0});for(let e of a||[]){let t=await ae({clase_id:r.claseId,level_id:r.nivelIdIA,nombre:e.titulo||e.id||`Objetivo generado por IA`});for(let n of e.indicadores||[])await ie({objetivo_id:t.id,clase_id:r.claseId,descripcion:typeof n==`string`?n:n.titulo||n.id||`Indicador generado por IA`})}e.show(`Estructura generada por IA creada en el mapa de la clase ⭐`,`success`),y.navigate(`planificacion-acm`)}catch(t){console.error(`[DisenadorCurricularView] Error generando con IA:`,t),e.show(`Error al generar con IA: ${t.message}`,`error`),i.disabled=!1}})}var ot=n({renderRutaPedagogicaView:()=>G});async function G(e){if(!e)return;e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5" style="min-height: 450px;">
      <div class="text-center">
        <div class="spinner-border text-primary mb-3" style="width: 3rem; height: 3rem;" role="status"></div>
  ...
  `;let t=[],n=[];try{let[e,r]=await Promise.all([o(),s()]);t=e||[],n=r||[]}catch(e){console.error(`[RutaPedagogicaView] Error:`,e)}st(e,t,n)}function st(n,r,i){let a=r[0]?.id||``,o=null,s=[],c=new Map,l=!0,u=async()=>{c.clear(),l=!0,s=await _(a),p()},d=()=>{let e=n.querySelector(`#panel-alumnos-evaluacion-nodo`),r=n.querySelector(`#lbl-nodo-seleccionado`);e&&(e.style.display=o?`block`:`none`),r&&(r.innerHTML=`<i class="bi bi-award-fill text-warning me-2"></i>${t(o?.titulo||`Selecciona un Nodo`)}`)},f=()=>{let e=n.querySelector(`#tbody-alumnos-ruta`);if(!e)return;e.innerHTML=s.map(e=>{let n=e.nombre.split(` `).slice(0,2).map(e=>e[0]).join(``).toUpperCase();return`
          <tr class="row-alumno-ruta${l?``:` opacity-50`}" data-id="${e.id}" style="cursor: pointer;">
            ...
            <td class="text-center">
              ...
              <small class="fw-bold text-body-secondary">${e.estrellas>0?`${e.estrellas} Estrellas (${ut(e.estrellas)})`:`Sin Registrar (0★)`}</small>
            </td>
            ...
            <div class="col-md-3 col-6">
              <div class="d-flex align-items-center gap-2">
                ...
                <div>
                  ...
                  <div class="fw-bold fs-5" id="kpi-evaluados-count">${b} / ${g}</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-6">
