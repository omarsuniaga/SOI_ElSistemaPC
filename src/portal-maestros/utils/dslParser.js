const TOKEN_PATTERNS = {
  ...
  alumnos:
    /#(todos\b|[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+(?:de|la|las|los|del|y|el)\b)?(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)*|[A-Za-zÁÉÍÓÚáéíóúÑñ]+)/g,
  contenido: /\[([^\]]+)\]/g,
  sugerencias: /\(([^)]+)\)/g,
  tareas: /\{([^}]+)\}/g,
  medidas: /\$([^\s$]+)/g,
...
export function highlightDSL(text) {
  ...
  result = result.replace(TOKEN_PATTERNS.capas, (_, capa) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-capa" style="background:${TOKEN_COLORS.capas}22; color:${TOKEN_COLORS.capas}; font-weight:800; padding:2px 6px; border-radius:4px">:::CAPA: ${capa}</span>`,
    )
  })
  ...
  result = result.replace(/&gt;NIVEL-(\d{1,2})/g, (_, nivel) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-nivel" style="color:${TOKEN_COLORS.niveles}; font-weight:700">&gt;NIVEL-${nivel}</span>`,
    )
  })
  ...
  result = result.replace(/&gt;NODO:([A-Z_]+)/g, (_, nodo) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-nodo" style="color:${TOKEN_COLORS.nodos}; font-weight:600">&gt;NODO:${nodo}</span>`,
    )
  })
  ...
  result = result.replace(/&gt;([A-Z]{1,3}(?:-[A-Za-z]+)?-[0-9]+(?:\.[0-9]+)?)/g, (_, objetivo) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-objetivo" data-objetivo="${objetivo}">&gt;${objetivo}</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.alumnos, (_, nombre) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-alumno" data-nombre="${nombre}">#${nombre}</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.contenido, (_, contenido) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-contenido" data-contenido="${contenido}">[${contenido}]</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.sugerencias, (_, sugerencia) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-sugerencia" data-sugerencia="${sugerencia}">(${sugerencia})</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.tareas, (_, tarea) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-tarea" data-tarea="${tarea}">{${tarea}}</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.medidas, (_, medida) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-medida" data-medida="${medida}">$${medida}</span>`,
    )
  })
  ...
  result = result.replace(TOKEN_PATTERNS.calificacion, (_, v, s) => {
    return pushPlaceholder(
      `<span class="dsl-token dsl-calificacion" data-valor="${v}" data-sobre="${s}">${v}/${s}</span>`,
    )
  })
...
export function getTokenSummary(parsed) {
  ...
  if (parsed.estados && parsed.estados.length > 0)
    summary.push(`${parsed.estados.length} estado(s)`)
  ...
  if (Object.keys(parsed.por_capas).length > 0)
    summary.push(`${Object.keys(parsed.por_capas).length} capa(s)`)
