const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/pushService-DVAfSSV4.js","assets/rolldown-runtime-DlOssbPu.js","assets/supabase-KnARm58N.js","assets/maestroAuth-lT-ZcZZd.js","assets/permisoService-C_6egrWz.js","assets/permisosSupabase-BFQ2fEUb.js","assets/sistemaView-BjwJO1bj.js","assets/router-DSDN-AHb.js","assets/vendor-BUMNmNsE.js","assets/vendor-COf7rB16.css","assets/CHANGELOG-BUFzQeSb.js","assets/groqService-DqCFHp7y.js","assets/errorReporter-CDvSLm79.js","assets/AppToast-Bli1nFQQ.js","assets/ausenciasPanel-Di0cvPNk.js","assets/ausenciaModal-D8oQIM5E.js","assets/focusTrap-_L6o1rch.js","assets/portalUtils-DbrsCFDo.js","assets/AppModal-Fjeb_yOo.js"])))=>i.map(i=>d[i]);
import{i as e}from"./supabase-KnARm58N.js";import{i as t,n,o as r}from"./maestroAuth-lT-ZcZZd.js";import{a as i,c as a,d as o,f as s,i as c,l,n as u,p as d,r as f}from"./pushService-DVAfSSV4.js";import{t as p}from"./focusTrap-_L6o1rch.js";import{c as m,i as h}from"./portalUtils-DbrsCFDo.js";import{t as g}from"./preload-helper-lqsI3teB.js";import{t as _}from"./AppModal-Fjeb_yOo.js";import{n as v}from"./phoneUtils-BhES4eRi.js";import{t as y}from"./sanitize-rcBeFe96.js";import{n as b}from"./disponibilidadApi-CWsB8tCY.js";import{n as x}from"./CHANGELOG-BUFzQeSb.js";import{t as S}from"./ausenciaModal-D8oQIM5E.js";var C={async open(){let e=await u(),t=await c();_.open({title:`Notificaciones`,size:`md`,body:this._renderBody(e,t),saveText:`Hecho`,onShow:e=>this._initLogic(e),onSave:async e=>(await this._handleSave(e),!0)})},_renderBody(e,t){return`
    ...
    `},_initLogic(e){e.querySelector(`#modal-notif-test`).addEventListener(`click`,async()=>{await s()||window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Primero activa las notificaciones`,type:`warning`}}))});let t=e.querySelector(`#modal-notif-push`);t.addEventListener(`change`,async()=>{if(t.checked,t.checked){let e=await o();e.success?this._toast(`Notificaciones activadas`,`success`):(t.checked=!1,this._toast(e.error||`Error al suscribir`,`danger`))}else (await d()).success&&this._toast(`Notificaciones desactivadas`,`info`)})},async _handleSave(e){await l({alerta_pre_clase:e.querySelector(`#modal-notif-pre`).checked,min_antes_clase:parseInt(e.querySelector(`#modal-notif-min-antes`).value,10),alerta_post_clase:e.querySelector(`#modal-notif-post`).checked,min_post_clase_sin_registro:parseInt(e.querySelector(`#modal-notif-min-post`).value,10),alerta_24h:e.querySelector(`#modal-notif-24h`).checked,alerta_48h:!0}),this._toast(`Preferencias guardadas`,`success`)},_toast(e,t){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:e,type:t}}))}},w={container:null,async init(){document.getElementById(`push-diagnostic-panel`)||(this.createPanel(),await this.checkStatus())},createPanel(){this.container=document.createElement(`div`),this.container.id=`push-diagnostic-panel`,this.container.innerHTML=`
    ...
    `,document.head.appendChild(e)},bindEvents(){document.getElementById(`push-diagnostic-close`).addEventListener(`click`,()=>this.close()),document.getElementById(`push-diagnostic-overlay`).addEventListener(`click`,e=>{e.target.id===`push-diagnostic-overlay`&&this.close()}),document.getElementById(`btn-enable-push`).addEventListener(`click`,()=>this.enablePush()),document.getElementById(`btn-test-push`).addEventListener(`click`,()=>this.testPush())},async checkStatus(){let e=[],t=document.getElementById(`diagnostic-details`),n=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent),r=/iPhone|iPad|iPod/i.test(navigator.userAgent),a=/^((?!chrome|android).)*safari/i.test(navigator.userAgent);e.push({text:`📱 Dispositivo: ${n?r?`iOS`:`Android`:`Desktop`}`,type:`info`}),n&&a&&e.push({text:`⚠️ iOS Safari: Requiere iOS 16.4+ y agregar a pantalla de inicio`,type:`warn`});let o=i();e.push({text:`Navegador: ${o?`✅ Compatible`:`❌ No compatible`}`,type:o?`ok`:`error`}),this.updateStatusItem(`status-browser`,o);let s=`default`;`Notification`in window&&(s=Notification.permission);let c=s===`granted`;e.push({text:`Permiso: ${s===`granted`?`✅ Otorgado`:s===`denied`?`❌ Denegado - ve a Configuración del navegador`:`⚠️ No solicitado - click en Activar abajo`}`,type:c?`ok`:`warn`}),this.updateStatusItem(`status-permission`,c,s);let l=`no-registrado`,u=!1;if(`serviceWorker`in navigator)try{let t=await navigator.serviceWorker.getRegistration(`/sw.js`);t?(l=t.active?`✅ Activo`:`⏳ Registrado`,u=!!t.active,e.push({text:`Service Worker: ${l}`,type:`ok`})):e.push({text:`Service Worker: ❌ No registrado`,type:`error`}),this.updateStatusItem(`status-serviceworker`,u)}catch(t){e.push({text:`Service Worker: ❌ Error - ${t.message}`,type:`error`}),this.updateStatusItem(`status-serviceworker`,!1)}else e.push({text:`Service Worker: ❌ No soportado`,type:`error`}),this.updateStatusItem(`status-serviceworker`,!1);let d=`no-suscrito`;if(u)try{let t=await f();d=t.subscribed?`✅ Suscrito`:`❌ No suscrito`,e.push({text:`Suscripción: ${d}`,type:t.subscribed?`ok`:`warn`}),this.updateStatusItem(`status-subscription`,t.subscribed)}catch(t){e.push({text:`Suscripción: ❌ Error - ${t.message}`,type:`error`}),this.updateStatusItem(`status-subscription`,!1)}else e.push({text:`Suscripción: ⚠️ SW inactivo`,type:`warn`}),this.updateStatusItem(`status-subscription`,!1);return t.innerHTML=e.map(e=>`<div class="log-item log-${e.type}">${e.text}</div>`).join(``),{browserSupported:o,permOk:c,swActive:u}},updateStatusItem(e,t,n=``){let r=document.getElementById(e);r.className=`push-status-item ${t?`success`:`warning`}`;let i=r.querySelector(`.status-value`);i.textContent=t?`✓ Listo`:`⚠ Revisar`,n&&(i.textContent+=` (${n})`)},async enablePush(){let e=document.getElementById(`diagnostic-result`),t=document.getElementById(`btn-enable-push`);t.disabled=!0,t.innerHTML=`<span class="pm-spinner-sm me-2"></span> Configurando...`;try{e.className=`push-diagnostic-result info`;let t=/Android|webOS|iPhone|iPad|iPod/i.test(navigator.userAgent),n=/iPhone|iPad|iPod/i.test(navigator.userAgent);t&&n?e.innerHTML=`📱 iOS detectado: Se abrirá una solicitud de permiso...`:t?e.innerHTML=`📱 Android detectado: Solicitando permiso...`:e.innerHTML=`Solicitando permiso de notificaciones...`;let{granted:r,error:i}=await a();if(!r)throw Error((i||`Permiso denegado`)+(t?`<br><br><strong>En móvil:</strong> Ve a Configuración → Safari → Notificaciones → Permitir`:``));e.innerHTML=`Registrando en el sistema de notificaciones...`;let s=await o();if(!s.success)throw Error(s.error||`Error al suscribirse a push`);e.className=`push-diagnostic-result success`;let c=/Android|webOS|iPhone|iPad|iPod/i.test(navigator.userAgent),l=`✅ ¡Notificaciones push activadas!`;c&&(l+=`<br><small>💡 En móvil, agrega la app a pantalla de inicio para notificaciones completas (botón Compartir → Agregar a pantalla de inicio)</small>`),e.innerHTML=l,await this.checkStatus(),setTimeout(()=>this.testPush(),2e3)}catch(t){e.className=`push-diagnostic-result error`,e.innerHTML=`❌ ${t.message}`}finally{t.disabled=!1,t.innerHTML=`<i class="bi bi-bell me-2"></i>Activar Notificaciones Push`}},async testPush(){let e=document.getElementById(`diagnostic-result`);try{let t=await s();t.success?(e.className=`push-diagnostic-result success`,e.innerHTML=`✅ ${t.method===`serviceWorker`?`¡Notificación del sistema enviada! Deberías verla en tu escritorio.`:`Notificación enviada (modo local).`}`):(e.className=`push-diagnostic-result error`,e.innerHTML=`❌ ${y(t.error)}`)}catch(t){e.className=`push-diagnostic-result error`,e.innerHTML=`❌ Error: ${y(t.message)}`}},open(){this.init();let e=document.getElementById(`push-diagnostic-overlay`);e.style.display=`flex`;let t=document.querySelector(`#push-diagnostic-panel .push-diagnostic-card`);t&&(this._trap&&this._trap.dispose(),this._trap=p(t,{onClose:()=>this.close()}))},close(){this._trap&&=(this._trap.dispose(),null);let e=document.getElementById(`push-diagnostic-overlay`);e&&(e.style.display=`none`)}},T={dirty:!1,saving:!1,theme:localStorage.getItem(`portal-maestros-theme`)||`system`,pushEnabled:!1},E=[{key:`lunes`,label:`Lunes`},{key:`martes`,label:`Martes`},{key:`miércoles`,label:`Miércoles`},{key:`jueves`,label:`Jueves`},{key:`viernes`,label:`Viernes`},{key:`sábado`,label:`Sábado`},{key:`domingo`,label:`Domingo`}];function D(e){let n=t();if(!n){e.innerHTML=`
      <div class="pm-settings-empty">
        <i class="bi bi-shield-lock"></i>
        <p>No hay sesión activa.</p>
      </div>`;return}T.dirty=!1,T.saving=!1,g(()=>import(`./pushService-DVAfSSV4.js`).then(e=>e.s).then(async e=>{T.pushEnabled=(await e.getNotificationPreferences()).push_activo;let t=document.querySelector(`#btn-toggle-push-main input`);t&&(t.checked=T.pushEnabled);let n=document.getElementById(`pm-notif-sub-badge`);n&&(n.textContent=T.pushEnabled?`✅ Suscripción activa`:`⏸ Pausada`)}),__vite__mapDeps([0,1,2,3])),e.innerHTML=`
    <div class="pm-settings pm-fade-in" role="main" aria-label="Configuración del perfil">
      ...
      <footer class="pm-settings-footer">
        ...
        <p class="pm-settings-footer__version">
          v${x} &copy; 2026
          ${n?.es_admin?`<button id="pm-ver-sistema-btn" style="
    ...
    </div>`;let r=document.getElementById(`col-izquierda`),i=document.getElementById(`col-derecha`);O(r,n),k(r,n),z(r,n),A(i),j(i,n),M(i),N(i,n),i.insertAdjacentHTML(`beforeend`,`<div id="pm-collaboration-container"></div>`),I(i),L(i),q(n),H(n),J(),g(async()=>{let{getPermisos:e,solicitarPermiso:t}=await import(`./permisoService-C_6egrWz.js`).then(e=>e.n);return{getPermisos:e,solicitarPermiso:t}},__vite__mapDeps([4,1,5,2])).then(async({getPermisos:e,solicitarPermiso:t})=>{try{let r=await e(n.id),i=document.getElementById(`pm-collaboration-container`);i&&R(i,r,n.id,t)}catch(e){console.warn(`[PerfilView] Error cargando permisos de colaboración:`,e.message)}})}function O(e,t){let n=m(t.nombre_completo);e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-profile-hero" aria-label="Información del perfil">
      <div class="pm-profile-hero__content">
        <div class="pm-settings-avatar">
          ${t.avatar_url?`<img src="${h(t.avatar_url)}" alt="Avatar" class="pm-settings-avatar__img">`:`<div class="pm-settings-avatar__placeholder" aria-hidden="true">${h(n)}</div>`}
        ...
        <div class="pm-profile-hero__info">
          <h2 class="pm-profile-hero__name">${h(t.nombre_completo)}</h2>
          <p class="pm-profile-hero__email">${h(t.email)}</p>
          ${t.especialidad?`
            <span class="chip-apple active" aria-label="Especialidad: ${h(t.especialidad)}">
              <i class="bi bi-mortarboard" aria-hidden="true"></i> ${h(t.especialidad)}
            </span>`:``}
        </div>
      </div>
    ...
    <section class="card-apple pm-settings-section" aria-labelledby="datos-title">
      ...
      <div class="pm-settings-form-grid">
        <div class="pm-settings-field">
          ...
          <input type="text" class="input-apple" id="perfilNombre" value="${h(t.nombre_completo)}" placeholder="Tu nombre">
        </div>
        <div class="pm-settings-field">
          <label for="perfilTelefono" class="apple-caption">Teléfono</label>
          <input type="tel" class="input-apple" id="perfilTelefono" value="${h(t.tlf||t.telefono||``)}" placeholder="809-000-0000" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}">
        </div>
        <div class="pm-settings-field">
          <label for="perfilEspecialidad" class="apple-caption">Especialidad</label>
          <input type="text" class="input-apple" id="perfilEspecialidad" value="${h(t.especialidad||``)}" placeholder="Ej. Violín">
        </div>
      </div>
    ...
    </section>`)}function j(e,t){let n=i(),r=n?`<span class="pm-badge-sub" id="pm-notif-sub-badge" aria-live="polite" aria-atomic="true">${T.pushEnabled?`✅ Suscripción activa`:`⏸ Pausada`}</span>`:``;e.insertAdjacentHTML(`beforeend`,`
    ...
    </section>`),P(t)}async function P(t){await F(t.id),document.getElementById(`form-sol-necesidad`)?.addEventListener(`submit`,async n=>{n.preventDefault();let r=document.getElementById(`sol-tipo`)?.value?.trim(),i=document.getElementById(`sol-titulo`)?.value?.trim(),a=document.getElementById(`sol-descripcion`)?.value?.trim(),o=document.getElementById(`sol-prioridad`)?.value||`media`;if(!r||!i||!a){[{id:`sol-tipo`,v:r},{id:`sol-titulo`,v:i},{id:`sol-descripcion`,v:a}].forEach(({id:e,v:t})=>document.getElementById(e)?.classList.toggle(`is-invalid`,!t));return}let s=document.getElementById(`btn-sol-submit`),c=document.getElementById(`sol-status`);s&&(s.disabled=!0,s.innerHTML=`<span class="spinner-border spinner-border-sm me-1"></span>Enviando...`),c&&(c.style.display=`none`);try{let{error:n}=await e.from(`solicitudes_necesidades`).insert({maestro_id:t.id,maestro_nombre:t.nombre_completo||t.nombre||``,tipo_necesidad:r,categoria:document.getElementById(`sol-categoria`)?.value||null,titulo:i,descripcion:a,prioridad:o,cantidad:parseInt(document.getElementById(`sol-cantidad`)?.value)||null,area:document.getElementById(`sol-area`)?.value?.trim()||null,observaciones:document.getElementById(`sol-observaciones`)?.value?.trim()||null,estado:`pendiente`,fecha_solicitud:new Date().toISOString().split(`T`)[0]});if(n)throw n;document.getElementById(`form-sol-necesidad`)?.reset(),document.querySelectorAll(`#pm-solicitudes-section .is-invalid`).forEach(e=>e.classList.remove(`is-invalid`)),c&&(c.textContent=`✓ Solicitud enviada correctamente`,c.className=`small text-success`,c.style.display=`inline`),await F(t.id)}catch(e){console.error(`[solicitudes]`,e),c&&(c.textContent=`Error al enviar. Intentá de nuevo.`,c.className=`small text-danger`,c.style.display=`inline`)}finally{s&&(s.disabled=!1,s.innerHTML=`<i class="bi bi-send me-1"></i>Enviar solicitud`)}})}async function F(t){let n=document.getElementById(`sol-historial`);if(n)try{let{data:r,error:i}=await e.from(`solicitudes_necesidades`).select(`*`).eq(`maestro_id`,t).order(`created_at`,{ascending:!1}).limit(20);if(i)throw i;if(!r||r.length===0){n.innerHTML=`<p class="text-muted small fst-italic mb-0">No tenés solicitudes anteriores.</p>`;return}let a={pendiente:`bg-warning-subtle text-warning-emphasis`,en_revision:`bg-info-subtle text-info-emphasis`,aprobada:`bg-success-subtle text-success-emphasis`,rechazada:`bg-danger-subtle text-danger-emphasis`,resuelta:`bg-secondary-subtle text-secondary-emphasis`},o={urgente:`text-danger`,alta:`text-warning`,media:`text-primary`,baja:`text-secondary`};n.innerHTML=r.map(e=>`
      <div class="card border-0 shadow-sm mb-2">
        <div class="card-body py-2 px-3">
          <div class="d-flex justify-content-between align-items-start gap-2">
            <div class="flex-grow-1 overflow-hidden">
              <div class="fw-semibold small text-truncate">${h(e.titulo)}</div>
              <div class="text-muted" style="font-size:0.72rem;">
                ${h(e.tipo_necesidad)} · ${h(e.fecha_solicitud||`—`)} ·
                <span class="${o[e.prioridad]||`text-secondary`}">${h(e.prioridad)}</span>
              </div>
            </div>
            <span class="badge flex-shrink-0 ${a[e.estado]||`bg-secondary-subtle text-secondary-emphasis`}">${h((e.estado||``).replace(`_`,` `))}</span>
          </div>
          ${e.respuesta_admin?`
            <div class="border-start border-2 border-primary ps-2 mt-2">
              ...
              <div class="small">${h(e.respuesta_admin)}</div>
            </div>`:``}
        </div>
      </div>`).join(``)}catch(e){console.error(`[historial sol]`,e),n&&(n.innerHTML=`<p class="text-danger small mb-0">Error al cargar el historial.</p>`)}}function I(e){e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-settings-section" aria-labelledby="install-title" id="pm-install-section">
      ...
      <div id="pm-install-body">
        ...
        • <b>Firefox:</b> no admite PWA nativa.`)}),window.addEventListener(`beforeinstallprompt`,()=>{t&&(t.disabled=!1)},{once:!0})}function L(e){e.insertAdjacentHTML(`beforeend`,`
    ...
    </section>`)}function R(e,t,n,r){let i=t?.solicitud_actual,a=i?.solicita_alumnos||!1,o=i?.solicita_clases||!1,s=i?.estado||null;e.innerHTML=`
  ...
  `,e.querySelectorAll(`.pm-collab-request-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{let i=t.dataset.key;t.disabled=!0;let a=t.innerHTML;t.innerHTML=`<span class="pm-settings-spinner"></span> Enviando...`;try{await r(n,i),window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Solicitud enviada correctamente. Esperando aprobación admin.`,type:`success`}}));let{getPermisos:t}=await g(async()=>{let{getPermisos:e}=await import(`./permisoService-C_6egrWz.js`).then(e=>e.n);return{getPermisos:e}},__vite__mapDeps([4,1,5,2]));R(e,await t(n),n,r)}catch(e){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Error al enviar solicitud: `+e.message,type:`danger`}})),t.disabled=!1,t.innerHTML=a}})}),e.querySelectorAll(`.pm-collab-action-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.route;t===`registrar-alumno`?window.router&&window.router.navigate(`registrar-alumno`):t===`gestionar-clases`&&window.router&&window.router.navigate(`gestionar-clases`)})})}function z(e,t){let n=t.disponibilidad||{},r=!t.especialidad||!t.disponibilidad||Object.keys(t.disponibilidad).length===0;e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-settings-section ${r?`pm-section-warning`:``}" aria-labelledby="disp-title">
      ...
      <div id="pm-avail-days" class="pm-avail-days" role="list">
        ${E.map(e=>B(e.key,n[e.key]||[],e.label)).join(``)}
      </div>
    </section>`)}function B(e,t,n){let r=t.length>0;return`
    ...
    </div>`}function H(e){document.querySelectorAll(`#perfilNombre, #perfilTelefono, #perfilEspecialidad, .pm-apple-time`).forEach(e=>{e.addEventListener(`input`,()=>{T.dirty=!0;let e=document.getElementById(`btnGuardarPerfil`);e&&(e.disabled=!1)})}),document.getElementById(`btnGuardarPerfil`)?.addEventListener(`click`,()=>U(e)),document.getElementById(`btnCerrarSesion`)?.addEventListener(`click`,G),document.getElementById(`pm-ver-sistema-btn`)?.addEventListener(`click`,()=>{g(async()=>{let{renderSistemaView:e}=await import(`./sistemaView-BjwJO1bj.js`);return{renderSistemaView:e}},__vite__mapDeps([6,2,7,1,8,9,10,11,12,13])).then(({renderSistemaView:e})=>{_.open({title:`<i class="bi bi-cpu-fill me-2"></i>Sistema SOI — v${x}`,size:`xl`,saveText:null,cancelText:`Cerrar`,body:`<div id="pm-sistema-modal-body"></div>`,onOpen:t=>{e(t.querySelector(`#pm-sistema-modal-body`))}})})}),document.getElementById(`btnCambiarAvatar`)?.addEventListener(`click`,()=>{window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Cambio de avatar disponible próximamente`,type:`info`}}))});let t=document.querySelector(`#btn-toggle-push-main input`);t?.addEventListener(`change`,async e=>{if(t.disabled=!0,t.checked){let e=await o();e.success?(T.pushEnabled=!0,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Notificaciones activadas`,type:`success`}}))):(t.checked=!1,T.pushEnabled=!1,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:e.error||`Error al activar`,type:`danger`}})))}else await d(),T.pushEnabled=!1,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Notificaciones desactivadas`,type:`info`}}));t.disabled=!1;let n=document.getElementById(`pm-notif-sub-badge`);n&&(n.textContent=T.pushEnabled?`✅ Suscripción activa`:`⏸ Pausada`)}),document.getElementById(`btn-abrir-config-notif`)?.addEventListener(`click`,()=>C.open()),document.getElementById(`btn-push-diagnostic`)?.addEventListener(`click`,()=>w.open()),document.getElementById(`btn-probar-notificacion`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`btn-probar-notificacion`);e.disabled=!0,e.innerHTML=`<span class="pm-settings-spinner"></span> Enviando...`;let t=await s();t.success?(e.innerHTML=`<i class="bi bi-check2"></i> Notificación enviada`,setTimeout(()=>{e.innerHTML=`<i class="bi bi-send"></i> Probar notificación`,e.disabled=!1},2e3)):(e.innerHTML=`<i class="bi bi-exclamation-triangle"></i> Error`,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:t.error||`No se pudo enviar notificación de prueba. Verifica los permisos.`,type:`danger`}})),setTimeout(()=>{e.innerHTML=`<i class="bi bi-send"></i> Probar notificación`,e.disabled=!1},2e3))}),document.getElementById(`pm-theme-light`)?.addEventListener(`click`,()=>K(`light`)),document.getElementById(`pm-theme-dark`)?.addEventListener(`click`,()=>K(`dark`)),document.getElementById(`pm-theme-system`)?.addEventListener(`click`,()=>K(`system`)),document.getElementById(`pm-btn-ver-ausencias`)?.addEventListener(`click`,async()=>{let{ausenciasPanel:e}=await g(async()=>{let{ausenciasPanel:e}=await import(`./ausenciasPanel-Di0cvPNk.js`);return{ausenciasPanel:e}},__vite__mapDeps([14,2,15,3,16,17,18,13]));e.open()}),document.getElementById(`pm-btn-solicitar-ausencia`)?.addEventListener(`click`,()=>S.open()),document.querySelectorAll(`.pm-avail-dia__header`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.dia;let t=e.closest(`.pm-avail-dia`),n=e.getAttribute(`aria-expanded`)===`true`;e.setAttribute(`aria-expanded`,!n),t.classList.toggle(`open`,!n)})}),document.querySelectorAll(`.pm-avail-add-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.dia,n=e.closest(`.pm-avail-dia`),r=document.getElementById(`pm-avail-franjas-${t}`),i=r.querySelectorAll(`.pm-avail-franja`).length;r.insertAdjacentHTML(`beforeend`,V(t,i,{inicio:`08:00`,fin:`12:00`})),n.classList.add(`open`),n.querySelector(`.pm-avail-dia__header`).setAttribute(`aria-expanded`,`true`),T.dirty=!0,document.getElementById(`btnGuardarPerfil`).disabled=!1})}),document.addEventListener(`click`,e=>{let t=e.target.closest(`.pm-avail-franja__del`);t&&(t.closest(`.pm-avail-franja`).remove(),T.dirty=!0,document.getElementById(`btnGuardarPerfil`).disabled=!1)})}async function U(t){let r=document.getElementById(`perfilNombre`).value.trim(),i=v(document.getElementById(`perfilTelefono`).value.trim())||document.getElementById(`perfilTelefono`).value.trim(),a=document.getElementById(`perfilEspecialidad`).value.trim(),o=W();if(!r){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`El nombre es obligatorio`,type:`danger`}}));return}T.saving=!0;let s=document.getElementById(`btnGuardarPerfil`),c=s.innerHTML;s.disabled=!0,s.innerHTML=`<span class="pm-settings-spinner"></span><span>Guardando...</span>`;try{let s=await b(t.id,o);if(!s.success){let e=s.errors.join(`
`);throw Error(e)}let{error:c}=await e.from(`maestros`).update({nombre_completo:r,tlf:i,especialidad:a}).eq(`id`,t.id);if(c)throw c;let l={...t,nombre_completo:r,nombre:r,telefono:i,tlf:i,especialidad:a,disponibilidad:o};localStorage.setItem(n,JSON.stringify(l)),T.dirty=!1,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Perfil actualizado`,type:`success`}})),window.pwaInstaller&&window.pwaInstaller.evaluateInsights()}catch(e){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Error al guardar: `+e.message,type:`danger`}}))}finally{T.saving=!1,s.disabled=!1,s.innerHTML=c}}function W(){let e={};return E.forEach(({key:t})=>{let n=[];document.querySelectorAll(`[data-dia="${t}"].pm-avail-franja`).forEach(e=>{let t=e.querySelector(`[data-field="inicio"]`)?.value,r=e.querySelector(`[data-field="fin"]`)?.value;t&&r&&n.push({inicio:t,fin:r})}),e[t]=n}),e}function G(){_.open({title:`¿Cerrar Sesión?`,size:`sm`,body:`
      ...
      </div>`,saveText:`Salir`,cancelText:`Cancelar`,onSave:async()=>(await r(),window.location.reload(),!0)})}function K(e){let t=e===`system`?window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`:e;document.documentElement.setAttribute(`data-bs-theme`,t),document.documentElement.setAttribute(`data-portal-theme`,t),document.documentElement.classList.toggle(`pm-dark`,t===`dark`),document.querySelectorAll(`.pm-theme-opt`).forEach(t=>{t.setAttribute(`aria-checked`,t.dataset.theme===e?`true`:`false`),t.classList.toggle(`active`,t.dataset.theme===e)}),localStorage.setItem(`portal-maestros-theme`,e),T.theme=e}function q(e){let t=!e.especialidad||!e.disponibilidad||Object.keys(e.disponibilidad||{}).length===0,n=document.getElementById(`pm-banner-perfil-incompleto`);n&&(t?(n.style.display=`block`,n.innerHTML=`
      ...
      </div>`):n.style.display=`none`)}function J(){document.querySelectorAll(`.card-apple`).forEach((e,t)=>{e.style.opacity=`0`,e.style.transform=`translateY(12px)`,setTimeout(()=>{e.style.transition=`opacity 0.4s ease, transform 0.4s ease`,e.style.opacity=`1`,e.style.transform=`translateY(0)`},50*t)})}var Y=`
  .pm-profile-alert {
    grid-column: 1 / -1;
    padding: 0 0 0.5rem;
  }
...
`;if(!document.getElementById(`pm-avail-styles`)){let e=document.createElement(`style`);e.id=`pm-avail-styles`,e.textContent=Y,document.head.appendChild(e)}export{D as renderPerfilView};