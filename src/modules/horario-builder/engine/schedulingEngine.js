...
export function generateOptimizedSchedule({ clasesConMaestro, maestros, salones, config }) {
  ...
  classesToSchedule.forEach((clase) => {
    ...
    Object.keys(currentConfig.jornada).forEach((day) => {
      ...
      const effectiveDisp = getEffectiveAvailability(
        teacher.disponibilidad || {},
        day,
        currentConfig.jornada,
      )
      ...
      const candidateSalones = salones.filter(
        (s) => s.capacidad >= clase.totalAlumnos && s.is_active !== false,
      )
      ...
      effectiveDisp.forEach((availSlot) => {
        ...
        for (let start = availSlot.start; start + duration <= availSlot.end; start += 30) {
          ...
          const teacherConflicts = (teacherSchedules[teacher.id] || []).some(
            (s) =>
          ...
          candidateSalones.forEach((salon) => {
            const salonConflicts = (salonSchedules[salon.id] || []).some(
              (s) =>
                s.day === day && isOverlapping(start, end, s.start, s.end, currentConfig.gapMinimo),
            )
    ...
    if (candidates.length === 0) {
      ...
      const roomsCapable = salones.filter(
        (s) => s.capacidad >= clase.totalAlumnos && s.is_active !== false,
      )
    ...
    candidates.forEach((cand) => {
      ...
      const teacherHours =
        (teacherSchedules[cand.teacher.id] || []).reduce((acc, s) => acc + (s.end - s.start), 0) /
        60
      ...
      const hasAdjacent = (teacherSchedules[cand.teacher.id] || []).some(
        (s) => s.day === cand.day && (s.end === cand.start || s.start === cand.end),
      )
      if (hasAdjacent) {
  ...
  salones.forEach((s) => {
    const occupiedMins = (salonSchedules[s.id] || []).reduce(
  ...
  const cargaMaestros = {}
  maestros.forEach((m) => {
    const occupiedMins = (teacherSchedules[m.id] || []).reduce(
      (acc, slot) => acc + (slot.end - slot.start),
      0,
    )
    cargaMaestros[m.id] = {
