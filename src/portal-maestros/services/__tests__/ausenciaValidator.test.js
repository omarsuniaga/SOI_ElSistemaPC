...
describe('ausenciaValidator', () => {
  describe('validateDateRange', () => {
    ...
    it('rejects an end date before the start date', () => {
      ...
      expect(result.errors.fechaFin).toBe(
  ...
  describe('validateAbsenceRequest', () => {
    it('requires a replacement activity for each selected affected class', () => {
      const result = validateAbsenceRequest({
        ...
        clasesAfectadas: [{ claseId: 'clase-1', selected: true, actividadReemplazo: '' }],
      })

      expect(result.valid).toBe(false)
      expect(result.errors['actividad_clase-1']).toBe(
        'Indicá la actividad de reemplazo para esta clase.',
      )
    })
        ...
        clasesAfectadas: [
          {
            claseId: 'clase-1',
            selected: true,
            actividadReemplazo: 'Repasar escala mayor y registrar dudas.',
          },
        ],
        archivo: null,
      })
