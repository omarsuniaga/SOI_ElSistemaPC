-- fn_estado_asistencia_maestro no reconocía una clase regular como "cubierta"
-- por una actividad especial INSTITUCIONAL (alcance_tipo='institucion',
-- clase_id NULL, creada por otro maestro): solo miraba sesiones con
-- emergente_id ligadas a la propia clase del maestro (auto-justificación del
-- CREADOR de la actividad, ver emergenteJustificacionService.js). Una
-- actividad institucional queda invisible para las clases de los demás
-- maestros, que aparecen "pendientes" aunque la actividad ya cubrió a todos.
...
BEGIN
  ...
  WITH fechas AS (
    ...
    SELECT
      ...
      (inst.id IS NOT NULL) AS cubierta_institucional
    FROM programadas p
    LEFT JOIN LATERAL (
      SELECT sc.*
    ...
    -- Actividad especial institucional ese día (clase_id NULL, cualquier
    -- maestro creador): cubre a TODAS las clases regulares programadas,
    -- no solo a las del maestro que la creó.
    LEFT JOIN LATERAL (
      SELECT sc2.id
      FROM public.sesiones_clase sc2
      WHERE sc2.clase_id IS NULL
        AND sc2.alcance_tipo = 'institucion'
        AND sc2.fecha = p.fecha
        AND sc2.estado IN ('registrada', 'cerrada')
        AND NOT COALESCE(sc2.borrador, false)
      LIMIT 1
    ) inst ON true
  )
  SELECT
    ...
    e.sesion_id,
    CASE
      WHEN e.emergente_id IS NOT NULL OR e.cubierta_institucional THEN 'cubierta_emergente'
    ...
    (e.emergente_id IS NOT NULL OR e.cubierta_institucional) AS cubierta_emergente
  FROM evaluadas e
