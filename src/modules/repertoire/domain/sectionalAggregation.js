...
function filaContribution(fila) {
  if (fila.collectiveState) return { source: 'EXPLICIT_COLLECTIVE', states: [fila.collectiveState] }
  ...
  return { source: states.length ? 'DERIVED_FROM_STUDENTS' : 'SIN_EVALUAR', states: states.length ? states : ['SIN_EVALUAR'] }
}
...
export function aggregateSectionalPreparation({ evidence = [], authorizationScope = {} } = {}) {
  ...
  return [...byMeasure.entries()].map(([key, filas]) => {
    ...
    const states = contributions.flatMap((item) => item.states)
    const result = distribution(states)
    ...
    return {
      ...
      distribution: result,
      criticalFilaCount: result.counts.SIN_ESTUDIAR,
