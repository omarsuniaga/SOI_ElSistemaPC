...
export function createConflictReport({
  ...
  const unscheduledHtml = noAsignadas.length > 0
    ? noAsignadas.map(c => `
        <div class="hb-conflict-item" style="border-left: 4px solid var(--soi-color-danger); background: var(--soi-color-danger-light); padding: 10px 14px; border-radius: 4px; margin-bottom: 8px; font-size: 0.85rem; color: var(--soi-text);">
          <div style="font-weight: 700; display: flex; align-items: center; gap: 6px;">
            <i class="bi bi-exclamation-triangle-fill text-danger"></i>
            <span>${c.nombre}</span>
          </div>
          <div style="font-size: 0.75rem; color: var(--soi-text-muted); margin-top: 2px;">
            ⚠️ Razón: ${c.razon}
          </div>
        </div>
      `).join('')
    : `
      <div style="text-align: center; padding: 1.25rem; border: 1px dashed var(--soi-border); border-radius: 12px; background: var(--soi-color-success-light); color: var(--soi-color-success); font-size: 0.85rem; font-weight: 650; display: flex; align-items: center; justify-content: center; gap: 8px;">
    ...
    return `
      <div style="margin-bottom: 10px;">
        <div style="display: flex; justify-content: space-between; font-size: 0.8rem; margin-bottom: 3px;">
          ...
          <span style="font-weight: 700; color: var(--soi-text-muted);">${pct}%</span>
        </div>
        <div class="progress" style="height: 8px; border-radius: 20px; background: var(--soi-border);">
  ...
  const teachersLoadHtml = Object.keys(cargaMaestros).map(id => {
    ...
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px dashed var(--soi-border); font-size: 0.8rem;">
        <span style="font-weight: 600; color: var(--soi-text);">👤 ${m.nombre}</span>
  ...
  return `
    <div class="hb-card" style="padding: 1.25rem;">
      ...
      <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 1.5rem; background: var(--soi-bg-subtle); padding: 12px; border-radius: 12px; border: 1px solid var(--soi-border);">
        ...
        <div>
          <div style="font-size: 0.75rem; color: var(--soi-text-muted); font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Efectividad de Asignación</div>
          <div style="font-weight: 750; font-size: 1rem; color: var(--soi-text);">
            ${metricas.clasesAsignadas} de ${metricas.totalClases} clases
          </div>
        </div>
      </div>

      <ul class="nav nav-tabs hb-metrics-tabs" id="hbMetricsTabs" role="tablist" style="margin-bottom: 1rem; border-bottom: 1px solid var(--soi-border);">
      ...
      <div class="tab-content" id="hbMetricsTabsContent" style="max-height: 250px; overflow-y: auto; padding-right: 4px;">
        ...
        <div class="tab-pane fade" id="hb-panel-classrooms" role="tabpanel" aria-labelledby="hb-tab-classrooms">
          ${salonsHtml || '<p style="font-size:0.8rem;color:var(--soi-text-muted);">Carga salones para ver ocupación.</p>'}
        </div>
        <div class="tab-pane fade" id="hb-panel-teachers" role="tabpanel" aria-labelledby="hb-tab-teachers">
          ${teachersLoadHtml || '<p style="font-size:0.8rem;color:var(--soi-text-muted);">Carga maestros para ver carga.</p>'}
        </div>
      </div>
    </div>
  `;
