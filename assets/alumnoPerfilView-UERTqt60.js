import{t as e}from"./AppToast-DNGTRY9B.js";import{i as t}from"./supabase-DtQm9tmr.js";import{i as n}from"./maestroAuth-BSssKFds.js";import{t as r}from"./phoneUtils-Cpl-jyW9.js";import{i}from"./portalUtils-CkF82Yyk.js";var a=`alumno_plan_entradas`;async function o(e){let{data:n,error:r}=await t.from(a).select(`id, tipo, titulo, descripcion, nivel_referencia, objetivo_id, sesion_id, created_at, maestro_id`).eq(`alumno_id`,e).order(`created_at`,{ascending:!1});if(r)throw Error(r.message);return n||[]}async function s(e){if(!e.titulo?.trim())throw Error(`titulo requerido`);let{data:n,error:r}=await t.from(a).insert({alumno_id:e.alumno_id,maestro_id:e.maestro_id,tipo:e.tipo,titulo:e.titulo.trim(),descripcion:e.descripcion?.trim()||null,nivel_referencia:e.nivel_referencia||null,objetivo_id:e.objetivo_id||null,sesion_id:e.sesion_id||null}).select().single();if(r)throw Error(r.message);return n}async function c(e){let{error:n}=await t.from(a).delete().eq(`id`,e);if(n)throw Error(n.message)}var l={diagnostico:{label:`Diagnóstico`,icon:`🔍`,color:`#6366f1`,bg:`#6366f115`},logro:{label:`Logro`,icon:`✅`,color:`#16a34a`,bg:`#16a34a15`},en_progreso:{label:`En progreso`,icon:`📈`,color:`#2563eb`,bg:`#2563eb15`},dificultad:{label:`Dificultad`,icon:`⚠️`,color:`#dc2626`,bg:`#dc262615`},objetivo:{label:`Objetivo`,icon:`🎯`,color:`#d97706`,bg:`#d9770615`}},u=[`diagnostico`,`logro`,`en_progreso`,`dificultad`,`objetivo`],d=class{constructor({container:e,alumnoId:t,maestroId:n}){this._container=e,this._alumnoId=t,this._maestroId=n,this._entries=[],this._formOpen=!1}async init(){this._container.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`,await this._load(),this._render()}async _load(){this._entries=await o(this._alumnoId)}_render(){this._container.innerHTML=this._buildHTML(),this._attachEvents()}_buildHTML(){let e=this._entries.length>0,t=this._entries.some(e=>e.tipo===`diagnostico`);return`
      <div class="pe-panel">
...
`).trim():``}var y={LOGRADO:{label:`Logrado`,color:`#198754`,bg:`#19875418`,icon:`✅`},EN_PROGRESO:{label:`En progreso`,color:`#0d6efd`,bg:`#0d6efd18`,icon:`📈`},INICIADO:{label:`Iniciado`,color:`#6c757d`,bg:`#6c757d18`,icon:`🔰`},DIFICULTAD:{label:`Dificultad`,color:`#dc3545`,bg:`#dc354518`,icon:`⚠️`}};async function b(n,r,a=0){let o=n.querySelector(`#pm-alumno-progresos-root`);if(!o)return;let s=a===0;s&&(o.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`);try{let{data:e,error:c}=await t.from(`progresos`).select(`id, contenido_dsl, estado_cualitativo, calificacion, observaciones, fecha_evaluacion, clase_id, indicadores`).eq(`alumno_id`,r).not(`contenido_dsl`,`is`,null).neq(`contenido_dsl`,``).order(`fecha_evaluacion`,{ascending:!1}).range(a,a+19);if(c)throw c;if(!e||e.length===0){s&&(o.innerHTML=`<p style="font-size:0.82rem;color:var(--pm-text-muted);text-align:center;padding:1rem 0;">Sin registros de progreso generados por IA aún.</p>`);return}let l=[...new Set(e.map(e=>e.clase_id).filter(Boolean))],{data:u}=l.length?await t.from(`clases`).select(`id, nombre`).in(`id`,l):{data:[]},d=new Map((u||[]).map(e=>[e.id,e.nombre])),f=new Map;for(let t of e){let e=t.contenido_dsl;f.has(e)||f.set(e,{contenido:e,entries:[]}),f.get(e).entries.push(t)}let p=Array.from(f.values()).map(({contenido:e,entries:t})=>{let n=t[0],r=y[n.estado_cualitativo]??y.EN_PROGRESO,a=new Date(n.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`2-digit`});return`
      ...
      `;else{let e=o.querySelector(`.pm-prog-list`);if(e){let t=document.createElement(`div`);t.innerHTML=p,e.append(...t.children)}}if(e&&e.length===20){let e=document.createElement(`button`);e.className=`pm-btn pm-btn-outline`,e.style.cssText=`display:block;margin:0.75rem auto 0;font-size:0.82rem;`,e.textContent=`Cargar más`,e.onclick=()=>{e.remove(),b(n,r,a+20)},o.appendChild(e)}}catch(t){s?o.innerHTML=`<p style="color:var(--pm-danger);font-size:0.82rem;">Error al cargar historial: ${i(t.message)}</p>`:(console.error(`[_renderProgresos] Error loading page:`,t),e!==void 0&&e&&e.error(`Error al cargar más registros: `+t.message))}}async function x(e,{alumnoId:a}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let o=n();if(!o){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}if(!a){e.innerHTML=`<p class="pm-empty">No se especificó el alumno.</p>`;return}try{let{data:n,error:s}=await t.from(`alumnos`).select(`id, nombre_completo, instrumento_principal, tlf_alumno, fecha_nacimiento, created_at, nivel_actual, representante_nombre, representante_tlf, correo_representante, direccion`).eq(`id`,a).single();if(s||!n){console.error(`[AlumnoPerfil] Error al obtener alumno:`,s),e.innerHTML=`
      ...
      `;return}let{data:c}=await t.from(`alumnos_clases`).select(`clase_id`).eq(`alumno_id`,a).eq(`activo`,!0),l=(c||[]).map(e=>e.clase_id).filter(Boolean),{data:u}=await t.from(`sesiones_clase`).select(`id, clase_id, fecha, contenido_dsl, asistencia`).filter(`asistencia`,`cs`,JSON.stringify([{alumno_id:a}])).order(`fecha`,{ascending:!1}).limit(60),p=(u||[]).map(e=>{let t=e.asistencia?.find(e=>e.alumno_id===a);return t?{fecha:e.fecha,estado:t.estado,clase_id:e.clase_id,sesion_id:e.id,contenido_dsl:e.contenido_dsl,observaciones:t.observaciones||null}:null}).filter(Boolean);new Map(p.map(e=>[e.sesion_id,e.contenido_dsl]));let{data:m}=await t.from(`indicator_attempts`).select(`id, nota, observations, tarea, created_at, indicator_id, covered_by_clase_id`).eq(`student_id`,a).order(`created_at`,{ascending:!1}).limit(30),{data:y}=await t.from(`ausencias`).select(`id, fecha_inicio, fecha_fin, motivo, estado, clase_id`).eq(`alumno_id`,a).order(`fecha_inicio`,{ascending:!1}).limit(10),{data:x}=await t.from(`justificaciones`).select(`sesion_id, motivo, evidencia_url, estado, fecha`).eq(`alumno_id`,a).order(`fecha`,{ascending:!1}),S=new Map((x||[]).map(e=>[e.sesion_id,e])),C=p.length,w=p.filter(e=>e.estado===`P`).length,T=p.filter(e=>e.estado===`A`).length,E=p.filter(e=>e.estado===`J`).length,D=p.filter(e=>e.estado===`T`).length,O=C>0?Math.round(w/C*100):0,k=m?.filter(e=>e.nota!=null&&e.nota!==0)||[],A={};k.forEach(e=>{let t=e.covered_by_clase_id||`sin_clase`;A[t]||(A[t]=[]),A[t].push(e.nota)});let j=Object.values(A).map(e=>e.reduce((e,t)=>e+t,0)/e.length),M=j.length>0?Math.round(j.reduce((e,t)=>e+t,0)/j.length*10)/10:0,N=k.filter(e=>e.nota>=4).length,P=k.length,F=P>0?Math.round(N/P*100):0,I={};p.forEach(e=>{I[e.clase_id]||(I[e.clase_id]={P:0,A:0,J:0,T:0,total:0}),e.estado&&(I[e.clase_id][e.estado]=(I[e.clase_id][e.estado]||0)+1,I[e.clase_id].total++)});let L=new Set([...l,...Object.keys(I)]),{data:R}=L.size>0?await t.from(`clases`).select(`id, nombre, instrumento, nivel`).in(`id`,[...L]):{data:[]},z=new Map((R||[]).map(e=>[e.id,e])),B=p.filter(e=>e.estado===`P`&&e.contenido_dsl?.trim()),V=m?.filter(e=>e.observations?.trim())||[],H=`—`;if(n.fecha_nacimiento){let e=new Date(n.fecha_nacimiento),t=new Date;H=t.getFullYear()-e.getFullYear(),(t.getMonth()<e.getMonth()||t.getMonth()===e.getMonth()&&t.getDate()<e.getDate())&&H--}let U=n.created_at?new Date(n.created_at).toLocaleDateString(`es-ES`,{month:`long`,year:`numeric`}):`Reciente`,W=(n.instrumento_principal||``).toLowerCase(),G=`var(--pm-primary)`;(W.includes(`violin`)||W.includes(`cuerda`))&&(G=`#FF3B30`),(W.includes(`piano`)||W.includes(`teclado`))&&(G=`#FF9500`),W.includes(`guitarra`)&&(G=`#5856D6`),(W.includes(`canto`)||W.includes(`voz`))&&(G=`#AF52DE`),e.innerHTML=`
        ...
        <div class="pm-zen-hero" style="--accent-gradient: ${G}">
          ...
          <div class="pm-zen-hero__content">
            ...
            <div class="pm-zen-info">
              <h1 class="pm-zen-name">${i(n.nombre_completo)}</h1>
              <p class="pm-zen-instrument">${i(n.instrumento_principal||`Estudiante`)}</p>
              <p style="font-size:0.8rem;opacity:0.8;margin-top:4px;">Nivel ${n.nivel_actual||1} • ${H} años</p>
            </div>
          </div>
        </div>
            ...
            <div class="pm-zen-details-grid">
              <div class="pm-zen-detail">
                ...
                <div style="flex:1;min-width:0;">
                  <span>${n.tlf_alumno?`Teléfono alumno`:n.representante_tlf?`Teléfono representante`:`Teléfono`}</span>
                  <strong>${r(n.tlf_alumno||n.representante_tlf)||`—`}</strong>
                </div>
                ${n.tlf_alumno||n.representante_tlf?`
                <button
                  id="btn-whatsapp-alumno"
                  class="pm-btn-whatsapp"
                  data-phone="${n.tlf_alumno||n.representante_tlf}"
              ...
              <div class="pm-zen-detail">
                ...
                <div>
                  ...
                  <strong>${i(n.representante_nombre)}</strong>
                </div>
              </div>`:``}
              ${n.correo_representante?`
              <div class="pm-zen-detail">
                <i class="bi bi-envelope"></i>
                <div>
                  <span>Correo representante</span>
                  <strong>${i(n.correo_representante)}</strong>
                </div>
              </div>`:``}
              ${n.direccion?`
              <div class="pm-zen-detail">
    ...
    `,e.querySelector(`#pm-alumno-back`).onclick=()=>window.history.back();let K=f(n.tlf_alumno||n.representante_tlf);if(K){let t=n.nombre_completo||`el alumno`,r=n.representante_nombre||t,a={alumno:t,contacto:r},s=document.createElement(`div`);s.id=`pm-wa-modal`,s.innerHTML=`
