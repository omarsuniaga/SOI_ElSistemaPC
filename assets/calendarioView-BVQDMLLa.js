import{i as e}from"./AppModal-Du6jXNYA.js";import{c as t,f as n,o as r,p as i,r as a}from"./pwaInstaller-CABasb_l.js";import{i as o}from"./supabase-Cgh_dhNB.js";import{i as s}from"./maestroAuth-BMzDPnai.js";import{r as c}from"./main-maestros-BsZRL9Kd.js";import{S as l}from"./planificacion-BdwKIwFz.js";import{n as u,r as d}from"./asistenciasSupabase-DayI2_pO.js";import{i as f,n as p,t as m}from"./portalUtils-CkF82Yyk.js";import{t as h}from"./claseEmergenteModal-DzBloOSJ.js";var g=[`domingo`,`lunes`,`martes`,`miercoles`,`jueves`,`viernes`,`sabado`];function _(e){return Array.from((e||``).normalize(`NFD`)).filter(e=>{let t=e.codePointAt(0);return t<768||t>879}).join(``).trim().toLowerCase()}async function v(e,t){let n=[],r=0,[i,a,s]=e.fecha.split(`-`).map(Number),c=g[new Date(i,a-1,s).getDay()],{data:l,error:u}=await o.from(`clases`).select(`id, nombre`).or(`maestro_principal_id.eq.${t},maestro_suplente_id.eq.${t},maestro_id.eq.${t}`);if(u||!l?.length)return{justificadas:0,errores:[]};let d=l.map(e=>e.id),{data:f,error:p}=await o.from(`clase_horarios`).select(`clase_id, dia, hora_inicio, hora_fin`).in(`clase_id`,d);if(p||!f?.length)return{justificadas:0,errores:[]};let m=_(c),h=f.filter(e=>_(e.dia)===m);if(!h.length)return{justificadas:0,errores:[]};let v=h.map(e=>({...e,nombre:l.find(t=>t.id===e.clase_id)?.nombre||``}));for(let i of v)try{let{data:a}=await o.from(`alumnos_clases`).select(`alumno_id`).eq(`clase_id`,i.clase_id).eq(`activo`,!0),s=(a||[]).map(e=>({alumno_id:e.alumno_id,estado:`justificado`})),c=`Clase suspendida por actividad especial: "${e.actividad||`Actividad especial`}".`+(e.motivo?` Motivo: ${e.motivo}.`:``)+` Todos los alumnos quedan justificados.`,{error:l}=await o.from(`sesiones_clase`).upsert({clase_id:i.clase_id,fecha:e.fecha,maestro_id:t,emergente_id:e.id,hora_inicio:i.hora_inicio,hora_fin:i.hora_fin,estado:`registrada`,borrador:!1,asistencia:s,contenido:c},{onConflict:`clase_id,fecha,maestro_id`});l?n.push(`${i.nombre}: ${l.message}`):r++}catch(e){n.push(`${i.nombre}: ${e.message}`)}return{justificadas:r,errores:n}}var y=[`Do`,`Lu`,`Ma`,`Mi`,`Ju`,`Vi`,`Sa`],b=7;async function x(e,{onFechaClick:t}={}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let n=s();if(!n){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}let r=new Date,i=r.getFullYear(),a=r.getMonth();async function o(){try{let{estadoMap:s,dotsMap:c}=await S(n.id,i,a);w(e,i,a,r,s,c,{onFechaClick:e=>{T(e),t?.(e)},onPrev:()=>{a===0?(i--,a=11):a--,o()},onNext:()=>{a===11?(i++,a=0):a++,o()}})}catch(t){e.innerHTML=`<p class="pm-empty" style="color:var(--pm-danger)">Error al cargar calendario: ${f(t.message)}</p>`}}await o()}async function S(e,r,i){let o=new Date(r,i,1),s=new Date(r,i+1,0),c=o.toISOString().split(`T`)[0],l=s.toISOString().split(`T`)[0],f=(await t()).map(e=>e.id);if(f.length===0)return{estadoMap:new Map,dotsMap:new Map};let p=await a(f),h=new Set(p.map(e=>e.dia?.toLowerCase())),g=new Map;p.forEach(e=>{let t=e.dia?.toLowerCase(),n=e.hora_fin||`23:59`;(t&&!g.has(t)||n>g.get(t))&&g.set(t,n)});let _=await n(e,c,l),v=e=>{if(!e||e.borrador===!0||e.estado===`pendiente`)return!1;let t=Array.isArray(e.asistencia)&&e.asistencia.length>0,n=typeof e.contenido==`string`&&e.contenido.trim().length>0;return e.estado===`registrada`||e.estado===`cerrada`||e.borrador===!1&&(t||n)},y=_.filter(v),x=new Set(y.map(e=>e.fecha)),S=new Set(_.filter(e=>e.clase_id&&e.emergente_id).map(e=>e.fecha)),w=new Map;_.filter(e=>!e.clase_id).forEach(e=>{w.has(e.fecha)||w.set(e.fecha,[]),w.get(e.fecha).push(e)});let T=new Map,E=new Map;p.forEach(e=>{let t=e.dia?.toLowerCase();if(!t||!e.clase_id)return;T.has(t)||T.set(t,new Set),T.get(t).add(e.clase_id);let n=`${t}|${e.clase_id}`,r=e.hora_fin||`23:59`;(!E.has(n)||r>E.get(n))&&E.set(n,r)});let D=new Map;_.filter(e=>e.clase_id).forEach(e=>{let t=`${e.fecha}|${e.clase_id}`,n=D.get(t);(!n||!v(n)&&v(e))&&D.set(t,e)});let O=new Map,k=new Map,A=new Date;A.setHours(0,0,0,0);let j=await u().catch(()=>null);await d(e,j?.id).catch(()=>({esCompleto:!0,pendientesCount:0}));for(let e=new Date(o);e<=s;e.setDate(e.getDate()+1)){let t=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`,n=m[e.getDay()],r=h.has(n),i=w.get(t)||[],a=new Date(e),o=Math.floor((A-a)/864e5),s=j&&(t<j.fecha_inicio||t>j.fecha_fin),c=Array.from(D.keys()).some(e=>e.startsWith(`${t}|`))||i.length>0;if(s&&!c){O.set(t,`receso-academico`),k.set(t,[]);continue}let l=[];if((T.get(n)||new Set).forEach(e=>{let r=D.get(`${t}|${e}`);if(o===0){r&&r.borrador===!1&&(r.emergente_id||v(r))?l.push(`verde`):r?l.push(`amarillo`):C(E.get(`${n}|${e}`))?l.push(`rojo`):l.push(`gris`);return}r&&(r.emergente_id||v(r))?l.push(`verde`):r?l.push(`amarillo`):o<0?l.push(`gris`):l.push(`rojo`)}),i.forEach(e=>{l.push(v(e)?`verde`:`amarillo`)}),k.set(t,l),!r&&i.length===0){O.set(t,`sin-clase`);continue}if(o===0){let e=_.find(e=>e.fecha===t);if(e&&e.borrador===!1&&v(e)){O.set(t,`registrada`);continue}if(e&&(e.borrador===!0||e.estado===`pendiente`)){O.set(t,`pendiente`);continue}if(S.has(t)){O.set(t,`cubierta-emergente`);continue}let r=g.get(n);if(r){let e=new Date,[n,i]=r.split(`:`),a=parseInt(n)*60*60*1e3+parseInt(i||0)*60*1e3;if(e.getHours()*60*60*1e3+e.getMinutes()*60*1e3<a){O.set(t,`sin-clase`);continue}}O.set(t,`pendiente`);continue}if(o>0&&S.has(t)){O.set(t,`cubierta-emergente`);continue}if(o>0&&x.has(t)){O.set(t,`registrada`);continue}o<0?O.set(t,`sin-clase`):o<=b?O.set(t,`pendiente`):O.set(t,`vencida`)}return{estadoMap:O,dotsMap:k}}function C(e){let t=(e||`23:59`).slice(0,5),n=new Date;return`${String(n.getHours()).padStart(2,`0`)}:${String(n.getMinutes()).padStart(2,`0`)}`>=t}function w(e,t,n,r,i,a,{onFechaClick:o,onPrev:s,onNext:c}){let l=new Date(t,n,1),u=new Date(t,n+1,0),d=l.getDay(),f=`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,`0`)}-${String(r.getDate()).padStart(2,`0`)}`,m=u.getDate(),h=`${t}-${String(n+1).padStart(2,`0`)}-01`,g=`${t}-${String(n+1).padStart(2,`0`)}-${String(m).padStart(2,`0`)}`,_=f>=h&&f<=g?f:h,v=y.map(e=>`<div class="pm-cal-day-header">${e}</div>`).join(``);for(let e=0;e<d;e++)v+=`<div class="pm-cal-day otro-mes"></div>`;for(let e=1;e<=m;e++){let r=`${t}-${String(n+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,o=i.get(r)||`sin-clase`,s=a?.get(r)||[],c=s.includes(`rojo`)||s.includes(`amarillo`),l=s.length?`<div class="pm-day-dots">${s.map(e=>`<span class="pm-day-dot pm-dot-${e}"></span>`).join(``)}</div>`:``,u=r===f?`today`:``,d=r===_,m=`${e} de ${p[n]} ${t}${s.length?`, ${s.length} clase(s)`:``}`;v+=`
  ...
  `,e.querySelector(`#pm-cal-prev`).addEventListener(`click`,s),e.querySelector(`#pm-cal-next`).addEventListener(`click`,c),e.querySelectorAll(`.pm-cal-day[data-fecha]`).forEach(t=>{t.addEventListener(`click`,()=>{e.querySelectorAll(`.pm-cal-day[data-fecha]`).forEach(e=>e.setAttribute(`aria-selected`,`false`)),t.setAttribute(`aria-selected`,`true`),o?.(t.dataset.fecha)})});let b=e.querySelector(`.pm-cal-grid`);b&&b.addEventListener(`keydown`,function(e){let t=[...b.querySelectorAll(`.pm-cal-day[data-fecha]`)];if(t.length===0)return;let n=b.querySelector(`[tabindex="0"]`),r=n?t.indexOf(n):-1,i=e=>{e<0||e>=t.length||(t.forEach(e=>e.setAttribute(`tabindex`,`-1`)),t[e].setAttribute(`tabindex`,`0`),t[e].focus())};switch(e.key){case`ArrowLeft`:e.preventDefault(),r>0&&i(r-1);break;case`ArrowRight`:e.preventDefault(),r<t.length-1&&i(r+1);break;case`ArrowUp`:e.preventDefault(),i(Math.max(0,r-7));break;case`ArrowDown`:e.preventDefault(),i(Math.min(t.length-1,r+7));break;case`Home`:e.preventDefault(),i(Math.floor(Math.max(r,0)/7)*7);break;case`End`:e.preventDefault(),i(Math.min(t.length-1,Math.floor(Math.max(r,0)/7)*7+6));break;case`PageUp`:e.preventDefault(),typeof s==`function`&&s();break;case`PageDown`:e.preventDefault(),typeof c==`function`&&c();break;case`Enter`:case` `:e.preventDefault(),n&&n.click();break}})}async function T(t){let n=s();if(!n)return;let r=new Date;`${r.getFullYear()}${String(r.getMonth()+1).padStart(2,`0`)}${String(r.getDate()).padStart(2,`0`)}`;let a=document.getElementById(`pm-action-drawer`);a||(a=document.createElement(`div`),a.id=`pm-action-drawer`,a.className=`pm-drawer-overlay`,document.body.appendChild(a));let p=[],m=[],h=[],g=[];try{let{data:e}=await o.from(`clases_emergentes`).select(`*`).eq(`maestro_id`,n.id).eq(`fecha`,t).order(`hora_inicio`,{ascending:!0,nullsFirst:!1}),{data:r}=await o.from(`sesiones_clase`).select(`*`).eq(`maestro_id`,n.id).eq(`fecha`,t);p=r||[],g=p.filter(e=>e.clase_id&&e.emergente_id);let{data:i}=await o.from(`clases`).select(`id, nombre, instrumento`).or(`maestro_principal_id.eq.${n.id},maestro_suplente_id.eq.${n.id},maestro_id.eq.${n.id}`);m=i||[];let a=m.map(e=>e.id);if(a.length>0){let{data:e}=await o.from(`clase_horarios`).select(`clase_id, hora_inicio, hora_fin, dia`).in(`clase_id`,a);h=e||[]}}catch(e){console.error(`Error fetching drawer data:`,e)}let[_,v,y]=t.split(`-`).map(Number),b=new Date(_,v-1,y),S=b.toLocaleDateString(`es-ES`,{weekday:`long`}).toLowerCase(),C=await u().catch(()=>null),w=await d(n.id,C?.id).catch(()=>({esCompleto:!0,pendientesCount:0})),T=C&&(t<C.fecha_inicio||t>C.fecha_fin),D=``;T&&(D=w.esCompleto?`
    ...
    <div class="pm-drawer-content">
      <div class="pm-drawer-header">
        <div style="flex:1">
          <h3 style="margin:0; font-size:1.1rem; font-weight:700;">${b.toLocaleDateString(`es-ES`,{weekday:`long`,day:`numeric`,month:`long`})}</h3>
  ...
  `,!document.getElementById(`pm-drawer-styles`)){let e=document.createElement(`style`);e.id=`pm-drawer-styles`,e.textContent=`
    ...
    `,document.head.appendChild(e)}let M=()=>a.classList.remove(`open`),N=a.querySelector(`#pm-drawer-close-btn`);N&&(N.onclick=M),a.addEventListener(`click`,e=>{e.target===a&&M()}),a.querySelectorAll(`.btn-pasar-asistencia, .btn-ver-sesion, .btn-continuar-sesion`).forEach(e=>{e&&e.addEventListener(`click`,()=>{let n=e.dataset.clase;M(),window.location.hash=`#/asistencia?clase=${n}&fecha=${t}`})}),a.querySelectorAll(`.btn-ver-sesion-emergente`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.sesion;M(),window.location.hash=`#/asistencia?sesion=${n}&fecha=${t}`})}),a.querySelectorAll(`.btn-ver-clase-suspendida`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.clase;M(),window.location.hash=`#/asistencia?clase=${n}&fecha=${t}`})}),a.querySelectorAll(`.btn-descartar-borrador`).forEach(t=>{t.addEventListener(`click`,async n=>{n.stopPropagation();let r=t.dataset.sesion;if(r&&confirm(`¿Deseas descartar este borrador? La fecha se desmarcará por completo.`))try{await l(r),i(),c(`calendario`),e.show(`Borrador descartado. Fecha desmarcada.`,`success`),M(),await x(container)}catch(t){e.show(`Error al descartar: `+t.message,`danger`)}})}),a.querySelectorAll(`#pm-drawer-emergente, #pm-drawer-emergente-body`).forEach(e=>{e.addEventListener(`click`,()=>{E(t,m)})}),setTimeout(()=>a.classList.add(`open`),10)}async function E(t,n){let i=[];try{let e=await r(n.map(e=>e.id)),t={};e.forEach(e=>{if(!e.alumnos)return;t[e.alumno_id]||(t[e.alumno_id]=[]);let r=n.find(t=>t.id===e.clase_id);r&&t[e.alumno_id].push(r.nombre)});let a=new Set;i=e.map(e=>e.alumnos).filter(Boolean).filter(e=>a.has(e.id)?!1:(a.add(e.id),!0)).map(e=>({...e,clase_nombres:t[e.id]||[]}))}catch(e){console.warn(`[calendario] No se pudieron cargar alumnos para clase emergente:`,e)}h({fecha:t,clases:n,alumnos:i,maestroId:s().id,onSave:async t=>{try{let{data:n,error:r}=await o.from(`sesiones_clase`).insert([t]).select().single();if(r)throw r;let i=await v(n,s().id);i.errores.length>0?(console.warn(`[calendario] Auto-justificación parcial:`,i.errores),e.warning(`Clase emergente creada. ${i.justificadas} clase(s) justificada(s) automáticamente (${i.errores.length} con error).`)):i.justificadas>0?e.success(`Clase emergente creada. ${i.justificadas} clase(s) programada(s) marcada(s) como justificadas.`):e.success(`Clase emergente creada. Procedé a pasar asistencia.`);let a=document.getElementById(`pm-action-drawer`);a&&a.classList.remove(`open`),window.location.hash=`#/asistencia?sesion=${n.id}&fecha=${t.fecha}`}catch(t){console.error(`Error creando clase emergente:`,t),e.error(`No se pudo crear la clase emergente`)}}})}export{x as renderCalendarioView};