import{r as e}from"./rolldown-runtime-DlOssbPu.js";import{i as t}from"./supabase-KnARm58N.js";import{i as n}from"./maestroAuth-lT-ZcZZd.js";import{i as r,s as i}from"./maestroDataService-BGjCE976.js";import{i as a}from"./portalUtils-DbrsCFDo.js";import{t as o}from"./a11yUtils-DoZA0IX7.js";import{t as s}from"./claseAnalysisModal-CVDuqLFh.js";var c=e({getAlumnoIndexFromMetricas:()=>m,renderMetricasView:()=>h}),l={periodo:4,maestroId:null,clasesData:[],todasSesiones:[],inscripcionesPorClase:{},alertasRiesgo:[]};async function u(e,n){let a=await r();a.sort((e,t)=>e.nombre.localeCompare(t.nombre));let o=new Date;o.setDate(o.getDate()-e*7);let s=o.toISOString().split(`T`)[0],c=new Date().toISOString().split(`T`)[0],l=await i(n,s,c)||[],u=a.map(e=>e.id);if(u.length===0)return{clases:a,sesiones:l,inscripcionesPorClase:{}};let{data:d}=await t.from(`alumnos_clases`).select(`clase_id, alumno:alumnos(id, nombre_completo)`).in(`clase_id`,u).eq(`activo`,!0),f={};for(let e of d||[])!e.clase_id||!e.alumno||(f[e.clase_id]||(f[e.clase_id]=[]),f[e.clase_id].push(e.alumno));return{clases:a,sesiones:l,inscripcionesPorClase:f}}function d({clases:e,sesiones:t,inscripcionesPorClase:n}){let r=t.filter(e=>e.estado===`registrada`).length,i=t.filter(e=>e.estado===`pendiente`).length,a=t.filter(e=>e.borrador===!0).length,o=0,s=0,c=0,l=0;t.forEach(e=>{(e.asistencia||[]).forEach(e=>{l++,e.estado===`P`?o++:e.estado===`A`?s++:e.estado===`J`&&c++})});let u=l>0?Math.round(o/l*100):0,d=e.map(e=>{let r=t.filter(t=>t.clase_id===e.id),i=r.filter(e=>e.estado===`registrada`).length,a=r.filter(e=>e.estado===`pendiente`).length,o=n[e.id]||[],s=o.length,c=r.filter(e=>e.estado===`registrada`).slice(-8).map(e=>{let t=(e.asistencia||[]).filter(e=>e.estado===`P`).length,n=(e.asistencia||[]).length;return n>0?Math.round(t/n*100):0}),l=0,u=0;r.forEach(e=>{(e.asistencia||[]).forEach(e=>{u++,e.estado===`P`&&l++})});let d=u>0?Math.round(l/u*100):0,f=r.filter(e=>e.contenido_dsl?.trim()).length,p=r.length>0?Math.min(100,Math.round(f/Math.max(i,1)*100)):0,m=[];for(let e of o){let t=r.filter(t=>t.asistencia?.some(t=>t.alumno_id===e.id)).map(t=>t.asistencia.find(t=>t.alumno_id===e.id)),n=t.filter(e=>e?.estado===`P`).length,i=t.length>0?Math.round(n/t.length*100):0;i>0&&i<70&&m.push({id:e.id,nombre:e.nombre_completo,pct:i})}return{...e,totalAlumnos:s,sesionesCompletadas:i,sesionesPendientes:a,sessionAttendance:c,avgAttendance:d,progress:p,riskStudents:m,alumnos:o}}),f=[];for(let e of d)for(let t of e.riskStudents)f.push({tipo:`baja_asistencia`,alumnoId:t.id,nombre:t.nombre,clase:e.nombre,valor:t.pct,mensaje:`${t.pct}%`});return{totalClases:e.length,sesionesCompletadas:r,sesionesPendientes:i+a,totalPresentes:o,totalAusentes:s,totalJustificados:c,totalRegistros:l,asistenciaPromedio:u,clasesData:d,alertasRiesgo:f,inscripcionesPorClase:n}}function f(e){let{totalClases:t,sesionesCompletadas:n,sesionesPendientes:r,totalPresentes:i,totalAusentes:o,totalJustificados:s,totalRegistros:c,asistenciaPromedio:u,clasesData:d,alertasRiesgo:f}=e,p=c>0?Math.round(i/c*100):0,m=c>0?Math.round(o/c*100):0,h=c>0?Math.round(s/c*100):0;return`
    <div class="pm-dashboard" role="main" aria-label="Panel de métricas">
      <div role="status" aria-live="polite" aria-atomic="true" class="pm-visually-hidden">${a(`Dashboard: ${u}% asistencia general, ${t} clases, ${n} sesiones registradas, ${r} pendientes.`)}</div>
      ...
      <section class="pm-dashboard-overview" aria-label="Indicadores generales">
        <div class="pm-overview-card primary">
          ...
          <div class="pm-overview-info">
            ...
            <span class="pm-overview-detail">${i} de ${c} registros</span>
          </div>
        </div>
      ...
      <section class="pm-dashboard-section" aria-label="Alumnos en riesgo">
        ...
        <div class="pm-risk-list" role="list">
          ${f.slice(0,5).map(e=>`
            <div class="pm-risk-item" role="listitem" tabindex="0" data-alumno="${e.alumnoId}" aria-label="Ver perfil de ${a(e.nombre)}">
              <div class="pm-risk-avatar" aria-hidden="true">${(e.nombre||`A`)[0].toUpperCase()}</div>
              <div class="pm-risk-info">
                <span class="pm-risk-name">${a(e.nombre)}</span>
                <span class="pm-risk-class">${a(e.clase)}</span>
              </div>
      ...
      <section class="pm-dashboard-section" aria-label="Resumen por clase">
        ...
        <div class="pm-classes-list" id="pm-clases-grid">
          ${d.map(e=>{let t=e.avgAttendance,n=t<70?`danger`:t<85?`warning`:`success`,r=t<70?`linear-gradient(135deg,#ff3b30,#ff6b6b)`:t<85?`linear-gradient(135deg,#ff9500,#ffcc00)`:`linear-gradient(135deg,#30d158,#34c759)`,i=e.sessionAttendance.length>0?e.sessionAttendance.map((e,t,n)=>{let r=Math.max(8,e),i=e<70?`#ff3b30`:e<85?`#ff9500`:`#30d158`;return`<div class="pm-spark-bar ${t===n.length-1?`pm-spark-last`:``}" style="height:${r}%;background:${i};" title="${e}%"></div>`}).join(``):`<span class="pm-spark-empty">—</span>`;return`
            <div class="pm-class-card2" data-clase-id="${e.id}" role="article" aria-label="Clase ${a(e.nombre)}">
              ...
              <div class="pm-class-card2__body">
                <div class="pm-class-card2__top">
                  <div class="pm-class-card2__info">
                    <span class="pm-class-card2__name">${a(e.nombre)}</span>
                    ${e.instrumento?`<span class="pm-class-card2__inst"><i class="bi bi-music-note-beamed"></i> ${a(e.instrumento)}</span>`:``}
                  </div>
  ...
  `}function p(e){console.log(`[MetricasView.bindEvents] Iniciando bind, container:`,e),console.log(`[MetricasView.bindEvents] HTML length:`,e?.innerHTML?.length||0),e.querySelector(`#pm-filter-periodo`)?.addEventListener(`change`,async t=>{let n=parseInt(t.target.value,10);l.periodo=n,e.innerHTML=`<div class="pm-loading" style="padding:2rem;"><div class="pm-spinner"></div></div>`;try{let t=await u(n,l.maestroId),r=d(t);l.clasesData=r.clasesData,l.todasSesiones=t.sesiones,l.inscripcionesPorClase=t.inscripcionesPorClase,l.alertasRiesgo=r.alertasRiesgo,e.innerHTML=f(r),p(e),o(`Período actualizado a ${n} semanas. ${r.asistenciaPromedio}% de asistencia general.`)}catch(t){e.innerHTML=`<p class="pm-empty">Error al cargar datos: ${a(t.message)}</p>`}}),e.querySelectorAll(`.pm-risk-item`).forEach(e=>{let t=e.dataset.alumno,n=()=>{window.location.hash=`#/alumno?id=${t}`};e.addEventListener(`click`,n),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&n()})});let t=new Date,n=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-${String(t.getDate()).padStart(2,`0`)}`,r=l.periodo||4;e.querySelectorAll(`.pm-analisis-btn-metrics`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),t.preventDefault();let i=e.dataset.claseId;s(i,n,r)})}),e.querySelectorAll(`.pm-class-btn, .pm-class-btn2`).forEach(e=>{e.addEventListener(`click`,async t=>{t.stopPropagation();let n=e.closest(`.pm-class-card2, .pm-class-card`),r=n.querySelector(`.pm-clase-students-panel`);if(r){r.remove();return}let i=e.dataset.claseId,o=l.clasesData.find(e=>e.id===i)?.alumnos||[],s=l.todasSesiones.filter(e=>e.clase_id===i),c=o.map(e=>{let t=s.filter(t=>t.asistencia?.some(t=>t.alumno_id===e.id)).map(t=>t.asistencia.find(t=>t.alumno_id===e.id)),n=t.filter(e=>e?.estado===`P`).length,r=t.length,i=r>0?Math.round(n/r*100):0,a=s.filter(t=>t.asistencia?.some(t=>t.alumno_id===e.id)).sort((e,t)=>t.fecha.localeCompare(e.fecha))[0];return{...e,pct:i,total:r,lastFecha:a?.fecha}});c.sort((e,t)=>e.pct-t.pct);let u=document.createElement(`div`);u.className=`pm-clase-students-panel`,u.innerHTML=`
        <div class="pm-clase-students-header">
          <span>Alumnos (${c.length})</span>
          ...
          ${c.map(e=>`
            <div class="pm-clase-student-row" role="listitem" tabindex="0" data-alumno="${e.id}">
              <div class="pm-student-info">
                <span class="pm-student-nombre">${a(e.nombre_completo)}</span>
      ...
      <div class="pm-empty" style="padding:3rem 1rem;text-align:center;" role="alert">
        ...
        <p style="font-size:0.85rem;color:var(--pm-text-muted);">${a(t.message)}</p>
      </div>`}}export{c as n,m as t};