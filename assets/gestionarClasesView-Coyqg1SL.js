import{i as e}from"./AppModal-Du6jXNYA.js";import{i as t}from"./maestroAuth-BMzDPnai.js";import{i as n,o as r}from"./main-maestros-D98x9e4o.js";import{o as i,r as a}from"./planificacion-BdwKIwFz.js";import{d as o,i as s,o as c,s as l}from"./clasesApi-DGHemn9O.js";function u(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function d(e){return String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]?.toUpperCase()??``).join(``)}function f(e){if(!e||e.length===0)return`<span style="color:var(--pm-text-muted);font-size:.8rem;">Sin horario asignado</span>`;let t={lunes:`Lun`,martes:`Mar`,miercoles:`Mié`,miércoles:`Mié`,jueves:`Jue`,viernes:`Vie`,sabado:`Sáb`,sábado:`Sáb`,domingo:`Dom`};return e.map(e=>`<span class="gcv-horario-chip">${t[e.dia]||e.dia||``} ${(e.hora_inicio||``).slice(0,5)}–${(e.hora_fin||``).slice(0,5)}</span>`).join(` `)}var p=null,m=[],h=new Set;async function g(e){e.innerHTML=k();let r=t();if(!r){e.innerHTML=A(`bi-lock`,`Sin sesión activa`,`Por favor ingresá nuevamente.`);return}try{let t=await n(r.id);if(!t.puede_inscribir_clases){e.innerHTML=v(t),y(r.id);return}let[a,s]=await Promise.all([o(r.id),i().catch(()=>[])]);m=s.filter(e=>e.activo!==!1&&e.is_active!==!1),e.innerHTML=b(a),E(a),a.length>0&&await S(a[0].id,a)}catch(t){console.error(`[GestionarClases]`,t),e.innerHTML=A(`bi-exclamation-triangle`,`Error al cargar`,u(t.message))}}function _(e){let t=e?.solicitudes||[],n=e?.solicitud_actual;return t.includes(`clases:enroll`)||t.includes(`inscribir_clases`)||n?.estado===`pendiente`&&n?.solicita_clases}function v(e){return`
  ...
  `}function y(t){let n=document.getElementById(`gcv-btn-request-classes`);n&&n.addEventListener(`click`,async()=>{n.disabled=!0;let i=n.innerHTML;n.innerHTML=`<span class="gcv-spinner-sm"></span> Enviando...`;try{await r(t,`clases:enroll`),e.success(`Solicitud de permiso enviada correctamente.`);let n=document.getElementById(`gcv-permission-action`);n&&(n.innerHTML=`
          <div class="gcv-pending-badge">
            <i class="bi bi-clock-history"></i>
            Solicitud Pendiente de Aprobación
          </div>`)}catch(t){e.error(`Error al solicitar: `+t.message),n.disabled=!1,n.innerHTML=i}})}function b(e){return`
    <div class="gcv-root">
