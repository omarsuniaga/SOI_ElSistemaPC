...
export class GamificacionNodos {
  ...
  renderNodo(nodo) {
    ...
    return `
      <g class="gn-nodo gn-nodo-${estado}" data-nodo-id="${nodo.id}" data-estado="${estado}" transform="translate(${nodo.x}, ${nodo.y})" style="cursor:${estado !== 'locked' ? 'pointer' : 'default'};opacity:${opacity}">
  ...
  attachEvents() {
    this.container.querySelectorAll('.gn-nodo').forEach((el) => {
      const id = el.dataset.nodoId
      const estado = el.dataset.estado

      if (estado !== 'locked') {
        el.addEventListener('click', () => {
          // Invocar el callback seleccionarNodo pasado en opciones
          if (typeof this.seleccionarNodo === 'function') {
            this.seleccionarNodo(id)
          }

          // Disparar el evento personalizado 'nodo-select' para comunicación entre widgets/vistas
          const event = new CustomEvent('nodo-select', {
            detail: { id },
            bubbles: true,
            composed: true,
          })
          el.dispatchEvent(event)
          console.log(`[GamificacionNodos] Evento 'nodo-select' disparado para nodo: ${id}`)
        })
      }
    })
  }
