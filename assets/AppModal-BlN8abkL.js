var e=`app-global-modal`,t=`app-global-backdrop`;function n(){if(document.getElementById(e))return;let n=document.createElement(`div`);n.id=t,n.style.cssText=`
    ...
    opacity:0;
  `,document.body.appendChild(n);let r=document.createElement(`div`);r.id=e,r.setAttribute(`role`,`dialog`),r.setAttribute(`aria-modal`,`true`),r.style.cssText=`
    display:none;position:fixed;inset:0;
    z-index:11001;
    overflow-y:auto;
    padding:1.5rem;
  ...
  `,document.body.appendChild(r)}function r(e){return e?e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`):``}function i(){return{backdrop:document.getElementById(t),modal:document.getElementById(e),dialog:document.querySelector(`#${e} .app-modal-dialog`),title:document.querySelector(`#${e} .app-modal-title`),body:document.querySelector(`#${e} .app-modal-body`),closeX:document.querySelector(`#${e} .app-modal-close-x`),btnCancel:document.querySelector(`#${e} .app-modal-btn-cancel`),btnSave:document.querySelector(`#${e} .app-modal-btn-save`),btnDelete:document.querySelector(`#${e} .app-modal-btn-delete`),saveText:document.querySelector(`#${e} .app-modal-save-text`)}}var a={sm:`400px`,md:`520px`,lg:`720px`,xl:`960px`},o={_saveHandler:null,_cancelHandler:null,_keydownHandler:null,open({title:e=``,body:t=``,saveText:r=`Guardar`,cancelText:o=`Cancelar`,deleteText:s=`Eliminar`,onSave:c=null,onCancel:l=null,onDelete:u=null,onShow:d=null,onOpen:f=null,size:p=`md`,hideSave:m=!1}={}){n();let h=i(),g=h.dialog.querySelector(`.app-modal-footer`);g&&g.style.removeProperty(`display`),h.dialog.style.maxWidth=a[p]||a.md,h.title.textContent=e,typeof t==`string`?h.body.innerHTML=t:t instanceof HTMLElement&&(h.body.innerHTML=``,h.body.appendChild(t)),d&&d(h.body),f&&setTimeout(()=>f(h.body),280),this.resetSaveBtn(r),h.btnCancel.textContent=o,h.btnSave.style.display=m?`none`:``,u?(h.btnDelete.textContent=s,h.btnDelete.style.display=`block`):h.btnDelete.style.display=`none`,setTimeout(()=>{let e=h.body.querySelector(`input,select,textarea`);e&&e.focus()},280),this._detachHandlers(),this._keydownHandler=e=>{e.key===`Escape`&&(this._cancelHandler?this._cancelHandler():this.close())},document.addEventListener(`keydown`,this._keydownHandler),this._saveHandler=async()=>{if(c){let e=h.btnSave,t=e.innerHTML;e.disabled=!0,e.innerHTML=`<span class="spinner-border spinner-border-sm me-1" role="status"></span>`;try{await c(h.body)===!1?(e.disabled=!1,e.innerHTML=t):this.close()}catch{e.disabled=!1,e.innerHTML=t}}else this.close()},this._cancelHandler=()=>{l&&l(),this.close()},this._deleteHandler=async()=>{if(!u||!confirm(`¿Estás seguro de que querés eliminar este elemento? Esta acción no se puede deshacer.`))return;let e=h.btnDelete.innerHTML;h.btnDelete.disabled=!0,h.btnDelete.innerHTML=`<span class="spinner-border spinner-border-sm" role="status"></span>`;try{await u()===!1?(h.btnDelete.disabled=!1,h.btnDelete.innerHTML=e):this.close()}catch{h.btnDelete.disabled=!1,h.btnDelete.innerHTML=e}},h.btnSave.addEventListener(`click`,this._saveHandler),h.btnCancel.addEventListener(`click`,this._cancelHandler),h.closeX.addEventListener(`click`,this._cancelHandler),h.btnDelete.addEventListener(`click`,this._deleteHandler),h.closeX.onmouseenter=()=>{h.closeX.style.background=`var(--bs-secondary-bg)`,h.closeX.style.color=`var(--bs-body-color)`},h.closeX.onmouseleave=()=>{h.closeX.style.background=`none`,h.closeX.style.color=`var(--bs-secondary-color)`},h.backdrop.style.display=`block`,h.modal.style.display=`flex`,document.body.style.overflow=`hidden`,requestAnimationFrame(()=>{h.backdrop.style.opacity=`1`,h.dialog.style.opacity=`1`,h.dialog.style.transform=`translateY(0) scale(1)`})},close(){if(!document.getElementById(e))return;let t=i();t.backdrop.style.opacity=`0`,t.dialog.style.opacity=`0`,t.dialog.style.transform=`translateY(20px) scale(0.97)`,this._detachHandlers(),setTimeout(()=>{t.backdrop.style.display=`none`,t.modal.style.display=`none`,t.body.innerHTML=``,document.body.style.overflow=``},220)},_detachHandlers(){let e=i();e.btnSave&&(this._saveHandler&&e.btnSave.removeEventListener(`click`,this._saveHandler),this._cancelHandler&&(e.btnCancel.removeEventListener(`click`,this._cancelHandler),e.closeX.removeEventListener(`click`,this._cancelHandler)),this._deleteHandler&&e.btnDelete.removeEventListener(`click`,this._deleteHandler),this._keydownHandler&&document.removeEventListener(`keydown`,this._keydownHandler),this._saveHandler=null,this._cancelHandler=null,this._deleteHandler=null,this._keydownHandler=null)},resetSaveBtn(t=`Guardar`){let n=document.querySelector(`#${e} .app-modal-btn-save`);n&&(n.disabled=!1,n.innerHTML=`<span class="app-modal-save-text">${t}</span>`)},setSaveHandler(e,t=null){let n=i();n.btnSave&&(this._saveHandler&&n.btnSave.removeEventListener(`click`,this._saveHandler),t&&this.resetSaveBtn(t),this._saveHandler=async()=>{let t=n.btnSave,r=t.innerHTML;t.disabled=!0,t.innerHTML=`<span class="spinner-border spinner-border-sm me-1" role="status"></span>`;try{await e(n.body)===!1?(t.disabled=!1,t.innerHTML=r):this.close()}catch{t.disabled=!1,t.innerHTML=r}},n.btnSave.addEventListener(`click`,this._saveHandler))},showLoading(e=`Cargando...`){let t=i();t.body&&(t.body.innerHTML=`
      <div class="d-flex flex-column align-items-center justify-content-center py-5">
        <div class="spinner-border text-primary mb-3" role="status">
          <span class="visually-hidden">Cargando...</span>
        </div>
        <p class="text-muted mb-0">${r(e)}</p>
      </div>
    `,t.btnSave.style.display=`none`,t.btnCancel.style.display=`none`)},hideLoading(){let e=i();e.btnSave&&(e.btnSave.style.display=``,e.btnCancel.style.display=``)}};export{o as t};