-- ============================================================================
-- vw_seguimiento_ausentes v2:
--  - el corte del contador ahora combina retenciones_instrumento.fecha_reincorporacion
--    Y seguimiento_ausencias_reinicio.fecha_corte (reinicio manual).
--  - se excluyen los alumnos con una suspension temporal activa y vigente.
-- Aplicar a zmhmdvmyeyswunurcyow. Depende de 20260904120000_*.
-- ============================================================================
...
corte AS (
  SELECT alumno_id, MAX(f) AS fr FROM (
    ...
    UNION ALL
    SELECT alumno_id, fecha_corte
      FROM public.seguimiento_ausencias_reinicio
  ) x
  GROUP BY alumno_id
),
dias AS (
...
COMMENT ON VIEW public.vw_seguimiento_ausentes IS
  'Una fila por alumno activo con >=1 dia de ausencia injustificada en el periodo vigente, despues del ultimo corte (reincorporacion o reinicio manual) y sin suspension temporal activa. dias_ausente (distinct fecha) determina el nivel. security_invoker=false intencional.';

GRANT SELECT ON public.vw_seguimiento_ausentes TO authenticated;

-- ROLLBACK: restaurar la definicion de 20260904023243_seguimiento_ausentes_view.sql
