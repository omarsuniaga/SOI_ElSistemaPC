import{i as e}from"./supabase-Cgh_dhNB.js";import{s as t}from"./main-maestros-D8KYVIBy.js";function n(e,{alumnos:t=[],onSelect:n}){let r=document.getElementById(`pm-alumno-picker-modal`);r||(r=document.createElement(`div`),r.id=`pm-alumno-picker-modal`,r.className=`pm-modal-overlay`,r.innerHTML=`
  ...
  `,r.querySelector(`#eme-alumnos-chips`);let a=[];try{let{data:t}=await e.from(`alumnos_clases`).select(`alumno:alumnos(id, nombre_completo)`).eq(`activo`,!0);a=(t||[]).map(e=>e.alumno).filter(Boolean)}catch(e){console.error(e)}let o=n(r,{alumnos:a,onSelect:e=>{alert(`Alumnos seleccionados correctamente`)}});r.querySelector(`#btn-eme-pick-alumnos`).onclick=()=>o.open(),r.querySelector(`#btn-eme-guardar`).onclick=async()=>{let e={maestro_id:i,fecha:r.querySelector(`#eme-fecha`).value,motivo:r.querySelector(`#eme-motivo`).value,nombre_clase:r.querySelector(`#eme-nombre`).value,contenido:r.querySelector(`#eme-contenido`).value,created_at:new Date().toISOString()};if(!e.nombre_clase){alert(`Por favor ingresa un nombre para la clase.`);return}await t({tabla:`clases_emergentes`,operacion:`insert`,payload:e}),alert(`Clase emergente registrada. Redirigiendo...`),window.location.hash=`#/hoy`}}export{r as renderClaseEmergenteView};