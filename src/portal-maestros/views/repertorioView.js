...
import { renderSegmentedControl, renderHorizontalFilaTabs, renderBottomSheet, renderActionSheet, renderConfirmDialog, renderContextMenu, renderEmptyState, renderSkeleton, renderDerivedPresentation, renderStateBadge, bindRepertoireOverlay } from '../components/repertoirePrimitives.js'
...
function mapMarkup(montaje, selected, selectionMode, pickerMeasure, canEditApplicability, syncMessage, activeStudentId, passages = [], groups = [], action = null, linkedScope = null, measuresPerRow = DEFAULT_MEASURES_PER_ROW, historyEvents = [], trajectoryVisible = false, trajectoryTargets = [], canEditTargets = false, priorityVisible = false, priorityCandidates = [], mode = 'real', detailMode = 'fila') {
  ...
  const scopeLabel = activeStudent ? `Alumno · ${studentLabel}` : `Fila · ${activeFila.nombre || 'Pendiente'}`
  const actionSheet = selectionMode && selected.size ? renderActionSheet({ id: 'repertoire-measure-actions', selectedCount: selected.size, open: true, title: `${selected.size} compases seleccionados`, body: `<p class="repertoire-action-sheet-scope"><strong>Aplicar a …</strong> ${html(scopeLabel)}</p><fieldset><legend>PREPARACIÓN</legend><div class="repertoire-action-sheet-grid">${ESTADOS_PREPARACION.map((state) => `<button type="button" class="pm-button pm-button--secondary repertoire-sheet-preparation" data-state="${state}">${STATE_LABELS[state]}</button>`).join('')}</div></fieldset><fieldset><legend>APLICABILIDAD</legend><div class="repertoire-action-sheet-grid">${Object.entries(APPLICABILITY_LABELS).filter(([key]) => key !== 'DESCONOCIDO').map(([key, label]) => `<button type="button" class="pm-button pm-button--secondary repertoire-sheet-applicability" data-applicability="${key}">${label}</button>`).join('')}</div></fieldset><p class="repertoire-action-sheet-help">La preparación y la aplicabilidad se guardan por separado.</p>`, actions: [{ id: 'close', label: 'Cerrar selección' }] }) : ''
  ...
  return `<section class="repertoire-map" data-repertoire-mode="${html(mode)}" aria-label="Mapa de preparación de ${montaje.obra.titulo}">
    ...
    ${montaje.backendWarnings?.length ? `<div class="repertoire-backend-warning" role="status"><strong>Actualización del módulo requerida</strong><span>${html(montaje.backendWarnings.join(' '))}</span></div>` : ''}${renderHorizontalFilaTabs({ filas: montaje.filas || [], activeId: activeFilaId, label: 'Filas de la obra' })}
...
export async function renderRepertoireView(container, { adapter } = {}) {
  ...
  const bindMap = () => {
    container.querySelectorAll('.repertoire-fila-tab, .pm-fila-tab').forEach((button) => button.addEventListener('click', () => { setActiveFila(button.dataset.filaId); activeStudentId = null; pickerMeasure = null; selected = new Set(); selectionMode = false; anchorIndex = null; render() }))
    container.querySelectorAll('#repertoire-detail-mode .pm-segment').forEach((button) => button.addEventListener('click', () => { detailMode = button.dataset.value; if (detailMode === 'alumnos') activeStudentId ||= filaStudentsForActive()[0]?.id || null; pickerMeasure = null; selected = new Set(); selectionMode = false; anchorIndex = null; render() }))
    ...
    container.querySelector('.repertoire-student-prev')?.addEventListener('click', () => { const students = filaStudentsForActive(); const index = students.findIndex((student) => student.id === activeStudentId); if (index > 0) { activeStudentId = students[index - 1].id; selected = new Set(); selectionMode = false; anchorIndex = null; render() } })
    container.querySelector('.repertoire-student-next')?.addEventListener('click', () => { const students = filaStudentsForActive(); const index = students.findIndex((student) => student.id === activeStudentId); if (index >= 0 && index < students.length - 1) { activeStudentId = students[index + 1].id; selected = new Set(); selectionMode = false; anchorIndex = null; render() } })
    ...
    container.querySelectorAll('.repertoire-student').forEach((button) => button.addEventListener('click', () => { activeStudentId = button.dataset.studentId || null; pickerMeasure = null; selected = new Set(); selectionMode = false; anchorIndex = null; render() }))
    ...
    container.querySelector('#repertoire-measure-actions')?.addEventListener('click', async (event) => {
      if (event.target.closest('[data-action="close"]') || event.target.closest('[data-sheet-close]')) { selected = new Set(); selectionMode = false; anchorIndex = null; render(); return }
      const prep = event.target.closest('.repertoire-sheet-preparation')
      if (prep) { const bulk = container.querySelector('.repertoire-bulk'); if (bulk) { bulk.value = prep.dataset.state; container.querySelector('.repertoire-apply')?.click() } return }
      const applicability = event.target.closest('.repertoire-sheet-applicability')
      if (applicability) {
        if (detailMode === 'alumnos' || !(canEditApplicability && adapter.supportsFilaPreparation)) { syncMessage = { label: 'Actualización de backend requerida', className: 'is-error' }; render(); return }
        const measures = [...selected].map((id) => active.compases.find((measure) => measure.id === id)).filter(Boolean)
        const previous = measures.map((measure) => [measure, measure.aplicabilidad])
        measures.forEach((measure) => { measure.aplicabilidad = applicability.dataset.applicability })
        selected = new Set(); selectionMode = false; syncMessage = { label: 'Guardando…', className: 'is-pending' }; render()
        try { await Promise.all(measures.map((measure) => adapter.updateMeasureApplicability(measure.id, measure.aplicabilidad, { montageId: active.id, filaId: active.activeFilaId }))); syncMessage = { label: savedLabel, className: 'is-saved' } } catch (error) { previous.forEach(([measure, value]) => { measure.aplicabilidad = value }); syncMessage = { label: error.message === 'Actualización de backend requerida' ? error.message : 'No se pudo guardar; se revirtió', className: 'is-error' } }
        render()
      }
    })
    container.querySelector('#repertoire-measure-actions [data-action="close"]')?.addEventListener('click', () => { selected = new Set(); selectionMode = false; anchorIndex = null; render() })
    ...
    container.querySelectorAll('.repertoire-measure').forEach((button) => {
      ...
      button.addEventListener('pointerdown', () => { pressTimer = setTimeout(() => { const measure = active.compases.find((item) => item.id === button.dataset.measureId); selected = new Set([measure.id]); selectionMode = true; anchorIndex = active.compases.indexOf(measure); pickerMeasure = null; longPressTriggered = true; render() }, 550) })
    ...
    container.querySelector('.repertoire-apply')?.addEventListener('click', async () => {
      ...
      const bulkStudentWriter = adapter.updateStudentStates || adapter.updateStudentPreparationBulk
      if (activeStudentId && typeof bulkStudentWriter !== 'function') { syncMessage = { label: 'Actualización de backend requerida', className: 'is-error' }; render(); return }
