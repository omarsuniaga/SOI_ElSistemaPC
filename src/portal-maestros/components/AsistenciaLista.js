...
export function createAsistenciaLista(container, { alumnos = [], onStateChange }) {
  ...
  function _getCardStyle(estado) {
    if (estado === 'P')
      return 'border-left-color: var(--pm-success); background: rgba(34, 197, 94, 0.05);'
    if (estado === 'A')
      return 'border-left-color: var(--pm-danger); background: rgba(239, 68, 68, 0.05);'
    if (estado === 'J')
      return 'border-left-color: var(--pm-warning); background: rgba(234, 179, 8, 0.05);'
    return ''
  }
