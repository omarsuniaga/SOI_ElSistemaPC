...
Sé directo, pedagógico y actionable. Responde SOLO JSON válido.`,a={model:`llama-3.1-70b-versatile`,messages:[{role:`user`,content:i}],temperature:.7,max_tokens:500};console.log(`[ClaseAnalysis] Enviando a Groq:`,{apiKey:n?.slice(0,10)+`...`,model:a.model,prompt:i.slice(0,100)+`...`});let o=await fetch(pn,{method:`POST`,headers:{Authorization:`Bearer ${n}`,"Content-Type":`application/json`},body:JSON.stringify(a)});if(console.log(`[ClaseAnalysis] Respuesta status:`,o.status),!o.ok){let e=await o.json().catch(()=>({}));return console.error(`[ClaseAnalysis] Groq error status:`,o.status),console.error(`[ClaseAnalysis] Groq error body:`,JSON.stringify(e,null,2)),console.error(`[ClaseAnalysis] Full error:`,e),null}let s=(await o.json()).choices?.[0]?.message?.content||``;try{return JSON.parse(s)}catch(e){return console.error(`[ClaseAnalysis] Parse error:`,e),null}}catch(e){return console.error(`[ClaseAnalysis] Error:`,e),null}}async function _n(e,t,n=4){try{let r=new Date(new Date(t).getTime()-n*7*24*60*60*1e3).toISOString().split(`T`)[0],{data:i}=await S.from(`sesiones_clase`).select(`id, fecha, contenido, asistencia`).eq(`clase_id`,e).gte(`fecha`,r).lte(`fecha`,t).order(`fecha`,{ascending:!1});if(!i||i.length===0)return{sesiones:[],tracking:[]};let{data:a}=await S.from(`observaciones_alumnos`).select(`id, alumno_id, contenido, created_at`).eq(`clase_id`,e).gte(`created_at`,r+`T00:00:00`),o={};a&&a.forEach(e=>{o[e.alumno_id]||(o[e.alumno_id]=[]),o[e.alumno_id].push(e.contenido)});let{data:s}=await S.from(`alumnos`).select(`id, nombre_completo`),c=Object.fromEntries((s||[]).map(e=>[e.id,e.nombre_completo]));return{sesiones:i.map(e=>{let t=Array.isArray(e.asistencia)?e.asistencia:[],n=t.filter(e=>e.estado===`P`).map(e=>e.alumno_id||e),r=t.filter(e=>e.estado===`A`).map(e=>e.alumno_id||e);return{fecha:e.fecha,contenido:e.contenido||`(sin descripción)`,totalPresentes:n.length,totalAusentes:r.length,asistentes:n.map(e=>c[e]||`Desconocido`),ausentes:r.map(e=>c[e]||`Desconocido`),detalleAlumnos:t.map(e=>({alumnoId:e.alumno_id||e,alumnoNombre:c[e.alumno_id||e]||`Desconocido`,estado:e.estado,tieneObs:o[e.alumno_id||e]?.length>0,obsPreview:o[e.alumno_id||e]?.[0]||null}))}}),alumnoMap:c,obsPorAlumno:o}}catch(e){return console.error(`[ContentTracking] Error:`,e),{sesiones:[],tracking:[],alumnoMap:{},obsPorAlumno:{}}}}async function vn(e,t,n=4){try{console.log(`[getClaseDataForAnalysis] Iniciando con claseId:`,e,`fecha:`,t,`semanas:`,n);let{data:r,error:i}=await S.from(`clases`).select(`id, nombre, instrumento`).eq(`id`,e).maybeSingle();if(console.log(`[getClaseDataForAnalysis] Clase:`,r,`Error:`,i),!r)return console.warn(`[getClaseDataForAnalysis] No se encontró clase con id:`,e),null;let{data:a}=await S.from(`inscripciones`).select(`alumno_id`).eq(`clase_id`,e),o=a?.length||0,s=new Date(new Date(t).getTime()-n*7*24*60*60*1e3).toISOString().split(`T`)[0],{data:c}=await S.from(`sesiones_clase`).select(`asistencia, contenido, borrador`).eq(`clase_id`,e).gte(`fecha`,s).order(`fecha`,{ascending:!1}).limit(1),l=c?.[0],u=Array.isArray(l?.asistencia)?l.asistencia.filter(e=>e.estado===`P`).length:0,d=o-u,{data:f}=await S.from(`sesiones_clase`).select(`asistencia, contenido, borrador`).eq(`clase_id`,e).gte(`fecha`,s).order(`fecha`,{ascending:!1}).limit(10),p=(f||[]).filter(e=>{let t=Array.isArray(e.asistencia)&&e.asistencia.length>0,n=typeof e.contenido==`string`&&e.contenido.trim().length>0;return t||e.borrador===!1&&n}).length,m=(f||[]).length-p,h=f&&f.length>0?Math.round(p/f.length*100):0,g=new Date(new Date(t).getTime()-672*60*60*1e3).toISOString().split(`T`)[0],{data:_}=await S.from(`asistencias`).select(`alumno_id, estado`).eq(`clase_id`,e).gte(`fecha`,g),v={};_&&_.forEach(e=>{v[e.alumno_id]||(v[e.alumno_id]={total:0,presentes:0}),v[e.alumno_id].total++,e.estado===`P`&&v[e.alumno_id].presentes++});let y=Object.values(v).filter(e=>e.total>=3&&e.presentes/e.total<.7).length;return{id:e,nombre:r.nombre,instrumento:r.instrumento||`Sin especificar`,cumplimiento:h,totalAlumnos:o,alumnosPresentes:u,alumnosAusentes:d,registrosCompletos:p,registrosPendientes:m,alumnosEnRiesgo:y,alumnosEnRiesgoDetalle:y>0?`${y} alumno(s) con <70% asistencia`:null}}catch(e){return console.error(`[ClaseDataForAnalysis] Error:`,e),null}}async function yn(e,t=new Date().toISOString().split(`T`)[0],n=4){let r=document.createElement(`div`);r.className=`clase-analysis-backdrop`,r.innerHTML=`
  ...
  `}function wn(e){console.log(`[MetricasView.bindEvents] Iniciando bind, container:`,e),console.log(`[MetricasView.bindEvents] HTML length:`,e?.innerHTML?.length||0),e.querySelector(`#pm-filter-periodo`)?.addEventListener(`change`,async t=>{let n=parseInt(t.target.value,10);U.periodo=n,e.innerHTML=`<div class="pm-loading" style="padding:2rem;"><div class="pm-spinner"></div></div>`;try{let t=await xn(n,U.maestroId),r=Sn(t);U.clasesData=r.clasesData,U.todasSesiones=t.sesiones,U.inscripcionesPorClase=t.inscripcionesPorClase,U.alertasRiesgo=r.alertasRiesgo,e.innerHTML=Cn(r),wn(e),ln(`Período actualizado a ${n} semanas. ${r.asistenciaPromedio}% de asistencia general.`)}catch(t){e.innerHTML=`<p class="pm-empty">Error al cargar datos: ${I(t.message)}</p>`}}),e.querySelectorAll(`.pm-risk-item`).forEach(e=>{let t=e.dataset.alumno,n=()=>{window.location.hash=`#/alumno?id=${t}`};e.addEventListener(`click`,n),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&n()})});let t=new Date,n=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-${String(t.getDate()).padStart(2,`0`)}`,r=U.periodo||4;e.querySelectorAll(`.pm-analisis-btn-metrics`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),t.preventDefault();let i=e.dataset.claseId;yn(i,n,r)})}),e.querySelectorAll(`.pm-class-btn, .pm-class-btn2`).forEach(e=>{e.addEventListener(`click`,async t=>{t.stopPropagation();let n=e.closest(`.pm-class-card2, .pm-class-card`),r=n.querySelector(`.pm-clase-students-panel`);if(r){r.remove();return}let i=e.dataset.claseId,a=U.clasesData.find(e=>e.id===i)?.alumnos||[],o=U.todasSesiones.filter(e=>e.clase_id===i),s=a.map(e=>{let t=o.filter(t=>t.asistencia?.some(t=>t.alumno_id===e.id)).map(t=>t.asistencia.find(t=>t.alumno_id===e.id)),n=t.filter(e=>e?.estado===`P`).length,r=t.length,i=r>0?Math.round(n/r*100):0,a=o.filter(t=>t.asistencia?.some(t=>t.alumno_id===e.id)).sort((e,t)=>t.fecha.localeCompare(e.fecha))[0];return{...e,pct:i,total:r,lastFecha:a?.fecha}});s.sort((e,t)=>e.pct-t.pct);let c=document.createElement(`div`);c.className=`pm-clase-students-panel`,c.innerHTML=`
  ...
  `,document.head.appendChild(e)}var Un=null,Wn=null;function Gn(e={}){let{fecha:t=``,claseId:n=``,clases:r=[],alumnos:i=[],maestroId:a=null,onSave:o=null}=e;Un=o,Wn=a,Te.open({title:`Nueva Clase Emergente`,size:`lg`,saveText:`Crear Clase`,cancelText:`Cancelar`,body:`
...
`);r&&(n[r]=or(i))}),{...or(e),por_capas:n}}function cr(e){if(!e)return``;let t=lr(e),n=[];function r(e){let t=`__DSL_TOKEN_${n.length}__`;return n.push({id:t,html:e}),t}t=t.replace(W.capas,(e,t)=>r(`<span class="dsl-token dsl-capa" style="background:${rr.capas}22; color:${rr.capas}; font-weight:800; padding:2px 6px; border-radius:4px">:::CAPA: ${t}</span>`)),t=t.replace(/&gt;NIVEL-(\d{1,2})/g,(e,t)=>r(`<span class="dsl-token dsl-nivel" style="color:${rr.niveles}; font-weight:700">&gt;NIVEL-${t}</span>`)),t=t.replace(/&gt;NODO:([A-Z_]+)/g,(e,t)=>r(`<span class="dsl-token dsl-nodo" style="color:${rr.nodos}; font-weight:600">&gt;NODO:${t}</span>`)),t=t.replace(/&gt;([A-Z]{1,3}(?:-[A-Za-z]+)?-[0-9]+(?:\.[0-9]+)?)/g,(e,t)=>r(`<span class="dsl-token dsl-objetivo" data-objetivo="${t}">&gt;${t}</span>`)),t=t.replace(W.alumnos,(e,t)=>r(`<span class="dsl-token dsl-alumno" data-nombre="${t}">#${t}</span>`)),t=t.replace(W.contenido,(e,t)=>r(`<span class="dsl-token dsl-contenido" data-contenido="${t}">[${t}]</span>`)),t=t.replace(W.sugerencias,(e,t)=>r(`<span class="dsl-token dsl-sugerencia" data-sugerencia="${t}">(${t})</span>`)),t=t.replace(W.tareas,(e,t)=>r(`<span class="dsl-token dsl-tarea" data-tarea="${t}">{${t}}</span>`)),t=t.replace(W.medidas,(e,t)=>r(`<span class="dsl-token dsl-medida" data-medida="${t}">$${t}</span>`)),t=t.replace(/!(LOGRADO|EN_PROGRESO|INICIADO)/gi,(e,t)=>{let n=t.toUpperCase(),i={LOGRADO:`#198754`,EN_PROGRESO:`#0d6efd`,INICIADO:`#6c757d`}[n]??`#6c757d`;return r(`<span class="dsl-token dsl-estado" style="color:${i};font-weight:700;background:${i}18;padding:1px 4px;border-radius:3px">!${n}</span>`)}),t=t.replace(W.calificacion,(e,t,n)=>r(`<span class="dsl-token dsl-calificacion" data-valor="${t}" data-sobre="${n}">${t}/${n}</span>`));for(let e=n.length-1;e>=0;e--)t=t.replace(n[e].id,n[e].html);return t}function lr(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}var ur=[{trigger:`escalas`,label:`Escalas`,icon:`🎼`,expand:`[Escala Do Mayor] [Escala Re Mayor] [Escala Sol Mayor]`},{trigger:`arpegios`,label:`Arpegios`,icon:`🎹`,expand:`[Arpegio Do Mayor] [Arpegio La menor] [Arpegio Sol Mayor]`},{trigger:`tecnica`,label:`Técnica`,icon:`🎸`,expand:`$Tecnica_mano_derecha $Tecnica_mano_izquierda`},{trigger:`postura`,label:`Postura`,icon:`🧘`,expand:`$Postura_corporal $Posicion_manos`},{trigger:`evaluar`,label:`Evaluar`,icon:`📝`,expand:`4/5 (buen trabajo) {practicar 30 min diarios}`},{trigger:`mejorar`,label:`Mejorar`,icon:`💪`,expand:`(continuar mejorando la digitación) {repetir练习}`},{trigger:`ritmo`,label:`Ritmo`,icon:`🥁`,expand:`$Ritmo_binario $Ritmo_ternario`},{trigger:`dinamica`,label:`Dinámica`,icon:`🔊`,expand:`$Dinamica_piano $Dinamica_forte $Dinamica_mezzo`},{trigger:`afinacion`,label:`Afinación`,icon:`🎵`,expand:`$Afinacion_precisa $Afinacion_relativa`},{trigger:`lectura`,label:`Lectura`,icon:`📖`,expand:`[Lectura a primera vista] [Lectura de notas]`},{trigger:`respiracion`,label:`Respiración`,icon:`🌬️`,expand:`$Respiracion_diafragmatica $Respiracion_costeado`},{trigger:`memo`,label:`Memoria`,icon:`🧠`,expand:`[Técnica de memorización] {practicar de memoria}`}];function dr(e){if(!e||e.length===0)return ur.slice(0,6);let t=e.toLowerCase();return ur.filter(e=>e.trigger.toLowerCase().includes(t)||e.label.toLowerCase().includes(t)).slice(0,6)}function fr(e){let t=ur.find(t=>t.trigger===e);return t?t.expand:null}function pr(e,{onAccept:t}){let n=document.getElementById(`pm-structure-modal`);if(!n&&(n=document.createElement(`div`),n.id=`pm-structure-modal`,n.className=`pm-modal-overlay`,n.innerHTML=`
    ...
    `,document.body.appendChild(n),!document.getElementById(`pm-structure-modal-styles`))){let e=document.createElement(`style`);e.id=`pm-structure-modal-styles`,e.textContent=`
      ...
      `,document.head.appendChild(e)}let r=n.querySelector(`#pm-structure-original`),i=n.querySelector(`#pm-structure-dsl`);function a({original:e,dsl:t}){r.textContent=e,i.textContent=t,n.classList.add(`open`)}function o(){n.classList.remove(`open`)}return n.querySelector(`#pm-structure-close`).onclick=o,n.querySelector(`#pm-structure-reject`).onclick=o,n.querySelector(`#pm-structure-accept`).onclick=()=>{t&&t(i.textContent),o()},{open:a,close:o}}function mr(e,t={}){let n=document.getElementById(`pm-toolbar-help-modal`);if(!n&&(n=document.createElement(`div`),n.id=`pm-toolbar-help-modal`,n.className=`pm-help-modal-overlay`,n.innerHTML=`
  ...
  `,!document.getElementById(`pm-dsl-toolbar-styles`)){let e=document.createElement(`style`);e.id=`pm-dsl-toolbar-styles`,e.textContent=`
    ...
    `,document.head.appendChild(e)}e.querySelector(`#btn-snippets`).onclick=()=>{h.style.display===`block`?_():g()};let v=mr(e);return e.querySelector(`#btn-help`).onclick=()=>{v.open()},{setContext(e={}){e.presentes!==void 0&&(l.presentes=e.presentes),e.indicadorActivo!==void 0&&(l.indicadorActivo=e.indicadorActivo),e.indicadoresDisponibles!==void 0&&(l.indicadoresDisponibles=e.indicadoresDisponibles)}}}var G=null,gr=[],_r=null,K=-1,vr=!1,yr=null,br=!1,xr=0,Sr=0;function Cr(){if(!G){if(G=document.createElement(`div`),G.id=`pm-autocomplete-popup`,G.className=`pm-autocomplete-popup`,G.style.cssText=`
    position: fixed;
    display: none;
    background: var(--pm-surface, #fff);
  ...
  `,!document.getElementById(`pm-ac-styles`)){let e=document.createElement(`style`);e.id=`pm-ac-styles`,e.textContent=`
    ...
    `,document.head.appendChild(e)}document.body.appendChild(G)}}function wr(e,t,n={}){if(Cr(),gr=e||[],_r=t,yr=n.trigger||null,K=-1,vr=!0,Ar(e),n.position){let e=n.position,t=window.innerWidth,r=window.innerHeight,i=e.x,a=e.y+6;i+320>t-20&&(i=Math.max(10,e.x-320-10)),a+280>r-20&&(a=Math.max(10,e.y-280-10)),G.style.left=`${i}px`,G.style.top=`${a}px`}G.onmousedown=Tr,document.addEventListener(`mousemove`,Er),document.addEventListener(`mouseup`,Dr),G.style.display=`block`}function Tr(e){e.target.closest(`.pm-ac-option`)||(br=!0,xr=e.clientX-G.offsetLeft,Sr=e.clientY-G.offsetTop,G.style.cursor=`grabbing`,G.style.transition=`none`)}function Er(e){if(!br)return;let t=e.clientX-xr,n=e.clientY-Sr;G.style.left=`${Math.max(0,t)}px`,G.style.top=`${Math.max(0,n)}px`}function Dr(){br&&(br=!1,G.style.cursor=``)}function Or(){G&&(G.style.display=`none`,br=!1,document.removeEventListener(`mousemove`,Er),document.removeEventListener(`mouseup`,Dr)),gr=[],_r=null,K=-1,vr=!1,yr=null}function kr(e){gr=e||[],K=-1,Ar(e)}function Ar(e){if(!G)return;if(!e||e.length===0){G.innerHTML=`
      <div class="pm-ac-empty">
        <span>No hay opciones disponibles</span>
      </div>
    `;return}let t=`<div class="pm-ac-header">${Pr(yr)}</div>`;e.forEach((e,n)=>{let r=e.nombre||e.name||e.label||e.description||``,i=e.instrumento||e.descripcion||e.codigo||e.type||``,a=n===K,o=Fr(yr,e),s=e.is_historial?`<span class="pm-ac-badge">Reciente</span>`:``;t+=`
      <div class="pm-ac-option ${a?`selected`:``}" data-index="${n}">
        <div class="pm-ac-icon">${o}</div>
        <div class="pm-ac-text">
          <div class="pm-ac-label">${Lr(r)}</div>
          ${i?`<div class="pm-ac-sublabel">${Lr(i)}</div>`:``}
        </div>
        ${s}
      </div>
    `}),G.innerHTML=t,G.querySelectorAll(`.pm-ac-option`).forEach(e=>{e.addEventListener(`click`,()=>{jr(parseInt(e.dataset.index,10))})})}function jr(e){if(e>=0&&e<gr.length){let t=gr[e];_r&&_r(t),Or()}}function Mr(e){if(!(!vr||gr.length===0))switch(e.key){case`ArrowDown`:e.preventDefault(),K=Math.min(K+1,gr.length-1),Ar(gr),Nr();break;case`ArrowUp`:e.preventDefault(),K=Math.max(K-1,0),Ar(gr),Nr();break;case`Enter`:e.preventDefault(),K>=0?jr(K):gr.length>0&&jr(0);break;case`Escape`:e.preventDefault(),Or();break;case`Tab`:gr.length>0&&K===-1&&(e.preventDefault(),jr(0));break}}function Nr(){if(!G||K<0)return;let e=G.querySelector(`.pm-ac-option[data-index="${K}"]`);e&&e.scrollIntoView({block:`nearest`,behavior:`smooth`})}function Pr(e){switch(e){case`#`:return`👤 Alumnos`;case`[`:return`📚 Contenidos`;case`(`:return`💡 Sugerencias`;case`{`:return`📝 Tareas`;case`$`:return`🎯 Medidas`;case`>`:return`🎓 Objetivos`;default:return`Opciones`}}function Fr(e,t){if(e===`#`){let e=t.nombre||t.name||``;return t.value===`todos`||e.toLowerCase()===`todos`?`👥`:e.charAt(0).toUpperCase()}return e===`$`?`🎯`:e===`>`&&t.level_number?t.level_number:e===`>`&&t.type?Ir(t.type):`•`}function Ir(e){return{ESCALA:`🎼`,ARPEGIO:`🎹`,MANO_IZQ:`✋`,ARCO:`🎻`,SONIDO:`🔊`,AFINACION:`🎵`,TECNICA:`⚙️`,REPERTORIO:`📖`}[e]||`•`}function Lr(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}function Rr(){let e=window.getSelection();if(!e||e.rangeCount===0)return null;let t=e.getRangeAt(0).getBoundingClientRect();return{x:t.left,y:t.bottom}}function zr(){return vr}function Br(){return K}var Vr={show:wr,hide:Or,updateOptions:kr,handleKeyDown:Mr,getCursorPosition:Rr,isOpen:zr,getSelectedIndex:Br},Hr=`portal-maestros-catalogs`,Ur=1,Wr={alumnos:{ttl:1440*60*1e3},contenidos:{ttl:10080*60*1e3},medidas:{ttl:720*60*60*1e3},sugerencias:{ttl:720*60*60*1e3},tareas:{ttl:720*60*60*1e3},nodos:{ttl:10080*60*1e3},niveles:{ttl:10080*60*1e3},indicadores:{ttl:10080*60*1e3},historial:{ttl:null}},Gr=null;async function Kr(){return Gr||(Gr=await ee(Hr,Ur,{upgrade(e){for(let[t,n]of Object.entries(Wr))if(!e.objectStoreNames.contains(t)){let n=e.createObjectStore(t,{keyPath:`id`});n.createIndex(`by_updated`,`updated_at`),t===`alumnos`&&n.createIndex(`by_clase`,`clase_id`)}}}),Gr)}async function qr(e,t){let n=await Kr(),r=await n.get(e,t);if(!r)return null;let i=Wr[e];if(i?.ttl&&r.updated_at){let a=new Date(r.updated_at).getTime()+i.ttl;if(Date.now()>a)return await n.delete(e,t),null}return r}async function Jr(e){let t=await(await Kr()).getAll(e),n=Wr[e];if(!n?.ttl)return t;let r=Date.now();return t.filter(e=>e.updated_at?r<=new Date(e.updated_at).getTime()+n.ttl:!0)}async function Yr(e,t,n){return(await Kr()).getAllFromIndex(e,t,n)}async function Xr(e,t){let n=await Kr(),r={...t,updated_at:new Date().toISOString()};return await n.put(e,r),r}async function Zr(e,t){let n=(await Kr()).transaction(e,`readwrite`);for(let e of t)await n.store.put({...e,updated_at:new Date().toISOString()});await n.done}async function Qr(e,t){await(await Kr()).delete(e,t)}async function $r(e){await(await Kr()).clear(e)}async function ei(e){let t=await Kr(),n=Wr[e];if(!n?.ttl)return;let r=await t.getAll(e),i=Date.now();for(let a of r)a.updated_at&&i>new Date(a.updated_at).getTime()+n.ttl&&await t.delete(e,a.id)}async function ti(){let e=await Kr();for(let t of Object.keys(Wr))await e.clear(t)}async function ni(e){return(await Jr(e)).length}async function ri(e,t){let n=await Kr(),r=new Date().toISOString(),i=await n.get(`historial`,e);i?(i.count=(i.count||0)+1,i.last_used=r,i.recent_selections=[t,...(i.recent_selections||[]).slice(0,9)],await n.put(`historial`,i)):await n.put(`historial`,{id:e,trigger:e,count:1,last_used:r,recent_selections:[t],updated_at:r})}async function ii(e){return(await Kr()).get(`historial`,e)}async function ai(e,t=5){return(await(await Kr()).getAll(`historial`)).filter(t=>t.trigger===e).sort((e,t)=>(t.count||0)-(e.count||0)).slice(0,t)}var q={get:qr,getAll:Jr,getByIndex:Yr,set:Xr,setBulk:Zr,remove:Qr,clear:$r,cleanExpired:ei,clearAll:ti,getStoreSize:ni,addToHistorial:ri,getHistorial:ii,getTopUsed:ai};async function oi(e){if(!e)return[];try{let{data:t,error:n}=await S.from(`alumnos_clases`).select(`alumno_id, alumnos(id, nombre_completo, instrumento_principal)`).eq(`clase_id`,e).eq(`activo`,!0);if(n)throw n;if(t)return t.map(e=>e.alumnos).filter(Boolean).map(e=>({id:e.id,nombre:e.nombre_completo||``,instrumento:e.instrumento_principal}))}catch(e){console.warn(`[CatalogService] Error cargando alumnos:`,e)}return[]}async function si(){let e=await q.getAll(`contenidos`);if(e.length>0)return e;try{let{data:e,error:t}=await S.from(`catalogos`).select(`id, nombre, descripcion`).eq(`tipo`,`contenidos`).eq(`activo`,!0).order(`orden`,{ascending:!0});if(t)throw t;if(e)return await q.setBulk(`contenidos`,e),e}catch(e){console.warn(`[CatalogService] Error cargando contenidos:`,e)}return[]}async function ci(){let e=await q.getAll(`medidas`);if(e.length>0)return e;try{let{data:e,error:t}=await S.from(`catalogos`).select(`id, nombre, codigo, categoria`).eq(`tipo`,`medidas`).eq(`activo`,!0).order(`orden`,{ascending:!0});if(t)throw t;if(e)return await q.setBulk(`medidas`,e),e}catch(e){console.warn(`[CatalogService] Error cargando medidas:`,e)}return[]}async function li(){let e=await q.getAll(`sugerencias`);if(e.length>0)return e;try{let{data:e,error:t}=await S.from(`catalogos`).select(`id, nombre, descripcion`).eq(`tipo`,`sugerencias`).eq(`activo`,!0).order(`orden`,{ascending:!0});if(t)throw t;if(e)return await q.setBulk(`sugerencias`,e),e}catch(e){console.warn(`[CatalogService] Error cargando sugerencias:`,e)}return[]}async function ui(){let e=await q.getAll(`tareas`);if(e.length>0)return e;try{let{data:e,error:t}=await S.from(`catalogos`).select(`id, nombre, descripcion`).eq(`tipo`,`tareas`).eq(`activo`,!0).order(`orden`,{ascending:!0});if(t)throw t;if(e)return await q.setBulk(`tareas`,e),e}catch(e){console.warn(`[CatalogService] Error cargando tareas:`,e)}return[]}async function di(){let e=await q.getAll(`niveles`);if(e.length>0)return e;try{let{data:e}=await S.from(`routes`).select(`id`).eq(`instrument`,`violín`).eq(`status`,`published`).limit(1);if(!e||e.length===0)return[];let t=e[0].id,{data:n}=await S.from(`route_versions`).select(`id`).eq(`route_id`,t).eq(`status`,`published`).order(`version`,{ascending:!1}).limit(1);if(!n||n.length===0)return[];let r=n[0].id,{data:i,error:a}=await S.from(`levels`).select(`id, level_number, name, main_objective`).eq(`route_version_id`,r).order(`level_number`,{ascending:!0});if(a)throw a;if(i)return await q.setBulk(`niveles`,i),i}catch(e){console.warn(`[CatalogService] Error cargando niveles:`,e)}return[]}async function fi(e=null){let t=await q.getAll(`nodos`);if(e&&t.length>0){if(t=t.filter(t=>t.level_id===e),t.length>0)return t}else if(t.length>0)return t;try{let t=S.from(`nodes`).select(`id, name, type, is_critical, is_required, objective, level_id, order_index`);e&&(t=t.eq(`level_id`,e));let{data:n,error:r}=await t.order(`order_index`,{ascending:!0});if(r)throw r;if(n)return await q.setBulk(`nodos`,n),n}catch(e){console.warn(`[CatalogService] Error cargando nodos:`,e)}return[]}async function pi(e,t=``,n={}){let r=[];switch(e){case`#`:r=[{label:`todos`,value:`todos`,icon:`👥`,description:`Todos los presentes`}],r=r.concat(await oi(n.claseId));break;case`[`:r=await si();break;case`(`:r=await li();break;case`{`:r=await ui();break;case`$`:r=await ci();break;case`>`:r=t.toUpperCase().startsWith(`NIVEL`)?await di():await fi(n.nivelId);break;default:r=[]}if(t&&r.length>0&&(r=mi(r,t)),e&&e!==`#`){let t=(await q.getTopUsed(e,3)).flatMap(e=>e.recent_selections||[]).filter(Boolean).slice(0,3);for(let e of t)r.some(t=>(t.nombre||t.name||``).toLowerCase()===e.toLowerCase())||r.unshift({nombre:e,id:`hist-${e}`,is_historial:!0})}return r}function mi(e,t,n=`nombre`){if(!t)return e;let r=t.toLowerCase(),i=r.length;return e.map(e=>{let t=(e[n]||e.name||e.nombre||``).toLowerCase(),a=0;if(t.startsWith(r))a+=10;else if(t.includes(r))a+=5;else{let e=hi(t,r);if(e<=2&&i>3)a+=3-e;else return null}return t.length<20&&(a+=1),{...e,_score:a}}).filter(Boolean).sort((e,t)=>(t._score||0)-(e._score||0)).slice(0,15)}function hi(e,t){let n=[];for(let e=0;e<=t.length;e++)n[e]=[e];for(let t=0;t<=e.length;t++)n[0][t]=t;for(let r=1;r<=t.length;r++)for(let i=1;i<=e.length;i++)t.charAt(r-1)===e.charAt(i-1)?n[r][i]=n[r-1][i-1]:n[r][i]=Math.min(n[r-1][i-1]+1,n[r][i-1]+1,n[r-1][i]+1);return n[t.length][e.length]}async function gi(e,t){await q.addToHistorial(e,t)}var _i=`
...
`;function vi(e,{initialContent:t=``,onChange:n,onAlumnosNeeded:r}){let i=t,a=!1,o=!1,s={};e.innerHTML=`
    <div class="pm-dsl-editor-container">
      <div
        ...
        spellcheck="false"
      ></div>
      <div class="pm-dsl-placeholder" id="pm-dsl-placeholder">${_i}</div>
  ...
  `,e.appendChild(o);let s=o.querySelector(`#pm-evaluation-cards-container`);s&&i.forEach(e=>{yi(s,{indicator:e,sessionId:n,studentId:t.id,teacherId:r,onSave:e=>{console.log(`Progress saved:`,e)}})}),setTimeout(()=>o.classList.add(`open`),10);let c=()=>{o.classList.remove(`open`),setTimeout(()=>o.remove(),400)},l=o.querySelector(`#pm-close-eval-drawer`),u=o.querySelector(`#pm-finish-eval`);return l&&l.addEventListener(`click`,c),o.addEventListener(`click`,e=>{e.target===o&&c()}),u&&u.addEventListener(`click`,c),{close:c}}function xi(e,{onAceptar:t}){let n=document.getElementById(`pm-generar-informe-modal`);if(!n&&(n=document.createElement(`div`),n.id=`pm-generar-informe-modal`,n.className=`pm-modal-overlay`,n.innerHTML=`
...
`,Ei=class{constructor(e,t=wi){this._container=e,this._steps=t,this._step=0,this._autoTimer=null,this._overlay=null,this._spotlight=null,this._tooltip=null,this._mounted=!1,this._styleEl=null}mount(){if(!this._mounted)try{this._injectStyles(),this._injectDOM(),this._bindEvents(),this._mounted=!0,localStorage.getItem(Si)||(this._autoTimer=setTimeout(()=>this.start(),Ci))}catch(e){console.error(`[AsistenciaTour] Error al montar el tour:`,e),this._mounted=!1}}start(){this._overlay&&(this._step=0,this._tooltip.style.display=`block`,this._spotlight.style.display=`block`,this._overlay.style.display=`block`,this._overlay.offsetHeight,this._overlay.style.opacity=`1`,this._showStep(0),localStorage.setItem(Si,`true`))}destroy(){this._autoTimer!==null&&(clearTimeout(this._autoTimer),this._autoTimer=null),this._overlay&&=(this._overlay.style.transition=`none`,this._overlay.style.opacity=`0`,this._overlay.style.display=`none`,this._overlay.remove(),null),this._spotlight&&=(this._spotlight.remove(),null),this._tooltip&&=(this._tooltip.remove(),null),this._styleEl&&=(this._styleEl.remove(),null),this._mounted=!1}_injectStyles(){if(document.getElementById(`pm-tour-styles`))return;let e=document.createElement(`style`);e.id=`pm-tour-styles`,e.textContent=Ti,document.head.appendChild(e),this._styleEl=e}_injectDOM(){document.getElementById(`pm-tour-overlay`)?.remove(),document.getElementById(`pm-tour-spotlight`)?.remove(),document.getElementById(`pm-tour-tooltip`)?.remove();let e=document.createElement(`div`);e.id=`pm-tour-overlay`,e.className=`pm-tour-overlay`,e.setAttribute(`role`,`dialog`),e.setAttribute(`aria-modal`,`true`),e.setAttribute(`aria-label`,`Guía interactiva`),document.body.appendChild(e),this._overlay=e;let t=document.createElement(`div`);t.id=`pm-tour-spotlight`,t.className=`pm-tour-spotlight`,t.style.display=`none`,document.body.appendChild(t),this._spotlight=t;let n=document.createElement(`div`);n.id=`pm-tour-tooltip`,n.className=`pm-tour-tooltip`,n.style.display=`none`,n.innerHTML=`
  ...
  `,await Ai()}async function Oi(e,t,n,r){Te.open({title:`Editar ${e}`,body:`
  ...
  `,e.querySelector(`#btn-add-class`).onclick=()=>ki(`Clase`,null,()=>Ai()),Y.activeClassId?ji(Y.activeClassId):Li(`#pm-rc-levels-wrapper`,`Elegí Clase`),e.querySelectorAll(`.pm-rc-item`).forEach(e=>{let t=e.dataset.id;e.querySelector(`.btn-edit`).onclick=n=>{n.stopPropagation(),Oi(`Clase`,t,e.querySelector(`.pm-rc-item-text`).innerText,()=>Ai())},e.onclick=()=>{Y.activeClassId=t,Y.activeLevelId=Y.activeNodeId=Y.activeObjectiveId=null,Ai(),ji(t),Li(`#pm-rc-nodes-wrapper`,`Elegí Nivel`),Li(`#pm-rc-objs-wrapper`,`Elegí Tema`),Li(`#pm-rc-inds-wrapper`,`Elegí Objetivo`)}})}async function ji(e){let t=document.getElementById(`pm-rc-levels-wrapper`),n=await J.getLevelsByClass(e);!n.some(e=>e.id===Y.activeLevelId)&&n.length>0?Y.activeLevelId=n[0].id:n.length===0&&(Y.activeLevelId=null),t.innerHTML=`
    <div class="pm-rc-header">
      <h4>2. Nivel</h4> 
  ...
  `,t.querySelector(`#btn-add-level`).onclick=()=>ki(`Nivel`,e,()=>ji(e)),Y.activeLevelId?Mi(Y.activeLevelId):Li(`#pm-rc-nodes-wrapper`,`Elegí Nivel`),t.querySelectorAll(`.pm-rc-item`).forEach(t=>{let n=t.dataset.id;t.querySelector(`.btn-edit`).onclick=r=>{r.stopPropagation(),Oi(`Nivel`,n,t.querySelector(`.pm-rc-item-text`).innerText,()=>ji(e))},t.onclick=()=>{Y.activeLevelId=n,Y.activeNodeId=Y.activeObjectiveId=null,ji(e),Mi(n),Li(`#pm-rc-objs-wrapper`,`Elegí Tema`),Li(`#pm-rc-inds-wrapper`,`Elegí Objetivo`)}})}async function Mi(e){let t=document.getElementById(`pm-rc-nodes-wrapper`),n=await J.getNodesByLevel(e);!n.some(e=>e.id===Y.activeNodeId)&&n.length>0?Y.activeNodeId=n[0].id:n.length===0&&(Y.activeNodeId=null),t.innerHTML=`
    <div class="pm-rc-header">
      <h4>3. Tema</h4> 
  ...
  `,t.querySelector(`#btn-add-node`).onclick=()=>ki(`Tema`,e,()=>Mi(e)),Y.activeNodeId?Ni(Y.activeNodeId):Li(`#pm-rc-objs-wrapper`,`Elegí Tema`),t.querySelectorAll(`.pm-rc-item`).forEach(t=>{let n=t.dataset.id;t.querySelector(`.btn-edit`).onclick=r=>{r.stopPropagation(),Oi(`Tema`,n,t.querySelector(`.pm-rc-item-text`).innerText,()=>Mi(e))},t.onclick=()=>{Y.activeNodeId=n,Y.activeObjectiveId=null,Mi(e),Ni(n),Li(`#pm-rc-inds-wrapper`,`Elegí Objetivo`)}})}async function Ni(e){let t=document.getElementById(`pm-rc-objs-wrapper`),n=await J.getObjectivesByNode(e);!n.some(e=>e.id===Y.activeObjectiveId)&&n.length>0?Y.activeObjectiveId=n[0].id:n.length===0&&(Y.activeObjectiveId=null),t.innerHTML=`
    <div class="pm-rc-header">
      <h4>4. Objetivo</h4> 
  ...
  `,t.querySelector(`#btn-add-obj`).onclick=()=>ki(`Objetivo`,e,()=>Ni(e)),Y.activeObjectiveId?Ii(Y.activeObjectiveId):Li(`#pm-rc-inds-wrapper`,`Elegí Objetivo`),t.querySelectorAll(`.pm-rc-item`).forEach(t=>{let n=t.dataset.id;t.querySelector(`.btn-edit`).onclick=r=>{r.stopPropagation(),Oi(`Objetivo`,n,t.querySelector(`.pm-rc-item-text`).innerText,()=>Ni(e))},t.onclick=()=>{Y.activeObjectiveId=n,Ni(e),Ii(n)}})}var Pi={0:`Sin eval.`,1:`Inicial`,2:`En desarrollo`,3:`Logrado`,4:`Destacado`,5:`Superado`},Fi={0:`var(--pm-text-muted)`,1:`#ef4444`,2:`#f97316`,3:`#22c55e`,4:`#06b6d4`,5:`#8b5cf6`};async function Ii(e){let t=document.getElementById(`pm-rc-inds-wrapper`);t.innerHTML=`
    <div class="pm-rc-header">
      <h4>5. Indicador</h4> 
          ...
          <div class="pm-rc-calif-row" style="display:flex;gap:4px;padding-left:1.6rem;" data-indicator-id="${e.id}">
            ${[1,2,3,4,5].map(t=>`
              <button class="pm-rc-calif-btn" data-calif="${t}" style="
                width:28px;height:28px;border-radius:50%;border:2px solid ${Fi[t]};
                background:${e.calificacion===t?Fi[t]:`transparent`};
                color:${e.calificacion===t?`#fff`:Fi[t]};
            ...
            `:``}
            <span style="font-size:0.6rem;color:${Fi[e.calificacion]||`var(--pm-text-muted)`};margin-left:auto;align-self:center;font-weight:600;">
              ${Pi[e.calificacion]||Pi[0]}
            </span>
          </div>
        </div>
      `).join(``)}
    </div>
  `,t.querySelector(`#btn-add-ind`).onclick=()=>ki(`Indicador`,e,()=>Ii(e)),t.querySelectorAll(`.btn-edit-ind`).forEach(t=>{t.onclick=n=>{n.stopPropagation();let r=t.dataset.id,i=t.closest(`.pm-rc-ind-card`).querySelector(`.ind-text`).innerText;Oi(`Indicador`,r,i,()=>Ii(e))}}),t.querySelectorAll(`.pm-rc-calif-btn, .pm-rc-calif-clear`).forEach(t=>{t.onclick=async n=>{n.stopPropagation();let r=t.closest(`.pm-rc-calif-row`).dataset.indicatorId,i=parseInt(t.dataset.calif,10);try{await J.updateIndicatorCalificacion(r,i),Ii(e)}catch(e){console.error(`Error updating calificacion:`,e)}}})}function Li(e,t){let n=document.querySelector(e);n&&(n.innerHTML=`<div class="pm-rc-empty">${t}</div>`)}function Ri(e){let t=document.createElement(`div`);return t.textContent=e??``,t.innerHTML}function zi(e,{claseId:t,rutaId:n,completedTopics:r=[],onIndicadorSelect:i}){let a=[],o=!1,s=null,c=document.createElement(`div`);if(c.className=`pm-route-bar-wrapper`,e.appendChild(c),!document.getElementById(`pm-route-bar-styles`)){let e=document.createElement(`style`);e.id=`pm-route-bar-styles`,e.textContent=`
...
`)}function r(e,t){let n=sa[e.estado]||sa.EN_PROGRESO,r=e.alumnos.length,i=e.estado===`MIXTO`,a=e.alumnos.map(e=>`<span class="${i?`ssp-alumno-chip ${ca[e.estado]||``}`:`ssp-alumno-chip`}">${ua(e.nombre)}</span>`).join(``),o=e.observaciones?`<div class="ssp-group-obs">${ua(e.observaciones)}</div>`:``,s=e.tarea?`<div class="ssp-group-tarea">📝 Tarea: ${ua(e.tarea)}</div>`:``;return`
  ...
  `,document.head.appendChild(e)}function ha({saveFn:e,debounceMs:t=3e4}){let n=null,r=[];function i(i){n!==null&&(clearTimeout(n),n=null),!(!i||!i.trim())&&(n=setTimeout(async()=>{n=null,await e(i),r.forEach(e=>e(i))},t))}function a(){n!==null&&(clearTimeout(n),n=null)}function o(e){r.push(e)}return{onInput:i,destroy:a,onSaved:o}}async function ga(e,t,n){let{data:r,error:i}=await S.from(`observaciones_sesion`).select(`id`).eq(`sesion_id`,e).eq(`maestro_id`,t).eq(`es_borrador`,!0).limit(1).maybeSingle();if(i)throw i;if(r){let{data:e,error:t}=await S.from(`observaciones_sesion`).update({contenido_raw:n}).eq(`id`,r.id).select().single();if(t)throw t;return e}else{let{data:r,error:i}=await S.from(`observaciones_sesion`).insert({sesion_id:e,maestro_id:t,contenido_raw:n,es_borrador:!0}).select().single();if(i)throw i;return r}}async function _a(e,t){let{data:n,error:r}=await S.from(`observaciones_sesion`).select(`id, contenido_raw, updated_at`).eq(`sesion_id`,e).eq(`maestro_id`,t).eq(`es_borrador`,!0).limit(1).maybeSingle();if(r)throw r;return n??null}async function va(e){let{error:t}=await S.from(`observaciones_sesion`).delete().eq(`id`,e);if(t)throw t}async function ya(e,t,n,r,i=null,a=null){try{let{error:o}=await S.from(`observaciones_sesion`).delete().eq(`sesion_id`,e).eq(`maestro_id`,t).eq(`es_borrador`,!0);if(o)throw o;let{data:s,error:c}=await S.from(`observaciones_sesion`).insert({sesion_id:e,maestro_id:t,contenido_raw:n,contenido_parsed:r,contenido_ia_dsl:i,contenido_ia_mejorado:a,es_borrador:!1}).select().single();if(c)throw c;return s}catch(o){if(!navigator.onLine||o.message?.includes(`Failed to fetch`))return console.warn(`[autoDraftService] Offline, encolando saveObservation...`),await P({tabla:`observaciones_sesion`,operacion:`upsert`,payload:{sesion_id:e,maestro_id:t,contenido_raw:n,contenido_parsed:r,contenido_ia_dsl:i,contenido_ia_mejorado:a,es_borrador:!1}}),{_offline:!0,sesion_id:e};throw o}}var ba=`soi_ruta_tema_pendiente`;function xa(e){sessionStorage.setItem(ba,JSON.stringify(e))}function Sa(){let e=sessionStorage.getItem(ba);if(!e)return null;sessionStorage.removeItem(ba);try{return JSON.parse(e)}catch{return null}}function Ca(e,{onSave:t,onCancel:n,onDelete:r}){let i=document.getElementById(`pm-justif-modal`);if(!i&&(i=document.createElement(`div`),i.id=`pm-justif-modal`,i.className=`pm-justif-modal-overlay`,i.innerHTML=`
...
`),s=[];for(let e of o){let t=sr(e);if(!t.estados||t.estados.length===0||!t.contenido||t.contenido.length===0)continue;let n=t.estados[0],r=t.contenido[0],i=t.alumnos.length>0?t.alumnos:[`todos`],a=t.calificacion?.valor??null;s.push({alumnos:i,contenido:r,tipo:`tecnica`,estado:n,nota:a,tarea:t.tareas[0]||null,observacion:t.sugerencias[0]||null,es_colectivo:i.includes(`todos`)})}return s.length===0?{saved:[],errors:[]}:Ia({sesionId:e,claseId:t,maestroId:n,fechaHoy:r,progressRecords:s,alumnos:a})}async function Ra({claseId:e,objetivos:t}){if(!e||!t||t.length===0)return{linked:0};let{data:n,error:r}=await S.from(`progresos`).select(`id, contenido_dsl`).eq(`clase_id`,e).is(`objetivo_id`,null).not(`contenido_dsl`,`is`,null).neq(`contenido_dsl`,``);if(r)return console.warn(`[Progress] linkProgresosToObjetivos fetch error:`,r.message),{linked:0};if(!n||n.length===0)return{linked:0};let i=t.map(e=>({id:e.id,norm:ja(e.descripcion),raw:e.descripcion})),a=new Map;for(let e of n){let t=ja(e.contenido_dsl);if(!t)continue;let n=i.find(e=>e.norm===t);if(!n&&t.length>=5&&(n=i.find(e=>e.norm.length>=5&&e.norm.includes(t))),!n&&t.length>=5&&(n=i.find(e=>e.norm.length>=5&&t.includes(e.norm))),n){let t=a.get(n.id)||[];t.push(e.id),a.set(n.id,t)}}if(a.size===0)return{linked:0};let o=0;for(let[e,t]of a.entries()){let{error:n}=await S.from(`progresos`).update({objetivo_id:e}).in(`id`,t);n?console.warn(`[Progress] linkProgresosToObjetivos update error:`,n.message):o+=t.length}return console.debug(`[Progress] linkProgresosToObjetivos: linked ${o} records`),{linked:o}}var za={LOGRADO:{label:`Logrado`,color:`var(--pm-success, #198754)`,bg:`#19875418`},EN_PROGRESO:{label:`En Progreso`,color:`var(--pm-primary, #0d6efd)`,bg:`#0d6efd18`},INICIADO:{label:`Iniciado`,color:`var(--pm-muted,   #6c757d)`,bg:`#6c757d18`}},Ba=[`LOGRADO`,`EN_PROGRESO`,`INICIADO`];function Va(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}var Ha={CONDUCTA:{label:`conducta`,icon:`🚨`},ATENCION:{label:`atención`,icon:`🔔`},RIESGO_PEDAGOGICO:{label:`riesgo pedagógico`,icon:`📉`}};function Ua(e){let t={};for(let n of e){let e=n.alertaTipo??n.alertDetails?.type??`CONDUCTA`;t[e]=(t[e]??0)+1}return`${Object.entries(t).map(([e,t])=>{let n=Ha[e]??{label:e.toLowerCase(),icon:`⚠️`};return`${n.icon} ${t} ${n.label}${t>1?`s`:``}`}).join(` · `)} — revisá antes de guardar`}function Wa(e,{onConfirm:t,onCancel:n}){let r=[],i=null;function a(e){let t=e.scope||(e.es_colectivo?`grupo`:`individual`),n=e.alumnos||[];if(e.requires_confirmation)return`<span class="ppp-scope-chip ppp-scope--unknown">❓ Subgrupo sin identificar</span>`;switch(t){case`grupo`:case`all`:return`<span class="ppp-scope-chip ppp-scope--all">👥 Todos los presentes</span>`;case`grupo_excluyendo`:case`group_excluding`:return`<span class="ppp-scope-chip ppp-scope--excluding">👥 Resto del grupo</span>`;case`subgrupo_indeterminado`:case`subgroup_unknown`:return`<span class="ppp-scope-chip ppp-scope--unknown">❓ Subgrupo sin identificar</span>`;default:return n.length?n.length===1?`<span class="ppp-scope-chip ppp-scope--individual">👤 ${Va(n[0])}</span>`:`<span class="ppp-scope-chip ppp-scope--individual">👤 ${Va(n.join(`, `))}</span>`:``}}function o(e,t){let n=za[e.estado]??za.EN_PROGRESO,r=e.nota?` · ${Va(e.nota)}/5`:``,i=e.tarea?`<div class="ppp-tarea">📝 ${Va(e.tarea)}</div>`:``,o=!!e.alerta,s=a(e);if(o){let n=Ha[e.alertaTipo]??{label:`Alerta pedagógica`,icon:`⚠️`};return`
  ...
  `,document.head.appendChild(e)}async function Ka(e,t=12){let n=new Date;n.setDate(n.getDate()-t*7);let r=n.toISOString().split(`T`)[0],{data:i,error:a}=await S.from(`progresos`).select(`
      contenido_dsl,
      tipo,
      estado_cualitativo,
      fecha_evaluacion,
  ...
  `;let j=e.querySelector(`#pm-sync-badge-container`);if(j){let e=nr();j.appendChild(e.el)}let ee=e.querySelector(`#pm-dsl-toolbar-container`),M=e.querySelector(`#pm-dsl-editor-container`),N=vi(M,{initialContent:g,onChange:e=>{D=e}});N.setContext({claseId:m});let te=xi(e,{onAceptar:e=>{N.setValue(e)}}),ne=pr(e,{onAccept:e=>{N.setValue(e)}}),re=Wa(M.parentNode,{onConfirm:async t=>{try{let n=s.map(e=>({id:e.id,nombre:e.nombre_completo||e.nombre||``,nombreCorto:(e.nombre_completo||e.nombre||``).split(` `)[0]})),{saved:r,errors:i}=await Ia({sesionId:C,claseId:m,maestroId:f.id,fechaHoy:p,progressRecords:t,alumnos:n});i.length&&console.warn(`[Progress] Errores parciales:`,i);let a=e.querySelector(`#btn-guardar`);a&&(a.style.removeProperty(`display`),a.click())}catch(t){e.querySelector(`#btn-guardar`)?.style.removeProperty(`display`),F.error(`Error al guardar progreso: `+t.message)}},onCancel:()=>{e.querySelector(`#btn-guardar`)?.style.removeProperty(`display`)}}),ie=Xa(e.querySelector(`#pm-planificacion-dropdown`)||e,{onAdopt:async({instrumento:e,nivel:t,resumen:r,pilares:i})=>{try{let{curriculo:a,allObjetivos:o}=await n({instrumento:e,nivel:t,descripcion:r,pilares:i}),{linked:s}=await Ra({claseId:m,objetivos:o}),c=s>0?`Plan creado · ${s} registro${s===1?``:`s`} vinculado${s===1?``:`s`}`:`Plan curricular creado correctamente.`;F.success(c)}catch(e){F.error(`Error al crear el plan: `+e.message)}},onCancel:()=>{}}),ae=hr(ee,{onInsert:(e,t,n)=>N.insertText(e,t,n),getEditorContent:()=>N.getValue(),onLoading:e=>{},onIaProposal:async e=>{},onImproveClick:async e=>{let t=ee.querySelector(`#btn-generar-informe`);t&&(t.disabled=!0);try{let t=await De(e);te.open({original:e,improved:t})}catch(e){F.error(`Error al generar informe: `+e.message)}finally{t&&(t.disabled=!1)}},onStructureClick:async e=>{let t=ee.querySelector(`#btn-ia-magic`);t&&(t.disabled=!0);try{let t=A?.getActiveIndicador(),n=await Ae(e,{presentes:s.filter(e=>l[e.id]===`P`).map(e=>e.nombre_completo),indicadorActivo:t?.nombre||null});ne.open({original:e,dsl:n})}catch(e){F.error(`Error al estructurar con IA: `+e.message)}finally{t&&(t.disabled=!1)}},onAnalyzeClick:async t=>{await Za(async()=>{let n=s.filter(e=>l[e.id]&&l[e.id]!==`A`),r=(e,t)=>{let n=e.trim().split(/\s+/),r=n[0];return t.filter(e=>e.trim().split(/\s+/)[0]===r).length>1?n.slice(0,2).join(` `):r},a=s.map(e=>e.nombre_completo||e.nombre||``),o={alumnos:s.map(e=>{let t=e.nombre_completo||e.nombre||``;return{id:e.id,nombre:t,nombreCorto:r(t,a)}}),presentes:n.map(e=>{let t=e.nombre_completo||e.nombre||``;return{id:e.id,nombre:t,nombreCorto:r(t,a)}}),tipoClase:Re(i),instrumento:i.instrumento||``,sesionesRecientes:(h||[]).slice(-2).map(e=>e.contenido||``).filter(Boolean),indicadorActivo:A?.getActiveIndicador()?.nombre||``};e.querySelector(`#btn-guardar`)?.style.setProperty(`display`,`none`);let c=await je(t,o);if(!c?.progreso?.length){e.querySelector(`#btn-guardar`)?.style.removeProperty(`display`),F!==void 0&&F&&F.warning(`La IA no detectó registros de progreso en este texto.`);return}re.open({progreso:c.progreso,resumen:c.resumen})},{onError:t=>{e.querySelector(`#btn-guardar`)?.style.removeProperty(`display`),F!==void 0&&F&&F.error(`Error al analizar con IA: `+t.message)}})}}),oe=null,se=e.querySelector(`#pm-planificacion-card`),ce=e.querySelector(`#pm-planificacion-dropdown`),le=e.querySelector(`#pm-planificacion-nombre`);e.querySelector(`#pm-planificacion-detail`);let ue=e.querySelector(`#pm-plan-list-rutas`),de=e.querySelector(`#pm-plan-list-planificaciones`),fe=e.querySelector(`#pm-planificacion-header`);fe&&fe.addEventListener(`click`,()=>{let e=se.classList.toggle(`open`);ce.style.display=e?`block`:`none`}),e.querySelectorAll(`.pm-plan-tab-pill`).forEach(t=>{t.addEventListener(`click`,()=>{e.querySelectorAll(`.pm-plan-tab-pill`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`);let n=t.dataset.tab;ue.style.display=n===`rutas`?``:`none`,de.style.display=n===`planificaciones`?``:`none`})});function pe(t){oe=t.id,localStorage.setItem(`pm_default_plan_${m}`,t.id),le&&(le.textContent=t.nombre||t.name||`Sin nombre`),de.querySelectorAll(`.pm-plan-item`).forEach(e=>{e.classList.toggle(`active`,e.dataset.planId===t.id)}),A&&=(A.destroy(),null);let n=e.querySelector(`#pm-route-tree-container`),r=e.querySelector(`#pm-active-tema-badge`);oe&&n&&(n.innerHTML=``,He(m).then(e=>{A=zi(n,{claseId:m,rutaId:oe,completedTopics:e,onIndicadorSelect:e=>{N.insertText(`[${e.nombre}] `),ae.setContext({indicadorActivo:e.nombre}),r&&(r.textContent=e.nombre,r.style.display=`inline-block`),se.classList.remove(`open`),ce.style.display=`none`}}),T.push(()=>A.destroy())}))}let me=e.querySelector(`#btn-manage-planning`);me&&(me.onclick=e=>{if(e.stopPropagation(),!oe){Te.open({title:`Atención`,body:`<p>Seleccioná una planificación primero para poder gestionarla.</p>`,confirmText:`Entendido`,hideCancel:!0});return}Te.open({title:`Gestionar Estructura: ${le.textContent}`,size:`xl`,body:`<div id="modal-route-config-root"></div>`,saveText:`Cerrar y Actualizar`,onSave:async()=>(A&&A.refresh(),!0)});let t=document.getElementById(`modal-route-config-root`);t&&Di(t,oe)}),se&&(async()=>{try{let e=await J.getClasses(f?f.id:null),t=localStorage.getItem(`pm_default_plan_${m}`),n=(i.instrumento||``).toLowerCase().split(`,`).map(e=>e.trim()),r=e.filter(e=>{if(e.id===t)return!0;let r=(e.nombre||``).toLowerCase();return n.some(e=>r.includes(e))});ue&&(ue.innerHTML=r.length?r.map(e=>`
      ...
      `,e.innerHTML=`
        ...
        ">✕</button>
      `,r.parentElement.insertBefore(e,r)}}let ye=Ca(document.body,{onDelete:async({alumnoId:e,justificacionId:t,existingUrl:n})=>{if(n){let e=n.match(/\/storage\/v1\/object\/public\/[^/]+\/(.+)/);e&&S.storage.from(`documentos`).remove([e[1]]).catch(()=>{})}t&&Oa(t).catch(console.warn),l[e]=null,delete u[e],z(e),Oe();try{await Ne(!0)}catch(e){console.warn(`[asistencia] autoSave error on delete justif:`,e)}ln(`Justificación eliminada.`)},onSave:async({alumnoId:e,motivo:t,evidenciaFile:n,justificacionId:r,existingUrl:i,isEdit:a})=>{let o=document.getElementById(`pm-justif-save`);o&&(o.disabled=!0);try{let o=null;if(a&&r){let e=i;if(n){if(i){let e=i.match(/\/storage\/v1\/object\/public\/[^/]+\/(.+)/);e&&await S.storage.from(`documentos`).remove([e[1]]).catch(()=>{})}let{data:t}=await S.storage.from(`documentos`).upload(`justificaciones/${Date.now()}_${Math.random().toString(36).slice(2)}.${n.name.split(`.`).pop()}`,n).catch(()=>({data:null}));if(t){let{data:n}=S.storage.from(`documentos`).getPublicUrl(t.path);e=n.publicUrl}}let{data:a,error:s}=await S.from(`justificaciones`).update({motivo:t,evidencia_url:e}).eq(`id`,r).select().single();if(s)throw s;o=a}else{C||await Ne(!0,!1);let r=await Ea({sesionId:C,alumnoId:e,claseId:m,fecha:p,motivo:t,creadoPor:f.id},n);if(r.error)throw r.error;o=r.data}o&&(u[e]=o),ye.close()}catch(e){console.error(`[justificacion] Error guardando:`,e),alert(`Error al guardar la justificación: `+e.message)}finally{o&&(o.disabled=!1)}},onCancel:(e,t)=>{l[e]=t,z(e),Oe()}});T.push(()=>{try{ye.close()}catch{}});let be=null,xe=e.querySelector(`#pm-draft-indicator`);if(C){be=ha({saveFn:async e=>{C&&await ga(C,f.id,e)},debounceMs:3e4}),be.onSaved(()=>{let e=new Date;xe.textContent=`Borrador guardado ${String(e.getHours()).padStart(2,`0`)}:${String(e.getMinutes()).padStart(2,`0`)}`,xe.style.display=``}),N.getValue;let e=M.querySelector(`#pm-dsl-editable`);if(e){let t=e.oninput;e.oninput=function(e){t&&t.call(this,e),be&&be.onInput(N.getValue())}}T.push(()=>be.destroy()),b?.borrador===!0&&_a(C,f.id).then(e=>{if(e&&e.contenido_raw&&e.contenido_raw.trim()){let t=e.updated_at?new Date(e.updated_at).toLocaleString(`es-AR`):``;confirm(`Hay un borrador guardado${t?` (${t})`:``}.\n\n¿Deseas recuperarlo?`)?(N.setValue(e.contenido_raw),D=e.contenido_raw):va(e.id).catch(e=>console.warn(`[autoDraft] Error discarding:`,e))}}).catch(e=>console.warn(`[autoDraft] Error loading draft:`,e))}e.querySelector(`#pm-academic-tools`);let L=e.querySelector(`#btn-guardar-obs`);L&&(y&&(L.style.display=``),L.onclick=async()=>{let t=N.getValue();if(!t||!t.trim()){F.warning(`El editor está vacío. Escribe observaciones antes de guardar.`);return}if(!C){F.warning(`Primero guarda la sesión (asistencia) para poder registrar observaciones.`);return}let n=null,r=await Ue(t),a=A?.getActiveIndicador();if(n=r||a,!n){F.warning(`Seleccione un indicador en la ruta antes de guardar la observación o escríbalo entre corchetes [Ejemplo].`);return}let o=e.querySelector(`#pm-active-tema-badge`);o&&n.nombre&&(o.textContent=n.nombre,o.style.display=`inline-block`),L.disabled=!0,L.textContent=`Procesando...`;try{let r=s.filter(e=>l[e.id]===`P`),a=await Ki(t,n.id,r,n.nombre);if(a.error)throw Error(a.error);if(a.modo===`natural`&&a.dslGenerado&&!confirm(`📝 Texto convertido a formato estructurado:

`+a.dslGenerado+`

¿Guardar la evaluación?`)){L.disabled=!1,L.textContent=`Guardar observación`;return}if(a.missing.length>0&&!confirm(`Faltan ${a.missing.length} alumno(s) sin evaluar:\n${a.missing.join(`, `)}\n\n¿Guardar de todas formas?`)){L.disabled=!1,L.textContent=`Guardar observación`;return}if(a.evaluaciones.length>0){let{error:e}=await Wi(C,n.id,a.evaluaciones,f.id,r);if(e)throw e}let o={indicador_id:n.id,evaluaciones:a.evaluaciones};await ya(C,f.id,t,o,a.dslGenerado||null,a.textoMejorado||null);let c=sr(t);if(c.estados&&c.estados.length>0){let e=s.map(e=>({id:e.id,nombre:e.nombre_completo||e.nombre||``,nombreCorto:(e.nombre_completo||e.nombre||``).split(` `)[0]}));La({sesionId:C,claseId:m,maestroId:f.id,fechaHoy:p,dslText:t,alumnos:e}).then(({saved:e,errors:t})=>{t.length&&console.warn(`[Progress DSL] Errores:`,t),e.length&&ze(e,M)}).catch(e=>console.warn(`[Progress DSL] Error:`,e.message))}let u=await ka(C,m,f.id,a.evaluaciones,i.nombre||`Clase`,r);u.success||console.warn(`[Fase C] Fallo parcial en promoción:`,u.error),A&&await A.refresh(),N.setValue(``),D=``;let d=document.createElement(`div`);if(d.innerHTML=`
...
[Texto truncado — el reporte completo excede el límite de caracteres]`;F.warn(`El texto se truncó (${t.length} caracteres, máximo ${n}). Usá la opción PDF para ver el reporte completo.`),window.open(e+encodeURIComponent(r),`_blank`)}else window.open(e+encodeURIComponent(t),`_blank`)}e.querySelector(`#pm-asist-back`).onclick=()=>{We.destroy();try{ye.close()}catch{}T.forEach(e=>{try{e()}catch{}}),w(`hoy`)},e.querySelector(`#btn-bulk-p`).onclick=async()=>{s.forEach(e=>{l[e.id]=`P`}),z(),Oe();try{await Ne(!0)}catch(e){console.warn(`[asistencia] autoSave error on bulk P:`,e)}ln(`Todos los ${s.length} alumnos marcados como presentes.`)},e.querySelector(`#btn-bulk-a`).onclick=async()=>{s.forEach(e=>{l[e.id]=`A`}),z(),Oe();try{await Ne(!0)}catch(e){console.warn(`[asistencia] autoSave error on bulk A:`,e)}ln(`Todos los ${s.length} alumnos marcados como ausentes.`)},z();async function He(e){try{let{data:t}=await S.from(`sesiones_clase`).select(`contenido`).eq(`clase_id`,e).not(`contenido`,`is`,null);if(!t)return[];let n=new Set,r=/\[(.*?)\]/g;return t.forEach(e=>{if(!e.contenido)return;let t;for(;(t=r.exec(e.contenido))!==null;)t[1]&&n.add(t[1].trim())}),Array.from(n)}catch(e){return console.warn(`[AsistenciaView] Error calculando progreso histórico:`,e),[]}}async function Ue(e){if(!e||!oe)return null;let t=e.match(/\[(.*?)\]/);if(!t||!t[1])return null;let n=t[1].trim().toLowerCase(),r=e=>{let t=[`se`,`hizo`,`la`,`el`,`los`,`las`,`un`,`una`,`de`,`del`,`en`,`con`,`por`,`para`,`y`,`o`,`tema`,`indicador`];return e.toLowerCase().replace(/[^\w\sáéíóúñ]/g,``).split(/\s+/).filter(e=>e.length>2&&!t.includes(e))},i=r(n);if(i.length===0)return null;try{let e=await J.getRouteHierarchy(oe),t=null,a=0;for(let o of e)for(let e of o.plan_temas||[])for(let o of e.plan_objetivos||[]){let e=r(o.nombre),s=0;for(let t of i)e.some(e=>e.includes(t)||t.includes(e))&&s++;o.nombre.toLowerCase().includes(n)&&(s+=5);let c=s/(e.length||1);s>0&&c>a&&(a=c,t={id:o.id,nombre:o.nombre})}if(t)return console.log(`[asistencia] Indicador auto-resuelto con fuzzy match: '${t.nombre}' (Score: ${a.toFixed(2)})`),t}catch(e){console.warn(`[asistencia] Error en auto-resolución de indicador:`,e)}return null}let We=new Ei(e);We.mount();let Ge=e.querySelector(`#pm-btn-help`);return Ge&&(Ge.onclick=()=>We.start()),()=>{console.log(`[AsistenciaView] Cleanup ejecutado por el Router`),We.destroy();try{ye.close()}catch{}T.forEach(e=>{try{e()}catch{}})}}function to(e,t){return[...e].sort((e,n)=>{let r=t[e.id]!==null,i=t[n.id]!==null;return!r&&i?-1:r&&!i?1:0})}function no(e,{alumnos:t=[],onSelect:n}){let r=document.getElementById(`pm-alumno-picker-modal`);r||(r=document.createElement(`div`),r.id=`pm-alumno-picker-modal`,r.className=`pm-modal-overlay`,r.innerHTML=`
  ...
  `,e.querySelector(`#eme-alumnos-chips`);let n=[];try{let{data:e}=await S.from(`alumnos_clases`).select(`alumno:alumnos(id, nombre_completo)`).eq(`activo`,!0);n=(e||[]).map(e=>e.alumno).filter(Boolean)}catch(e){console.error(e)}let r=no(e,{alumnos:n,onSelect:e=>{alert(`Alumnos seleccionados correctamente`)}});e.querySelector(`#btn-eme-pick-alumnos`).onclick=()=>r.open(),e.querySelector(`#btn-eme-guardar`).onclick=async()=>{let n={maestro_id:t,fecha:e.querySelector(`#eme-fecha`).value,motivo:e.querySelector(`#eme-motivo`).value,nombre_clase:e.querySelector(`#eme-nombre`).value,contenido:e.querySelector(`#eme-contenido`).value,created_at:new Date().toISOString()};if(!n.nombre_clase){alert(`Por favor ingresa un nombre para la clase.`);return}await P({tabla:`clases_emergentes`,operacion:`insert`,payload:n}),alert(`Clase emergente registrada. Redirigiendo...`),window.location.hash=`#/hoy`}}var io={async open(){let e=await ce(),t=await oe();Te.open({title:`Notificaciones`,size:`md`,body:this._renderBody(e,t),saveText:`Hecho`,onShow:e=>this._initLogic(e),onSave:async e=>(await this._handleSave(e),!0)})},_renderBody(e,t){return`
    ...
    </section>`)}function mo(e,t){let n=ne(),r=n?`<span class="pm-badge-sub" id="pm-notif-sub-badge" aria-live="polite" aria-atomic="true">${Z.pushEnabled?`✅ Suscripción activa`:`⏸ Pausada`}</span>`:``;e.insertAdjacentHTML(`beforeend`,`
    ...
    </section>`)}function go(e,t){e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-settings-section" aria-labelledby="solicitudes-title" id="pm-solicitudes-section">
      ...
      </div>`).join(``)}catch(e){console.error(`[historial sol]`,e),t&&(t.innerHTML=`<p class="text-danger small mb-0">Error al cargar el historial.</p>`)}}function yo(e){e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-settings-section" aria-labelledby="install-title" id="pm-install-section">
      ...
      <div id="pm-install-body">
        ...
        • <b>Firefox:</b> no admite PWA nativa.`)}),window.addEventListener(`beforeinstallprompt`,()=>{t&&(t.disabled=!1)},{once:!0})}function bo(e){e.insertAdjacentHTML(`beforeend`,`
    ...
    </section>`)}function xo(e,t,n,r){let i=t?.solicitud_actual,a=i?.solicita_alumnos||!1,o=i?.solicita_clases||!1,s=i?.estado||null;e.innerHTML=`
  ...
  `,e.querySelectorAll(`.pm-collab-request-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{let i=t.dataset.key;t.disabled=!0;let a=t.innerHTML;t.innerHTML=`<span class="pm-settings-spinner"></span> Enviando...`;try{await r(n,i),window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Solicitud enviada correctamente. Esperando aprobación admin.`,type:`success`}}));let{getPermisos:t}=await R(async()=>{let{getPermisos:e}=await import(`./permisoService-3I2LDTJa.js`).then(e=>e.n);return{getPermisos:e}},__vite__mapDeps([8,1,9,3]));xo(e,await t(n),n,r)}catch(e){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Error al enviar solicitud: `+e.message,type:`danger`}})),t.disabled=!1,t.innerHTML=a}})}),e.querySelectorAll(`.pm-collab-action-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.route;t===`registrar-alumno`?window.router&&window.router.navigate(`registrar-alumno`):t===`gestionar-clases`&&window.router&&window.router.navigate(`gestionar-clases`)})})}function So(e,t){let n=t.disponibilidad||{},r=!t.especialidad||!t.disponibilidad||Object.keys(t.disponibilidad).length===0;e.insertAdjacentHTML(`beforeend`,`
    <section class="card-apple pm-settings-section ${r?`pm-section-warning`:``}" aria-labelledby="disp-title">
      ...
      <div id="pm-avail-days" class="pm-avail-days" role="list">
        ${co.map(e=>Co(e.key,n[e.key]||[],e.label)).join(``)}
      </div>
...
`);throw Error(e)}let{error:o}=await S.from(`maestros`).update({nombre_completo:t,tlf:n,especialidad:r}).eq(`id`,e.id);if(o)throw o;let c={...e,nombre_completo:t,nombre:t,telefono:n,tlf:n,especialidad:r,disponibilidad:i};localStorage.setItem(T,JSON.stringify(c)),Z.dirty=!1,window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Perfil actualizado`,type:`success`}})),window.pwaInstaller&&window.pwaInstaller.evaluateInsights()}catch(e){window.dispatchEvent(new CustomEvent(`showToast`,{detail:{message:`Error al guardar: `+e.message,type:`danger`}}))}finally{Z.saving=!1,a.disabled=!1,a.innerHTML=o}}function Do(){let e={};return co.forEach(({key:t})=>{let n=[];document.querySelectorAll(`[data-dia="${t}"].pm-avail-franja`).forEach(e=>{let t=e.querySelector(`[data-field="inicio"]`)?.value,r=e.querySelector(`[data-field="fin"]`)?.value;t&&r&&n.push({inicio:t,fin:r})}),e[t]=n}),e}function Oo(){Te.open({title:`¿Cerrar Sesión?`,size:`sm`,body:`
      ...
      </div>`,saveText:`Salir`,cancelText:`Cancelar`,onSave:async()=>(await E(),window.location.reload(),!0)})}function ko(e){let t=e===`system`?window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`:e;document.documentElement.setAttribute(`data-bs-theme`,t),document.documentElement.setAttribute(`data-portal-theme`,t),document.documentElement.classList.toggle(`pm-dark`,t===`dark`),document.querySelectorAll(`.pm-theme-opt`).forEach(t=>{t.setAttribute(`aria-checked`,t.dataset.theme===e?`true`:`false`),t.classList.toggle(`active`,t.dataset.theme===e)}),localStorage.setItem(`portal-maestros-theme`,e),Z.theme=e}function Ao(e){let t=!e.especialidad||!e.disponibilidad||Object.keys(e.disponibilidad||{}).length===0,n=document.getElementById(`pm-banner-perfil-incompleto`);n&&(t?(n.style.display=`block`,n.innerHTML=`
      ...
      </div>`):n.style.display=`none`)}function jo(){document.querySelectorAll(`.card-apple`).forEach((e,t)=>{e.style.opacity=`0`,e.style.transform=`translateY(12px)`,setTimeout(()=>{e.style.transition=`opacity 0.4s ease, transform 0.4s ease`,e.style.opacity=`1`,e.style.transform=`translateY(0)`},50*t)})}var Mo=`
  .pm-profile-alert {
    grid-column: 1 / -1;
    padding: 0 0 0.5rem;
  }
...
`;if(!document.getElementById(`pm-avail-styles`)){let e=document.createElement(`style`);e.id=`pm-avail-styles`,e.textContent=Mo,document.head.appendChild(e)}async function No(e,{indicator:t,claseId:n,maestroId:r,routeVersionId:i,onSave:a}){let o=new Set,s=document.createElement(`div`);s.style.cssText=`
    position: fixed;
    top: 0;
    left: 0;
    right: 0;
  ...
  `;let a=`pm-planning-active-tab`,o=e.querySelectorAll(`.pm-planning-tab`),s=e.querySelectorAll(`.pm-planning-pane`);function l(e){o.forEach(t=>{let n=t.dataset.tab===e;t.classList.toggle(`active`,n),t.setAttribute(`aria-selected`,n?`true`:`false`)}),s.forEach(t=>{t.hidden=t.dataset.pane!==e});try{sessionStorage.setItem(a,e)}catch{}C(e)}o.forEach(e=>{e.addEventListener(`click`,()=>l(e.dataset.tab))});let d=e.querySelector(`#pm-planning-clase-select`),f=e.querySelector(`#pm-planning-ruta-select`),p=e.querySelector(`#pm-planning-content`);try{d.innerHTML=`<option value="">Selecciona una clase...</option>`+(await c()).map(e=>`<option value="${e.id}">${e.nombre} (${e.instrumento||`S/I`})</option>`).join(``)}catch(e){console.error(`[planning] Error cargando clases:`,e),F.error(`Error cargando clases`)}d.addEventListener(`change`,async()=>{if(r=d.value,f.value=``,n=null,p.innerHTML=`<div class="pm-planning-empty"><p>Selecciona una ruta...</p></div>`,y=null,_=null,v=null,e.querySelector(`.pm-planning-tab.active`)?.dataset.tab===`historial`&&D(),r)try{f.innerHTML=`<option value="">Selecciona una ruta...</option>`+(await u(r)).map(e=>`<option value="${e.route_version_id}">${e.name}${e.instrumento?` (${e.instrumento})`:``}</option>`).join(``)}catch(e){console.error(`[planning] Error cargando rutas:`,e),F.error(`Error cargando rutas`)}}),f.addEventListener(`change`,()=>{n=f.value||null,C(e.querySelector(`.pm-planning-tab.active`)?.dataset.tab||`semaforo`)});let m=e.querySelector(`#pm-planning-route-tree`),h=e.querySelector(`#pm-planning-manager`),g=e.querySelector(`#pm-planning-historial`),_=null,v=null,y=null,b=!1,x=!1,S=!1;function C(e){if(e===`semaforo`)return w();if(e===`ruta`)return T();if(e===`gestionar`)return E();if(e===`historial`)return D()}async function w(){if(!n||!r){p.innerHTML=`<div class="pm-planning-empty"><p>Selecciona una clase y ruta para comenzar</p></div>`;return}p.innerHTML=`<div class="pm-planning-empty"><p>Cargando indicadores...</p></div>`;try{i=await ze(n,t,r),O()}catch(e){console.error(`[planning] Error cargando indicadores:`,e),F.error(`Error cargando indicadores`),p.innerHTML=`<div class="pm-planning-empty"><p>Error al cargar indicadores. Intenta de nuevo.</p></div>`}}async function T(){if(!n){m.innerHTML=`<div class="pm-planning-empty"><p>Selecciona una ruta para ver su estructura</p></div>`;return}if(!(_===n||b)){b=!0,m.innerHTML=`<div class="pm-planning-empty"><p>Cargando estructura...</p></div>`;try{let{renderPlanningRouteTree:e}=await R(async()=>{let{renderPlanningRouteTree:e}=await import(`./PlanningRouteTree-BUEdXSqy.js`);return{renderPlanningRouteTree:e}},__vite__mapDeps([20,21,3,5]));await e(m,{routeVersionId:n}),_=n}catch(e){console.error(`[planning] Error cargando estructura de ruta:`,e),m.innerHTML=`<div class="pm-planning-empty"><p>Error al cargar la estructura. Intenta de nuevo.</p></div>`}finally{b=!1}}}async function E(){if(!n){h.innerHTML=`<div class="pm-planning-empty"><p>Selecciona una ruta para gestionar su contenido</p></div>`;return}if(!(v===n||x)){x=!0,h.innerHTML=`<div class="pm-planning-empty"><p>Preparando tu borrador editable…</p></div>`;try{let{renderPlanningManager:e}=await R(async()=>{let{renderPlanningManager:e}=await import(`./PlanningManagerPanel-ITOqnDFw.js`);return{renderPlanningManager:e}},__vite__mapDeps([22,23,3,21,5,19,16]));await e(h,{publishedRouteVersionId:n,maestroId:t,onChanged:()=>{_=null}}),v=n}catch(e){console.error(`[planning] Error cargando gestor:`,e),h.innerHTML=`<div class="pm-planning-empty"><p>Error al cargar el gestor. Intenta de nuevo.</p></div>`}finally{x=!1}}}async function D(){if(!r){g.innerHTML=`<div class="pm-planning-empty"><p>Selecciona una clase para ver el historial</p></div>`;return}let e=`${r}:${n}`;if(!(y===e||S)){S=!0,g.innerHTML=`<div class="pm-planning-empty"><p>Cargando historial...</p></div>`;try{let{renderPlanningHistorialPane:i}=await R(async()=>{let{renderPlanningHistorialPane:e}=await import(`./PlanningHistorialPane-C1QiQrJ6.js`);return{renderPlanningHistorialPane:e}},__vite__mapDeps([24,3,23,21,5,19,16]));await i(g,{maestroId:t,claseId:r,publishedRouteVersionId:n,onPromoted:()=>{n&&w()}}),y=e}catch(e){console.error(`[planning] Error cargando historial:`,e),g.innerHTML=`<div class="pm-planning-empty"><p>Error al cargar el historial. Intenta de nuevo.</p></div>`}finally{S=!1}}}try{let e=sessionStorage.getItem(a);e&&e!==`semaforo`&&l(e)}catch{}function O(){if(!i||i.length===0){p.innerHTML=`<div class="pm-planning-empty"><p>No hay indicadores en esta ruta</p></div>`;return}let e={};i.forEach(t=>{let n=t.tipo||`Sin Categoría`;e[n]||(e[n]=[]),e[n].push(t)}),p.innerHTML=`<div class="pm-planning-grid">${Object.entries(e).map(([e,t])=>`
    ...
    `}async function A(i){Po(e,{indicator:i,claseId:r,maestroId:t,routeVersionId:n})}async function j(a){No(e,{indicator:a,claseId:r,maestroId:t,routeVersionId:n,onSave:async()=>{try{i=await ze(n,t,r),O(),F.success(`✓ Indicadores actualizados`)}catch(e){console.error(`[planning] Error reloading:`,e)}}})}}var Io=`alumno_plan_entradas`;async function Lo(e){let{data:t,error:n}=await S.from(Io).select(`id, tipo, titulo, descripcion, nivel_referencia, objetivo_id, sesion_id, created_at, maestro_id`).eq(`alumno_id`,e).order(`created_at`,{ascending:!1});if(n)throw Error(n.message);return t||[]}async function Ro(e){if(!e.titulo?.trim())throw Error(`titulo requerido`);let{data:t,error:n}=await S.from(Io).insert({alumno_id:e.alumno_id,maestro_id:e.maestro_id,tipo:e.tipo,titulo:e.titulo.trim(),descripcion:e.descripcion?.trim()||null,nivel_referencia:e.nivel_referencia||null,objetivo_id:e.objetivo_id||null,sesion_id:e.sesion_id||null}).select().single();if(n)throw Error(n.message);return t}async function zo(e){let{error:t}=await S.from(Io).delete().eq(`id`,e);if(t)throw Error(t.message)}var Bo={diagnostico:{label:`Diagnóstico`,icon:`🔍`,color:`#6366f1`,bg:`#6366f115`},logro:{label:`Logro`,icon:`✅`,color:`#16a34a`,bg:`#16a34a15`},en_progreso:{label:`En progreso`,icon:`📈`,color:`#2563eb`,bg:`#2563eb15`},dificultad:{label:`Dificultad`,icon:`⚠️`,color:`#dc2626`,bg:`#dc262615`},objetivo:{label:`Objetivo`,icon:`🎯`,color:`#d97706`,bg:`#d9770615`}},Vo=[`diagnostico`,`logro`,`en_progreso`,`dificultad`,`objetivo`],Ho=class{constructor({container:e,alumnoId:t,maestroId:n}){this._container=e,this._alumnoId=t,this._maestroId=n,this._entries=[],this._formOpen=!1}async init(){this._container.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`,await this._load(),this._render()}async _load(){this._entries=await Lo(this._alumnoId)}_render(){this._container.innerHTML=this._buildHTML(),this._attachEvents()}_buildHTML(){let e=this._entries.length>0,t=this._entries.some(e=>e.tipo===`diagnostico`);return`
      <div class="pe-panel">
    ...
    `}_buildEntry(e){let t=Bo[e.tipo]||Bo.logro,n=new Date(e.created_at).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`numeric`});return`
      ...
      <div class="pe-form-overlay">
        <div class="pe-form">
          ...
          <select class="pe-select" id="pe-tipo">
            ${Vo.map(e=>`
              <option value="${e}" ${e===t?`selected`:``}>${Bo[e].icon} ${Bo[e].label}</option>
            `).join(``)}
          </select>
...
`).trim():``}var Xo={LOGRADO:{label:`Logrado`,color:`#198754`,bg:`#19875418`,icon:`✅`},EN_PROGRESO:{label:`En progreso`,color:`#0d6efd`,bg:`#0d6efd18`,icon:`📈`},INICIADO:{label:`Iniciado`,color:`#6c757d`,bg:`#6c757d18`,icon:`🔰`},DIFICULTAD:{label:`Dificultad`,color:`#dc3545`,bg:`#dc354518`,icon:`⚠️`}};async function Zo(e,t,n=0){let r=e.querySelector(`#pm-alumno-progresos-root`);if(!r)return;let i=n===0;i&&(r.innerHTML=`<div class="pm-loading-zen"><div class="pm-pulse"></div></div>`);try{let{data:a,error:o}=await S.from(`progresos`).select(`id, contenido_dsl, estado_cualitativo, calificacion, observaciones, fecha_evaluacion, clase_id, indicadores`).eq(`alumno_id`,t).not(`contenido_dsl`,`is`,null).neq(`contenido_dsl`,``).order(`fecha_evaluacion`,{ascending:!1}).range(n,n+19);if(o)throw o;if(!a||a.length===0){i&&(r.innerHTML=`<p style="font-size:0.82rem;color:var(--pm-text-muted);text-align:center;padding:1rem 0;">Sin registros de progreso generados por IA aún.</p>`);return}let s=[...new Set(a.map(e=>e.clase_id).filter(Boolean))],{data:c}=s.length?await S.from(`clases`).select(`id, nombre`).in(`id`,s):{data:[]},l=new Map((c||[]).map(e=>[e.id,e.nombre])),u=new Map;for(let e of a){let t=e.contenido_dsl;u.has(t)||u.set(t,{contenido:t,entries:[]}),u.get(t).entries.push(e)}let d=Array.from(u.values()).map(({contenido:e,entries:t})=>{let n=t[0],r=Xo[n.estado_cualitativo]??Xo.EN_PROGRESO,i=new Date(n.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`2-digit`});return`
        <details class="pm-prog-card">
          ...
          <div class="pm-prog-card__timeline">
            ${t.map(e=>{let t=Xo[e.estado_cualitativo]??Xo.EN_PROGRESO,n=new Date(e.fecha_evaluacion).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`,year:`numeric`}),r=l.get(e.clase_id)||`Clase`,i=e.indicadores?.tarea,a=e.calificacion==null?null:`${e.calificacion}/5`;return`
        ...
        <div class="pm-zen-body">
          ...
          <div class="pm-zen-section">
            ...
            <div class="pm-zen-bitacora">
              ${ee.map(e=>{let t=j.get(e.clase_id),n=Yo(e.contenido_dsl);return`
    ...
    `,e.querySelector(`#pm-alumno-back`).onclick=()=>window.history.back();let re=Uo(r.tlf_alumno||r.representante_tlf);if(re){let t=r.nombre_completo||`el alumno`,i=r.representante_nombre||t,a={alumno:t,contacto:i},o=document.createElement(`div`);o.id=`pm-wa-modal`,o.innerHTML=`
                ...
                <div class="pm-level-nodes">
                  ${n.map(e=>{let t=s?.find(t=>t.node_id===e.id),n=t?.status||`pending`,r=e.indicators||[],i=t?.indicator_attempts||[];return`
                      <div class="pm-node-card" onclick="this.classList.toggle('expanded')">
                        <div class="pm-node-icon">${ns(e.name)}</div>
                        <div class="pm-node-info">
                          <span class="pm-node-name">${I(e.name)}</span>
                          <span class="pm-node-status">${ts[n]}</span>
                        </div>
                        ...
                        <div class="pm-indicators">
                          ${r.length===0?`<p style="font-size:0.7rem;color:var(--pm-text-muted);">Sin indicadores</p>`:r.map(e=>{let t=i.find(t=>t.indicator_id===e.id);return`
                                <div class="pm-indicator">
                                  <span class="pm-indicator-dot" style="background:${es[t?.status||`pending`]};"></span>
  ...
  `,e.querySelector(`.block-section-header`)?.addEventListener(`click`,()=>{o(n)})}var us=()=>typeof globalThis<`u`&&globalThis.getMaestroLocal?globalThis.getMaestroLocal():w(),ds=(...e)=>typeof globalThis<`u`&&globalThis.getMisClases?globalThis.getMisClases(...e):c(...e),fs=(...e)=>typeof globalThis<`u`&&globalThis.loadRouteTree?globalThis.loadRouteTree(...e):os(...e),Q={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:null},ps=!1;async function ms(e,{onTopicSelected:t}={}){if(Q={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:t},e.innerHTML=`<div class="pm-ruta-gamificada"><div class="pm-loading"><div class="pm-spinner"></div></div></div>`,!us()){e.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No hay sesión activa.</p></div>`;return}try{if(d(),Q.clases=await ds(!0),!Q.clases?.length){e.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No tienes clases asignadas.</p></div>`;return}Q.activeClaseId=Q.clases[0].id,await hs(),gs(e),bs(e),xs(e)}catch(t){console.error(`[rutaGameificadaView]`,t),e.innerHTML=`<div style="color:red;padding:20px;"><i class="bi bi-exclamation"></i> Error: ${t.message}</div>`}}async function hs(){Q.loading=!0,Q.rutaId=await as(Q.activeClaseId),Q.rutaId?Q.blocks=await fs(Q.rutaId,Q.activeClaseId):Q.blocks=[],Q.loading=!1}function gs(e){e.innerHTML=`
  ...
  `;let t=e.querySelector(`#ruta-tree-area`);if(!Q.rutaId){t.innerHTML=`<div style="padding:60px; text-align:center; color:#94a3b8;"><i class="bi bi-map fs-1 d-block mb-3"></i>No se encontró ruta publicada para esta clase.</div>`;return}if(Q.blocks.length===0){t.innerHTML=`<div style="padding:60px; text-align:center; color:#94a3b8;">La ruta no tiene bloques configurados.</div>`;return}Q.blocks.forEach(n=>{let r=document.createElement(`div`);if(t.appendChild(r),ls(r,{blockId:n.id,blockName:n.nombre,isExpanded:Q.expandedBlocks.has(n.id),childCount:n.levels?.length||0,onToggle:t=>{Q.expandedBlocks.has(t)?Q.expandedBlocks.delete(t):Q.expandedBlocks.add(t),gs(e)}}),Q.expandedBlocks.has(n.id)){let t=r.querySelector(`.block-section-content`);n.levels.forEach(n=>{t.appendChild(_s(n,e))})}}),vs(e)}function _s(e,t){let n=document.createElement(`div`);n.className=`pm-level-row`,n.style.cssText=`
  ...
  `;let r=Q.expandedLevels.has(e.id);return n.innerHTML=`
    <div class="level-header" style="padding: 12px 16px; display: flex; align-items: center; gap: 12px; cursor: ${e.locked?`not-allowed`:`pointer`};">
      <div class="level-icon" style="width: 32px; height: 32px; border-radius: 8px; background: ${ys(e.semaphore)}; display: flex; align-items: center; justify-content: center; color: white;">
      ...
      <div style="flex: 1;">
        ...
        <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
          <div style="flex: 1; height: 6px; background: #e2e8f0; border-radius: 3px; overflow: hidden;">
            <div style="width: ${e.percentage}%; height: 100%; background: ${ys(e.semaphore)}; transition: width 0.3s ease;"></div>
          </div>
      ...
      ${(e.nodes||[]).map(t=>`
        <div class="node-item" style="margin-bottom: 12px;">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
            <span style="width: 8px; height: 8px; border-radius: 50%; background: ${ys(t.semaphore)};"></span>
  ...
  `,e.locked||n.querySelector(`.level-header`).addEventListener(`click`,()=>{Q.expandedLevels.has(e.id)?Q.expandedLevels.delete(e.id):Q.expandedLevels.add(e.id),gs(t)}),n.querySelectorAll(`.indicator-row`).forEach(e=>{e.addEventListener(`click`,n=>{n.stopPropagation(),Q.selectedIndicator={id:e.dataset.id,nombre:e.dataset.nombre,nodeNombre:e.dataset.node,levelNombre:e.dataset.level,blockNombre:e.dataset.block},gs(t)})}),n}function vs(e){let t=e.querySelector(`#ruta-action-panel`);if(!Q.selectedIndicator){t.innerHTML=``;return}t.innerHTML=`
  ...
  `,t.querySelector(`#btn-cancel-select`).addEventListener(`click`,()=>{Q.selectedIndicator=null,gs(e)}),t.querySelector(`#btn-use-topic`).addEventListener(`click`,()=>{xa({...Q.selectedIndicator,indicatorId:Q.selectedIndicator.id,claseId:Q.activeClaseId}),Q.onTopicSelected&&Q.onTopicSelected(Q.activeClaseId)})}function ys(e){switch(e){case`green`:return`#22c55e`;case`yellow`:return`#eab308`;case`gray`:return`#94a3b8`;default:return`#94a3b8`}}function bs(e){e.querySelector(`#ruta-clase-select`)?.addEventListener(`change`,async t=>{Q.activeClaseId=t.target.value,e.innerHTML=`<div class="pm-ruta-gamificada"><div class="pm-loading"><div class="pm-spinner"></div></div></div>`,await hs(),gs(e),bs(e)})}function xs(e){ps||(ps=!0,cs.on(`node-covered`,()=>{hs().then(()=>{gs(e),bs(e)})}))}var Ss=[`lunes`,`martes`,`miércoles`,`jueves`,`viernes`,`sábado`,`domingo`];async function Cs(e){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let t=w();if(!t){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}try{let{data:n}=await S.from(`system_config`).select(`value`).eq(`key`,`maestros_pueden_crear_clases`).maybeSingle();if(n?.value!==`true`){e.innerHTML=`
    ...
    `}}async function ws(e,{alumnoId:t}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;try{let n=await z.getStudent(t),r=await z.fetchRoutes();e.innerHTML=`
      <div class="pm-asist-header">
    ...
    `,r.querySelector(`#back-to-library`).onclick=()=>{window.location.hash=`#/ruta-libreria`},t?this.loadRouteDetail(r,t,n):r.innerHTML=`<div class="pm-placeholder"><i class="bi bi-x-circle"></i><p>ID de ruta no proporcionado.</p></div>`,r},async loadRouteDetail(e,t,n){let r=e.querySelector(`#route-header-content`),i=e.querySelector(`#route-hierarchy`);try{let[e,a]=await Promise.all([z.getRouteDetail(t),n?z.getStudentProgress(n,t):Promise.resolve([])]),o=new Map(a.map(e=>[e.node_id,e]));if(r.innerHTML=`
      ...
      `).join(``)}catch(e){console.error(`Error al cargar detalle de ruta:`,e),i.innerHTML=`<p class="pm-error-msg">Error al cargar la estructura académica.</p>`}}};function Os(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function ks(e){return String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]?.toUpperCase()??``).join(``)}function As(e){if(!e||e.length===0)return`<span style="color:var(--pm-text-muted);font-size:.8rem;">Sin horario asignado</span>`;let t={lunes:`Lun`,martes:`Mar`,miercoles:`Mié`,miércoles:`Mié`,jueves:`Jue`,viernes:`Vie`,sabado:`Sáb`,sábado:`Sáb`,domingo:`Dom`};return e.map(e=>`<span class="gcv-horario-chip">${t[e.dia]||e.dia||``} ${(e.hora_inicio||``).slice(0,5)}–${(e.hora_fin||``).slice(0,5)}</span>`).join(` `)}var js=null,Ms=[],Ns=new Set;async function Ps(e){e.innerHTML=qs();let t=w();if(!t){e.innerHTML=Js(`bi-lock`,`Sin sesión activa`,`Por favor ingresá nuevamente.`);return}try{let n=await he(t.id);if(!n.puede_inscribir_clases){e.innerHTML=Is(n),Ls(t.id);return}let[r,i]=await Promise.all([m(t.id),Pe().catch(()=>[])]);Ms=i.filter(e=>e.activo!==!1&&e.is_active!==!1),e.innerHTML=Rs(r),Ws(r),r.length>0&&await Bs(r[0].id,r)}catch(t){console.error(`[GestionarClases]`,t),e.innerHTML=Js(`bi-exclamation-triangle`,`Error al cargar`,Os(t.message))}}function Fs(e){let t=e?.solicitudes||[],n=e?.solicitud_actual;return t.includes(`clases:enroll`)||t.includes(`inscribir_clases`)||n?.estado===`pendiente`&&n?.solicita_clases}function Is(e){return`
    <div class="gcv-root">
  ...
  `}function Ls(e){let t=document.getElementById(`gcv-btn-request-classes`);t&&t.addEventListener(`click`,async()=>{t.disabled=!0;let n=t.innerHTML;t.innerHTML=`<span class="gcv-spinner-sm"></span> Enviando...`;try{await me(e,`clases:enroll`),F.success(`Solicitud de permiso enviada correctamente.`);let t=document.getElementById(`gcv-permission-action`);t&&(t.innerHTML=`
          <div class="gcv-pending-badge">
            <i class="bi bi-clock-history"></i>
            Solicitud Pendiente de Aprobación
          </div>`)}catch(e){F.error(`Error al solicitar: `+e.message),t.disabled=!1,t.innerHTML=n}})}function Rs(e){return`
    <div class="gcv-root">
  ...
  `}function zs(e){let t=Os(e.nombre||`Clase sin nombre`),n=As(e.horarios||[]),r=Os(e.nivel||``),i=e.capacidad_maxima??e.max_alumnos??`–`;return`
  ...
  `}async function Bs(e,t){js=e,document.querySelectorAll(`.gcv-clase-card`).forEach(e=>e.classList.remove(`active`)),document.getElementById(`gcv-card-${e}`)?.classList.add(`active`);let n=document.getElementById(`gcv-panel`);if(!n)return;let r=t.find(t=>t.id===e);if(r){n.innerHTML=`<div class="gcv-loading"><div class="gcv-spinner"></div></div>`;try{let i=await v(e),a=i.map(e=>e.alumno).filter(Boolean);Ns=new Set(i.map(e=>e.alumno_id)),n.innerHTML=Vs(r,a,Ms.filter(e=>!Ns.has(e.id))),Gs(e,t)}catch(e){n.innerHTML=Js(`bi-exclamation-circle`,`Error al cargar alumnos`,Os(e.message))}}}function Vs(e,t,n){return`
    <div class="gcv-panel-inner">
      <div class="gcv-panel-header">
        <h3 class="gcv-panel-title"><i class="bi bi-people-fill"></i> ${Os(e.nombre||`Clase`)}</h3>
      ...
      <div class="gcv-section">
        ...
        <div id="gcv-lista-inscritos" class="gcv-student-list">
          ${t.length===0?`<p class="gcv-empty-list">Sin alumnos inscritos aún.</p>`:t.map(e=>Hs(e)).join(``)}
        </div>
      </div>
      ...
      <div class="gcv-section">
        ...
        <div id="gcv-lista-disponibles" class="gcv-student-list gcv-available-list">
          ${n.length===0?`<p class="gcv-empty-list">Todos los alumnos activos ya están inscritos.</p>`:n.map(e=>Us(e)).join(``)}
        </div>
        ${n.length>0?`
  ...
  `}function Hs(e){let t=Os(e.nombre_completo||e.nombre||`Alumno`),n=Os(e.instrumento_principal||e.instrumento||``);return`
    <div class="gcv-student-row inscrito-item"
         data-alumno-id="${e.id}"
         data-name="${t.toLowerCase()}"
         data-instrumento="${n.toLowerCase()}">
      <div class="gcv-student-avatar gcv-avatar-success">${ks(t)}</div>
  ...
  `}var Ys=[`login`,`logout`,`register`,`pending-approval`,`calendario`,`clases`,`hoy`,`asistencia`,`metricas`,`perfil`,`clase-emergente`,`planificacion`,`alumno`,`gamificacion`,`ruta`,`crear-clase`,`ruta-plan-builder`,`ruta-semanal`,`ruta-libreria`,`ruta-detalle`,`gestionar-clases`],Xs=new Set([`hoy`,`calendario`,`metricas`,`perfil`,`ruta`,`gamificacion`,`crear-clase`,`planificacion`,`ruta-libreria`]);function Zs(e,t,n){[`login`,`logout`,`calendario`,`clases`,`hoy`,`asistencia`,`metricas`,`perfil`,`clase-emergente`,`planificacion`,`alumno`,`gamificacion`,`ruta`,`crear-clase`,`ruta-plan-builder`,`ruta-semanal`,`ruta-libreria`,`gestionar-clases`,`register`,`pending-approval`].forEach(t=>e.on(t,(e,r)=>n(t,r))),e.on(`ruta-detalle/:id`,(e,t)=>n(`ruta-detalle`,t)),e.onNotFound(()=>n(`hoy`))}function Qs(){let e=document.getElementById(`pm-view-container`);if(!e)return{};e.innerHTML=``;let t={};return Ys.forEach(n=>{let r=document.createElement(`div`);r.id=`pm-view-${n}`,r.className=`pm-view-content`,r.style.display=`none`,e.appendChild(r),t[n]=r}),t}async function $s(e,t,n,r,i){let{maestroId:a,permisos:o,router:s,showLoginScreen:c,cleanupPushService:l,stopRealtime:u,logoutMaestro:d}=i;switch(e){case`login`:Nn(t,{onSuccess:i.onLoginSuccess});break;case`register`:Pn(t,{onSuccess:()=>s.navigate(`pending-approval`)});break;case`pending-approval`:Fn(t,{onBackToLogin:()=>s.navigate(`login`)});break;case`logout`:c(),l(),u(),d().then(()=>window.location.reload());break;case`calendario`:case`clases`:return await Zn(t);case`hoy`:return await zn(t,{onClaseClick:e=>s.navigate(`asistencia?clase=${e}`)});case`asistencia`:return await $a(t,{claseId:r.get(`clase`),fecha:r.get(`fecha`),sesionId:r.get(`sesion`),router:s});case`metricas`:return En(t);case`perfil`:return lo(t);case`clase-emergente`:return ro(t,{maestroId:a});case`planificacion`:return await Fo(t,{maestroId:a});case`alumno`:return Qo(t,{alumnoId:r.get(`id`)||n.id});case`gamificacion`:await rs(t);break;case`ruta`:await ms(t,{onTopicSelected:e=>s.navigate(`asistencia?clase=${e}`)});break;case`crear-clase`:Cs(t);break;case`ruta-plan-builder`:ws(t,{alumnoId:r.get(`id`)});break;case`ruta-semanal`:Ts(t,{alumnoId:r.get(`id`)});break;case`ruta-libreria`:Es.render().then(e=>{t.innerHTML=``,t.appendChild(e)});break;case`ruta-detalle`:Ds.render(n).then(e=>{t.innerHTML=``,t.appendChild(e)});break;case`gestionar-clases`:if(!o?.puede_inscribir_clases){s.navigate(`hoy`);return}return await Ps(t);default:break}return null}var ec=!1,tc=null;function nc({isAdmin:e,getMaestro:t,getPermisosCached:n,onPermisosUpdate:r,onNavigate:i,onResize:a}){if(ec)return;ec=!0,Ft(()=>{let e=document.getElementById(`pm-notif-badge`);if(!e)return;let t=Ht();t>0?(e.textContent=t>9?`9+`:t,e.style.display=`flex`):e.style.display=`none`}),Lt(),Wt(),e||rc({getMaestro:t,getPermisosCached:n,onPermisosUpdate:r,onNavigate:i}),document.addEventListener(`keydown`,e=>{if(On()!==`desktop`||e.target.tagName===`INPUT`||e.target.tagName===`TEXTAREA`)return;window._globalAppKeys||(window._globalAppKeys=[]);let t=window._globalAppKeys;if(t.push(e.key.toLowerCase()),t[t.length-2]===`g`){let n={h:`hoy`,c:`calendario`,r:`ruta`,m:`metricas`,p:`perfil`}[e.key.toLowerCase()];n&&(i(n),t.length=0)}t.length>3&&t.splice(0,t.length-2)});let o=null,s=On();window.addEventListener(`resize`,()=>{clearTimeout(o),o=setTimeout(()=>{let e=On();e!==s&&(s=e,a())},250)},{passive:!0})}function rc({getMaestro:e,getPermisosCached:t,onPermisosUpdate:n,onNavigate:r}){let i=e();i?.id&&(tc&&=(S.removeChannel(tc),null),tc=S.channel(`permisos-maestro:${i.id}`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`permisos_maestros`,filter:`maestro_id=eq.${i.id}`},async e=>{console.log(`[Realtime] Permisos actualizados:`,e.new);try{let e=await he(i.id),r=t(),a=[],o=[];e.puede_inscribir_clases&&!r?.puede_inscribir_clases&&a.push(`Gestionar e Inscribir Clases`),r?.puede_inscribir_clases&&!e.puede_inscribir_clases&&o.push(`Gestionar e Inscribir Clases`),await n(e,{ganados:a,perdidos:o}),a.length>0?F.success(`¡Nuevos permisos activados: ${a.join(`, `)}! Ahora podés acceder desde el Perfil o la barra de navegación.`):o.length>0?F.show(`El administrador removió tu acceso a: ${o.join(`, `)}.`,`warning`):F.show(`Tus permisos fueron actualizados por el administrador.`,`info`)}catch(e){console.warn(`[Realtime] Error actualizando permisos:`,e.message)}}).subscribe(e=>console.log(`[Realtime] Canal permisos_maestros:`,e)),window.addEventListener(`beforeunload`,()=>S.removeChannel(tc),{once:!0}))}if(h(),`serviceWorker`in navigator){let e=async()=>{try{let e=await navigator.serviceWorker.register(`/sw.js`);console.log(`[PWA] Service Worker registered:`,e.scope)}catch(e){console.log(`[PWA] Service Worker registration failed:`,e)}};document.readyState===`complete`?e():window.addEventListener(`load`,e)}else `serviceWorker`in navigator;$e(),Ye({windowMs:6e4,max:100}),qe({enabled:!1,consent:!1}),nt({debug:!1}),A({dsn:null,environment:`production`}),window.addEventListener(`showToast`,e=>{let{message:t,type:n=`info`}=e.detail||{};t&&F.show(t,n)});var ic=null,ac=null,oc=!1,$=mt();window.router=$;var sc={},cc=null,lc=new Set;function uc(e){let t=[{id:`calendario`,label:`Calendario`,icon:`bi-calendar3`},{id:`hoy`,label:`Hoy`,icon:`bi-house-door`},{id:`planificacion`,label:`Plan`,icon:`bi-signpost-split`},{id:`metricas`,label:`Métricas`,icon:`bi-bar-chart-line`}];return e?.puede_inscribir_clases&&t.push({id:`gestionar-clases`,label:`Clases`,icon:`bi-mortarboard`}),t}async function dc(e){let{tabla:t,operacion:n,payload:r}=e,i={...r};t===`sesiones_clase`&&(i.contenido_dsl!==void 0&&(i.contenido=i.contenido_dsl,delete i.contenido_dsl),i.asistencias!==void 0&&i.asistencia===void 0&&(i.asistencia=i.asistencias,delete i.asistencias)),console.log(`[SYNC] Intentando ${n} en ${t}:`,i);try{if(n===`insert`){let{error:e}=await S.from(t).insert([i]);if(e)throw e}else if(n===`update`){let{id:e,...n}=i,{error:r}=await S.from(t).update(n).eq(`id`,e);if(r)throw r}else if(n===`delete`){let{error:e}=await S.from(t).delete().eq(`id`,i.id);if(e)throw e}}catch(e){if(e.code===`PGRST204`){let{data:e}=await S.from(t).select().limit(1);e?.length>0?console.warn(`[SYNC] Columnas REALES encontradas:`,Object.keys(e[0])):console.warn(`[SYNC] No se pueden leer las columnas. ¿Ejecutaste el SQL en Supabase?`)}throw console.error(`[SYNC] Error crítico:`,e),e}}var fc=null;async function pc(){let e=document.getElementById(`pm-sync-indicator`);if(e)try{let t=await N();t.length===0?(e.className=`pm-online-dot synced`,e.title=`Sincronizado`):(e.className=`pm-online-dot pending`,e.title=`Pendiente (${t.length})`)}catch{e.className=`pm-online-dot error`,e.title=`Error de sincronización`}}async function mc(){clearTimeout(fc),fc=setTimeout(async()=>{if(navigator.onLine)try{await M(dc)}finally{await pc()}},1e3)}window.addEventListener(`online`,mc),window.addEventListener(`offline`,pc);function hc(){lc.clear()}function gc(e){lc.delete(e)}async function _c(e,t={},{silent:n=!1}={}){let r=window.location.hash.includes(`?`)?window.location.hash.split(`?`)[1]:``,i=new URLSearchParams(r),a=e.split(`?`)[0];if(!n){let e=document.getElementById(`pm-header`);if(e?.classList.contains(`search-active`)){e.classList.remove(`search-active`);let t=document.getElementById(`pm-header-search-input`);t&&(t.value=``)}An(a),window.pwaInstaller?.evaluateInsights()}let o=sc[a];if(!o){console.warn(`[Router] Contenedor no encontrado: ${a}`);return}if(n||(typeof cc==`function`&&(cc(),cc=null),Object.values(sc).forEach(e=>{e.style.display=`none`,e.classList.remove(`active`)}),o.style.display=`block`,o.offsetHeight,o.classList.add(`active`)),lc.has(a))return;let s=setTimeout(()=>{o.querySelectorAll(`.pm-loading-overlay`).forEach(e=>e.remove());let e=document.createElement(`div`);e.className=`pm-loading pm-loading-overlay`,e.innerHTML=`<div class="pm-spinner"></div>`,o.prepend(e)},300);try{let e=await $s(a,o,t,i,{maestroId:ic?.id,permisos:ac,router:$,showLoginScreen:yc,cleanupPushService:fe,stopRealtime:()=>{},logoutMaestro:ft,onLoginSuccess:()=>Sc()});e&&(cc=e),clearTimeout(s),o.querySelector(`.pm-loading-overlay`)?.remove(),Xs.has(a)&&lc.add(a)}catch(e){clearTimeout(s),o.innerHTML=`<p class="pm-error">Error cargando vista: ${e.message}</p>`}}function vc(e,t,n){ic=t,ac=n||ac,jn(e,t,uc(ac),(e,t)=>$.navigate(e,t),pc),document.getElementById(`pm-sync-indicator`)?.addEventListener(`click`,async e=>{e.target.classList.contains(`error`)&&await mc()});let r=($.currentRoute?.()||`hoy`).split(`?`)[0];An(r)}function yc(){let e=document.getElementById(`portal-app`);if(!e)return;let t=[`login`,`register`,`pending-approval`],n=($.currentRoute?.()||`login`).split(`?`)[0];if(t.includes(n)&&n!==`login`){document.getElementById(`pm-view-container`)||(e.innerHTML=`<main class="pm-view" id="pm-view-container"></main>`),Object.assign(sc,Qs()),bc(),$.setAuthGuard(()=>dt.isAuthenticated(),t),$.start();return}let r=sc.login;if(r){kn(),r.style.display=`block`,r.innerHTML=``,$s(`login`,r,{},new URLSearchParams,{router:$,onLoginSuccess:e=>{e&&e!==`login`?$.navigate(e):Sc()}});return}e.innerHTML=`<main class="pm-view" id="pm-view-container"></main>`,Object.assign(sc,Qs()),bc(),$.setAuthGuard(()=>dt.isAuthenticated(),t),history.replaceState({route:`login`},``,`#/login`),_c(`login`)}function bc(){Zs($,oc,_c)}async function xc(){if(ic)try{let t=new Date,n=t.toLocaleDateString(`es-ES`,{weekday:`long`}).toLowerCase(),i=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-${String(t.getDate()).padStart(2,`0`)}`,[a,o,s]=await Promise.all([c(),c().then(e=>r(e.map(e=>e.id))),c().then(()=>e(ic.id,i,i))]),l=Object.fromEntries(a.map(e=>[e.id,e]));await pe(o.filter(e=>e.dia?.toLowerCase()===n).map(e=>({...e,clase_nombre:l[e.clase_id]?.nombre||`Clase`})),s.filter(e=>e.borrador===!1||e.estado===`registrada`).map(e=>e.clase_id))}catch(e){console.warn(`[Alerts] Error programando alertas:`,e.message)}}async function Sc(){let e=document.getElementById(`portal-app`);if(!e)return;console.log(`[Init] Iniciando Portal...`);let t=await dt.init();if(console.log(`[Init] Auth:`,t?`con maestro`:`sin maestro`),dt.isPendingApproval()){console.log(`[Init] Cuenta pendiente de aprobación — mostrando pantalla de espera`),document.getElementById(`pm-view-container`)||(e.innerHTML=`<main class="pm-view" id="pm-view-container"></main>`),Object.assign(sc,Qs(!1)),bc(),history.replaceState({route:`pending-approval`},``,`#/pending-approval`),_c(`pending-approval`);return}let n=[`login`,`register`,`pending-approval`],r=(window.router||$).currentRoute().split(`?`)[0],i=n.includes(r);if(!t&&!i){yc();return}if(!t&&i){document.getElementById(`pm-view-container`)||(e.innerHTML=`<main class="pm-view" id="pm-view-container"></main>`),Object.assign(sc,Qs()),bc(),$.setAuthGuard(()=>dt.isAuthenticated(),n),$.start();return}if(t.es_admin&&!t.es_maestro){console.log(`[Init] Admin puro detectado → redirigiendo a /admin`),window.location.href=`/admin`;return}let a=null;try{a=await he(t.id)}catch(e){console.warn(`[Init] Error fetching permissions:`,e.message)}vc(e,t,a),Object.assign(sc,Qs()),nc({isAdmin:!1,getMaestro:()=>ic,getPermisosCached:()=>ac,onPermisosUpdate:async(t,{ganados:r,perdidos:i})=>{let a=($.currentRoute?.()||`perfil`).split(`?`)[0],o=a===`gestionar-clases`&&!t.puede_inscribir_clases||a===`pending-approval`&&r.length>0?`hoy`:a;vc(e,ic,t),Object.assign(sc,Qs()),bc(),$.setAuthGuard(()=>dt.isAuthenticated(),n),lc.clear(),await _c(o),$.navigate(o)},onNavigate:e=>$.navigate(e),onResize:()=>{vc(e,ic,ac),Object.assign(sc,Qs());let t=($.currentRoute?.()||`hoy`).split(`?`)[0];An(t)}}),gt(gc,hc),bc(),$.setAuthGuard(()=>dt.isAuthenticated(),n),$.start();let o=($.currentRoute?.()||``).split(`?`)[0];(!o||o===`login`||o===`logout`)&&$.navigate(`hoy`),y().then(async()=>{let e=[`hoy`,`calendario`,`metricas`],t=($.currentRoute?.()||`hoy`).split(`?`)[0];await e.filter(e=>e!==t&&!lc.has(e)).reduce((e,t)=>e.then(()=>{if(sc[t])return _c(t,{},{silent:!0})}),Promise.resolve()),xc(),window.pwaInstaller?.evaluateInsights()}).catch(e=>console.warn(`[Prefetch] Error:`,e.message)),mc()}var Cc=(e,t,n,r)=>`
  ...
  </div>`;window.addEventListener(`error`,e=>{if([`useCache`,`WebSocket`,`content.js`].some(t=>(e.message||``).includes(t))){console.warn(`[Ignored Error]`,e.message);return}j(Error(e.message),{context:`window.error`,filename:e.filename,lineno:e.lineno});let t=document.getElementById(`portal-app`);t&&(t.innerHTML=Cc(`bi-x-circle-fill`,`Ups! Algo salió mal`,`Se ha producido un error inesperado en la aplicación.`,`<div style="color:#ef4444;font-weight:bold;margin-bottom:8px;">${e.message}</div><div style="color:rgba(255,255,255,0.4);">${e.filename?.split(`/`).pop()}:${e.lineno}</div>`))}),window.addEventListener(`unhandledrejection`,e=>{j(e.reason instanceof Error?e.reason:Error(String(e.reason)),{context:`unhandledRejection`});let t=document.getElementById(`portal-app`);t&&(t.innerHTML=Cc(`bi-exclamation-triangle-fill`,`Error de Sincronización`,`Hubo un problema al procesar una solicitud de red.`,`<div style="color:#ef4444;font-weight:bold;margin-bottom:8px;">Promise Rejection</div><div style="color:rgba(255,255,255,0.4);">${String(e.reason)}</div>`))}),Sc().catch(e=>{let t=document.getElementById(`portal-app`);t&&(t.innerHTML=`<div style="padding:20px;color:red;font-family:monospace;background:#fff;z-index:9999;position:fixed;top:0;left:0;right:0;bottom:0;overflow:auto;"><h2>❌ initPortal() falló</h2><pre>${e?.message||e}\n${e?.stack||``}</pre></div>`)});