import{i as e}from"./supabase-KnARm58N.js";import{f as t,m as n}from"./planificacionAdapter-BkWRUKTF.js";import{a as r}from"./alumnosApi-DIQGy8Mo.js";var i=class{constructor(e=`default`){this.scope=e,this.subscriptions=[],this.intervals=[],this.listeners=[]}registerChannel(e){e&&!this.subscriptions.includes(e)&&this.subscriptions.push(e)}registerInterval(e){e!==null&&!this.intervals.includes(e)&&this.intervals.push(e)}registerListener(e,t,n){e&&t&&n&&this.listeners.push({el:e,event:t,fn:n})}destroy(){this.subscriptions.forEach(t=>{try{t&&e.removeChannel(t)}catch(e){console.warn(`[LifecycleManager] Error removing channel (${this.scope}):`,e)}}),this.intervals.forEach(e=>{try{clearInterval(e)}catch(e){console.warn(`[LifecycleManager] Error clearing interval (${this.scope}):`,e)}}),this.listeners.forEach(({el:e,event:t,fn:n})=>{try{e&&t&&n&&e.removeEventListener(t,n)}catch(e){console.warn(`[LifecycleManager] Error removing listener (${this.scope}):`,e)}}),this.subscriptions=[],this.intervals=[],this.listeners=[],console.log(`[LifecycleManager] Cleanup completed for scope: ${this.scope}`)}},a={PRESENTE:`presente`,AUSENTE:`ausente`,JUSTIFICADO:`justificado`,TARDE:`tarde`},o={P:a.PRESENTE,A:a.AUSENTE,J:a.JUSTIFICADO,T:a.TARDE,presente:a.PRESENTE,ausente:a.AUSENTE,justificado:a.JUSTIFICADO,tarde:a.TARDE};function s(e){return e?o[e]||e:a.PRESENTE}var c={presente:{short:`P`,label:`Presente`,css:`success`},ausente:{short:`A`,label:`Ausente`,css:`danger`},justificado:{short:`J`,label:`Justificado`,css:`warning`}};function l(e,t){throw console.error(e,t?.message),Error(e)}function u(e){let t=e?.message?.toLowerCase()||``;return t.includes(`unique constraint`)||t.includes(`duplicate key`)||t.includes(`uk_asistencias`)||t.includes(`unique`)&&t.includes(`duplicate`)}async function d({fechaInicio:t,fechaFin:n,periodoId:r,claseId:i,maestroId:o}={}){let s=e.from(`sesiones_clase`).select(`
      id,
      fecha,
      hora_inicio,
      hora_fin,
    ...
    `).order(`fecha`,{ascending:!1}).order(`hora_inicio`,{ascending:!0});if(t&&(s=s.gte(`fecha`,t)),n&&(s=s.lte(`fecha`,n)),i&&(s=s.eq(`clase_id`,i)),r){let{data:i}=await e.from(`periodos`).select(`fecha_inicio, fecha_fin`).eq(`id`,r).single();i&&(t||(s=s.gte(`fecha`,i.fecha_inicio)),n||(s=s.lte(`fecha`,i.fecha_fin)))}let{data:c,error:u}=await s;u&&l(`No se pudieron cargar las sesiones`,u);let d=(c||[]).map(e=>{let t=e.asistencias||[];return{sesionId:e.id,fecha:e.fecha,horaInicio:e.hora_inicio,horaFin:e.hora_fin,temaPrincipal:e.tema_principal,observacionesGenerales:e.observaciones_generales,estado:e.estado,claseId:e.clase_id,claseNombre:e.clases?.nombre??`—`,instrumento:e.clases?.instrumento??`—`,maestroId:e.clases?.maestro_principal_id??null,maestroNombre:e.clases?.maestros?.nombre_completo??`—`,totalPresentes:t.filter(e=>e.estado===a.PRESENTE).length,totalAusentes:t.filter(e=>e.estado===a.AUSENTE).length,totalJustificados:t.filter(e=>e.estado===a.JUSTIFICADO).length,totalRegistros:t.length}}),p=d;return o&&(p=d.filter(e=>e.maestroId&&e.maestroId.toString()===o.toString())),f(p)}function f(e){let t=new Map;for(let n of e)t.has(n.fecha)||t.set(n.fecha,[]),t.get(n.fecha).push(n);return Array.from(t.entries()).sort(([e],[t])=>t.localeCompare(e)).map(([e,t])=>({fecha:e,sesiones:t}))}async function p(t){t||l(`Se requiere sesionId`);let{data:n,error:r}=await e.from(`sesiones_clase`).select(`
    ...
    `).eq(`id`,t).single();r&&l(`No se pudo cargar la sesión`,r);let{data:i,error:a}=await e.from(`asistencias`).select(`
      id, estado, justificacion_texto, observaciones, alumno_id,
      alumnos ( id, nombre_completo )
    `).eq(`sesion_clase_id`,t).order(`alumnos(nombre_completo)`,{ascending:!0});a&&l(`No se pudieron cargar las asistencias`,a);let{data:o}=await e.from(`justificaciones`).select(`motivo, descripcion, archivo_url, estado, alumno_id`).eq(`sesion_id`,t),s={};o&&o.forEach(e=>{s[e.alumno_id]=e});let{data:c,error:u}=await e.from(`observaciones_alumnos`).select(`
      id, tipo, observacion, titulo, descripcion, prioridad,
      alumnos ( id, nombre_completo )
    `).eq(`sesion_clase_id`,t);u&&l(`No se pudieron cargar las observaciones`,u);let{data:d,error:f}=await e.from(`contenidos_sesion`).select(`
      id, descripcion, nivel_logro,
      planificaciones ( titulo, contenidos )
    `).eq(`sesion_clase_id`,t);return f&&l(`No se pudieron cargar los contenidos`,f),{sesion:{id:n.id,fecha:n.fecha,horaInicio:n.hora_inicio,horaFin:n.hora_fin,temaPrincipal:n.tema_principal,observacionesGenerales:n.observaciones_generales,estado:n.estado,claseNombre:n.clases?.nombre??`—`,instrumento:n.clases?.instrumento??`—`,maestroNombre:n.clases?.maestros?.nombre_completo??`—`},asistencias:(i||[]).map(e=>({id:e.id,estado:e.estado,justificacionTexto:e.justificacion_texto,observacion:e.observaciones,alumnoId:e.alumno_id,alumnoNombre:e.alumnos?.nombre_completo??`—`,justificacion:s[e.alumno_id]??null})),observaciones:(c||[]).map(e=>({id:e.id,tipo:e.tipo,titulo:e.titulo,descripcion:e.descripcion??e.observacion,prioridad:e.prioridad,alumnoId:e.alumnos?.id,alumnoNombre:e.alumnos?.nombre_completo??`—`})),contenidos:(d||[]).map(e=>({id:e.id,descripcion:e.descripcion,nivelLogro:e.nivel_logro,planTitulo:e.planificaciones?.titulo}))}}async function m(){let{data:t,error:n}=await e.from(`periodos`).select(`id, nombre, fecha_inicio, fecha_fin, activo`).order(`fecha_inicio`,{ascending:!1});return n&&l(`No se pudieron cargar los períodos`,n),t||[]}async function h(){let{data:t,error:n}=await e.from(`periodos`).select(`id, nombre, fecha_inicio, fecha_fin`).eq(`activo`,!0).single();return n?null:t}async function g(){let{data:t,error:n}=await e.from(`clases`).select(`id, nombre, instrumento`).order(`nombre`,{ascending:!0});return n&&l(`No se pudieron cargar las clases`,n),t||[]}async function _(t){t?.length||l(`No hay asistencias para registrar`);let n=[...new Set(t.map(e=>e.alumno_id))];n.some(e=>!e)&&l(`Todas las asistencias deben tener alumno_id`);let{data:r,error:i}=await e.from(`alumnos`).select(`id`).in(`id`,n);i&&l(`No se pudo validar alumnos en la base de datos`,i);let a=new Set(r?.map(e=>e.id)||[]),o=n.filter(e=>!a.has(e));o.length>0&&l(`Los siguientes alumnos no existen: ${o.join(`, `)}`);let c=t.filter(e=>e.sesion_clase_id?!0:(console.warn(`[asistenciasApi] Saltando alumno ${e.alumno_id} sin sesion_clase_id (se sincronizará vía offline queue)`),!1)).map(e=>{if(!e.clase_id)throw Error(`clase_id es requerido para alumno ${e.alumno_id}`);if(!e.fecha)throw Error(`fecha es requerido para alumno ${e.alumno_id}`);return{sesion_clase_id:e.sesion_clase_id,clase_id:e.clase_id,alumno_id:e.alumno_id,fecha:e.fecha,estado:s(e.estado),justificacion_texto:(e.justificacion_texto||``).trim()||null,observaciones:(e.observaciones||``).trim()||null,...e.registrado_por?{registrado_por:e.registrado_por}:{}}});if(c.length===0)return console.warn(`[asistenciasApi] No hay registros válidos con sesion_clase_id para insertar`),[];let{data:d,error:f}=await e.from(`asistencias`).upsert(c,{onConflict:`clase_id,alumno_id,fecha`}).select();if(f&&u(f)){console.warn(`[registrarAsistenciaBulk] Constraint detected, trying plain INSERT:`,f.message);let{data:t,error:n}=await e.from(`asistencias`).insert(c,{returning:`representation`}).select();return n&&l(`No se pudieron registrar las asistencias (UPSERT y INSERT fallidos)`,f),t||[]}return f&&l(`No se pudieron registrar las asistencias`,f),d}async function v({periodoId:t,fecha:n,claseId:r}={}){try{let i,a;if(t){let{data:n,error:r}=await e.from(`periodos`).select(`fecha_inicio, fecha_fin`).eq(`id`,t).single();r&&console.warn(`No se pudo cargar el período, mostrando todas las sesiones`,r),n&&(i=n.fecha_inicio,a=n.fecha_fin)}let o=e.from(`vw_asistencias_consolidada`).select(`
        fecha,
        sesion_clase_id,
        clase_id,
        nombre_clase,
...
`),M=null,N=``;function se(e=``,t=null){let n=document.createElement(`div`);n.className=`dsl-editor-container`;let r=document.createElement(`div`);r.className=`dsl-editor-wrapper position-relative`;let i=document.createElement(`div`);i.className=`dsl-editor form-control`,i.contentEditable=`true`,i.spellcheck=`false`,i.setAttribute(`data-placeholder`,j),i.innerHTML=e?O(e):``;let a=document.createElement(`div`);a.className=`dsl-highlight-layer position-absolute top-0 start-0 w-100 h-100 overflow-hidden pe-none`,a.setAttribute(`aria-hidden`,`true`),r.appendChild(a),r.appendChild(i),n.appendChild(r),ce();function o(){return i.innerText||i.textContent||``}function s(){let e=o();e!==N&&(N=e,a.innerHTML=O(e)+`<br>`)}function c(){a.scrollTop=i.scrollTop,a.scrollLeft=i.scrollLeft}function l(){clearTimeout(M),M=setTimeout(()=>{s();let e=D(o());t&&t(o(),e)},A)}function u(e){e.key===`Tab`&&(e.preventDefault(),document.execCommand(`insertText`,!1,`  `))}function d(){c()}i.addEventListener(`input`,l),i.addEventListener(`keydown`,u),i.addEventListener(`scroll`,d),i.addEventListener(`focus`,()=>{n.classList.add(`focused`)}),i.addEventListener(`blur`,()=>{n.classList.remove(`focused`),s()});function f(e){i.innerHTML=e?O(e):``,N=o(),a.innerHTML=O(N)+`<br>`}function p(){return o()}function m(){return D(o())}function h(){i.focus()}function g(e){i.focus(),document.execCommand(`insertText`,!1,e)}function _(e,t=``){i.focus();let n=window.getSelection();if(!n.rangeCount)return;let r=n.getRangeAt(0);r.deleteContents();let a=document.createTextNode(e),s=document.createTextNode(t);r.insertNode(a),r.collapse(!1),r.insertNode(s),r.setStartAfter(a),r.setEndAfter(a),n.removeAllRanges(),n.addRange(r),N=o()}return n.component={element:n,editor:i,setContent:f,getContent:p,getParsed:m,focus:h,insertText:g,insertAtCursor:_},n.setContent=f,n.getContent=p,n.getParsed=m,n.focus=h,n.insertText=g,n.insertAtCursor=_,e&&(N=e),n}function ce(){if(document.getElementById(`dsl-editor-styles`))return;let e=document.createElement(`style`);e.id=`dsl-editor-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}function I(e={}){let{initialContent:t=``,onChange:n=null,onAlumnoClick:r=null,onGrabarClick:i=null,onEnriquecerClick:a=null}=e,o=document.createElement(`div`);o.className=`dsl-editor-with-toolbar`;let s=se(t,n),c=P({onAlumnoClick:r,onGrabarClick:i,onEnriquecerClick:a,editor:s});return o.appendChild(c),o.appendChild(s),o.container=o,o}function L(e={}){let{onSelect:t=null,onClose:n=null}=e,i=new Set,a=``,o=document.createElement(`div`);o.className=`modal fade`,o.setAttribute(`tabindex`,`-1`),o.setAttribute(`role`,`dialog`),o.innerHTML=`
  ...
  `;let s=o.querySelector(`.search-input`),c=o.querySelector(`.alumno-list`),l=o.querySelector(`.confirm-btn`),u=o.querySelector(`.cancel-btn`);async function d(){let e=await r();f(a?e.filter(e=>e.nombre_completo.toLowerCase().includes(a.toLowerCase())):e)}function f(e){if(c.innerHTML=``,e.length===0){let e=document.createElement(`div`);e.className=`text-center text-muted py-4`,e.textContent=`No se encontraron alumnos`,c.appendChild(e);return}e.forEach(e=>{let t=document.createElement(`label`);t.className=`list-group-item d-flex align-items-center gap-2 cursor-pointer`,t.style.cursor=`pointer`;let n=document.createElement(`input`);n.type=`checkbox`,n.className=`form-check-input`,n.checked=i.has(e.id),n.value=e.id;let r=document.createElement(`div`);r.className=`flex-grow-1`,r.innerHTML=`
      ...
      </div>`),b.appendChild(e);let r=p.querySelector(`#pl-clase_id`);if(r&&(r.addEventListener(`change`,()=>{let e=r.value;if(!e)return;let t=a.find(t=>t.id===e);t?.instrumento&&t?.plan_estudio&&B(t.instrumento,t.plan_estudio,p)}),f.clase_id)){let e=a.find(e=>e.id===f.clase_id);e?.instrumento&&e?.plan_estudio&&B(e.instrumento,e.plan_estudio,p)}}requestAnimationFrame(()=>{p.classList.add(`open`),p.querySelector(`#pl-tema`)?.focus()})}async function B(e,t,n){let r=n.querySelector(`#pl-curriculo-body`);if(r){r.innerHTML=`<div class="text-center py-2"><div class="spinner-border spinner-border-sm text-muted"></div></div>`;try{let n=await y(e,t);if(!n){r.innerHTML=`<p class="text-muted small text-center py-2">Sin guía curricular<br>para ${e} — ${t}</p>`;return}r.innerHTML=n.curriculo_pilares.map(e=>`
      <div class="mb-2">
      ...
      </div>`).join(``)}catch(e){r.innerHTML=`<p class="text-danger small">${e.message}</p>`}}}function V(e,t,n,r,i=[]){let a=n.length?n.map(e=>`<option value="${e.id}" ${t.clase_id===e.id?`selected`:``}>${K(e.nombre||e.id)}</option>`).join(``):`<option value="">Sin clases disponibles</option>`,o=r.length?`<option value="">Sin asignar</option>`+r.map(e=>`<option value="${e.id}" ${t.maestro_id===e.id?`selected`:``}>${K(e.nombre||e.id)}</option>`).join(``):`<option value="">Sin maestros disponibles</option>`,s=Array.isArray(t.recursos)?t.recursos.join(`, `):``,c=i.map(e=>`<option value="${e.id}">${e.nombre}</option>`).join(``);return`
        ...
        <div class="pm-plan-section">
          ...
          <div class="pm-plan-field">
            ...
            <textarea class="pm-plan-textarea" id="pl-objetivos" rows="2" maxlength="1000"
              placeholder="¿Qué quieres lograr en esta clase?">${K(t.objetivos||``)}</textarea>
          ...
          <div class="pm-plan-field">
            ...
            <textarea class="pm-plan-textarea" id="pl-contenido" rows="3" maxlength="2000"
              placeholder="Desarrollo del tema, actividades...">${K(t.contenido||``)}</textarea>
        ...
        <div class="pm-plan-section">
          ...
          <div class="pm-plan-field">
            ...
            <textarea class="pm-plan-textarea" id="pl-evaluacion" rows="2" maxlength="500"
              placeholder="¿Cómo evaluarás el aprendizaje?">${K(t.evaluacion_metodo||``)}</textarea>
          ...
          <div class="pm-plan-field">
            ...
            <textarea class="pm-plan-textarea" id="pl-observaciones" rows="2" maxlength="1000"
              placeholder="Notas adicionales...">${K(t.observaciones||``)}</textarea>
