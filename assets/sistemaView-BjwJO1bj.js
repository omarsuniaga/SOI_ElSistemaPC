...
`;async function y(){let e=p(),n=f(),a=t(),o={queryStats:e,definedIndexes:n,recentErrors:a,cacheVersion:g(),timestamp:new Date().toISOString(),userAgent:typeof navigator<`u`?navigator.userAgent:`NodeJS-Test-Env`},s=[{role:`system`,content:v},{role:`user`,content:JSON.stringify(o)}];try{let e=r(await i(s));return{healthScore:typeof e.healthScore==`number`?Math.min(100,Math.max(0,e.healthScore)):100,findings:Array.isArray(e.findings)?e.findings:[],recommendations:{sql:e.recommendations?.sql||null,cache:e.recommendations?.cache||`keep`,advice:e.recommendations?.advice||`El sistema funciona de forma óptima.`}}}catch(t){console.error(`[aiDiagnosticService] Failed to run AI diagnostics:`,t);let n=a.length>0,r=e.indexMisses>e.indexHits;return{healthScore:n?70:r?85:95,findings:[...n?[{severity:`critical`,msg:`Se detectaron ${a.length} errores recientes en el reportero.`}]:[],...r?[{severity:`warning`,msg:`Elevada tasa de búsquedas secuenciales (misses de índice).`}]:[],{severity:`info`,msg:`Modo de contingencia: diagnóstico local básico (IA offline).`}],recommendations:{sql:r?`CREATE INDEX IF NOT EXISTS obs_student_date ON observations (student_id, created_at);`:null,cache:n?`clear`:`keep`,advice:`Conexión con el servicio de IA no disponible. Se aplicó el diagnóstico heurístico local.`}}}}var b=3e4,x=null,S=[{key:`aprobaciones`,title:`Aprobaciones de usuarios`,description:`Maestros y administradores pendientes de aprobación`,icon:`bi-person-check`,route:`admin-aprobacion`,color:`#22c55e`},{key:`ausencias`,title:`Solicitudes de ausencia`,description:`Ausencias pendientes de revisión`,icon:`bi-calendar-x`,route:`admin-ausencias`,color:`#f59e0b`},{key:`permisos`,title:`Solicitudes de permisos`,description:`Permisos solicitados por maestros`,icon:`bi-shield-lock`,route:`admin-dashboard-reportes`,color:`#8b5cf6`}];function C(e){e.innerHTML=`
  ...
  `,document.head.appendChild(e)}function I(e){let t=e.querySelector(`#btn-diagnose`),r=e.querySelector(`#diagnose-spinner`),i=e.querySelector(`#diagnose-icon`),a=e.querySelector(`#diagnose-btn-text`),o=e.querySelector(`#ai-output-container`),s=e.querySelector(`#ai-placeholder-container`),c=e.querySelector(`#health-score-val`),l=e.querySelector(`#health-score-status`),u=e.querySelector(`#svg-meter-progress`),d=e.querySelector(`#findings-list`),f=e.querySelector(`#sql-recommendation-box`),p=e.querySelector(`#sql-preview-text`),h=e.querySelector(`#btn-apply-indexes`),g=e.querySelector(`#cache-recommendation-box`),v=e.querySelector(`#cache-desc-text`),b=e.querySelector(`#btn-optimize-cache`),x=e.querySelector(`#ai-advice-text`),S=null;t&&(t.addEventListener(`click`,async()=>{t.disabled=!0,r.classList.remove(`d-none`),i.classList.add(`d-none`),a.textContent=`Analizando sistema...`;try{let e=await y();s.classList.add(`d-none`),o.classList.remove(`d-none`);let t=e.healthScore;c.textContent=t;let r=251.2-251.2*t/100;u.style.strokeDashoffset=r,l.className=`health-status-badge mt-2`,t>=90?(u.style.stroke=`#10b981`,l.textContent=`Saludable`,l.classList.add(`health-status-healthy`)):t>=80?(u.style.stroke=`#f59e0b`,l.textContent=`Advertencia`,l.classList.add(`health-status-warning`)):(u.style.stroke=`#ef4444`,l.textContent=`Crítico`,l.classList.add(`health-status-critical`)),d.innerHTML=e.findings.length>0?e.findings.map(e=>`
            ...
            `).join(``):`<div class="text-muted fs-7 p-2">No se detectaron anomalías ni advertencias en el sistema.</div>`,e.recommendations?.sql?(S=e.recommendations.sql,p.textContent=S,f.classList.remove(`d-none`),h.disabled=!1,h.innerHTML=`<i class="bi bi-arrow-repeat me-1"></i> Aplicar Índices`):(f.classList.add(`d-none`),S=null),e.recommendations?.cache===`clear`?(v.textContent=`Se detectaron errores recientes en el reportero. Se recomienda purgar la caché PWA para limpiar posibles inconsistencias de carga.`,g.classList.remove(`d-none`),b.disabled=!1,b.innerHTML=`<i class="bi bi-trash3 me-1"></i> Optimizar Caché`):g.classList.add(`d-none`),x.textContent=e.recommendations?.advice||`El sistema funciona de forma óptima.`,n.success(`Diagnóstico de IA completado con éxito.`)}catch(e){console.error(e),n.error(`Error al ejecutar diagnóstico: `+e.message)}finally{t.disabled=!1,r.classList.add(`d-none`),i.classList.remove(`d-none`),a.textContent=`Diagnosticar Sistema con IA`}}),h?.addEventListener(`click`,async()=>{if(S){h.disabled=!0,h.innerHTML=`<span class="spinner-border spinner-border-sm me-1" role="status"></span> Aplicando...`;try{let e=m(S);e.success?(n.success(e.message||`Índices aplicados con éxito en memoria.`),h.innerHTML=`<i class="bi bi-check-circle-fill me-1"></i> Aplicado`):(n.error(`Error al aplicar índices: `+e.error),h.disabled=!1,h.innerHTML=`<i class="bi bi-arrow-repeat me-1"></i> Reintentar`)}catch(e){n.error(`Fallo inesperado al optimizar base de datos: `+e.message),h.disabled=!1,h.innerHTML=`<i class="bi bi-arrow-repeat me-1"></i> Reintentar`}}}),b?.addEventListener(`click`,async()=>{b.disabled=!0,b.innerHTML=`<span class="spinner-border spinner-border-sm me-1" role="status"></span> Limpiando...`;try{let e=await _();e.success?(n.success(`Caché PWA depurada correctamente.`),b.innerHTML=`<i class="bi bi-check-circle-fill me-1"></i> Caché Limpia`):(n.error(`Error al limpiar caché: `+e.error),b.disabled=!1,b.innerHTML=`<i class="bi bi-trash3 me-1"></i> Optimizar Caché`)}catch(e){n.error(`Fallo inesperado al limpiar caché: `+e.message),b.disabled=!1,b.innerHTML=`<i class="bi bi-trash3 me-1"></i> Optimizar Caché`}}))}export{M as renderSistemaView};