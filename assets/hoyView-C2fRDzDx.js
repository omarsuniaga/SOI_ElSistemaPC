import{i as e}from"./supabase-KnARm58N.js";import{i as t}from"./maestroAuth-CgmSmLyS.js";import{a as n,i as r,n as i,o as a,r as o,t as s}from"./maestroDataService-9gztUGVK.js";import{t as c}from"./AppToast-3qbHkRVc.js";import{d as l}from"./weeklyPlanAdapter-BulGBGlw.js";import{t as u}from"./academicService-D-BIm-fm.js";import{a as d,i as f,o as p,r as m}from"./portalUtils-C92TBVO0.js";import{t as h}from"./claseEmergenteModal-Dknu0Ytj.js";import{t as g}from"./claseAnalysisModal--Ig8ZiUG.js";function _(e){let[t,n]=(e||`00:00`).split(`:`).map(Number);return t*60+n}function v(e,t,n){let r=_(e),i=_(t);return n>=r&&n<i?`en-curso`:n>=i?`pasada`:r-n<=15?`proxima`:`futura`}function y(e,t,n){let r=document.createElement(`div`);r.id=`pm-hoy-autonav-banner`,r.innerHTML=`
  ...
  `,document.body.appendChild(r);let i=3,a=!1,o=document.getElementById(`pm-autonav-count`),s=setInterval(()=>{a||(i--,o&&(o.textContent=i),i<=0&&(clearInterval(s),r.remove(),a||(window.router?window.router.navigate(`asistencia?clase=${e}&fecha=${t}`):n?.(e))))},1e3);document.getElementById(`pm-autonav-cancel`)?.addEventListener(`click`,()=>{a=!0,clearInterval(s),r.remove(),c.show(`Auto-navegación cancelada`,`info`)})}async function b(e,{onClaseClick:c}={}){e.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let h=t();if(!h){e.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}let _=new Date,b=_.toLocaleDateString(`es-ES`,{weekday:`long`}).toLowerCase(),S=`${_.getFullYear()}-${String(_.getMonth()+1).padStart(2,`0`)}-${String(_.getDate()).padStart(2,`0`)}`;try{let t=await s(h.id,S);if(t&&t.length>0){e.innerHTML=x(t,b,_),C(e,S,h.id);return}let T=await r();if(!T||T.length===0){e.innerHTML=`
      ...
      `,w(e,S,h.id,[]);return}let E=T.map(e=>e.id),D=Object.fromEntries(T.map(e=>[e.id,e])),O=await l(h.id).catch(()=>[]),k=Object.fromEntries((O||[]).map(e=>[String(e.group_id),e])),A=(await i(E)).filter(e=>e.dia?.toLowerCase()===b).sort((e,t)=>e.hora_inicio.localeCompare(t.hora_inicio));if(!A||A.length===0){e.innerHTML=`
      ...
      `,w(e,S,h.id,T);return}let j=new Date(_);j.setDate(j.getDate()-3);let M=`${j.getFullYear()}-${String(j.getMonth()+1).padStart(2,`0`)}-${String(j.getDate()).padStart(2,`0`)}`,N=new Date(_);N.setDate(N.getDate()-1);let P=`${N.getFullYear()}-${String(N.getMonth()+1).padStart(2,`0`)}-${String(N.getDate()).padStart(2,`0`)}`,F=(await a(h.id,M,P)||[]).filter(e=>{if(!E.includes(e.clase_id))return!1;let t=Array.isArray(e.asistencia)&&e.asistencia.length>0,n=typeof e.contenido==`string`&&e.contenido.trim().length>0;return!t&&!(e.borrador===!1&&n)}),I=(await a(h.id,S,S)).filter(e=>E.includes(e.clase_id)).filter(e=>{let t=Array.isArray(e.asistencia)&&e.asistencia.length>0,n=typeof e.contenido==`string`&&e.contenido.trim().length>0;return t||e.borrador===!1&&n}),L=new Set(I.map(e=>e.clase_id)),R=await o(E),z={};for(let e of R||[])e.clase_id&&(z[e.clase_id]=(z[e.clase_id]||0)+1);let B=[...new Set(A.map(e=>e.salon_id).filter(Boolean))],V=B.length>0?await n(B):[],H=Object.fromEntries(V.map(e=>[e.id,e.nombre])),U=_.getHours()*60+_.getMinutes(),W=null,G=null,K=A.map(e=>{let t=D[e.clase_id],n=L.has(t.id),r=z[t.id]||0,i=v(e.hora_inicio,e.hora_fin,U),a=k[String(t.id)]||null;i===`en-curso`&&(!n&&!W&&(W=t.id),n&&!G&&(G=t.id));let o=n?`<span class="pm-badge pm-badge-success"><i class="bi bi-check-circle-fill me-1"></i>Registrada</span>`:`<span class="pm-badge pm-badge-danger">Sin registrar</span>`,s=i===`en-curso`?`<span class="pm-badge pm-badge-en-curso"><i class="bi bi-circle-fill pm-pulse-dot me-1"></i>En curso</span>`:i===`proxima`?`<span class="pm-badge pm-badge-proxima"><i class="bi bi-clock me-1"></i>Próximamente</span>`:``;return`
