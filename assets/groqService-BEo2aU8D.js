...
- Responde solo con el JSON, sin markdown ni explicaciones fuera de él.`;async function W(e,t={}){if(!e||!e.trim())throw Error(`No hay texto para analizar`);let n={observacion:e,indicador:t.indicadorNombre||``,criterios:t.criterios||``,estudiantesPresentes:(t.estudiantesPresentes||[]).join(`, `)},r=L(await A([{role:`system`,content:U},{role:`user`,content:JSON.stringify(n)}],.2)),i=!!r?.tieneValoracionImplicita;return{panorama:r?.panorama||`No se pudo generar un panorama a partir del texto.`,tieneValoracionImplicita:i,sugerirCalificarConEstrellas:!i}}async function G(e,t,n){let r=t.alumnos||[],i=m(e,{...t,alumnos:r,presentes:n});if(!i.length)return{dsl:``,progreso:[],resumen:`Registro general de clase sin evaluaciones detectadas.`};let a={instrumento:t.instrumento||`música`,tipoClase:t.tipoClase||`instrumento`,groups:i.map((e,r)=>({id:`g_${r+1}`,fragment:K(e.fragment,n),estado:e.estado?.value||e.estado,tipo:l(e.fragment,t.tipoClase),scope:e.scope||`grupo`}))},o=i.map(()=>({contenido:``,observacion:``})),s;try{s=await A([{role:`system`,content:`Eres un asistente pedagógico musical.
