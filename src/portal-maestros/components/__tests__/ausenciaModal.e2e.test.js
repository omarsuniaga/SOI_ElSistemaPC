...
describe('AusenciaModal E2E — full flow', () => {
  ...
  it('hides reschedule panel when switching back to activities coverage', () => {
    ...
    const rescheduleRadio = document.querySelector(
      'input[name="coverage-type"][value="reschedule"]',
    )
    rescheduleRadio.checked = true
    rescheduleRadio.dispatchEvent(new Event('change'))

    // Then switch back to activities
    const activitiesRadio = document.querySelector(
      'input[name="coverage-type"][value="activities"]',
    )
  ...
  it('shows empty state message when no classes are affected', async () => {
    ...
    await vi.waitFor(() => {
      expect(document.getElementById('clases-afectadas-container').textContent).toContain(
        'No hay clases',
      )
    })
  })
  ...
  it('whatsapp copy button writes text to clipboard after success', async () => {
    ...
    await vi.waitFor(() => expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expectedText))
