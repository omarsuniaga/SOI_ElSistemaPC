-- Migration: Allow institutional admin roles to approve pending maestros.
--
-- The Admin UI can be used by institutional administrative roles, not only
-- literal "admin" / "superadmin" profiles. The approval RPC must mirror that
-- authorization model or the UI shows the action but the database rejects it.
...
'Atomic approval of pending maestro profiles by institutional admin roles. Confirms email, syncs metadata role, creates/updates maestros with registration instrument and reseña, and grants default permissions.';
