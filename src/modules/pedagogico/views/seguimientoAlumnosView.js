...
async function _loadData() {
  ...
  const [alumnosRes, asistenciasRes, progresosRes, observacionesRes] = await Promise.all([
    supabase
      .from('alumnos')
      .select('id, nombre_completo, instrumento_principal, activo')
      .eq('activo', true)
  ...
  ;(asistenciasRes.data || []).forEach((a) => {
    if (!state.asistenciaMap[a.alumno_id])
      state.asistenciaMap[a.alumno_id] = { total: 0, presentes: 0 }
...
function _getAlumnoRisk(alumnoId) {
  ...
  if ((state.observacionesMap[alumnoId] || []).some((o) => o.tipo === 'disciplina'))
...
function _render() {
  ...
  const filtrados = state.alumnos.filter(
    (a) =>
      !term ||
...
function _attachEvents() {
  state.container.querySelector('#btn-help-seguimiento')?.addEventListener('click', () => {
    HelpPanel.open({
      ...
      intro:
        'Vista unificada del estado académico de cada alumno. Los alumnos con riesgo aparecen primero, destacados con una barra lateral amarilla.',
      sections: [
        {
          icon: 'bi-search',
          title: 'Buscador',
        ...
        },
        {
          ...
          description:
            'Aparece cuando hay alumnos que requieren atención. Muestra el total con algún indicador activo.',
          color: '#f59e0b',
        },
        {
          icon: 'bi-person-fill',
          ...
          description:
            'Nombre, instrumento, % de asistencia (últimas 4 semanas) y promedio de las últimas 3 calificaciones. Barra amarilla izquierda = en riesgo.',
          color: '#3b82f6',
        },
        {
          icon: 'bi-tags-fill',
          ...
          description:
            '"Asistencia baja" < 70% en 4 semanas. "Nota baja" promedio < 6.0. "Observación" cuando hay observaciones de disciplina activas.',
          color: '#ef4444',
        },
        {
          ...
          description:
            'Clic en cualquier alumno → panel con asistencia reciente (20 clases), últimas calificaciones por clase y observaciones activas.',
          color: '#10b981',
        },
      ],
    })
                  ...
                  <div class="d-flex justify-content-between align-items-center py-1 border-bottom" style="font-size:0.78rem;">
                    ...
                    <span class="badge rounded-pill ${a.estado === 'P' ? 'bg-success-subtle text-success' : a.estado === 'J' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger'}">
                      ${a.estado === 'P' ? 'Presente' : a.estado === 'J' ? 'Justificado' : 'Ausente'}
                        ...
                        .join('')
                    : '<p class="text-muted small mb-0">Sin registros</p>'
                }
              </div>
            </div>
          </div>
                ...
                ${
                  ...
                  <div class="d-flex justify-content-between align-items-center py-1 border-bottom" style="font-size:0.78rem;">
                    ...
                    <span class="fw-semibold ${p.calificacion >= 7 ? 'text-success' : p.calificacion >= 5 ? 'text-warning' : 'text-danger'}">${p.calificacion?.toFixed(1) ?? '–'}</span>
                        ...
                        .join('')
                    : '<p class="text-muted small mb-0">Sin calificaciones</p>'
                }
              </div>
            </div>
          </div>
