import { INTENCIONES } from './intencionParser.js'
import { puedeTransicionar } from './postuladoStateMachine.js'
...
function handleRespuestaInicial(intencion, confianza, fecha_sugerida, ctx) {
  const nombre = ctx.nombreRepresentante || 'estimado representante'
  const alumno = ctx.nombreAlumno || 'su hijo'

  switch (intencion) {
    case INTENCIONES.AGENDAR_CITA:
      if (confianza < 0.5) {
        return {
    ...
    case INTENCIONES.PREGUNTAR_REQUISITOS:
      return {
    ...
    case INTENCIONES.CONSULTA_GENERAL:
      return {
    ...
    case INTENCIONES.CONFIRMAR_ASISTENCIA:
      return {
...
function handleOfreciendoHorarios(intencion, confianza, fecha_sugerida, ctx) {
...
function handleAgendandoCita(intencion, confianza, fecha_sugerida, ctx) {
  const nombre = ctx.nombreRepresentante || 'estimado'
  ...
  switch (intencion) {
    ...
    case INTENCIONES.PREGUNTAR_REQUISITOS:
      return {
        mensaje: `Los requisitos son:\n\n${docsToList()}\n\n¿Confirmamos su cita para el ${formatearFecha(ctx.fechaCitaActual)}?`,
        siguienteEstado: CONV_ESTADOS.AGENDANDO_CITA,
        pipelineAction: null,
      }
...
function handleConfirmacionD1(intencion, confianza, fecha_sugerida, ctx) {
  const nombre = ctx.nombreRepresentante || 'estimado'
  const hora = ctx.fechaCitaActual ? formatearHora(ctx.fechaCitaActual) : 'la hora acordada'
...
function handleConfirmada(intencion, confianza, fecha_sugerida, ctx) {
  const nombre = ctx.nombreRepresentante || 'estimado'
  return {
...
function handleCancelada(intencion, confianza, fecha_sugerida, ctx) {
  return {
    mensaje: null,
    siguienteEstado: CONV_ESTADOS.CANCELADA,
    pipelineAction: null,
...
export function decidirAccion(conversacion, intencion) {
  ...
  switch (estadoConversacion) {
    ...
    case CONV_ESTADOS.CANCELADA:
      return handleCancelada(intento, confianza, fecha_sugerida, ctx)
...
export function conflictoToMensaje(nombre, alternativas) {
