...
export async function renderRepertoireView(container, { adapter = createRepertoireDemoAdapter() } = {}) {
  ...
  const render = () => {
    ...
    if (active) bindMap()
    else container.querySelectorAll('.repertoire-open').forEach((button) => button.addEventListener('click', () => { active = montajes.find((item) => item.id === button.dataset.montajeId); activeStudentId = active.alumnos?.[0]?.id || null; render() }))
  }

  const bindMap = () => {
    container.querySelectorAll('.repertoire-student').forEach((button) => button.addEventListener('click', () => { activeStudentId = button.dataset.studentId; render() }))
  ...
  async function saveState(measure, state) {
    ...
    measure.estado_preparacion = state
    syncMessage = { label: 'Guardando…', className: 'is-pending' }
    pickerMeasure = null
    render()
    try { await adapter.updateMeasureState(measure.id, state); syncMessage = { label: 'Guardado local (Demo)', className: 'is-saved' } } catch { measure.estado_preparacion = previous; syncMessage = { label: 'No se pudo guardar; se revirtió', className: 'is-error' } }
    render()
  }
