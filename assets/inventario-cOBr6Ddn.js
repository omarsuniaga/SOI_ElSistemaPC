import"./early-error-suppression-K9hxPIVV.js";import{i as e}from"./supabase-Cgh_dhNB.js";import"./vendor-mK9cUK6A.js";import{a as t,i as n,n as r,o as i,r as a,t as o}from"./historialInstrumentoView-W3husOzz.js";import{t as s}from"./tareasView-CGnZxlyx.js";function c(c,l){let u=l?.user?.email??`Usuario`;c.style.background=`#f8fafc`,c.innerHTML=`
  ...
  `;let d=c.querySelector(`#portal-content`),f=null;function p(e){c.querySelectorAll(`.portal-tab`).forEach(t=>{let n=t.dataset.view===e;t.style.borderBottomColor=n?`#2563eb`:`transparent`,t.style.color=n?`#2563eb`:`#64748b`}),f?.teardown?.(),e===`dashboard`?a(d).then(e=>{f=e}):e===`stock`?i(d).then(e=>{f=e}):e===`comodatos`?t(d).then(e=>{f=e}):e===`alertas`?n(d).then(e=>{f=e}):e===`tareas`&&s(d,{departamento:`LOG`,hideCalendarBtn:!0}).then(e=>{f=e})}c.querySelectorAll(`.portal-tab`).forEach(e=>{e.addEventListener(`click`,()=>p(e.dataset.view))}),c.querySelector(`#btn-logout`)?.addEventListener(`click`,async()=>{await e.auth.signOut(),window.location.reload()}),window.router={navigate:(e,t)=>{e===`inventario-dashboard`?p(`dashboard`):e===`inventario-stock`?p(`stock`):e===`inventario-comodatos`?p(`comodatos`):e===`inventario-alertas`?p(`alertas`):e===`inventario-tareas`?p(`tareas`):e===`inventario-detalle`?(f?.teardown?.(),r(d,t).then(e=>{f=e})):e===`inventario-historial`&&(f?.teardown?.(),o(d,t).then(e=>{f=e}))}},p(`dashboard`)}async function l(t){let{data:n}=await e.from(`profiles`).select(`rol`).eq(`id`,t).maybeSingle();return n?.rol===`admin`||n?.rol===`inventarista`}function u(t,n=null){t.style.background=``,t.innerHTML=`
