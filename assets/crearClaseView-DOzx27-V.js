import{i as e}from"./supabase-Cgh_dhNB.js";import{i as t}from"./maestroAuth-BMzDPnai.js";import{i as n}from"./portalUtils-CkF82Yyk.js";var r=[`lunes`,`martes`,`miércoles`,`jueves`,`viernes`,`sábado`,`domingo`];async function i(i){i.innerHTML=`<div class="pm-loading"><div class="pm-spinner"></div></div>`;let a=t();if(!a){i.innerHTML=`<p class="pm-empty">No hay sesión activa.</p>`;return}try{let{data:t}=await e.from(`system_config`).select(`value`).eq(`key`,`maestros_pueden_crear_clases`).maybeSingle();if(t?.value!==`true`){i.innerHTML=`
      ...
      `;return}let{data:o}=await e.from(`inventario_activos`).select(`tipo_instrumento`).eq(`activo`,!0).not(`tipo_instrumento`,`is`,null).order(`tipo_instrumento`),s=[...new Map((o||[]).map(e=>[e.tipo_instrumento,{nombre:e.tipo_instrumento}])).values()],{data:c}=await e.from(`maestros`).select(`id, nombre, email`).neq(`id`,a.id).order(`nombre`);i.innerHTML=`
        ...
        <div class="pm-form-section">
          ...
          <div class="pm-form-group">
            ...
            <select id="nueva-clase-instrumento" class="pm-input">
              ...
              ${(s||[]).map(e=>`<option value="${n(e.nombre)}">${n(e.nombre)}</option>`).join(``)}
            </select>
          </div>
        ...
        <div class="pm-form-section">
          ...
          <div class="pm-form-group">
            ...
            <select id="nueva-clase-maestro-aux" class="pm-input">
              ...
              ${(c||[]).map(e=>`<option value="${e.id}">${n(e.nombre||e.email)}</option>`).join(``)}
            </select>
          </div>
        </div>
