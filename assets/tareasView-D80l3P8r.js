import{t as e}from"./AppToast-BfaQtGFE.js";import{i as t}from"./supabase-Cgh_dhNB.js";import{n,t as r}from"./AppModal-C8uxOJPY.js";import{C as i,S as a,_ as o,d as s,f as c,g as l,h as u,i as d,m as f,n as p,t as m,v as h,x as g}from"./tareas-DeTW-RPg.js";var _={pendiente:{label:`Pendiente`,color:`secondary`,icon:`bi-clock`},en_progreso:{label:`En Progreso`,color:`info`,icon:`bi-play-circle`},completada:{label:`Completada`,color:`success`,icon:`bi-check-circle`},bloqueada:{label:`Bloqueada`,color:`danger`,icon:`bi-x-octagon`},cancelada:{label:`Cancelada`,color:`dark`,icon:`bi-dash-circle`},observada:{label:`Observada`,color:`warning`,icon:`bi-eye`}};function v(e){let t=_[e]??{label:e,color:`secondary`,icon:`bi-question-circle`};return`<span class="badge bg-${t.color} task-status-badge" data-estado="${e}"><i class="bi ${t.icon} me-1"></i>${t.label}</span>`}function y(){return{..._}}var b={alumno:`bi-person`,maestro:`bi-person-workspace`,postulante:`bi-person-plus`,representante:`bi-people`,instrumento:`bi-music-note-beamed`,evento:`bi-calendar-event`,otro:`bi-link-45deg`},x={alumno:`Alumno`,maestro:`Maestro`,postulante:`Postulante`,representante:`Representante`,instrumento:`Instrumento`,evento:`Evento`,otro:`Otro`};function S(e){if(!e?.entidad_tipo)return``;let t=e.entidad_tipo,n=e.entidad_label||t,r=b[t]??`bi-link-45deg`,i=x[t]??t,a=String(n).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`),o=String(i).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`);return`<span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 task-entity-chip" title="${o}: ${a}"><i class="bi ${r} me-1"></i>${o}: ${a}</span>`}function C(e){if(!e)return``;try{return new Date(e).toLocaleString(`es-VE`,{day:`2-digit`,month:`short`,year:`numeric`,hour:`2-digit`,minute:`2-digit`})}catch{return e}}function w(e,t=[]){let r=t.length===0?`<p class="text-muted small text-center py-2"><i class="bi bi-chat-square-dots me-1"></i>Sin comentarios aÃºn.</p>`:t.map(e=>`
        <div class="task-comment-item d-flex gap-2 mb-3" data-comment-id="${n(e.id)}">
          ...
          <div class="flex-grow-1">
            <div class="d-flex align-items-baseline gap-2 mb-1">
              <strong class="small">${n(e.autor_nombre||`AnÃ³nimo`)}</strong>
              <small class="text-muted">${C(e.created_at)}</small>
            </div>
            <p class="mb-0 small">${n(e.cuerpo)}</p>
          </div>
        </div>
      `).join(``);return`
    <div class="task-comments-panel" data-tarea-id="${n(e)}">
  ...
  `}function T(e){if(!e)return``;try{return new Date(e).toLocaleString(`es-VE`,{day:`2-digit`,month:`short`,year:`numeric`,hour:`2-digit`,minute:`2-digit`})}catch{return e}}var E={estado:`Estado`,asignado_a:`Asignado a`,prioridad:`Prioridad`,fecha_vencimiento:`Vencimiento`,entidad_tipo:`Tipo de entidad`,entidad_id:`ID de entidad`,correlation_id:`Correlation ID`};function D(e=[]){if(e.length===0)return`
    ...
    `;let t=e.map(e=>{let t=E[e.campo]??n(e.campo),r=e.actor_nombre?n(e.actor_nombre):`<em class="text-muted">Sistema</em>`,i=e.valor_anterior==null?`<span class="text-muted small">â€”</span>`:`<span class="text-danger text-decoration-line-through small">${n(e.valor_anterior)}</span>`,a=e.valor_nuevo==null?`<span class="text-muted small">â€”</span>`:`<span class="text-success fw-semibold small">${n(e.valor_nuevo)}</span>`;return`
      <div class="task-history-entry d-flex gap-3 mb-3" data-history-id="${n(e.id)}">
    ...
    <div class="task-attachment-item d-flex align-items-center gap-3 p-2 rounded border mb-2" data-adj-id="${n(e.id)}" data-storage-path="${n(e.storage_path)}">
      ...
      <div class="flex-shrink-0">
        <button class="btn btn-sm btn-outline-secondary task-attachment-download" type="button"
          data-storage-path="${n(e.storage_path)}"
          title="Descargar ${n(e.nombre)}">
          <i class="bi bi-download"></i>
        </button>
      </div>
    </div>
  `).join(``);return`
    <div class="task-attachments-panel" data-tarea-id="${n(e)}">
      <h6 class="mb-3">
  ...
  `,e.querySelector(`#retryBtn`)?.addEventListener(`click`,()=>K(e,{departamento:B.departamentoFijo,actor:B.actor}),{signal:V.signal})}function Y(e){let t=Z(),r=e=>B.tareas.filter(t=>t.estado===e).length;e.innerHTML=`
    <div class="page-container">
      <div class="tareas-header mb-4">
        <div class="d-flex align-items-center gap-3 mb-3">
          ...
          <div>
            <h1 class="tareas-title mb-0">${n(B.departamentoFijo?`Tareas â€” ${L[B.departamentoFijo]||B.departamentoFijo}`:B.processCode?`Tareas del caso ${B.processCode}`:`Tareas Institucionales`)}</h1>
            <p class="text-muted small mb-0">Sistema Hermes Â· delegaciÃ³n automÃ¡tica</p>
            ${B.correlationId?`<p class="text-muted small mb-0">Caso: <code>${n(B.correlationId)}</code></p>`:``}
          </div>
        </div>
      ...
      <div class="tareas-filters mb-4 d-flex gap-2 flex-wrap">
        <div class="flex-grow-1" style="min-width: 200px;">
          <input type="text" class="form-control form-control-sm" id="buscarTarea"
            placeholder="ðŸ” Buscar tarea..." autocomplete="off" value="${n(B.busqueda)}">
        </div>
      ...
      <div id="tareasList" class="tareas-list">
        ${t.length===0?`<div class="alert alert-info text-center py-4"><i class="bi bi-inbox"></i> No hay tareas que cumplan con los filtros</div>`:t.map(X).join(``)}
      </div>
    </div>
  `}function X(e){let t=R[e.estado]||R.pendiente,r=z[e.prioridad]||z.media,i=Array.isArray(e.checklist)?e.checklist:[],a=i.filter(e=>e.completado).length,o=i.length,s=o>0?a/o*100:0,c=e.fecha_vencimiento?Math.ceil((new Date(e.fecha_vencimiento)-new Date)/864e5):null,l=c===null?`text-muted`:c<0?`text-danger`:c<3?`text-warning`:`text-muted`,u=S(e),d=v(e.estado),f=e.entidad_tipo===`tool_call`;return`
    <div class="tarea-card card border-0 mb-3 shadow-sm" data-tarea-id="${e.id}">
      <div class="card-body p-3">
        <div class="d-flex align-items-start gap-3">
          ...
          <div class="flex-grow-1">
            <h5 class="card-title mb-1">${n(e.titulo)}</h5>
            <p class="card-text text-muted small mb-2">${n(e.descripcion||``)}</p>
            ...
            ${o>0?`<div class="mb-1">
                     ...
                     <div class="progress" style="height: 6px;">
                       <div class="progress-bar bg-${t.color}" style="width: ${s}%;"></div>
                     </div>
                   </div>`:``}
          </div>
  ...
  `}function Z(){let e=[...B.tareas];if(B.filtroEstado!==`todos`&&(e=e.filter(e=>e.estado===B.filtroEstado)),!B.departamentoFijo&&B.filtroDepartamento!==`todos`&&(e=e.filter(e=>e.departamento===B.filtroDepartamento)),B.filtroPrioridad!==`todos`&&(e=e.filter(e=>e.prioridad===B.filtroPrioridad)),B.busqueda){let t=B.busqueda.toLowerCase();e=e.filter(e=>e.titulo.toLowerCase().includes(t)||(e.descripcion||``).toLowerCase().includes(t))}return e.sort((e,t)=>(z[e.prioridad]?.orden??9)-(z[t.prioridad]?.orden??9)),e}function Q(e){let t=V.signal,n=()=>{Y(e),Q(e)};e.querySelector(`#buscarTarea`)?.addEventListener(`input`,t=>{B.busqueda=t.target.value;let n=e.querySelector(`#tareasList`),r=Z();n.innerHTML=r.length===0?`<div class="alert alert-info text-center py-4"><i class="bi bi-inbox"></i> No hay tareas que cumplan con los filtros</div>`:r.map(X).join(``),$(e)},{signal:t}),e.querySelector(`#filtroEstado`)?.addEventListener(`change`,e=>{B.filtroEstado=e.target.value,n()},{signal:t}),e.querySelector(`#filtroDepartamento`)?.addEventListener(`change`,e=>{B.filtroDepartamento=e.target.value,n()},{signal:t}),e.querySelector(`#filtroPrioridad`)?.addEventListener(`change`,e=>{B.filtroPrioridad=e.target.value,n()},{signal:t}),$(e)}function $(e){let t=V.signal;e.querySelectorAll(`.btn-detalle`).forEach(n=>{n.addEventListener(`click`,()=>{let t=B.tareas.find(e=>e.id===n.dataset.tareaId);t&&ee(e,t)},{signal:t})})}async function ee(t,s){let c=z[s.prioridad]||z.media,_=Array.isArray(s.checklist)?s.checklist:[],y=v(s.estado),b=S(s),x=[],C=[];try{[x,C]=await Promise.all([u(s.id),l(s.id)])}catch{}let T=Array.isArray(s.documentos_adjuntos)?s.documentos_adjuntos:[],E=Object.entries(R).filter(([e])=>e!==`observada`),O=P(s),k=O?F(s):null,A=k?I(k.args):[];r.open({title:s.titulo,size:`xl`,body:`
      <div class="modal-tarea-content">
        ...
        ${b?`<div class="mb-3"><strong>Entidad asociada</strong><div class="mt-1">${b}</div></div>`:``}

        ${O&&k?`<div class="mb-3 border rounded p-3 bg-dark bg-opacity-10" id="toolApprovalPanel">
                 <strong class="d-block mb-2"><i class="bi bi-robot me-1"></i>Solicitud de ejecuciÃ³n de tool</strong>
                 <div class="row mb-2 small">
                   <div class="col-md-6"><span class="text-muted">Tool</span><p class="mb-0 fw-semibold">${n(s.entidad_label||k.tool_name)}</p></div>
                   <div class="col-md-6"><span class="text-muted">Departamento</span><p class="mb-0">${L[s.departamento]||s.departamento}</p></div>
                 </div>
                 ${A.length>0?`<table class="table table-sm table-borderless mb-2">
                          <tbody>
                            ${A.map(e=>`<tr><td class="text-muted small" style="width:40%;">${n(e.clave)}</td><td class="small">${n(e.valor)}</td></tr>`).join(``)}
                          </tbody>
        ...
        <div class="mb-3">
          ...
          ${s.estado===`observada`?`<input type="hidden" id="modalEstado" value="observada">
                 <p class="text-muted small mt-1"><i class="bi bi-info-circle me-1"></i>Este estado sÃ³lo puede modificarse mediante una nueva transiciÃ³n.</p>`:`<select class="form-select form-select-sm" id="modalEstado">
                   ${E.map(([e,t])=>`<option value="${e}" ${s.estado===e?`selected`:``}>${t.label}</option>`).join(``)}
                 </select>`}
        </div>
                 ...
                 <div class="list-group list-group-flush mt-1" id="modalChecklist">
                   ${_.map((e,t)=>`
                     <label class="list-group-item px-0 d-flex align-items-center gap-2">
                       ...
                       <span class="${e.completado?`text-decoration-line-through text-muted`:``}">${n(e.item)}</span>
                     </label>`).join(``)}
                 </div>
               </div>`:``}
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control form-control-sm mt-1" id="modalFeedback" rows="2"
            placeholder="Comentario del responsable...">${n(s.feedback||``)}</textarea>
        </div>
    ...
    `,saveText:`Guardar cambios`,onOpen:n=>{let a=V.signal;M(n,i,a);let c=n.querySelector(`#btnObservar`);c?.addEventListener(`click`,async()=>{let i=n.querySelector(`#modalObservarComentario`)?.value?.trim()||``;if(!i){e.show(`El comentario es obligatorio para marcar como Observada`,`error`);return}try{c.disabled=!0;let n=B.actor||{id:null,nombre:`Usuario`};await o(s.id,i,n),e.show(`Tarea marcada como Observada`,`success`),r.close?.(),await K(t,{departamento:B.departamentoFijo,actor:B.actor})}catch(t){e.show(`Error: ${t.message}`,`error`),c.disabled=!1}},{signal:a});let l=n.querySelector(`#btnAprobarTool`),d=n.querySelector(`#btnRechazarTool`);l?.addEventListener(`click`,async()=>{try{l.disabled=!0,d&&(d.disabled=!0);let n=B.actor||{id:null,nombre:`Usuario`},i=await p(s.id,n);e.show(i?.mensaje||`Tool ejecutada correctamente`,`success`),r.close?.(),await K(t,{departamento:B.departamentoFijo,actor:B.actor})}catch(t){e.show(`Error al aprobar la tool: ${t.message}`,`error`),l.disabled=!1,d&&(d.disabled=!1)}},{signal:a}),d?.addEventListener(`click`,async()=>{let i=n.querySelector(`#toolRechazoMotivo`)?.value?.trim()||``;if(!i){e.show(`El motivo del rechazo es obligatorio`,`error`);return}try{d.disabled=!0,l&&(l.disabled=!0);let n=B.actor||{id:null,nombre:`Usuario`};await h(s.id,i,n),e.show(`Solicitud de tool rechazada`,`success`),r.close?.(),await K(t,{departamento:B.departamentoFijo,actor:B.actor})}catch(t){e.show(`Error al rechazar la tool: ${t.message}`,`error`),d.disabled=!1,l&&(l.disabled=!1)}},{signal:a});let f=n.querySelector(`.task-comment-submit`);f?.addEventListener(`click`,async()=>{let t=n.querySelector(`.task-comment-input`),r=t?.value?.trim()||``;if(!r){e.show(`El comentario no puede estar vacÃ­o`,`error`);return}try{f.disabled=!0;let i=B.actor||{id:null,nombre:`Usuario`};await m(s.id,r,i),e.show(`Comentario agregado`,`success`),t&&(t.value=``);let a=await u(s.id),o=n.querySelector(`.task-comments-panel`);o&&(o.outerHTML=w(s.id,a))}catch(t){e.show(`Error: ${t.message}`,`error`)}finally{f.disabled=!1}},{signal:a})},onSave:async n=>{let r=n.querySelector(`#modalEstado`).value,i=n.querySelector(`#modalFeedback`).value.trim();try{let o=n.querySelectorAll(`.chk-item`);for(let e of o){let t=parseInt(e.dataset.idx,10);!!_[t]?.completado!==e.checked&&await g(s.id,t,e.checked)}r===`completada`?await d(s.id,i||null):r!==`observada`&&(await a(s.id,r),i!==(s.feedback||``)&&await f(s.id,i)),e.show(`Tarea actualizada`,`success`),await K(t,{departamento:B.departamentoFijo,actor:B.actor})}catch(t){e.show(`Error: ${t.message}`,`error`)}}})}export{K as t};