import{i as e}from"./supabase-Cgh_dhNB.js";import{b as t,v as n,x as r,y as i}from"./hermesConsultaView-DfJGOdH_.js";async function a(e){e&&(e.innerHTML=`
    ...
    </div>`,e.querySelector(`#btn-doc-hist-back`)?.addEventListener(`click`,()=>window.router?.navigate(`exportar-datos`)),e.querySelectorAll(`.btn-doc-redownload`).forEach(e=>{e.addEventListener(`click`,n=>{n.stopPropagation();let o=a.find(t=>t.id===e.dataset.id);o&&t(r({title:o.titulo,content:o.contenido_final,metadata:{alumnoNombre:o.alumno_nombre,tipo:o.tipo}}),i({tipo:o.tipo,alumnoNombre:o.alumno_nombre,fecha:o.created_at?.slice(0,10)}))})}),e.querySelectorAll(`.btn-doc-archive`).forEach(t=>{t.addEventListener(`click`,async r=>{r.stopPropagation(),confirm(`¿Archivar este documento?`)&&(await n(t.dataset.id),await o(e))})})}export{a as renderGeneratedDocumentsView};