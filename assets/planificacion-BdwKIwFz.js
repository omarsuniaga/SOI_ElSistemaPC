import{i as e,r as t,s as n,t as r}from"./AppModal-Du6jXNYA.js";import{i}from"./supabase-Cgh_dhNB.js";import{t as a}from"./vendor-mK9cUK6A.js";import{t as o}from"./config-CNiOV0RX.js";import{a as s,c,i as l,n as u,o as d,r as f,t as p}from"./planificacionAdapter-C-rXyuPH.js";import{i as m,n as h,r as g}from"./evaluacionClaseService-PzaE8gD7.js";import{i as _,n as v,r as ee,t as y}from"./IndicadorLogro-CUm_IXl5.js";import{n as b}from"./aiEvaluacionService-_0RGpDzq.js";import{t as te}from"./MapaContenidoSVG-B-8-5_NT.js";import{a as x,b as ne,c as re,l as ie,o as ae,p as oe,s as se,u as ce,v as le,x as ue,y as de}from"./mapaClaseService-DaoOTdhF.js";var fe=n({actualizarPermiso:()=>me,aprobarSolicitud:()=>ve,crearSolicitud:()=>he,obtenerPermisoPorMaestro:()=>C,obtenerPermisos:()=>pe,obtenerSolicitudPorMaestro:()=>ge,obtenerSolicitudesPendientes:()=>_e,rechazarSolicitud:()=>ye});function S(e){return e?{...e,id:e.id,maestro_id:e.maestro_id??``,maestro_nombre:e.maestros?.nombre_completo??``,maestro_email:e.maestros?.correo??``,puede_registrar_alumnos:e.puede_registrar_alumnos??!1,puede_inscribir_clases:e.puede_inscribir_clases??!1,permisos:Array.isArray(e.permisos)?e.permisos:[],solicitudes:Array.isArray(e.solicitudes)?e.solicitudes:[],concedido_por:e.concedido_por??null,concedido_por_nombre:null,creado_en:e.creado_en||null,actualizado_en:e.actualizado_en||null}:null}async function pe(){let{data:e,error:t}=await i.from(`permisos_maestros`).select(`*, maestros!permisos_maestros_maestro_id_fkey(nombre_completo, correo)`).order(`creado_en`,{ascending:!1});if(t)throw console.error(`Error cargando permisos:`,t.message),Error(`No se pudieron cargar los permisos`);return e.map(S)}async function C(e){let{data:t,error:n}=await i.from(`permisos_maestros`).select(`*, maestros!permisos_maestros_maestro_id_fkey(nombre_completo, correo)`).eq(`maestro_id`,e).maybeSingle();if(n)throw console.error(`Error cargando permiso:`,n.message),Error(`No se pudo cargar el permiso`);return S(t)}async function me(e,t){let{data:n}=await i.from(`permisos_maestros`).select(`puede_registrar_alumnos, puede_inscribir_clases, permisos, solicitudes`).eq(`maestro_id`,e).maybeSingle(),r={maestro_id:e,puede_registrar_alumnos:`puede_registrar_alumnos`in t?t.puede_registrar_alumnos??!1:n?.puede_registrar_alumnos??!1,puede_inscribir_clases:`puede_inscribir_clases`in t?t.puede_inscribir_clases??!1:n?.puede_inscribir_clases??!1,permisos:Array.isArray(t.permisos)?t.permisos:n?.permisos??[],solicitudes:Array.isArray(t.solicitudes)?t.solicitudes:n?.solicitudes??[],concedido_por:t.concedido_por||null},{data:a,error:o}=await i.from(`permisos_maestros`).upsert(r,{onConflict:`maestro_id`}).select().single();if(o)throw console.error(`Error actualizando permiso:`,o.message),Error(`No se pudo actualizar el permiso`);return S(a)}function w(e){return e?{id:e.id,maestro_id:e.maestro_id??``,maestro_nombre:e.maestros?.nombre_completo??``,maestro_email:e.maestros?.correo??``,solicita_alumnos:e.solicita_alumnos??!1,solicita_clases:e.solicita_clases??!1,estado:e.estado??`pendiente`,creado_en:e.creado_en||null,actualizado_en:e.actualizado_en||null,aprobado_en:e.aprobado_en||null,aprobado_por:e.aprobado_por||null,aprobado_por_nombre:e.maestros_aprobado?.nombre_completo??``,motivo_rechazo:e.motivo_rechazo||null}:null}async function he(e,t,n){if(!e)throw Error(`maestroId es requerido`);let{data:r}=await i.from(`solicitudes_permisos`).select(`id, estado`).eq(`maestro_id`,e).eq(`estado`,`pendiente`).maybeSingle();if(r)throw Error(`Ya existe una solicitud pendiente para este maestro`);let a=[];t&&a.push(`alumnos:create`),n&&a.push(`clases:enroll`);let{data:o,error:s}=await i.from(`solicitudes_permisos`).insert([{maestro_id:e,solicita_alumnos:t??!1,solicita_clases:n??!1,tipos:a,estado:`pendiente`}]).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(s)throw console.error(`Error creando solicitud:`,s.message),Error(`No se pudo crear la solicitud de permisos: ${s.message}`);return w(o)}async function ge(e){let{data:t,error:n}=await i.from(`solicitudes_permisos`).select(`*, maestros!maestro_id(nombre_completo, correo)`).eq(`maestro_id`,e).order(`creado_en`,{ascending:!1}).maybeSingle();if(n)throw console.error(`Error obteniendo solicitud:`,n.message),Error(`No se pudo obtener la solicitud`);return w(t)}async function _e(){let{data:e,error:t}=await i.from(`solicitudes_permisos`).select(`*, maestros!maestro_id(nombre_completo, correo)`).eq(`estado`,`pendiente`).order(`creado_en`,{ascending:!0});if(t)throw console.error(`Error obteniendo solicitudes pendientes:`,t.message),Error(`No se pudieron cargar las solicitudes pendientes`);return(e||[]).map(w)}async function ve(e,t){if(!e||!t)throw Error(`solicitudId y adminId son requeridos`);let{data:n,error:r}=await i.from(`solicitudes_permisos`).update({estado:`aprobado`,aprobado_en:new Date().toISOString(),aprobado_por:t}).eq(`id`,e).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(r)throw console.error(`Error aprobando solicitud:`,r.message),Error(`No se pudo aprobar la solicitud`);if(n?.maestro_id){let e=w(n),r=[];e.solicita_alumnos&&r.push(`registrar_alumnos`,`alumnos:create`),e.solicita_clases&&r.push(`inscribir_clases`,`clases:enroll`,`clases:create`);let i=await C(n.maestro_id),a=Array.isArray(i?.permisos)?i.permisos:[];await me(n.maestro_id,{puede_registrar_alumnos:e.solicita_alumnos||(i?.puede_registrar_alumnos??!1),puede_inscribir_clases:e.solicita_clases||(i?.puede_inscribir_clases??!1),permisos:[...new Set([...a,...r])],concedido_por:t})}return w(n)}async function ye(e,t,n){if(!e||!t)throw Error(`solicitudId y adminId son requeridos`);let{data:r,error:a}=await i.from(`solicitudes_permisos`).update({estado:`rechazado`,aprobado_en:new Date().toISOString(),aprobado_por:t,motivo_rechazo:n||``}).eq(`id`,e).select(`*, maestros!maestro_id(nombre_completo, correo)`).single();if(a)throw console.error(`Error rechazando solicitud:`,a.message),Error(`No se pudo rechazar la solicitud`);return w(r)}var be=class{constructor(e=`default`){this.scope=e,this.subscriptions=[],this.intervals=[],this.listeners=[]}registerChannel(e){e&&!this.subscriptions.includes(e)&&this.subscriptions.push(e)}registerInterval(e){e!==null&&!this.intervals.includes(e)&&this.intervals.push(e)}registerListener(e,t,n){e&&t&&n&&this.listeners.push({el:e,event:t,fn:n})}destroy(){this.subscriptions.forEach(e=>{try{e&&i.removeChannel(e)}catch(e){console.warn(`[LifecycleManager] Error removing channel (${this.scope}):`,e)}}),this.intervals.forEach(e=>{try{clearInterval(e)}catch(e){console.warn(`[LifecycleManager] Error clearing interval (${this.scope}):`,e)}}),this.listeners.forEach(({el:e,event:t,fn:n})=>{try{e&&t&&n&&e.removeEventListener(t,n)}catch(e){console.warn(`[LifecycleManager] Error removing listener (${this.scope}):`,e)}}),this.subscriptions=[],this.intervals=[],this.listeners=[],console.log(`[LifecycleManager] Cleanup completed for scope: ${this.scope}`)}};async function xe(e){let{error:t}=await i.from(`sesiones_clase`).delete().eq(`id`,e);if(t)throw console.error(`Error eliminando sesión:`,t.message),Error(`No se pudo eliminar la sesión`);return{success:!0}}var Se=n({router:()=>T}),T={routes:{},_authCheck:null,_publicRoutes:[`login`,`register`],_guardEnabled:!1,register(e,t){this.routes[e]=t},setAuthGuard(e,t=[`login`,`register`]){this._authCheck=e,this._publicRoutes=t,this._guardEnabled=!0},_cleanupModals(){document.querySelectorAll(`.modal.show, .modal.fade`).forEach(e=>{try{let t=a.getInstance(e);t&&t.dispose()}catch{}}),document.querySelectorAll(`.modal-backdrop`).forEach(e=>e.remove()),document.body.classList.remove(`modal-open`),document.body.style.removeProperty(`overflow`),document.body.style.removeProperty(`padding-right`)},navigate(e,t={}){if(!this.routes[e]){console.error(`Route ${e} not found`);return}if(this._guardEnabled&&this._authCheck&&!this._publicRoutes.includes(e)&&!this._authCheck()){localStorage.setItem(`current-view`,`login`),localStorage.setItem(`intended-route`,e),this._navigateTo(`login`,{});return}this._navigateTo(e,t)},_navigateTo(e,t={}){let n=document.querySelector(`#app`);n&&(this._cleanupModals(),n.innerHTML=``,this.routes[e](n,t),localStorage.setItem(`current-view`,e),t&&Object.keys(t).length>0?localStorage.setItem(`current-view-params`,JSON.stringify(t)):localStorage.removeItem(`current-view-params`),window.dispatchEvent(new CustomEvent(`routeChanged`,{detail:e})))},init(){let e=localStorage.getItem(`current-view`)||`programas`,t=localStorage.getItem(`current-view-params`),n=t?JSON.parse(t):{};this.navigate(e,n)},initCustomEvents(){window.addEventListener(`navigate:alumno`,e=>{let t=e.detail?.alumnoId||e.detail?.id;t&&this.navigate(`alumnos`,{selectedId:t})}),window.addEventListener(`navigate:observaciones`,e=>{let t=e.detail?.alumnoId;t&&this.navigate(`observaciones`,{filtroAlumnoId:t})}),window.addEventListener(`navigate:metricas-alumno`,e=>{let t=e.detail?.alumnoId||e.detail?.id;t&&this.navigate(`metricas-riesgo`,{highlightId:t})})}},Ce=[{id:`nivel-0`,nombre:`Nivel 0: Iniciación / Descubrimiento`,color:`success`},{id:`nivel-1`,nombre:`Nivel 1: Básico / Formación Técnica`,color:`primary`},{id:`nivel-2`,nombre:`Nivel 2: Intermedio / Desarrollo Solista`,color:`warning`},{id:`nivel-3`,nombre:`Nivel 3: Avanzado / Maestría Institucional`,color:`danger`}];async function E({plan:n=null,esACM:i=!1,onSaved:a=null}={}){let o=[];try{o=await s()}catch(e){console.error(`[EditorPlanificacionModal] Error cargando clases:`,e)}let c=!!(n&&n.id),l=c?`Editar Estructura Curricular`:i?`Diseñador Curricular Institucional (ACM)`:`Nueva Planificación Didáctica`,d={nivelId:n?.nivelId||`nivel-1`,frecuenciaSemanal:n?.frecuenciaSemanal||2,semanasTotales:n?.semanasTotales||24,objetivos:n?.objetivosEstructurados||[{id:`obj-1`,titulo:`Dominio de Postura y Emisión Sonora`,indicadores:[{id:`ind-1`,titulo:`Postura corporal equilibrada y relajada`,prerrequisitoId:null},{id:`ind-2`,titulo:`Distribución fluida del arco en cuerdas abiertas`,prerrequisitoId:`ind-1`}]},{id:`obj-2`,titulo:`Afinación y Articulación Digital`,indicadores:[{id:`ind-3`,titulo:`Colocación exacta de 1er y 2do dedo`,prerrequisitoId:`ind-2`},{id:`ind-4`,titulo:`Independencia digital a pulso 60 BPM`,prerrequisitoId:`ind-3`}]}]},f=n?.clase_id||n?.claseId||``,m=n?.titulo||`Plan Didáctico Semestral (6 Meses)`,h=n?.semana||1,g=`
  ...
  `;try{we(e,await c())}catch(n){console.error(`[MaestroPlanificacionView] Error:`,n),e.innerHTML=`
    ...
    `}}}function we(n,r){let i=`todos`,a=()=>{let a=r.filter(e=>i===`todos`?!0:i===`borrador`?e.estado===`borrador`:i===`revisada`?e.estado===`revisada`:i!==`publicada`||e.estado===`publicada`),o=n.querySelector(`#maestro-planes-list`);if(o){if(a.length===0){o.innerHTML=`
      ...
      `,o.querySelector(`#btn-crear-primer-plan`)?.addEventListener(`click`,()=>{E({onSaved:()=>D(n)})});return}o.innerHTML=a.map(e=>{let n=Te(e.estado),r=Array.isArray(e.contenidos)?e.contenidos:typeof e.contenidos==`string`?[e.contenidos]:[];return`
  ...
  `,n.querySelector(`#btn-crear-disenador-full`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-disenador`)}),n.querySelector(`#btn-ver-ruta-full`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-ruta`)}),n.querySelector(`#chk-select-all`)?.addEventListener(`change`,e=>{let t=e.target.checked;n.querySelectorAll(`.chk-plan`).forEach(e=>{e.checked=t,t?a.add(e.dataset.id):a.delete(e.dataset.id)}),s()}),n.querySelector(`#btn-aprobar-masivo`)?.addEventListener(`click`,async()=>{if(a.size!==0)try{await l(Array.from(a)),e.show(`${a.size} planificaciones aprobadas con éxito`,`success`),a.clear(),O(n)}catch(t){e.show(`Error en aprobación masiva: ${t.message}`,`error`)}}),n.querySelectorAll(`#acm-filter-tabs button`).forEach(e=>{e.addEventListener(`click`,()=>{n.querySelectorAll(`#acm-filter-tabs button`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),i=e.dataset.filter,a.clear(),s(),o()})}),o()}function ke(e){return e===`publicada`?`<span class="badge bg-success-subtle text-success border border-success-subtle"><i class="bi bi-check-circle me-1"></i>Aprobado</span>`:e===`revisada`?`<span class="badge bg-warning-subtle text-warning-emphasis border border-warning-subtle"><i class="bi bi-clock me-1"></i>Pendiente ACM</span>`:`<span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle"><i class="bi bi-pencil-square me-1"></i>Borrador</span>`}async function Ae(e){if(e){e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5">
      <div class="spinner-border text-primary" role="status">
        <span class="visually-hidden">Cargando métricas de cobertura curricular...</span>
      </div>
    </div>
  `;try{je(e,await d(null))}catch(n){console.error(`[CoberturaCurricularView] Error:`,n),e.innerHTML=`
      <div class="alert alert-danger my-4">
        <i class="bi bi-exclamation-triangle me-2"></i>Error al cargar la cobertura curricular: ${t(n.message)}
      </div>
    `}}}function je(e,n){let r=Array.isArray(n)?n:n?.clases||[],i=r.length,a=r.filter(e=>e.plan_id||e.tienePlan||e.planificacion).length,o=i>0?Math.round(a/i*100):0;e.innerHTML=`
    <div class="container-fluid px-3 py-3">
      <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
  ...
  `}var k={route_versions:[{id:`demo-route-version-1`,route_id:`demo-route-1`,clase_id:`demo-clase-1`,origen:`acm`,status:`published`,propuesta_por:null,feedback:null,created_at:`2026-06-01T10:00:00Z`},{id:`demo-route-version-propuesta-1`,route_id:`demo-route-1`,clase_id:`demo-clase-1`,origen:`maestro`,status:`propuesta`,propuesta_por:`demo-maestro-1`,feedback:null,created_at:`2026-07-01T09:00:00Z`}],levels:[{id:`demo-level-1`,route_version_id:`demo-route-version-1`,level_number:1,name:`Nivel 1 — Iniciación`},{id:`demo-level-2`,route_version_id:`demo-route-version-1`,level_number:2,name:`Nivel 2 — Intermedio`},{id:`demo-level-propuesta-1`,route_version_id:`demo-route-version-propuesta-1`,level_number:1,name:`Nivel 1 — Propuesto`}],nodes:[{id:`demo-node-1`,level_id:`demo-level-1`,name:`Postura`,order_index:0},{id:`demo-node-2`,level_id:`demo-level-2`,name:`Escalas`,order_index:0},{id:`demo-node-propuesta-1`,level_id:`demo-level-propuesta-1`,name:`Repertorio inicial`,order_index:0}],objetivos:[{id:`demo-obj-1`,node_id:`demo-node-1`,nombre:`Mantener la espalda recta`,order_index:0,activo:!0},{id:`demo-obj-2`,node_id:`demo-node-2`,nombre:`Tocar la escala de Do Mayor`,order_index:0,activo:!0},{id:`demo-obj-propuesta-1`,node_id:`demo-node-propuesta-1`,nombre:`Interpretar una pieza simple`,order_index:0,activo:!0}],indicators:[{id:`demo-ind-1`,objetivo_id:`demo-obj-1`,description:`Espalda alineada durante toda la sesión`,is_required:!0,order_index:0},{id:`demo-ind-2`,objetivo_id:`demo-obj-1`,description:`Hombros relajados`,is_required:!1,order_index:1},{id:`demo-ind-3`,objetivo_id:`demo-obj-2`,description:`Ejecuta la escala ascendente sin errores`,is_required:!0,order_index:0},{id:`demo-ind-propuesta-1`,objetivo_id:`demo-obj-propuesta-1`,description:`Ejecuta la pieza de memoria`,is_required:!0,order_index:0}],indicator_attempts:[{id:`demo-attempt-1`,student_id:`demo-alumno-1`,indicator_id:`demo-ind-2`,result:`approved`}]},Me=`curriculo_tres_planos_route_versions_demo`,A=2,j=null;function M(){if(j===null){try{let e=localStorage.getItem(Me);if(e){let t=JSON.parse(e);if(t&&t.schemaVersion===A&&Array.isArray(t.rows)){j=t.rows;return}}}catch{}j=JSON.parse(JSON.stringify(k.route_versions)),N()}}function N(){try{localStorage.setItem(Me,JSON.stringify({rows:j,schemaVersion:A}))}catch(e){console.warn(`[curriculoTresPlanosStore] Failed to persist:`,e.message)}}function Ne(){return M(),j}function Pe(e){return M(),j.find(t=>t.id===e)||null}function Fe(e){return M(),j.push(e),N(),{...e}}function Ie(e,t){M();let n=j.findIndex(t=>t.id===e);if(n===-1)throw Error(`Propuesta no encontrada.`);return j[n]={...j[n],...t,updated_at:new Date().toISOString()},N(),{...j[n]}}function Le(e){return e?.estructura?.niveles?e.estructura.niveles.map((t,n)=>({id:`${e.id}-level-${n}`,level_number:t.numero_nivel??n+1,name:t.nombre,nodes:(t.temas||[]).map((t,r)=>({id:`${e.id}-node-${n}-${r}`,name:t.nombre,objetivos:(t.objetivos||[]).map((t,i)=>({id:`${e.id}-obj-${n}-${r}-${i}`,nombre:t.nombre,indicators:(t.indicadores||[]).map((t,a)=>({id:`${e.id}-ind-${n}-${r}-${i}-${a}`,description:t.descripcion,is_required:t.es_requerido!==!1}))}))}))})):k.levels.filter(t=>t.route_version_id===e.id).map(e=>({...e,nodes:k.nodes.filter(t=>t.level_id===e.id).map(e=>({...e,objetivos:k.objetivos.filter(t=>t.node_id===e.id).map(e=>({...e,indicators:k.indicators.filter(t=>t.objetivo_id===e.id)}))}))}))}var Re=n({devolverPropuesta:()=>Ve,listarPropuestasPendientes:()=>ze,publicarPropuesta:()=>Be});function P(e=80){return new Promise(t=>setTimeout(t,e))}async function ze(){return await P(),Ne().filter(e=>e.origen===`maestro`&&e.status===`propuesta`).map(e=>({...e,levels:Le(e)}))}async function Be(e){if(!e)throw Error(`publicarPropuesta: se requiere routeVersionId.`);if(await P(),!Pe(e))throw Error(`Propuesta no encontrada.`);return Ie(e,{status:`published`})}async function Ve(e,t){if(!e)throw Error(`devolverPropuesta: se requiere routeVersionId.`);if(!t||!t.trim())throw Error(`devolverPropuesta: se requiere feedback para explicar el motivo de la devolución.`);if(await P(),!Pe(e))throw Error(`Propuesta no encontrada.`);return Ie(e,{status:`devuelta`,feedback:t.trim()})}var He=n({devolverPropuesta:()=>Ge,listarPropuestasPendientes:()=>Ue,publicarPropuesta:()=>We});async function Ue(){let{data:e,error:t}=await i.from(`route_versions`).select(`*, levels(id, level_number, nodes(id, name, objetivos(id, nombre, indicators(id, description))))`).eq(`origen`,`maestro`).eq(`status`,`propuesta`).order(`created_at`,{ascending:!1});if(t)throw t;return e||[]}async function We(e){if(!e)throw Error(`publicarPropuesta: se requiere routeVersionId.`);let{data:t,error:n}=await i.from(`route_versions`).update({status:`published`,updated_at:new Date().toISOString()}).eq(`id`,e).select().single();if(n)throw n;return t}async function Ge(e,t){if(!e)throw Error(`devolverPropuesta: se requiere routeVersionId.`);if(!t||!t.trim())throw Error(`devolverPropuesta: se requiere feedback para explicar el motivo de la devolución.`);let{data:n,error:r}=await i.from(`route_versions`).update({status:`devuelta`,feedback:t.trim(),updated_at:new Date().toISOString()}).eq(`id`,e).select().single();if(r)throw r;return n}var F=o.isDemoMode?Re:He,Ke=()=>F.listarPropuestasPendientes(),qe=e=>F.publicarPropuesta(e),Je=(e,t)=>F.devolverPropuesta(e,t);async function Ye(e){e.innerHTML=`
  ...
  `;let n=e.querySelector(`#acm-propuestas-list`),r=e.querySelector(`#acm-propuestas-detail`),i=[];try{i=await Ke()}catch(e){console.error(`[acmPropuestasView] Error cargando propuestas:`,e),n.innerHTML=`<p class="acm-error">Error al cargar propuestas: ${t(e.message)}</p>`;return}if(!i.length){n.innerHTML=`<p class="acm-placeholder">No hay propuestas pendientes de revisión.</p>`;return}n.innerHTML=i.map(e=>`
    ...
    `).join(``),n.querySelectorAll(`[data-propuesta-id]`).forEach(t=>{t.addEventListener(`click`,()=>{let n=i.find(e=>e.id===t.dataset.propuestaId);n&&Xe(r,n,{onResolved:()=>Ye(e)})})})}function Xe(e,t,{onResolved:n}){e.innerHTML=`
    <div class="acm-propuesta-detail-inner">
      <h3>Árbol de contenido propuesto</h3>
      <div class="acm-tree">${Ze(t.levels||[])}</div>
  ...
  `;let r=``;e.querySelector(`[data-role="feedback-textarea"]`).addEventListener(`input`,e=>{r=e.target.value}),e.querySelector(`[data-action="publicar"]`).addEventListener(`click`,async()=>{try{await qe(t.id),n()}catch(e){console.error(`[acmPropuestasView] Error al publicar:`,e),window.alert(`Error al publicar: ${e.message}`)}}),e.querySelector(`[data-action="devolver"]`).addEventListener(`click`,async()=>{if(!r.trim()){window.alert(`Escriba un feedback antes de devolver la propuesta.`);return}try{await Je(t.id,r),n()}catch(e){console.error(`[acmPropuestasView] Error al devolver:`,e),window.alert(`Error al devolver: ${e.message}`)}})}function Ze(e){return e.length?e.slice().sort((e,t)=>(e.level_number||0)-(t.level_number||0)).map(e=>`
    ...
    `).join(``):`<p class="acm-placeholder">Esta propuesta no tiene niveles.</p>`}var Qe=[`borrador`,`activo`,`archivado`];async function $e(e,t){if(!e||!t)throw Error(`claseId y routeVersionId son requeridos`);let{data:n}=await i.from(`class_curriculum_plan`).select(`id`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();n?.id&&await i.from(`class_curriculum_plan`).update({estado:`archivado`}).eq(`id`,n.id);let{data:r,error:a}=await i.from(`class_curriculum_plan`).insert({clase_id:e,route_version_id:t,estado:`activo`}).select().single();if(a)throw a;return r}async function et(e){let{data:t,error:n}=await i.from(`class_curriculum_plan`).select(`*`).eq(`clase_id`,e).eq(`estado`,`activo`).maybeSingle();if(n)throw n;return t}async function tt(e,t){if(!Qe.includes(t))throw Error(`Estado no válido`);let{data:n,error:r}=await i.from(`class_curriculum_plan`).update({estado:t}).eq(`clase_id`,e).select().single();if(r)throw r;return n}var nt=[`nombre`,`descripcion`,`order_index`];async function rt(e){if(!Array.isArray(e)||e.length===0)throw Error(`Se requiere al menos un objetivo`);let t=e.map(e=>({clase_id:e.clase_id,level_id:e.level_id,nombre:e.nombre,descripcion:e.descripcion||null,orden_objetivo:e.orden_objetivo??null,origen_node_id:e.origen_node_id||null,origen_objetivo_id:e.origen_objetivo_id||null,created_by:e.created_by||null})),{data:n,error:r}=await i.from(`clase_mapa_objetivos`).insert(t).select();if(r)throw r;return n}async function it(e){let{data:t,error:n}=await i.from(`clase_mapa_objetivos`).select(`*`).eq(`clase_id`,e).is(`archived_at`,null).order(`order_index`);if(n)throw n;return t||[]}async function at(e,t){let n={};for(let e of nt)t[e]!==void 0&&(n[e]=t[e]);if(Object.keys(n).length===0)throw Error(`No hay campos válidos para actualizar (permitidos: nombre, descripcion, order_index)`);let{data:r,error:a}=await i.from(`clase_mapa_objetivos`).update(n).eq(`id`,e).select().single();if(a)throw a;return r}async function ot(e){let{error:t}=await i.from(`clase_mapa_objetivos`).delete().eq(`clase_id`,e);if(t)throw t}var st=class{constructor(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._listeners=[]}subscribe(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}_notify(){this._listeners.forEach(e=>e(this))}async fetchRutaDeClase(e){this.cargando=!0,this.error=null,this._notify();try{return this.rutaAsignada=await et(e),this.cargando=!1,this._notify(),this.rutaAsignada}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async asignarRuta(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await $e(e,t);return this.rutaAsignada=n,this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async cambiarEstado(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await tt(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{return this.objetivos=await it(e),this.cargando=!1,this._notify(),this.objetivos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async agregarObjetivos(e,t,n){this.cargando=!0,this.error=null,this._notify();try{let r=await rt(n.map(n=>({planificacion_id:e,class_curriculum_plan_id:t,node_id:n.node_id,indicator_id:n.indicator_id,semana:n.semana||null})));return this.objetivos=await it(e),this.cargando=!1,this._notify(),r}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async actualizarObjetivo(e,t){this.cargando=!0,this.error=null,this._notify();try{let n=await at(e,t);return this.cargando=!1,this._notify(),n}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async eliminarObjetivos(e){this.cargando=!0,this.error=null,this._notify();try{await ot(e),this.objetivos=[],this.cargando=!1,this._notify()}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async evaluarAlumno(e){this.cargando=!0,this.error=null,this._notify();try{let t=await m(e);return this.evaluaciones=await h(e.clase_id),this.cargando=!1,this._notify(),t}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchEvaluaciones(e){this.cargando=!0,this.error=null,this._notify();try{return this.evaluaciones=await h(e),this.cargando=!1,this._notify(),this.evaluaciones}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}async fetchProgresoAlumnos(e){this.cargando=!0,this.error=null,this._notify();try{return this.progresoAlumnos=await g(e),this.cargando=!1,this._notify(),this.progresoAlumnos}catch(e){throw this.error=e.message,this.cargando=!1,this._notify(),e}}reset(){this.planificacion=null,this.rutaAsignada=null,this.objetivos=[],this.evaluaciones=[],this.progresoAlumnos=[],this.cargando=!1,this.error=null,this._notify()}},ct=null;function lt(){return ct||=new st,ct}var I=lt();function ut(e){if(!e)return;let t=I.subscribe(()=>dt(e));return dt(e),I.fetchRutaDeClase(``).catch(()=>{}),()=>t()}function dt(e){if(I.cargando&&!I.rutaAsignada&&I.objetivos.length===0){e.innerHTML=`
      ...
      </div>`;return}if(I.error&&!I.rutaAsignada){e.innerHTML=`
      <div class="clase-plan-view">
        <div class="alert alert-warning d-flex align-items-start gap-3 m-3" role="alert">
          ...
          <div>
            ...
            <p class="mb-0 small">${L(I.error)}</p>
          </div>
        </div>
      </div>`;return}let t=I.rutaAsignada,n=I.objetivos||[];e.innerHTML=`
    <div class="clase-plan-view">
  ...
  `,ht(e)}function ft(e){let t=e.estado===`activo`?`Activa`:e.estado===`borrador`?`Borrador`:`Archivada`,n=e.estado===`activo`?`success`:e.estado===`borrador`?`warning`:`secondary`;return`
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            <small class="text-muted">ID: ${L(e.route_version_id||e.id)}</small>
          </div>
        </div>
    ...
    <div class="page-glass rounded p-4 mb-4 text-center">
    ...
    </div>`}function mt(e){if(e.length===0)return`
      <div class="page-glass rounded p-3 mb-4">
      ...
      <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
        <div class="d-flex align-items-center gap-2">
          ...
          <span class="small">${L(e.indicator_description||e.indicator_id)}</span>
        </div>
        <div class="d-flex align-items-center gap-2">
          ${t}
    ...
    <div class="page-glass rounded p-3 mb-4">
      ...
      ${t}
    </div>`}function ht(e){e.querySelectorAll(`[data-action="evaluar"]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.indicatorId;document.dispatchEvent(new CustomEvent(`evaluacion:open`,{detail:{indicatorId:t}}))})})}function L(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}var gt=n({renderDisenadorCurricularView:()=>R});async function R(e){if(!e)return;e.innerHTML=`
  ...
  `;let t=[];try{t=await s()}catch(e){console.error(`[DisenadorCurricularView] Error cargando clases:`,e)}let n={claseId:t[0]?.id||``,niveles:[],nivelIdClonar:``,nivelIdIA:``};await z(n),B(e,t,n)}async function z(e){if(!e.claseId){e.niveles=[],e.nivelIdClonar=``,e.nivelIdIA=``;return}try{e.niveles=await le(e.claseId)}catch(t){console.error(`[DisenadorCurricularView] Error cargando niveles del catálogo:`,t),e.niveles=[]}e.nivelIdClonar=e.niveles[0]?.id||``,e.nivelIdIA=e.niveles[0]?.id||``}function B(e,n,r){let i=!!r.claseId&&r.niveles.length===0,a=!r.claseId||i;e.innerHTML=`
      ...
      <div class="row g-4">
        <div class="col-lg-6">
          <div class="card border border-secondary-subtle bg-body-tertiary rounded-4 p-4 shadow-sm h-100">
            ...
            <select class="form-select border-secondary-subtle mb-3" id="select-nivel-clonar-disenador" ${r.niveles.length===0?`disabled`:``}>
              ${r.niveles.length===0?`<option value="">Sin niveles en el catálogo para esta clase</option>`:r.niveles.map(e=>`
                    <option value="${e.id}" ${e.id===r.nivelIdClonar?`selected`:``}>
                      ${t(e.nombre)}
                    </option>
                  `).join(``)}
            </select>

            <button type="button" class="btn btn-primary" id="btn-clonar-plantilla" ${a?`disabled`:``}>
  ...
  `,_t(e,n,r)}function _t(t,n,r){t.querySelector(`#btn-volver-acm`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-acm`)}),t.querySelector(`#btn-ir-catalogo`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-catalogo`)}),t.querySelector(`#select-clase-disenador`)?.addEventListener(`change`,async e=>{r.claseId=e.target.value,await z(r),B(t,n,r)}),t.querySelector(`#select-nivel-clonar-disenador`)?.addEventListener(`change`,e=>{r.nivelIdClonar=e.target.value}),t.querySelector(`#select-nivel-ia-disenador`)?.addEventListener(`change`,e=>{r.nivelIdIA=e.target.value}),t.querySelector(`#btn-clonar-plantilla`)?.addEventListener(`click`,async()=>{if(!r.claseId||!r.nivelIdClonar)return;let n=t.querySelector(`#btn-clonar-plantilla`);n.disabled=!0;try{await x(r.claseId,r.nivelIdClonar),e.show(`Nivel del catálogo clonado a la clase con éxito ⭐`,`success`),window.router?.navigate(`planificacion-acm`)}catch(t){e.show(`Error al clonar el catálogo: ${t.message}`,`error`),n.disabled=!1}}),t.querySelector(`#btn-generar-ia`)?.addEventListener(`click`,async()=>{if(!r.claseId||!r.nivelIdIA)return;let i=t.querySelector(`#btn-generar-ia`);i.disabled=!0,e.show(`Generando estructura curricular con IA (GROQ)...`,`info`);try{let t=n.find(e=>String(e.id)===String(r.claseId)),i=r.niveles.findIndex(e=>e.id===r.nivelIdIA),a=(await ue(r.claseId)).filter(e=>e.level_id===r.nivelIdIA),o=a.reduce((e,t)=>Math.max(e,t.orden_objetivo||0),0)+1,s=a.map(e=>e.nombre),c=await b({instrumento:t?.nombre||t?.name||`Música`,nivelIndex:i>=0?i:0,objetivosExistentes:s});for(let e of c||[]){let t=await re({clase_id:r.claseId,level_id:r.nivelIdIA,nombre:e.titulo||e.id||`Objetivo generado por IA`,orden_objetivo:o++});for(let n of e.indicadores||[])await ae({objetivo_id:t.id,clase_id:r.claseId,descripcion:typeof n==`string`?n:n.titulo||n.id||`Indicador generado por IA`})}e.show(`Estructura generada por IA creada en el mapa de la clase ⭐`,`success`),window.router?.navigate(`planificacion-acm`)}catch(t){console.error(`[DisenadorCurricularView] Error generando con IA:`,t),e.show(`Error al generar con IA: ${t.message}`,`error`),i.disabled=!1}})}var vt=n({renderRutaPedagogicaView:()=>V});async function V(e){if(!e)return;e.innerHTML=`
    <div class="d-flex justify-content-center align-items-center py-5" style="min-height: 450px;">
      <div class="text-center">
        <div class="spinner-border text-primary mb-3" style="width: 3rem; height: 3rem;" role="status"></div>
  ...
  `;let t=[],n=[];try{let[e,r]=await Promise.all([s(),c()]);t=e||[],n=r||[]}catch(e){console.error(`[RutaPedagogicaView] Error:`,e)}yt(e,t,n)}function yt(n,r,i){let a=r[0]?.id||``,o=null,s=[],c=new Map,l=!0,u=async()=>{c.clear(),l=!0,s=await v(a),p()},d=()=>{let e=n.querySelector(`#panel-alumnos-evaluacion-nodo`),r=n.querySelector(`#lbl-nodo-seleccionado`);e&&(e.style.display=o?`block`:`none`),r&&(r.innerHTML=`<i class="bi bi-award-fill text-warning me-2"></i>${t(o?.titulo||`Selecciona un Nodo`)}`)},f=()=>{let e=n.querySelector(`#tbody-alumnos-ruta`);if(!e)return;e.innerHTML=s.map(e=>{let n=e.nombre.split(` `).slice(0,2).map(e=>e[0]).join(``).toUpperCase();return`
          <tr class="row-alumno-ruta${l?``:` opacity-50`}" data-id="${e.id}" style="cursor: pointer;">
            ...
            <td class="text-center">
              ...
              <small class="fw-bold text-body-secondary">${e.estrellas>0?`${e.estrellas} Estrellas (${St(e.estrellas)})`:`Sin Registrar (0★)`}</small>
            </td>
    ...
    `;let x=n.querySelector(`#full-ruta-svg-canvas`);x&&te({container:x,nodos:h,onNodeClick:t=>{o=t,e.info(`Nodo Seleccionado: ${t.titulo}`),d();let n=c.get(t.id);if(n){s=n,l=!0,f();return}l=!1,f(),v(a,t.id).then(e=>{o?.id===t.id&&(c.set(t.id,e),s=e,l=!0,f())})}}),n.querySelector(`#btn-volver-plan`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-acm`)}),n.querySelector(`#btn-ir-disenador`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-disenador`)}),n.querySelector(`#select-clase-ruta`)?.addEventListener(`change`,e=>{a=e.target.value,o=null,u()}),n.querySelector(`#tbody-alumnos-ruta`)?.addEventListener(`click`,t=>{if(!l)return;let n=t.target.closest(`.row-alumno-ruta`);if(!n)return;let r=n.dataset.id,i=s.find(e=>String(e.id)===String(r));i&&i.presente&&(i.estrellas=y.siguienteEstrella(i.estrellas),ee.guardarLocal({alumnoId:i.id,claseId:a,nodoId:o?.id||`nodo-1`,estrellas:i.estrellas}),f(),e.show(`${i.nombre}: ${i.estrellas}★ registrad@s en LocalStorage/Offline`,`info`))}),d(),f()};u()}function bt(e){let t=[];return Array.isArray(e.objetivosEstructurados)&&e.objetivosEstructurados.forEach(e=>{Array.isArray(e.indicadores)&&e.indicadores.forEach(n=>{t.push({id:n.id,titulo:`${e.titulo}: ${n.titulo}`,estado:n.prerrequisitoId?`en_proceso`:`logrado`})})}),t}function xt(e){let t=``;for(let n=1;n<=5;n++)n<=e?t+=`<i class="bi bi-star-fill text-warning me-1"></i>`:t+=`<i class="bi bi-star text-secondary opacity-50 me-1"></i>`;return t}function St(e){return e===1?`Iniciado`:e===2?`En Proceso`:e===3?`Aprobado Básico`:e===4?`Logrado Fluido`:e===5?`Dominado Total`:`Sin Registrar`}async function Ct(e){if(!e)return;let t={instrumento:``,niveles:[],nivelId:``,generales:[],objetivoGeneralId:``,especificos:[]};await H(t),G(e,t)}async function H(e){e.niveles=await oe(e.instrumento||null),e.niveles.find(t=>t.id===e.nivelId)||(e.nivelId=``),await U(e)}async function U(e){e.generales=e.nivelId?await ne(e.nivelId):[],e.generales.find(t=>t.id===e.objetivoGeneralId)||(e.objetivoGeneralId=``),await W(e)}async function W(e){e.especificos=e.objetivoGeneralId?await de(e.objetivoGeneralId):[]}function G(e,n){let r=n.niveles.find(e=>e.id===n.nivelId),i=n.generales.find(e=>e.id===n.objetivoGeneralId);e.innerHTML=`
    <div class="container-fluid px-4 py-4">
      <div class="d-flex align-items-center gap-3 mb-4">
      ...
      <div class="mb-3" style="max-width: 320px;">
        ...
        <input type="text" class="form-control" id="input-filtro-instrumento" placeholder="Ej. Violín, Cello..." value="${t(n.instrumento)}">
      </div>
      ...
      <div class="row g-3">
        <div class="col-md-4">
          <div class="card border-0 shadow-sm h-100">
            ...
            <div class="list-group list-group-flush" id="lista-niveles">
              ${n.niveles.length===0?`<div class="list-group-item text-muted small">Sin niveles todavía</div>`:n.niveles.map(e=>`
                <button class="list-group-item list-group-item-action ${e.id===n.nivelId?`active`:``}" data-nivel-id="${e.id}">
                  ${t(e.nombre)} <span class="badge bg-secondary-subtle text-secondary ms-1">${t(e.instrumento)}</span>
                </button>`).join(``)}
            </div>
          </div>
        </div>
        ...
        <div class="col-md-4">
          <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
              <strong>Objetivos Generales${r?` — ${t(r.nombre)}`:``}</strong>
              <button class="btn btn-sm btn-primary" id="btn-add-general" ${n.nivelId?``:`disabled`}><i class="bi bi-plus-lg"></i></button>
            </div>
            <div class="list-group list-group-flush" id="lista-generales">
              ${n.nivelId?n.generales.length===0?`<div class="list-group-item text-muted small">Sin objetivos generales todavía</div>`:n.generales.map(e=>`
                <button class="list-group-item list-group-item-action ${e.id===n.objetivoGeneralId?`active`:``}" data-general-id="${e.id}">
                  ${t(e.nombre)}
                </button>`).join(``):`<div class="list-group-item text-muted small">Elegí un nivel</div>`}
            </div>
          </div>
        </div>
        ...
        <div class="col-md-4">
          <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
              <strong>Objetivos Específicos${i?` — ${t(i.nombre)}`:``}</strong>
              <button class="btn btn-sm btn-primary" id="btn-add-especifico" ${n.objetivoGeneralId?``:`disabled`}><i class="bi bi-plus-lg"></i></button>
            </div>
            <div class="list-group list-group-flush" id="lista-especificos">
              ${n.objetivoGeneralId?n.especificos.length===0?`<div class="list-group-item text-muted small">Sin objetivos específicos todavía</div>`:n.especificos.map(e=>`<div class="list-group-item small">${t(e.nombre)}</div>`).join(``):`<div class="list-group-item text-muted small">Elegí un objetivo general</div>`}
            </div>
          </div>
        </div>
      </div>
    </div>
  `,wt(e,n)}function wt(n,i){n.querySelector(`#btn-volver-catalogo`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-acm`)}),n.querySelector(`#input-filtro-instrumento`)?.addEventListener(`change`,async e=>{i.instrumento=e.target.value.trim(),await H(i),G(n,i)}),n.querySelector(`#lista-niveles`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`[data-nivel-id]`);t&&(i.nivelId=t.dataset.nivelId,await U(i),G(n,i))}),n.querySelector(`#lista-generales`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`[data-general-id]`);t&&(i.objetivoGeneralId=t.dataset.generalId,await W(i),G(n,i))}),n.querySelector(`#btn-add-nivel`)?.addEventListener(`click`,()=>{r.open({title:`Nuevo nivel del catálogo`,body:`
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="modal-nivel-instrumento" placeholder="Ej. Cello" value="${t(i.instrumento)}">
        </div>
      `,saveText:`Crear nivel`,onSave:async t=>{let r=t.querySelector(`#modal-nivel-nombre`).value.trim(),a=t.querySelector(`#modal-nivel-instrumento`).value.trim();if(!r||!a)return e.error(`Nombre e instrumento son obligatorios`),!1;await se({nombre:r,instrumento:a,orden:i.niveles.filter(e=>e.instrumento===a).length+1}),i.instrumento=a,await H(i),G(n,i),e.success(`Nivel creado`)}})}),n.querySelector(`#btn-add-general`)?.addEventListener(`click`,()=>{i.nivelId&&r.open({title:`Nuevo objetivo general`,body:`
        <div class="mb-3">
          <label class="form-label fw-semibold">Nombre *</label>
