import{i as e,r as t}from"./AppModal-Du6jXNYA.js";import{a as n,n as r,t as i}from"./MapaContenidoSVG-DzsxSvgK.js";import{c as a}from"./asistenciasApi-Vw-N6q8K.js";import{c as o,d as s,f as c,g as l,h as u,i as d,m as f,n as p,o as m,r as h,t as g,y as _}from"./mapaClaseService-R-RlGV7a.js";function v({claseId:e,niveles:t=[],objetivo:n=null,maestroId:r=null,onSaved:i=null,onClosed:a=null}){document.querySelectorAll(`.objetivo-editor-modal-overlay`).forEach(e=>e.remove());let o={objetivo:n,indicadores:[],archivarPendienteId:null,archivarObjetivoPendiente:!1},s=document.createElement(`div`);if(s.className=`objetivo-editor-modal-overlay`,document.body.appendChild(s),!document.getElementById(`objetivo-editor-modal-styles`)){let e=document.createElement(`style`);e.id=`objetivo-editor-modal-styles`,e.textContent=T(),document.head.appendChild(e)}let c=()=>{s.remove(),a?.()},l=()=>{s.innerHTML=x({claseId:e,niveles:t,state:o}),b({overlay:s,claseId:e,niveles:t,maestroId:r,state:o,close:c,render:l,onSaved:i})};l(),o.objetivo?.id&&y(o,l)}async function y(e,t){try{e.indicadores=await u(e.objetivo.id)}catch(t){console.error(`[objetivoEditorModal] Error cargando indicadores:`,t),e.indicadores=[]}t()}function b({overlay:e,claseId:t,niveles:n,maestroId:r,state:i,close:a,render:l,onSaved:u}){e.querySelector(`.objetivo-editor-modal-close-x`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-modal-backdrop`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-cancelar-btn`)?.addEventListener(`click`,a),e.querySelector(`.objetivo-editor-guardar-btn`)?.addEventListener(`click`,async()=>{let n=e.querySelector(`#objetivo-editor-nombre`)?.value?.trim()||``,a=e.querySelector(`#objetivo-editor-descripcion`)?.value?.trim()||``,s=e.querySelector(`#objetivo-editor-nivel`)?.value||null;if(!n){C(e,`El nombre del objetivo es requerido`);return}try{if(i.objetivo?.id){let e=await p(i.objetivo.id,{nombre:n,descripcion:a});i.objetivo={...i.objetivo,...e},u?.(i.objetivo)}else{if(!s){C(e,`Seleccioná un nivel`);return}let c=await o({clase_id:t,level_id:s,nombre:n,descripcion:a,orden_objetivo:(await _(t)).filter(e=>e.level_id===s).reduce((e,t)=>Math.max(e,t.orden_objetivo||0),0)+1,created_by:r});i.objetivo=c,u?.(c)}l(),i.objetivo?.id&&y(i,l)}catch(t){console.error(`[objetivoEditorModal] Error guardando objetivo:`,t),C(e,`No se pudo guardar el objetivo`)}}),e.querySelector(`.objetivo-editor-borrar-objetivo-btn`)?.addEventListener(`click`,async()=>{try{await c(i.objetivo.id),a()}catch(t){if(t instanceof g){i.archivarObjetivoPendiente=!0,l();return}console.error(`[objetivoEditorModal] Error borrando objetivo:`,t),C(e,`No se pudo borrar el objetivo`)}}),e.querySelector(`.objetivo-editor-archivar-objetivo-btn`)?.addEventListener(`click`,async()=>{try{await d(i.objetivo.id),a()}catch(t){console.error(`[objetivoEditorModal] Error archivando objetivo:`,t),C(e,`No se pudo archivar el objetivo`)}}),e.querySelector(`.objetivo-editor-agregar-indicador-btn`)?.addEventListener(`click`,async()=>{let n=e.querySelector(`#objetivo-editor-nuevo-indicador`)?.value?.trim()||``;if(n)try{await m({objetivo_id:i.objetivo.id,clase_id:t,descripcion:n}),await y(i,l)}catch(t){console.error(`[objetivoEditorModal] Error agregando indicador:`,t),C(e,`No se pudo agregar el indicador`)}}),e.querySelectorAll(`.objetivo-editor-indicador-row`).forEach(t=>{let n=t.dataset.indicadorId;t.querySelector(`.btn-borrar-indicador`)?.addEventListener(`click`,async()=>{try{await s(n),await y(i,l)}catch(t){if(t instanceof g){i.archivarPendienteId=n,l();return}console.error(`[objetivoEditorModal] Error borrando indicador:`,t),C(e,`No se pudo borrar el indicador`)}}),t.querySelector(`.btn-archivar-indicador`)?.addEventListener(`click`,async()=>{try{await h(n),i.archivarPendienteId=null,await y(i,l)}catch(t){console.error(`[objetivoEditorModal] Error archivando indicador:`,t),C(e,`No se pudo archivar el indicador`)}})})}function x({claseId:e,niveles:t,state:n}){let{objetivo:r}=n,i=!!r?.id,a=!i&&t.length===0,o=t.map(e=>`<option value="${e.id}">${w(e.nombre)}</option>`).join(``);return`
