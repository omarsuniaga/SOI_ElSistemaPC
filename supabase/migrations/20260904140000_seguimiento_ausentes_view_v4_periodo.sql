-- ============================================================================
-- vw_seguimiento_ausentes v4:
--  - el contador vuelve a acumularse DESDE EL INICIO DEL PERIODO ACADEMICO
--    vigente (NO se reinicia cada mes). Revierte v3.
--  - se mantiene la fraccion dias_ausente/dias_clase y la eliminacion de
--    sesiones_ausente. Se quita mes_nombre.
--  - seguimiento_reglas.config.periodo vuelve a 'academico'.
--  - Para poner el contador de un alumno en cero: boton "Reiniciar contador"
--    (inserta en seguimiento_ausencias_reinicio, ya soportado por el CTE corte).
-- DROP + CREATE (cambia columnas). Aplicado a zmhmdvmyeyswunurcyow.
-- ============================================================================
...
COMMENT ON VIEW public.vw_seguimiento_ausentes IS
  'Una fila por alumno activo con >=1 dia de ausencia injustificada acumulada DESDE EL INICIO DEL PERIODO ACADEMICO vigente (despues del ultimo corte manual, sin suspension activa). dias_ausente / dias_clase = faltas sobre dias con clase del periodo. security_invoker=false.';

GRANT SELECT ON public.vw_seguimiento_ausentes TO authenticated;

UPDATE public.seguimiento_reglas
SET config = config || '{"periodo":"academico"}'::jsonb, updated_at = now()
