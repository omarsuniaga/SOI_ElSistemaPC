import{t as e}from"./AppToast-DNGTRY9B.js";import{i as t}from"./supabase-Dhe7Tlxd.js";import{i as n}from"./maestroAuth-CdApllXF.js";import{t as r}from"./ausenciaModal-BGyz7Tee.js";var i=new class{constructor(){this.state={ausencias:[],loading:!0,error:null},this.listenersAttached=!1}async init(e){if(!e){console.error(`AusenciaHistorial: Container no proporcionado`);return}this.container=e,this._attachListeners(),await this._loadData(),this._render()}_attachListeners(){this.listenersAttached||=(window.addEventListener(`ausenciaSolicitada`,()=>this.refresh()),window.addEventListener(`ausenciaActualizada`,()=>this.refresh()),!0)}destroy(){window.removeEventListener(`ausenciaSolicitada`,()=>this.refresh()),window.removeEventListener(`ausenciaActualizada`,()=>this.refresh()),this.listenersAttached=!1}async refresh(){await this._loadData(),this._render()}async _loadData(){this.state.loading=!0,this.state.error=null,this._render();try{let e=n();if(!e)throw Error(`No hay sesión activa`);let{data:r,error:i}=await t.from(`ausencias_maestros`).select(`*`).eq(`maestro_id`,e.id).order(`created_at`,{ascending:!1});if(i)throw i;this.state.ausencias=r||[]}catch(e){console.error(`Error cargando ausencias:`,e),this.state.error=`No se pudieron cargar las ausencias. Intenta de nuevo.`,this.state.ausencias=[]}finally{this.state.loading=!1}}_render(){this.container&&(this.container.innerHTML=this._getHTML(),this._attachEvents())}_getHTML(){return this.state.loading?this._renderLoading():this.state.error?this._renderError():this.state.ausencias.length?`
      ${this._renderTable()}
      <div class="ah-cards">
        ${this._renderCards()}
      </div>
    ...
    `}_renderCards(){return this.state.ausencias.map(e=>{let t=this._formatDate(e.fecha_inicio),n=this._formatDate(e.fecha_fin),r=e.fecha_inicio===e.fecha_fin?t:`${t} - ${n}`;return`
      ...
      `}).join(``)}_attachEvents(){this.container?.querySelectorAll(`[data-action="retry"]`).forEach(e=>{e.addEventListener(`click`,()=>this.refresh())}),this.container?.querySelectorAll(`[data-action="cancel"]`).forEach(e=>{e.addEventListener(`click`,e=>{let t=e.currentTarget.dataset.id;this._cancelSolicitud(t)})})}async _cancelSolicitud(n){if(confirm(`¿Estás seguro que deseas cancelar esta solicitud?`))try{let{error:r}=await t.from(`ausencias_maestros`).update({estado:`cancelada`}).eq(`id`,n);if(r)throw r;e.success(`Solicitud cancelada correctamente`),this.refresh()}catch(t){console.error(`Error al cancelar:`,t),e.error(`No se pudo cancelar la solicitud`)}}_formatDate(e){if(!e)return`-`;let t=navigator.language||`es-ES`;return new Date(e).toLocaleDateString(t,{year:`numeric`,month:`short`,day:`numeric`})}_calcDias(e,t){if(!e||!t)return 0;let n=new Date(e),r=new Date(t);return Math.ceil((r-n)/(1e3*60*60*24))+1}_getTipoIcon(e){let t={enfermedad:`<i class="bi bi-thermometer-half ah-tipo-icon"></i>`,personal:`<i class="bi bi-person ah-tipo-icon"></i>`,capacitacion:`<i class="bi bi-book ah-tipo-icon"></i>`,vacaciones:`<i class="bi bi-sun ah-tipo-icon"></i>`,otro:`<i class="bi bi-three-dots ah-tipo-icon"></i>`};return t[e]||t.otro}_getTipoLabel(e){return{enfermedad:`Enfermedad`,personal:`Personal`,capacitacion:`Capacitación`,vacaciones:`Vacaciones`,otro:`Otro`}[e]||`Otro`}};async function a(e){let t=typeof e==`string`?document.querySelector(e):e;if(!t){console.error(`AusenciaHistorial: Contenedor no encontrado`);return}await i.init(t)}function o(){i.destroy()}var s=new class{constructor(){this.maestro=n(),this.isOpen=!1,this.container=null}initDOM(){document.getElementById(`pm-ausencias-drawer-overlay`)||(this.container=document.createElement(`div`),this.container.innerHTML=`
    ...
    `,document.body.appendChild(this.container),document.getElementById(`pm-ausencias-close`).addEventListener(`click`,()=>this.close()),document.getElementById(`pm-ausencias-drawer-overlay`).addEventListener(`click`,e=>{e.target.id===`pm-ausencias-drawer-overlay`&&this.close()}),document.getElementById(`pm-ausencias-btn-nueva`).addEventListener(`click`,()=>{this.close(),r.open()}))}async open(){if(!this.maestro){e.error(`Debes estar logueado para ver tus ausencias`);return}this.initDOM();let t=document.getElementById(`pm-ausencias-drawer-overlay`);t.style.display=`block`,t.offsetHeight,t.classList.add(`open`),this.isOpen=!0,await a(`#pm-ausencias-body`)}close(){let e=document.getElementById(`pm-ausencias-drawer-overlay`);e&&(e.classList.remove(`open`),setTimeout(()=>{e.style.display=`none`,o()},300)),this.isOpen=!1}};export{s as ausenciasPanel};