const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/router-D1rLi8eq.js","assets/rolldown-runtime-tcWNtVWY.js","assets/vendor-CAKU_njC.js","assets/vendor-COf7rB16.css"])))=>i.map(i=>d[i]);
import{n as e}from"./rolldown-runtime-tcWNtVWY.js";import{i as t}from"./supabase-BryBf0UA.js";import{t as n}from"./preload-helper-CQ36nxok.js";async function r(){try{let{data:e,error:n}=await t.from(`maestro_desempeño`).select(`
        id,
        maestro_id,
        ...
        updated_at
        `).order(`updated_at`,{ascending:!1});if(n)throw console.error(`[getMaestrosComplianceStatus] Error:`,n),n;return e||[]}catch(e){throw console.error(`[getMaestrosComplianceStatus] Exception:`,e),e}}async function i(e){try{let{data:n,error:r}=await t.from(`registros_pendientes`).select(`
        id,
        created_at,
        notification_state,
        notif_count,
        ...
        `).eq(`maestro_id`,e).eq(`estado`,`pendiente`).in(`tipo`,[`asistencia_pendiente`,`contenido_pendiente`]).order(`created_at`,{ascending:!1});if(r)throw console.error(`[getMaestroPendingRegistros] Error:`,r),r;return n||[]}catch(e){throw console.error(`[getMaestroPendingRegistros] Exception:`,e),e}}var a=e({CumplimientoMaestrosWidget:()=>o,default:()=>o}),o=class{constructor(e){this.containerId=e,this.container=document.getElementById(e),this.maestros=[],this.filteredMaestros=[],this.currentFilter={categoria:null,estado:null,diasAtrasoMin:0,diasAtrasoMax:999}}async init(){try{this.container.innerHTML=`
    ...
    `}attachEventListeners(){let e=document.getElementById(`filterCategoria`),t=document.getElementById(`filterDiasAtraso`),r=document.getElementById(`btnRefresh`);this._filterCategoriaHandler=e=>{this.applyFilter({categoria:e.target.value||null})},this._filterDiasAtrasoHandler=e=>{if(!e.target.value)this.applyFilter({diasAtrasoMin:0,diasAtrasoMax:999});else{let t=e.target.value.split(`-`);this.applyFilter({diasAtrasoMin:t[0]?parseInt(t[0]):0,diasAtrasoMax:t[1]?parseInt(t[1]):999})}},this._btnRefreshHandler=()=>{this.init()},e?.addEventListener(`change`,this._filterCategoriaHandler),t?.addEventListener(`change`,this._filterDiasAtrasoHandler),r?.addEventListener(`click`,this._btnRefreshHandler);let i=document.getElementById(`btnGotoNotificaciones`);this._btnGotoNotificacionesHandler=()=>{n(async()=>{let{router:e}=await import(`./router-D1rLi8eq.js`).then(e=>e.n);return{router:e}},__vite__mapDeps([0,1,2,3])).then(({router:e})=>{e.navigate(`admin-notificaciones`)})},i?.addEventListener(`click`,this._btnGotoNotificacionesHandler),this._contactarHandlers=[],this._detalleHandlers=[],this.container.querySelectorAll(`.btn-contactar`).forEach(e=>{let t=e=>{let t=e.target.closest(`.btn-contactar`).dataset.maestroId;this.onContactarMaestro(t)};this._contactarHandlers.push({btn:e,handler:t}),e.addEventListener(`click`,t)}),this.container.querySelectorAll(`.btn-detalle`).forEach(e=>{let t=e=>{let t=e.target.closest(`.btn-detalle`).dataset.maestroId;this.onDetalleMaestro(t)};this._detalleHandlers.push({btn:e,handler:t}),e.addEventListener(`click`,t)})}destroy(){let e=document.getElementById(`filterCategoria`),t=document.getElementById(`filterDiasAtraso`),n=document.getElementById(`btnRefresh`),r=document.getElementById(`btnGotoNotificaciones`);e&&this._filterCategoriaHandler&&e.removeEventListener(`change`,this._filterCategoriaHandler),t&&this._filterDiasAtrasoHandler&&t.removeEventListener(`change`,this._filterDiasAtrasoHandler),n&&this._btnRefreshHandler&&n.removeEventListener(`click`,this._btnRefreshHandler),r&&this._btnGotoNotificacionesHandler&&r.removeEventListener(`click`,this._btnGotoNotificacionesHandler),this._contactarHandlers&&=(this._contactarHandlers.forEach(({btn:e,handler:t})=>{e.removeEventListener(`click`,t)}),[]),this._detalleHandlers&&=(this._detalleHandlers.forEach(({btn:e,handler:t})=>{e.removeEventListener(`click`,t)}),[]),this.maestros=[],this.filteredMaestros=[],this.container=null}onContactarMaestro(e){let t=this.maestros.find(t=>t.maestro_id===e);if(!t)return;let n=t.maestros?.email;n?window.location.href=`mailto:${n}?subject=Seguimiento%20Registros%20Asistencias`:alert(`No hay email disponible para este maestro`)}onDetalleMaestro(e){this.maestros.find(t=>t.maestro_id===e)&&(window.location.href=`/admin/maestros/${e}/detail`)}};export{a as n,o as t};