...
const IMPORT_ENTITIES = {
  ...
  clases: {
    ...
    fields: [
      ...
      {
        name: 'maestro_principal_id',
        type: 'string',
        required: false,
        label: 'ID del maestro (UUID)',
      },
      {
        name: 'maestro_nombre',
        type: 'string',
        required: false,
        label: 'Nombre del maestro (lookup)',
        virtual: true,
      },
  ...
  progresos: {
    ...
    fields: [
      ...
      { name: 'clase_id', type: 'string', required: true, label: 'ID de la clase' },
      {
        ...
        required: true,
        label: 'Tipo',
        options: ['parcial', 'final', 'continua'],
      },
      ...
      { name: 'observaciones', type: 'string', required: false, label: 'Observaciones' },
      {
        ...
        required: false,
        label: 'Estado',
        options: ['en_progreso', 'completado', 'pendiente'],
        default: 'en_progreso',
      },
    ],
    ...
    .map((f) => {
      switch (f.type) {
        ...
        case 'date':
          return '2024-01-01'
        case 'select':
          return f.options?.[0] || 'valor'
        default:
          return `ejemplo_${f.name}`
      }
    })
    ...
    if (field.required) {
      switch (field.type) {
        ...
        case 'boolean':
          ...
          break
        case 'date':
          sample[field.name] = '2024-01-01'
          break
        case 'select':
          sample[field.name] = field.options?.[0] || ''
          break
        default:
          sample[field.name] = `ejemplo_${field.name}`
      }
    }
  }
...
export async function importData(entityKey, records) {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    for (const field of entity.fields) {
      ...
      const value =
        field.name in record
          ? record[field.name]
          : field.alias && field.alias in record
            ? record[field.alias]
            : undefined
    ...
    if (existingRecord) {
      ...
      // Insert new record
      const { error: insertError } = await supabase.from(entity.table).insert(cleanRecord)
