import{i as e,r as t}from"./AppModal-Du6jXNYA.js";import{o as n,s as r}from"./pwaInstaller-D9QnnKwe.js";import{i}from"./supabase-Cgh_dhNB.js";import{t as a}from"./vendor-mK9cUK6A.js";import{n as o,r as s,t as c}from"./IndicadorLogro-CUm_IXl5.js";import{t as l}from"./MapaContenidoSVG-B-8-5_NT.js";import{t as u}from"./a11yUtils-DRYT20ux.js";import{a as d,c as f,d as p,i as m,r as h}from"./weeklyPlanAdapter-E65PNMYx.js";function g(e){return e?String(e).split(`
`).map(e=>e.trim()).filter(Boolean):[]}function _(e){return e?Array.isArray(e)?e.join(`
`):typeof e==`string`?e:``:``}async function v(e,t=null){if(!e)return null;let n=i.from(`planificaciones`).select(`*`).eq(`clase_id`,e).eq(`activo`,!0).order(`created_at`,{ascending:!1}).limit(1);t&&(n=n.eq(`periodo_nombre`,t));let{data:r,error:a}=await n;if(a)throw Error(`No se pudo cargar el plan: ${a.message}`);return r?.[0]??null}async function y(){let{data:e}=await i.auth.getUser(),t=e?.user?.id;if(!t)throw Error(`Sesión no iniciada`);let{data:n,error:r}=await i.from(`maestros`).select(`id`).eq(`user_id`,t).maybeSingle();if(r)throw Error(`No se pudo identificar al maestro: ${r.message}`);if(!n?.id)throw Error(`Su usuario no está vinculado a un maestro`);return n.id}async function b(e){let t=(e.titulo||``).trim();if(!e.clase_id)throw Error(`Falta la clase`);if(!t)throw Error(`El título del plan es obligatorio`);let n={clase_id:e.clase_id,titulo:t,descripcion:(e.descripcion||``).trim()||null,periodo_nombre:e.periodo_nombre||null,fecha_inicio:e.fecha_inicio||null,fecha_fin:e.fecha_fin||null,instrumento:e.instrumento||null,contenidos:g(e.contenidos),obras:g(e.obras),tecnicas:g(e.tecnicas),escalas_arpegios:g(e.escalas_arpegios),estado:e.estado||`borrador`,activo:!0,updated_at:new Date().toISOString()};if(e.id){let{data:t,error:r}=await i.from(`planificaciones`).update(n).eq(`id`,e.id).select().maybeSingle();if(r)throw Error(`No se pudo guardar el plan: ${r.message}`);return t}n.maestro_id=await y();let{data:r,error:a}=await i.from(`planificaciones`).insert([n]).select().maybeSingle();if(a)throw Error(`No se pudo crear el plan: ${a.message}`);return r}async function x(e,{periodoNombre:t,fechaInicio:n=null,fechaFin:r=null}){if(!e)throw Error(`Falta el plan a duplicar`);if(!t)throw Error(`Indique el período de destino`);let{data:a,error:o}=await i.from(`planificaciones`).select(`*`).eq(`id`,e).maybeSingle();if(o)throw Error(`No se pudo leer el plan original: ${o.message}`);if(!a)throw Error(`El plan original ya no existe`);let{id:s,created_at:c,updated_at:l,...u}=a,{data:d,error:f}=await i.from(`planificaciones`).insert([{...u,periodo_nombre:t,fecha_inicio:n,fecha_fin:r,estado:`borrador`,activo:!0,maestro_id:await y()}]).select().maybeSingle();if(f)throw Error(`No se pudo duplicar el plan: ${f.message}`);return d}async function S(e){if(!e)return[];let{data:t,error:n}=await i.from(`clases`).select(`route_version_id`).eq(`id`,e).maybeSingle();if(n)throw Error(`No se pudo leer la clase: ${n.message}`);if(!t?.route_version_id)return[];let{data:r,error:a}=await i.from(`nodes`).select(`id, name, codigo, level:levels!inner(level_number, name, route_version_id)`).eq(`level.route_version_id`,t.route_version_id).not(`codigo`,`is`,null).order(`name`);if(a)throw Error(`No se pudo cargar el apoyo curricular: ${a.message}`);let o=new Set;return(r??[]).filter(e=>o.has(e.codigo)?!1:(o.add(e.codigo),!0))}async function C(){let{data:e,error:t}=await i.from(`periodos`).select(`id, nombre, fecha_inicio, fecha_fin, activo`).order(`fecha_inicio`,{ascending:!1});if(t)throw Error(`No se pudieron cargar los períodos: ${t.message}`);return e??[]}var w=[[`contenidos`,`Contenidos`,`Un contenido por línea`],[`obras`,`Obras y repertorio`,`Una obra por línea`],[`tecnicas`,`Técnicas`,`Una técnica por línea`],[`escalas_arpegios`,`Escalas y arpegios`,`Una por línea`]];function T(e,{clase:n,periodoActivo:r=null,onCambio:i}={}){let a=null,o=[],s=[],c=!0,l=null;function u(e,n,r){return`
          ...
          ${w.map(([e,n,r])=>`
            <div>
              ...
              <textarea name="${e}" rows="4" class="form-control form-control-sm rounded-3"
                style="background:var(--pm-surface); color:var(--pm-text); border-color:var(--pm-border); resize:vertical;"
                placeholder="${r}">${t(_(i[e]))}</textarea>
            </div>`).join(``)}
        </div>
...
`).some(e=>e.trim()===i)||(n.value=r?`${r}\n${i}`:i,n.focus())})});let t=e.querySelector(`#pcp-form`);t?.addEventListener(`submit`,async e=>{e.preventDefault();let o=t.querySelector(`button[type="submit"]`),s=o.textContent;o.disabled=!0,o.textContent=`Guardando…`;try{let e=new FormData(t),o=await b({id:a?.id,clase_id:n.id,instrumento:n.instrumento,periodo_nombre:r?.nombre??a?.periodo_nombre??null,fecha_inicio:r?.fecha_inicio??null,fecha_fin:r?.fecha_fin??null,titulo:e.get(`titulo`),descripcion:e.get(`descripcion`),contenidos:e.get(`contenidos`),obras:e.get(`obras`),tecnicas:e.get(`tecnicas`),escalas_arpegios:e.get(`escalas_arpegios`)}),s=!a;a=o,d(),f(s?`Plan creado.`:`Cambios guardados.`),i?.(o)}catch(e){o.disabled=!1,o.textContent=s,f(e.message,`error`)}}),e.querySelector(`#pcp-duplicar`)?.addEventListener(`click`,async()=>{let e=s.filter(e=>e.nombre!==a?.periodo_nombre);if(e.length===0){f(`No hay otro período al cual copiar.`,`error`);return}let t=e.find(e=>e.activo)??e[0];if(window.confirm(`Se copiará este plan a "${t.nombre}" como borrador.\n\nEl plan actual no se modifica.`))try{await x(a.id,{periodoNombre:t.nombre,fechaInicio:t.fecha_inicio,fechaFin:t.fecha_fin}),f(`Copiado a "${t.nombre}" como borrador.`)}catch(e){f(e.message,`error`)}})}async function m(){c=!0,d();try{let[e,t,i]=await Promise.all([v(n.id,r?.nombre??null),S(n.id).catch(()=>[]),C().catch(()=>[])]);a=e,o=t,s=i,l=null}catch(e){l=e.message}finally{c=!1,d()}}return m(),{recargar:m,getPlan:()=>a}}var E={achieved:{label:`Dominado`,icon:`🟢`,cardClass:`estado-completado`},exceeded:{label:`Sobresaliente`,icon:`🔵`,cardClass:`estado-completado`},in_process:{label:`En proceso`,icon:`🟡`,cardClass:`estado-parcial`},needs_reinforcement:{label:`Requiere refuerzo`,icon:`🟠`,cardClass:`estado-parcial`},failed:{label:`No logrado`,icon:`🔴`,cardClass:`estado-no_iniciado`},not_started:{label:`Sin iniciar`,icon:`⚪`,cardClass:`estado-no_iniciado`}},D={violin:`🎻`,viola:`🎻`,cello:`🎻`,contrabajo:`🎻`,chelo:`🎻`,piano:`🎹`,teclado:`🎹`,guitarra:`🎸`,bajo:`🎸`,ukulele:`🎸`,flauta:`🪈`,clarinete:`🎵`,oboe:`🎵`,fagot:`🎵`,saxofon:`🎵`,trompeta:`🎺`,trombon:`🎺`,tuba:`🎺`,corno:`🎺`,corneta:`🎺`,percusion:`🥁`,bateria:`🥁`,marimba:`🥁`,xilofono:`🥁`,timbal:`🥁`,canto:`🎤`,voz:`🎤`,vocal:`🎤`,arpa:`🪗`,acordeon:`🪗`,teoria:`📖`,solfeo:`📖`,armonia:`📖`,historia:`📖`};function O(e){return E[e]||E.not_started}function k(e){return String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function A(e){if(!e)return`🎼`;let t=e.toLowerCase().normalize(`NFD`).replace(/[\u0300-\u036f]/g,``);return Object.entries(D).find(([e])=>t.includes(e))?.[1]||`🎼`}function j(e){return e>=70?`#10b981`:e>=30?`#f59e0b`:`#ef4444`}function M(e){let t=new Set;return(e?.items||[]).filter(e=>{let n=e.indicator_id||`${e.node_id}:${e.topic}`;return t.has(n)?!1:(t.add(n),!0)})}function N(e,t,n){let r=n.length;return M(e).map(e=>{let i=n.map(n=>t[`${n}_${e.indicator_id}`]?.status||`not_started`),a=i.filter(e=>[`achieved`,`exceeded`].includes(e)).length,o=i.filter(e=>[`in_process`,`needs_reinforcement`].includes(e)).length,s=`not_started`;a>0&&a===r&&r>0?s=`achieved`:(a>0||o>0)&&(s=`in_process`);let c=O(s);return{id:e.indicator_id||e.node_id||e.id,weekNumber:e.week_number,topic:e.topic,objective:e.objective,evidence:e.evidence,assessmentMethod:e.assessment_method,teacherStrategy:e.teacher_strategy,progressPercentage:r>0?Math.round(a/r*100):0,achievedCount:a,totalStudents:r,overallStatus:s,meta:c}})}function P(e,t,n){let r=n.map(e=>e.alumno_id).filter(Boolean),i=M(e?.plan),a=i.length,o=0;if(a>0&&r.length>0){let e=0;i.forEach(n=>{r.forEach(r=>{let i=t[`${r}_${n.indicator_id}`]?.status||`not_started`;[`achieved`,`exceeded`].includes(i)&&e++})}),o=Math.round(e/(a*r.length)*100)}return{progressPercentage:o,totalStudents:r.length}}function F(e=3){return`
  ...
  `}function I(e,t){e.innerHTML=`<div class="pm-planning-empty"><p>${k(t)}</p></div>`}async function L(t,{maestroId:g}){let _=null,v=null,y=[],b={},x={},S=[];if(!document.getElementById(`pm-planning-styles`)){let e=document.createElement(`style`);e.id=`pm-planning-styles`,e.textContent=`
      /* ── Layout ── */
  ...
  `;let C=t.querySelector(`#pm-planning-content`),w=null;(function(){let e=t.querySelector(`#pm-step-slide`),n=t.querySelectorAll(`#pm-step-dots .pm-stepper-dot`),r=t.querySelector(`#pm-step-prev`),i=t.querySelector(`#pm-step-next`);if(!e)return;let a=n.length,o=0;function s(t){o=Math.max(0,Math.min(a-1,t)),e.style.transform=`translateX(-${o*100}%)`,n.forEach((e,t)=>{e.classList.toggle(`active`,t===o),e.setAttribute(`aria-selected`,String(t===o))}),r&&(r.disabled=o===0),i&&(i.disabled=o===a-1)}r?.addEventListener(`click`,()=>s(o-1)),i?.addEventListener(`click`,()=>s(o+1)),n.forEach(e=>e.addEventListener(`click`,()=>s(Number(e.dataset.step))));let c=setInterval(()=>{o<a-1?s(o+1):clearInterval(c)},5e3);t.querySelector(`#pm-guide-stepper`)?.addEventListener(`pointerdown`,()=>clearInterval(c)),s(0)})(),t.querySelector(`#btn-pm-header-disenador`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-disenador`)}),t.querySelector(`#btn-pm-header-ruta`)?.addEventListener(`click`,()=>{window.router?.navigate(`planificacion-ruta`)});async function E(e){let t=C.querySelector(`[data-clase-id="${e}"]`);if(t)try{let[r,i,a]=await Promise.all([d(e,g).catch(()=>null),f(e).catch(()=>({})),n([e]).catch(()=>[])]),{progressPercentage:o}=P(r,i,a),s=j(o),c=t.querySelector(`.pm-card-progress-bar`),l=t.querySelector(`.pm-progress-pct`);c&&(c.style.width=`${o}%`,c.style.backgroundColor=s),l&&(l.textContent=`${o}%`)}catch(e){console.warn(`[planning] No se pudo actualizar la tarjeta:`,e)}}async function D(){C.innerHTML=F(3);try{let e=await r();if(e.length===0){C.innerHTML=`
          ...
          </div>`,u(`No tienes clases asignadas actualmente.`);return}let t=(await Promise.allSettled(e.map(async e=>{let[t,r,i]=await Promise.all([d(e.id,g).catch(()=>null),f(e.id).catch(()=>({})),n([e.id]).catch(()=>[])]),{progressPercentage:a,totalStudents:o}=P(t,r,i);return{...e,currentWeek:t?.route?.current_week||1,hasGuide:!!t,progressPercentage:a,totalStudents:o}}))).map((t,n)=>t.status===`fulfilled`?t.value:{...e[n],currentWeek:1,hasGuide:!1,progressPercentage:0,totalStudents:0});C.innerHTML=`
        ...
        <div class="pm-planning-classes-grid">
          ${t.map((e,t)=>{let n=A(e.instrumento),r=j(e.progressPercentage),i=e.hasGuide&&e.totalStudents>0;return`
              <div class="pm-class-card-interactive" data-clase-id="${e.id}"
                   role="button" tabindex="0" aria-label="Abrir clase ${k(e.nombre)}"
                ...
                <div class="pm-class-card-body">
                  <div class="pm-class-card-top">
                    <h4 class="pm-class-card-title">${k(e.nombre)}</h4>
                  ...
                  <div class="pm-class-card-plan">${k(e.plan_estudio||`Sin plan curricular`)}</div>
      ...
      `,u(`${t.length} clases cargadas.`),C.querySelectorAll(`.pm-class-card-interactive`).forEach(e=>{let n=async()=>{let n=e.dataset.claseId,r=t.find(e=>String(e.id)===String(n));r&&(_=r.id,await O(),R(r))};e.addEventListener(`click`,n),e.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),n())})})}catch(t){console.error(`[planning] Error renderizando clases:`,t),e.error(`No se pudieron cargar las clases asignadas.`),I(C,`No se pudieron cargar tus clases.`)}}async function O(){if(_)try{let[e,t,r]=await Promise.all([d(_,g).catch(()=>null),n([_]).catch(()=>[]),f(_).catch(()=>({}))]);v=e,S=t,x=r,b=await M(e?.route?.weekly_plan_id);let i=t.map(e=>e.alumno_id).filter(Boolean);y=N(e?.plan,r,i)}catch(e){console.error(`[planning] Error al refrescar datos:`,e)}}async function M(e){return!_||!g||!e?{}:(await m(_,g,e).catch(()=>[])).reduce((e,t)=>(e[String(t.week_number)]=t,e),{})}function L(e){let t=b[String(e.week_number)]||null;return{...e,teacher_strategy:t?.teacher_strategy??e.teacher_strategy,student_activity:t?.student_activity??e.student_activity,homework:t?.homework??e.homework,evidence:t?.evidence??e.evidence,teacher_notes:t?.teacher_notes??``,hasTeacherAdjustment:!!t}}function R(t,n=`general`){w&&=(a.getInstance(w)?.dispose(),w.remove(),null);let r=v?.route,s=v?.plan?.items||[],c=r?.current_week||1,d={general:``,temas:s.length>0?s.length:``,indicadores:y.length>0?y.length:``};function f(e,t,r){let i=d[e];return`
        <button class="pm-tab-btn ${n===e?`active`:``}" data-tab="${e}" type="button">
          ${t} ${r}
          ${i?`<span class="pm-tab-count">${i}</span>`:``}
        </button>
      `}w=document.createElement(`div`),w.className=`modal fade`,w.setAttribute(`tabindex`,`-1`),w.setAttribute(`aria-hidden`,`true`),w.setAttribute(`aria-label`,`Detalle de clase: ${t.nombre}`),w.innerHTML=`
      <div class="modal-dialog modal-xxl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content" style="background:var(--pm-surface); color:var(--pm-text); border:1px solid var(--pm-border); border-radius:20px; box-shadow:0 24px 64px rgba(0,0,0,0.15);">
          ...
          <div style="display:flex; gap:0; padding:0 1.5rem; border-bottom:1px solid var(--pm-border); overflow-x:auto;">
            ${f(`general`,`📋`,`Perfil`)}
            ${f(`plan`,`📝`,`Mi Plan`)}
            ${f(`temas`,`📅`,`Temas y Ajustes`)}
            ${f(`indicadores`,`🎯`,`Indicadores`)}
            ${f(`ruta_svg`,`🗺️`,`Ruta SVG`)}
          </div>
          ...
          <div class="modal-body" style="padding:1.5rem; min-height:360px;">
              ...
              <div class="row g-3">
                <div class="col-12 col-md-6">
                  <div style="padding:1.1rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.015)); height:100%;">
                    ...
                    <div style="font-weight:700; font-size:1.05rem; margin-bottom:0.5rem; color:var(--pm-text);">${k(t.plan_estudio||`Sin plan asignado`)}</div>
                    <p style="font-size:0.82rem; color:var(--pm-text-muted); margin:0; line-height:1.5;">${k(t.descripcion||`Clase de formación activa dentro del plan institucional.`)}</p>
                  </div>
                </div>
                <div class="col-12 col-md-6">
                  <div style="padding:1.1rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.015)); height:100%;">
                    ...
                    ${[[`Instrumento`,k(t.instrumento||`General`)],[`Alumnos inscritos`,`${t.totalStudents||0}`],[`Semanas en plan`,`${s.length}`],[`Semana activa`,`${c}`]].map(([e,t])=>`
                ...
                <div class="col-12">
                  <div style="padding:1.25rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.01));">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem;">
                      ...
                      <span style="font-size:1.6rem; font-weight:800; color:${j(t.progressPercentage)}; min-width:56px; text-align:right;">${t.progressPercentage}%</span>
                    </div>
                    <div style="height:12px; border-radius:999px; background:var(--pm-border); overflow:hidden;">
                      <div style="height:100%; width:${t.progressPercentage}%; background:${j(t.progressPercentage)}; border-radius:999px; transition:width 0.6s ease;"></div>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:0.72rem; color:var(--pm-text-muted); margin-top:0.5rem;">
                      <span>0%</span>
                      <span style="color:${j(t.progressPercentage)}; font-weight:700;">
            ...
            <div class="pm-tab-pane ${n===`temas`?``:`d-none`}" data-pane="temas">
              ...
              `:s.map(e=>{let t=L(e),n=e.week_number<c,r=e.week_number===c,i=n?`past`:r?`current`:`upcoming`,a=n?`Pasada`:r?`Esta semana`:`Semana ${e.week_number}`;return`
                  <div class="pm-week-item ${r?`is-current`:``}" id="pm-week-${e.week_number}">
                    <button class="pm-week-header" data-week="${e.week_number}" type="button" aria-expanded="${r}">
                      ...
                      <span style="font-weight:700; font-size:0.92rem; flex:1; color:var(--pm-text);">${k(e.topic)}</span>
                    ...
                    <div class="pm-week-body ${r?`open`:``}" id="pm-week-body-${e.week_number}">
                      <div style="padding:1rem;">
                        ...
                        <div style="padding:0.85rem; background:var(--pm-surface-2,rgba(0,0,0,0.02)); border-radius:10px; margin-bottom:1rem; border:1px dashed var(--pm-border);">
                          ...
                          <div style="font-size:0.83rem; color:var(--pm-text); margin-bottom:0.5rem;">${k(e.objective||`Sin objetivo registrado`)}</div>
                          <div class="row g-2">
                            <div class="col-12 col-sm-6">
                              <div style="font-size:0.72rem; font-weight:700; color:var(--pm-text-muted);">Estrategia base:</div>
                              <div style="font-size:0.8rem; color:var(--pm-text-muted);">${k(e.teacher_strategy||`—`)}</div>
                            </div>
                            <div class="col-12 col-sm-6">
                              <div style="font-size:0.72rem; font-weight:700; color:var(--pm-text-muted);">Evidencia base:</div>
                              <div style="font-size:0.8rem; color:var(--pm-text-muted);">${k(e.evidence||`—`)}</div>
                            </div>
                          </div>
                        </div>
            ...
            <div class="pm-tab-pane ${n===`indicadores`?``:`d-none`}" data-pane="indicadores">
              ...
              `:`
                ...
                ${y.map(e=>{let t=j(e.progressPercentage);return`
                    <div class="pm-indicator-card">
                      <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; flex-wrap:wrap; margin-bottom:0.65rem;">
                        <div style="flex:1; min-width:220px;">
                          ...
                          <h5 style="font-weight:700; font-size:0.92rem; margin:0 0 0.25rem; color:var(--pm-text);">${k(e.topic)}</h5>
                          <p style="font-size:0.78rem; color:var(--pm-text-muted); margin:0;">${k(e.objective||`Sin objetivo registrado`)}</p>
                        </div>
            ...
            <div class="pm-tab-pane ${n===`ruta_svg`?``:`d-none`}" data-pane="ruta_svg">
              <div style="padding:1rem; border:1px solid var(--pm-border); border-radius:14px; background:var(--pm-surface-2,rgba(0,0,0,0.015));">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem; flex-wrap:wrap; gap:0.5rem;">
                  <div>
                    <h5 style="margin:0; font-weight:800; color:var(--pm-text);">Grafo Vectorial SVG - ${k(t.nombre)}</h5>
    ...
    `,document.body.appendChild(w);let m=new a(w),b=w.querySelectorAll(`.pm-tab-btn`),C=w.querySelectorAll(`.pm-tab-pane`);b.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.tab;b.forEach(e=>e.classList.toggle(`active`,e.dataset.tab===t)),C.forEach(e=>e.classList.toggle(`d-none`,e.dataset.pane!==t)),t===`plan`&&F(),t===`ruta_svg`&&N()})});let D=e=>{document.activeElement&&document.activeElement.blur(),m.hide(),setTimeout(()=>{a.getInstance(w)?.dispose(),document.querySelectorAll(`.modal-backdrop`).forEach(e=>e.remove()),document.body.classList.remove(`modal-open`),document.body.style.removeProperty(`overflow`),document.body.style.removeProperty(`padding-right`),w&&=(w.remove(),null),window.router?.navigate(e)},150)};w.querySelectorAll(`.btn-modal-disenador`).forEach(e=>{e.addEventListener(`click`,()=>D(`planificacion-disenador`))}),w.querySelectorAll(`.btn-modal-ruta-full`).forEach(e=>{e.addEventListener(`click`,()=>D(`planificacion-ruta`))});let M=!1;async function N(){if(M)return;M=!0;let e=w.querySelector(`#pm-svg-canvas-host-${t.id}`);if(!e)return;let n=await o(t.id),r=(s||[]).map((e,t)=>({id:`w-node-${e.week_number||t+1}`,titulo:e.topic||`Clase ${e.week_number||t+1}`,estado:t===0?`logrado`:t===1?`en_proceso`:`pendiente`}));l({container:e,nodos:r.length>0?r:[{id:`nd-1`,titulo:`Postura corporal y emisión sonora libre`,estado:`logrado`},{id:`nd-2`,titulo:`Escala de Do Mayor en cuerdas Re-Sol`,estado:`en_proceso`},{id:`nd-3`,titulo:`Estudio Nº 4: Control de pulso a 80 BPM`,estado:`pendiente`}],onNodeClick:e=>{z(w,t.id,e,n)}})}n===`ruta_svg`&&N();let P=null;async function F(){if(P)return;let e=w.querySelector(`#pm-plan-clase-host`);if(!e)return;let n=null;try{let{data:e}=await i.from(`periodos`).select(`nombre, fecha_inicio, fecha_fin`).eq(`activo`,!0).maybeSingle();n=e??null}catch{n=null}P=T(e,{clase:t,periodoActivo:n,onCambio:()=>u(`Plan de clase actualizado`)})}n===`plan`&&F(),w.querySelectorAll(`.pm-week-header`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.week,n=w.querySelector(`#pm-week-body-${t}`),r=e.querySelector(`.pm-week-chevron`),i=n?.classList.contains(`open`);n?.classList.toggle(`open`,!i),r?.classList.toggle(`open`,!i),e.setAttribute(`aria-expanded`,String(!i))})}),w.querySelectorAll(`.pm-week-adjustment-form`).forEach(n=>{n.addEventListener(`submit`,async i=>{i.preventDefault();let a=Number(n.dataset.week),o=new FormData(n),s=n.querySelector(`button[type="submit"]`),c=s?.querySelector(`.btn-text`),l=s?.querySelector(`.spinner-border`);s&&(s.disabled=!0),l?.classList.remove(`d-none`),c&&(c.textContent=`Guardando...`);try{await h({group_id:t.id,teacher_id:g,weekly_plan_id:r?.weekly_plan_id,week_number:a,teacher_strategy:String(o.get(`teacher_strategy`)||``).trim(),student_activity:String(o.get(`student_activity`)||``).trim(),homework:String(o.get(`homework`)||``).trim(),evidence:String(o.get(`evidence`)||``).trim(),teacher_notes:String(o.get(`teacher_notes`)||``).trim()}),e.success(`Ajustes guardados — Semana ${a}.`),u(`Ajuste de la semana ${a} guardado correctamente.`),await O(),R(t,`temas`)}catch(t){console.error(`[planning] Error guardando ajuste:`,t),e.error(t.message||`No se pudieron guardar los ajustes.`)}finally{s?.isConnected&&(s.disabled=!1,l?.classList.add(`d-none`),c&&(c.textContent=`Guardar ajuste`))}})}),w.querySelectorAll(`.btn-mark-seen-group`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.indicatorId,i=n.querySelector(`.btn-text`),a=n.querySelector(`.spinner-border`);n.disabled=!0,a?.classList.remove(`d-none`),i&&(i.textContent=`Procesando...`);try{let n=S.filter(e=>String(e.clase_id)===String(t.id)).map(e=>e.alumno_id).filter(Boolean);if(n.length===0){e.warning(`Esta clase no tiene alumnos inscritos para calificar.`);return}await Promise.all(n.map(e=>p(e,r,`achieved`,`Aprobado masivamente`,``,null))),e.success(`Indicador marcado como Dominado para todo el grupo.`),u(`Indicador marcado como dominado para todos los alumnos.`),await O(),R(t,`indicadores`)}catch(t){console.error(`[planning] Error al calificar indicador grupal:`,t),e.error(`Error al actualizar el progreso del indicador.`)}finally{n?.isConnected&&(n.disabled=!1,a?.classList.add(`d-none`),i&&(i.textContent=`🟢 Marcar Grupo`))}})}),w.querySelectorAll(`.btn-toggle-individual`).forEach(n=>{n.addEventListener(`click`,()=>{let r=n.dataset.indicatorId,i=w.querySelector(`#individual-eval-${r}`),a=w.querySelector(`#alumnos-list-ind-${r}`);if(!i)return;if(!i.classList.contains(`d-none`)){i.classList.add(`d-none`),n.classList.replace(`btn-secondary`,`btn-outline-secondary`);return}if(i.classList.remove(`d-none`),n.classList.replace(`btn-outline-secondary`,`btn-secondary`),a.children.length>0)return;let o=S.filter(e=>String(e.clase_id)===String(t.id)).map(e=>e.alumnos).filter(Boolean);if(o.length===0){a.innerHTML=`<div class="col-12 text-muted" style="font-size:0.82rem;">Sin alumnos inscritos en esta clase.</div>`;return}a.innerHTML=o.map(e=>{let t=(x||{})[`${e.id}_${r}`]?.status||`not_started`;return`
            <div class="col-12 col-sm-6" style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem 0.65rem; border-radius:10px; border:1px dashed var(--pm-border); gap:0.5rem;">
              <span style="font-size:0.82rem; font-weight:600; color:var(--pm-text); flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${k(e.nombre_completo)}</span>
          ...
          `}).join(``),a.querySelectorAll(`.select-student-indicator`).forEach(i=>{i.addEventListener(`change`,async()=>{let a=i.dataset.studentId,o=i.value;i.disabled=!0;try{await p(a,r,o,`Calificación individual`,``,null),e.success(`Calificación guardada.`),u(`Calificación del alumno guardada.`),await O();let i=x||{},s=S.filter(e=>String(e.clase_id)===String(t.id)).map(e=>e.alumno_id).filter(Boolean),c=s.filter(e=>[`achieved`,`exceeded`].includes(i[`${e}_${r}`]?.status||`not_started`)).length,l=s.length,d=l>0?Math.round(c/l*100):0,f=j(d),m=n.closest(`.pm-indicator-card`),h=m?.querySelector(`.ind-progress-bar`),g=m?.querySelector(`.ind-progress-bar`)?.parentElement?.previousElementSibling?.querySelector(`span:last-child`);h&&(h.style.width=`${d}%`,h.style.background=f),g&&(g.textContent=`${d}%`),n.textContent=`👥 ${c}/${l}`}catch(t){console.error(`[planning] Error actualizando indicador:`,t),e.error(`No se pudo guardar la calificación.`)}finally{i.disabled=!1}})})})}),w.addEventListener(`hidden.bs.modal`,()=>{a.getInstance(w)?.dispose(),w.remove(),w=null,_&&E(_)},{once:!0}),m.show(),u(`Panel de clase ${t.nombre} abierto.`)}function z(t,n,r,i){let a=t.querySelector(`#pm-nodo-alumnos-host-${n}`),o=t.querySelector(`#pm-nodo-titulo-${n}`),l=t.querySelector(`#pm-nodo-alumnos-tbody-${n}`);if(!a||!l)return;a.style.display=`block`,o&&(o.textContent=`Evaluación Alumnos: ${r.titulo}`);let u=()=>{l.innerHTML=(i||[]).map(e=>{let t=``;for(let n=1;n<=5;n++)t+=n<=e.estrellas?`<i class="bi bi-star-fill text-warning me-1"></i>`:`<i class="bi bi-star text-secondary opacity-50 me-1"></i>`;return`
          <tr class="pm-row-alumno-node" data-id="${e.id}" style="cursor:pointer;">
            <td class="fw-bold" style="color:var(--pm-text);">${k(e.nombre)}</td>
            <td class="text-center">
              <span class="fs-6">${t}</span>
              <small style="color:var(--pm-text-muted);" class="ms-1">(${e.estrellas>0?e.estrellas+`/5★`:`Sin Registrar`})</small>
            </td>
          </tr>
        `}).join(``),l.querySelectorAll(`.pm-row-alumno-node`).forEach(t=>{t.addEventListener(`click`,()=>{let a=t.dataset.id,o=i.find(e=>String(e.id)===String(a));o&&(o.estrellas=c.siguienteEstrella(o.estrellas),s.guardarLocal({alumnoId:o.id,claseId:n,nodoId:r.id,estrellas:o.estrellas}),u(),e.show(`${o.nombre}: ${o.estrellas}★ guardados`,`info`))})})};u()}await D()}export{L as renderPlanificacionView};