import{d as e,s as t}from"./pwaInstaller-DKgXOIzT.js";import{i as n}from"./supabase-Cgh_dhNB.js";import{i as r}from"./maestroAuth-BMzDPnai.js";import{s as i,t as a}from"./evaluationService-DR_iOLwx.js";async function o(e){let r=(await t())?.find(t=>t.id===e)?.instrumento;if(!r)return null;let i=r.split(`,`)[0].trim().toLowerCase(),{data:a,error:o}=await n.from(`routes`).select(`id, route_versions!inner(id)`).ilike(`instrument`,`%${i}%`).eq(`route_versions.status`,`published`).limit(1).maybeSingle();return o?(console.warn(`[rutaService] resolveRutaIdForClase error:`,o.message),null):a?.route_versions?.[0]?.id||a?.route_versions?.id||null}async function s(e,t){let{data:r,error:i}=await n.from(`blocks`).select(`id, nombre:name, order_index`).eq(`route_version_id`,e).order(`order_index`,{ascending:!0});if(i)throw Error(`[rutaService] blocks: `+i.message);if(!r||r.length===0)return[];let o=r.map(e=>e.id),{data:s,error:c}=await n.from(`levels`).select(`id, block_id, nombre:name, order_index`).in(`block_id`,o).eq(`route_version_id`,e).order(`order_index`,{ascending:!0});if(c)throw Error(`[rutaService] levels: `+c.message);let l=(s??[]).map(e=>e.id);if(l.length===0)return r.map(e=>({...e,levels:[]}));let{data:u,error:d}=await n.from(`nodes`).select(`id, level_id, nombre:name, order_index`).in(`level_id`,l).eq(`route_version_id`,e).order(`order_index`,{ascending:!0});if(d)throw Error(`[rutaService] nodes: `+d.message);let f=(u??[]).map(e=>e.id),{data:p,error:m}=f.length>0?await n.from(`indicators`).select(`id, node_id, nombre:description, order_index`).in(`node_id`,f).eq(`activo`,!0).order(`order_index`,{ascending:!0}):{data:[],error:null};if(m)throw Error(`[rutaService] indicators: `+m.message);let h=await Promise.all((u??[]).map(e=>a(e.id,t).then(t=>({nodeId:e.id,semaphore:t.semaphore})).catch(()=>({nodeId:e.id,semaphore:`gray`})))),g=new Map(h.map(e=>[e.nodeId,e.semaphore])),_=new Map;for(let e of p??[])_.has(e.node_id)||_.set(e.node_id,[]),_.get(e.node_id).push({...e,semaphore:g.get(e.node_id)??`gray`});let v=new Map;for(let e of u??[])v.has(e.level_id)||v.set(e.level_id,[]),v.get(e.level_id).push({...e,semaphore:g.get(e.id)??`gray`,indicators:_.get(e.id)??[]});let y=new Map;for(let[e]of o.map(e=>[e]))y.set(e,[]);let b=new Map;for(let e of s??[])b.has(e.block_id)||b.set(e.block_id,[]),b.get(e.block_id).push(e);for(let[e,t]of b){let n=t.map((e,t,n)=>{let r=v.get(e.id)??[],i=r.flatMap(e=>_.get(e.id)??[]),a=i.filter(e=>(g.get(e.node_id)??`gray`)===`green`).length,o=i.length>0?Math.round(a/i.length*100):0,s=r.map(e=>e.semaphore),c=s.every(e=>e===`green`)&&s.length>0?`green`:s.every(e=>e===`gray`)||s.length===0?`gray`:`yellow`,l=!1;if(t>0){let e=n[t-1],r=(v.get(e.id)??[]).flatMap(e=>_.get(e.id)??[]),i=r.filter(e=>(g.get(e.node_id)??`gray`)===`green`).length;l=r.length>0&&i/r.length<.8}return{...e,semaphore:c,locked:l,percentage:o,nodes:r}});y.set(e,n)}return r.map(e=>({...e,levels:y.get(e.id)??[]}))}var c=new Map,l={on(e,t){c.has(e)||c.set(e,[]),c.get(e).push(t)},off(e,t){if(!c.has(e))return;let n=c.get(e),r=n.indexOf(t);r>-1&&n.splice(r,1)},emit(e,t){c.has(e)&&c.get(e).forEach(n=>{try{n(t)}catch(t){console.error(`[rutaEventEmitter] Error in listener for ${e}:`,t)}})},clearAllListeners(){c.clear()}};function u(e,t){let{blockId:n,blockName:r,isExpanded:i,childCount:a,onToggle:o}=t;e.innerHTML=`
  ...
  `,e.querySelector(`.block-section-header`)?.addEventListener(`click`,()=>{o(n)})}var d=()=>typeof globalThis<`u`&&globalThis.getMaestroLocal?globalThis.getMaestroLocal():r(),f=(...e)=>typeof globalThis<`u`&&globalThis.getMisClases?globalThis.getMisClases(...e):t(...e),p=(...e)=>typeof globalThis<`u`&&globalThis.loadRouteTree?globalThis.loadRouteTree(...e):s(...e),m={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:null},h=!1;async function g(t,{onTopicSelected:n}={}){if(m={clases:[],activeClaseId:null,rutaId:null,blocks:[],loading:!1,expandedBlocks:new Set,expandedLevels:new Set,selectedIndicator:null,onTopicSelected:n},t.innerHTML=`<div class="pm-ruta-gamificada"><div class="pm-loading"><div class="pm-spinner"></div></div></div>`,!d()){t.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No hay sesión activa.</p></div>`;return}try{if(e(),m.clases=await f(!0),!m.clases?.length){t.innerHTML=`<div class="pm-ruta-gamificada"><p class="pm-empty">No tienes clases asignadas.</p></div>`;return}m.activeClaseId=m.clases[0].id,await _(),v(t),S(t),C(t)}catch(e){console.error(`[rutaGameificadaView]`,e),t.innerHTML=`<div style="color:red;padding:20px;"><i class="bi bi-exclamation"></i> Error: ${e.message}</div>`}}async function _(){m.loading=!0,m.rutaId=await o(m.activeClaseId),m.rutaId?m.blocks=await p(m.rutaId,m.activeClaseId):m.blocks=[],m.loading=!1}function v(e){e.innerHTML=`
